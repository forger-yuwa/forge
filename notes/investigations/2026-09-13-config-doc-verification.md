...
PASS スカラーのキーに辞書
PASS YAML 真偽値に見える化学種名 (NO) を壊さない (注釈を外すと元テキストに戻る)
PASS 空の節を潰さない (注釈を外すと元テキストに戻る)
PASS 引用符・バックスラッシュを含む文字列 (注釈を外すと元テキストに戻る)
...
- `annotate`: 注釈行を取り除くと元テキストに 1 文字違わず戻ることを出力前に検査する (戻らなければ出力しない)。
