# codex レビュー 2026-09-20 — case/53 C3X 翼列が収束しない件 (自由形式の診断依頼)

- 依頼: 「node median-dual + 低 Re SST の翼列で前縁が壊れる。何が壊れているか、起動をどうするか、
  メッシュは妥当か、forge では無理なのか」を read-only で診断させた (`codex exec`、plan 段/result 段の
  定型レビューではない)。
- 生ログ: セッション scratchpad (`codex_c3x.log`)。以下は最終回答の全文。

## 採否 (2026-09-20)

| 指摘 | 判定 | 対応 |
| --- | --- | --- |
| 等温壁の `roe` が凍結し EOS 温度 9.19e6 K → μ が 151 倍・壁 ω 8.1e15 | **採用** | `main.cpp` で壁ピンを EOS/物性/RANS 壁の**前**へ移動 + 陰解法の最終更新後にも適用。別記録 `2026-09-20-codex-node-isothermal-roe-frozen.md` |
| `inlet_Pressure_dir` CPG 分岐の $P_c>P_t$ で根号が負、方向ベクトルのゼロ割ガード無し | **採用** (codex は「今回の主因ではない」としたが、全圧入口へ切り替える際に実際に踏んだ) | `boundaryCond_d.cu` に 2 件のガードを追加。`methods/boundary.md` に記載 |
| `convMethod: 1, limiter: 0` は 1 次ではなく**無制限 2 次** | **採用** | 起動段を `convMethod: 0` に変更 (`setup_run.py`) |
| skew 0.964 の 4 セル (前縁から 14.6 mm) は局所再分割で直せる。第一層 2 µm は維持 | **保留** | 全圧入口で収束したので当面据え置き。壁解像は `check_wall_resolution.py` で別途判定する |
| 「k の暴走が最初」という証拠は無い。周期・双対体積・壁距離はすべて健全 (294 対一対一、最大誤差 2.15e-8 m) | **採用** (切り分けの結論として) | 周期・メッシュ由来の疑いを打ち切った |
| 同一メッシュ・同一壁温で SU2 低 Re SST の対照計算を推奨 | **採用 (残作業)** | plan §5.1 に登録 |
