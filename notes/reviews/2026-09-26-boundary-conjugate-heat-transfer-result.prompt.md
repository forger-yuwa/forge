...
1. **実装 diff の検証**: `git diff 6b7c6f4a...HEAD -- solver_density_cuda design methods procedures` (必要なら `git log 6b7c6f4a..HEAD --oneline`)
...
8 巡目。7 巡目以降の変更: V2 case/48 の判定と (ii) 1 点の例外受け入れ (ユーザ決定)、V6′ (case/58) の実装・判定 (§5.1 #86–#103、§6 V6′ と閉じる条件)。閉じる条件 3 つが揃ったとして status: done → accepted/ に移してよいかを判定すること。V6′ は範囲限定の PASS (§6 閉じる条件 2 の 1 文)。solver 変更は conjugate.node_log (診断出力のみ) と check_cht_interface.py --band-y、評価器 eval_v6p.py / sens_v6p.py
