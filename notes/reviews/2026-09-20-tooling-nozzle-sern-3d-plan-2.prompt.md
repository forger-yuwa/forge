...
§4.35 R5h: カウル後縁が厚さ0で閉じている件の診断と案(a)有限厚+cowl_base vs 案(b)後縁だけ壁層を粗く、の選択。§4.34 R5g: プラトーをリミットサイクルと断じ受理条件を残差から力の平均±振幅へ書き直す判断。§4.32 側壁のy+1はARと両立しないという結論。
...
### 4.34 R5g: プラトーは**ノイズ床ではなく小スケールのリミットサイクル**。力は 7 桁で定常 (2026-09-20)
...
### 4.35 R5h の正体: **カウル後縁は板厚ゼロで閉じている** — 壁解像がそこを壊した (2026-09-20)
...
| ~~**R5g**~~ **完了 (2026-09-20, §4.34)**: プラトーは**ノイズ床でない** (場は床の 3–62 倍動く) が、**減衰を止めて振動している = 小スケールのリミットサイクル** (振幅は床の 8–35 倍)。**力はそれでも 7 桁で定常** (`C_T_with_shear` 0.8595940 ± 8.0e-07)。**受理条件を「残差を下げる」から「派生量の定常性 + 力の平均±振幅 + リミットサイクル振幅/ノイズ床」へ書き直す** | `run_0417_noise_r1`–`r5` |
| **R5h** | **カウル後縁が板厚 0 で閉じている** (§4.35)。`np.interp(..., [t, t, 0])` で x = `L_cowl` の 25 z-station すべてが厚さ **0.000e+00** = 厚さ 0 スリット。第 1 層 4.0e-04 ではならされていたが **1.6e-05 にすると 2772 K と 1000 K が 3.2e-05 m で向き合い状態が潰れる** (T 37.6 K / ρ 0.671)。粗メッシュには**無い** (T 最小はプルーム過膨張の 157.9 K)。**推奨は案 (a): 後縁に有限厚 + `cowl_base` タグ** (機体ベース R4e 案 (d) と同じ型)。新タグなので **codex plan レビューを先に通す** | `mesh_sern3d.py` の `tk` 法則 / `runner_sern3d.py` の力の帳簿 |
