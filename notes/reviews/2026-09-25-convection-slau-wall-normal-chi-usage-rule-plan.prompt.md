...
対象の plan は下に全文を貼る (`plans/active/convection-slau-wall-normal-chi-usage-rule.md`)。これから実装に入る前の段階なので、次を順に評価せよ。
...
## plan 全文 (`plans/active/convection-slau-wall-normal-chi-usage-rule.md`)
...
## 参考: `plans/accepted/convection-slau-wall-normal-chi.md`
...
| R3-M3 | SERN 衝撃足は差系列だけで、各 run の準定常が未判定。$C_L$ の「実効果」を DRIFTING のまま確定 | 各 run の量 (絶対 $x_{foot}$・窓内壁圧) を判定 (#14)、$C_L$ の確定を撤回 | §5.1 #14、`case/46.sern_design/_r3_m1m3/PERRUN_QS_flag{0,1}.txt` | **閉** (両 run ALL STEADY、窓内分布の偏差上限 0.063 % / 0.035 % も追記 (result-4 m1)。$C_L$/$C_M$ は #11 の前提) |
