...
に帰属**させる (`gmshReader::buildMedianDual` / `buildMedianDual3D` (2026-09-07 追加, 同型), `inletCornerWall`)。角 CV の境界は壁半割面 (pressure-only, 流入なし) だけに
...
- **フォールバックの設計判断**: 退化方向の勾配成分は**ゼロ化** (その方向 1 次精度化)。GG 係数への差し替えでなく
  ...
  少数 (case/29 で 2.6%) の退化ノードの再構成勾配 1 次化は安全側。閾値は**コードに固定した 1e-2** (2026-09-18, plan [config-key-pruning](../plans/accepted/config-key-pruning.md)。旧 `mesh.gradLSQDegenThresh` は掃引の実測が無く使用実績もゼロだったため撤去。書くと起動時エラーになる)。
