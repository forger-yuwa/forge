...
| 量 | 低温側 | 高温側 |
| --- | --- | --- |
| N2 $L$ | 70 K 未満は Kirchhoff の線形 $L(70)+(c_{p,v}-c_l)(T-70)$、$c_l$=`condN2LiquidCp` (既定 2000)。$c_{p,v}$ は**凝縮する蒸気**の値で、pure-condensible CPG では `physProp.cp`、それ以外は N2 の 1038.8 (下記) | $T_c-0.5=125.69$ K でクランプ |
...
**$c_{p,v}$ の出どころ (2026-09-14)**: Kirchhoff の傾きに入る $c_{p,v}$ は**凝縮する蒸気**の比熱であって、
混合気の比熱ではない。したがって `physProp.cp` を無条件に使うのは誤りで、
**pure-condensible CPG (気相そのものが凝縮種) のときだけ** `physProp.cp` を渡す (`CondPropOpts::gasCp`)。
CPG carrier (空気) の `physProp.cp` は混合気の 1008.7 J/(kg·K) で N2 蒸気の 1038.8 ではないため渡さない
(渡すと 70 K 未満で $L$ が 0.45 % 変わり、$T_{on}\approx38$ K の onset が動く)。
なお二相 EOS 側は液相の顕熱に**混合気**の $c_v+R_w$ を当てている (空気で 1014.7 対 N2 の 1038.8) ので、
EOS 上の実効 $c_l$ は意図した 2000 ではなく 1976 になる (1.2 %)。これは CPG carrier 形の定式化そのものの近似で、
$c_{p,v}$ の出どころとは別の話。
