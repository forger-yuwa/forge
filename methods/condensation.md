...
| H2O $h_l$ | 273.15 K 未満は $c_{p,l}=4228$ J/(kg·K) 一定の線形外挿 (過冷却水) | **373.15 K でクランプ** |
| H2O $h_v$ | 200 K 未満は $c_p(200)$ 一定の線形外挿 (**種 DB と同規約**, 2026-09-14) | 1000 K 超は $c_p(1000)$ 一定の線形外挿 (同上) |
| H2O $L$ | 上の差。$[1.5,\,3.5]$ MJ/kg でクランプ (1000 K で上限に当たるので $h_v$ の高温外挿は場に出ない) | |
...
2. **H2O の $L$ は 373 K 以上で増えてしまう** (未解決, `plans/active/condensation-followups.md` F-cf5)。$h_l$ がクランプするので実質 $c_{p,l}=0$ となり、
   $dL/dT=c_{p,v}-c_{p,l}$ が $+1896$ J/(kg·K) に化けて符号が反転する。$L(400)=2.320$ MJ/kg (真値 2.183, $+6\%$)、
   $L(600)=2.711$ MJ/kg (本来は $T_c=647$ K で 0 に向かう)。凝縮が起きない温度域なので場には効かないが、
   $g>0$ のセルが高温へ飛ぶと潜熱が過大になる (蒸発の潜熱吸収が過大 → 過冷却しすぎる)。ずれは 473 K で $+27\%$
   (2.460 対 真値 1.940 MJ/kg)、573 K で $+89\%$ (2.657 対 1.404)、$T_c=647$ K では 0 であるべきところ 2.807 MJ/kg。
   1000 K 以上は $[1.5,3.5]$ MJ/kg クランプの上限に張り付く。
   直すなら Watson 型 $L(T)=L(373.15)\,[(T_c-T)/(T_c-373.15)]^{0.38}$ で臨界点まで伸ばす (蒸気表と 473 K で 1.5 %、
   573 K で 1.6 % 一致)。現行ケースは Wyslouzil (全温 ~300 K) も Arthur (N2) も 373 K 未満なので場には出ていないが、
   高全温の風洞 (case/44 は $T_t=1161$ K) で H2O 凝縮を入れると踏む。
