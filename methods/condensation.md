...
**$c_{p,v}$ の出どころ (2026-09-14)**: **CPG (`thermalMethod: 0`) では config の `physProp.cp` をそのまま使う**
(`CondPropOpts::gasCp`)。CPG は「気相の比熱はこの 1 つの定数」というモデルなので、物性相関だけ別の定数を
持たせると同じ run の中に 2 つの $c_p$ が並ぶ。TP (`thermalMethod: 2`) には気相の $c_p$ 定数が無いので、
内蔵の種固有値 (N2 1038.8 / H2O 1855 J/(kg·K)) を使う。

空気キャリアでは `physProp.cp` が混合気の 1008.7 で、窒素蒸気の 1038.8 とは 2.9 % 違う。厳密には
Kirchhoff の関係に入るべきは凝縮する窒素蒸気の値だが、CPG 二相 EOS は液相の顕熱にも混合気の熱容量
($c_v+R_w$ = 1014.7) を当てており、**そもそも凝縮種固有の $c_p$ を表現できない**。config の値に統一するのは
その枠内での自己整合を取る選択で、影響は次のとおり (case/34 空気 node, 12000 step の A/B)。

| 量 | 1038.8 固定 (旧) | `physProp.cp` = 1008.7 (現行) |
| --- | --- | --- |
| $L$ (38 K) | 240.1 kJ/kg | 241.1 kJ/kg (+0.40 %) |
| $p_{sat}$ (38 K) | — | ×0.983 |
| onset ($\Delta p/p_{dry}>1\%$) | 2.20 in | **2.20 in (変化なし)** |
| onset ($g>10^{-4}$) | 2.10 in | **2.10 in (変化なし)** |
| $g_{exit}$ | 0.0583 | 0.0581 (−0.3 %) |
| 液滴モーメント $Q_n$ | — | 最大 2.4 % 変化 (ノイズ床超) |

**検証されている量 (onset) は動かない**。純窒素の run は `physProp.cp: 1038.8` で内蔵値と同じなので
ノイズ床以内で不変。液相比熱 $c_l$ 自体が 30–60 K では測定が無く $\pm500$ J/(kg·K) の感度幅で扱う閉包
(下の「既知の限界」) なので、$c_{p,v}$ の 30 J/(kg·K) の差はその内側にある。
