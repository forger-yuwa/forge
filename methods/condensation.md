...
- **Kantrowitz 非等温補正** (H2O): 生成中の臨界核が放出潜熱を捨てきれず自己加熱し $J$ が下がる効果を
  ...
  **2026-09-10 以前の実装はセルの気相混合 $\gamma=c_{p,gas}/c_{v,gas}$ (H2O–N2 では N2 支配 ≈1.40) を使っていた**
  ...
  carrier 拡張 ($b^2$ に $p_c/p_v\sqrt{m_v/m_c}$ 重みの項が加わり $\theta$ が大幅に小さくなる; Wedekind et al.) は `condKantrowitz 2/3` として実装済み (次節「carrier 中の非等温核生成補正 (Feder 形)」; 既定は 0 のまま)。
...
- **carrier 中の非等温補正 (Feder 形, `condKantrowitz: 2, 3`; 計画 [condensation-kantrowitz-carrier](../plans/accepted/condensation-kantrowitz-carrier.md))**:
...
#### 潜熱 $L(T)$ と飽和圧の低温整合 (`condN2LatentLowT`, 計画 [condensation-air](../plans/accepted/condensation-air.md))
...
### 8b. 物性の外挿規約 — 有効域の外で何を返すか

非平衡凝縮は原理的に**フィットの有効域の外**で使う。相関は実験できる領域 (気液共存線、三重点〜臨界点) で作られているが、
ノズルの凝縮は三重点以下の過冷却液で起きるからである (§8「相の方針」: 全域で液相フィットを使い固相へ切り替えない)。
したがって「有効域外で何を返すか」がモデルの一部になる。規約は**熱力学的に繋がる相手がいるかどうか**で 3 通りに分かれる。

| 型 | 対象 | 外挿の根拠 |
| --- | --- | --- |
| **関係式で伸ばす** | N2 の $L$、N2 の $p_{sat}$ | Kirchhoff / Clausius–Clapeyron。互いに整合する |
| **定 $c_p$ で伸ばす** | 気相 NASA-9 の $c_p, h, s^\circ$、H2O の $h_l$ | 端で $c_p$ を凍結し、$h$ と $s^\circ$ をそれと整合する形で積分 |
| **端で凍結する** | N2 の $\sigma, \rho_l$ | 拘束する相手が無い。安全側に倒す設計判断 |

#### 気相 (NASA-9, `thermo_d.cuh`) — 定 $c_p$ 外挿で 3 量を揃える

多項式そのものは外挿しない (低温で発散する)。$T\notin[T_{lo},T_{hi}]$ では端の温度 $T_b$ で $c_p$ を凍結し、

$$c_p(T)=c_p(T_b),\qquad h(T)=h(T_b)+c_p(T_b)(T-T_b),\qquad s^\circ(T)=s^\circ(T_b)+c_p(T_b)\ln\frac{T}{T_b}$$

とする。$s^\circ$ を一定にクランプしていた旧実装は $h$ の定 $c_p$ 外挿と熱力学的に非整合で、200 K 未満の等エントロピー関係を
壊していた (TP の低温不安定の一因)。float ミラー (`SpeciesThermoF`) も分岐を同一にしてある。
NASA-9 は多くの種で $T_{lo}=200$ K なので、出口が 200 K を割るノズルでは全域が定 $c_p$ 外挿になり実質 CPG と変わらない
(冷間ケースで CPG を勧める理由)。

#### 凝縮物性 (`condensationProperties_d.cuh`)

| 量 | 低温側 | 高温側 |
| --- | --- | --- |
| N2 $L$ | 70 K 未満は Kirchhoff の線形 $L(70)+(c_{p,v}-c_l)(T-70)$、$c_l$=`condN2LiquidCp` (既定 2000)。$c_{p,v}$ は CPG なら `physProp.cp`、TP なら内蔵の 1038.8 (下記) | $T_c-0.5=125.69$ K でクランプ |
| N2 $p_{sat}$ | 50 K 未満は上の $L(T)$ を積分した Clausius–Clapeyron ($T$ は 5 K まで追随) | 同上 |
| N2 $\sigma,\rho_l$ | **45 K で凍結** (`COND_T_PROP_FLOOR`) | 同上 |
| H2O $p_{sat}$ | Murphy–Koop、120 K でクランプ | — |
| H2O $h_l$ | 273.15 K 未満は $c_{p,l}=4228$ J/(kg·K) 一定の線形外挿 (過冷却水) | 373.15 K でフィット終了。ここから先は $h_l$ ではなく $L$ を直接外挿する (下行) |
| H2O $h_v$ | 200 K 未満は $c_p(200)$ 一定の線形外挿 (**種 DB と同規約**, 2026-09-14) | 1000 K 超は $c_p(1000)$ 一定の線形外挿 (同上) |
| H2O $L$ | 373.15 K 以下は上の差 ($[1.5,\,3.5]$ MJ/kg の安全網つき。45–373 K では 2.27–3.04 MJ/kg で当たらない) | **373.15 K 超は Watson 型** $L=L(373.15)[(T_c-T)/(T_c-373.15)]^{0.38}$、646.15 K で凍結 (下記) |
| H2O $\rho_l$ | $1000-0.12(277-T)$、920 kg/m³ で床 | |

**N2 の潜熱 (70 K 未満)**: Kirchhoff の関係 $dL/dT=c_{p,v}-c_l$ を定数係数で積分した直線。傾きは $-961.2$ J/(kg·K²) で、
1 K 冷えるごとに $L$ が 961 J/kg 増える (低温ほど蒸発に要るエネルギーが大きい、という正しい向き)。
70 K で切るのは物理ではなくフィットの都合で、元の 4 次多項式は 70–126 K のフィットなので 60 K 未満で $dL/dT>0$ に符号反転する。
値だけ繋ぐ C0 接続で、傾きは不連続 (下記の暗黙 $c_l$ が 2110 → 2000 に 5.2 % 跳ぶ)。

**N2 の飽和圧 (50 K 未満)**: 上の $L(T)$ をそのまま Clausius–Clapeyron $d\ln p/dT=L/(R_vT^2)$ に入れて解析積分したもの。
$L=L_a+c_{pr}(T-T_a)$、$c_{pr}=c_{p,v}-c_l$ とすると

$$\ln\frac{p}{p_{50}}=\frac{1}{R_v}\Big[(L_a-c_{pr}T_a)\Big(\frac{1}{50}-\frac{1}{T}\Big)+c_{pr}\ln\frac{T}{50}\Big]$$

となり、コードの式に一致する。**潜熱の外挿と飽和圧の外挿が同じ $L$ を共有している**のが要点。
50 K で切るのは Jacobsen 相関が三重点 63.15 K 付近までのフィットで、低温外挿すると 33 K 付近で $dp/dT<0$ (非単調) になるため。

**$\sigma$ と $\rho_l$ の凍結**: この 2 つには $L$–$p_{sat}$ のような熱力学的拘束が無い ($\sigma$ は表面過剰量、$\rho_l$ は状態方程式)。
整合させる相手がいないので、液相フィットの有効域の下端 45 K で止める。3 つの中で唯一、物理ではなく安全側の設計判断。

#### $L(T)$ が液相の熱力学を決めている (CPG 経路)

CPG の二相 EOS は $e=(c_v+gR)T-gL(T)$ で、液相の物性を独立に与えていない。液のエンタルピーは
$h_l\equiv h_v-L=c_{p,v}T-L(T)$ として**暗黙に定義**され、液の比熱も $c_l=c_{p,v}-dL/dT$ として $L$ の傾きから決まる。

| $T$ [K] | 45–70 | 70 | 90 | 110 | 125 |
| --- | --- | --- | --- | --- | --- |
| 暗黙の $c_l$ [J/(kg·K)] | 2000 (定数) | 2110 | 2746 | 4613 | 9289 |

臨界点に近づくと発散する形になっており、液体として正しい振る舞い。実測の液体 N2 は 63 K で 2010、77 K で 2040 J/(kg·K) なので
70 K 近傍も妥当。**$L(T)$ のフィットを差し替えると液相の比熱まで変わる**ことに注意する。

**$c_{p,v}$ の出どころ (2026-09-14, [`plans/active/condensation-vapour-cp-source.md`](../plans/active/condensation-vapour-cp-source.md))**:
**CPG (`thermalMethod: 0`) では config の `physProp.cp`、TP では内蔵の種固有値** (N2 1038.8 / H2O 1855 J/(kg·K))。
CPG は「気相の比熱はこの 1 つの定数」というモデルなので、物性相関だけ別の定数を持たせない。

**`CondSpeciesProps::cp`/`cv`/`R` は上書きしない**。これは種固有の整合した組で、Kantrowitz の非等温補正が
$\gamma_v=c_p/c_v$ を使う。Kirchhoff の傾き専用に `kirchhoffCpv` を別に持つ。混ぜると空気で
$c_p-c_v=266.7\neq R$、$\gamma_v=1.359$ となり補正係数 $2(\gamma_v-1)/(\gamma_v+1)$ が 0.3333 → 0.3047 に動く。

空気キャリアでは `physProp.cp` が混合気の 1008.7 で、窒素蒸気の 1038.8 とは 2.9 % 違う。厳密には Kirchhoff に
入るべきは凝縮する窒素蒸気の値だが、CPG 二相 EOS は液相の顕熱にも混合気の熱容量 ($c_v+R_w$ = 1014.7) を
当てており、**そもそも凝縮種固有の $c_p$ を表現できない**。config に統一するのはその枠内で整合を取る選択だが、
**厳密な熱力学的整合ではない**: 空気では $c_v+R_w=1017.3$ なので 70 K 未満の実効液比熱は
$1017.3-(1008.7-2000)=2008.6$ J/(kg·K) となり、意図した 2000 から +0.43 % 残る。
差 30 J/(kg·K) は液相比熱 $c_l$ 自体の感度幅 ($\pm500$) の内側。実測 (case/34, 12000 step):

| 量 | 1038.8 固定 (旧) | `physProp.cp` = 1008.7 (現行) |
| --- | --- | --- |
| $L$ (38 K) | 240.1 kJ/kg | 241.1 kJ/kg (+0.40 %) |
| $p_{sat}$ (38 K) | — | ×0.983 |
| onset ($\Delta p/p_{dry}>1\%$) node / cell | 2.20 / 2.21 in | **2.20 / 2.21 in** |
| onset ($g>10^{-4}$) node / cell | 2.10 / 2.13 in | **2.10 / 2.13 in** |
| $g_{exit}$ node / cell | 0.0583 / 0.0590 | 0.0581 / 0.0589 |
| 保存モーメント $\rho Q_0$ の最大差/基準場最大値 | — | node 2.43 %, cell 2.92 % |

**検証されている量 (onset) は、格子刻みと同 run の時間変動の分解能内で動かない**。node は末尾窓 (step ≥ 8000) で
両 onset とも完全に一定、cell は圧力 onset が基準側で 0.020 in 揺れる (各 run の `ONSET_SERIES.txt`)。
連続位置まで不変という主張ではない。純窒素の run は `physProp.cp: 1038.8` で内蔵値と同じなのでノイズ床以内で不変。

#### 既知の限界

1. **N2 の 45 K 凍結は生産域に入っている。** case/34 の空気凝縮は $T_{on}=39.7$ K で onset するので、
   $\sigma$ と $\rho_l$ が 45 K の値で止まったまま核生成している。凍結を外して相関を延長すると $\sigma$ が
   39.2 K で $+9\%$ (16.75 → 18.26 mN/m)、$\Delta G^*\propto\sigma^3$ が指数に乗るので onset 点で $J$ が $1.2\times10^{-6}$ 倍になる。
   onset は数 mm 下流へ動く見込みで、現在の実験一致 (対応理論線と 1.7 K) と同オーダー。延長するなら
   `condSigmaScale` と同じ形の opt-in にして onset 感度とセットで測ること (σ×1.03 で onset ∓2.3 mm の既存データあり)。
2. ~~**H2O の $L$ は 373 K 以上で増えてしまう。**~~ **決着 (2026-09-14, 下の「373 K 超の $L$」)**。
3. ~~**H2O の $h_v$ が二重ソースになっている。**~~ **決着 (2026-09-14)**: `h2o_latent` 内の $h_v$ を種 DB と同じ
   「有効域端の $c_p$ 一定で線形外挿」に揃えた (`h2o_gas_h_mass`)。係数は元から同一なので、これで 200 K 未満でも
   同じ温度に対する $h_v$ が 1 通りになり、暗黙の $h_l$ は CEA の液相フィットそのものになる。
   $L$ の変化は運転域 200–240 K で $1.4\times10^{-6}$ 以下、表の下端 120 K で $8.4\times10^{-4}$。
   あわせて 273.15 K 未満の $c_{p,l}$ を $\pm0.5$ K 差分 (液相フィットの有効域外を踏んでいた) から解析形の
   $c_p(273.15)=4228.27$ J/(kg·K) に変えた (差 0.05 J/(kg·K))。**種 DB ポインタは不要**で、外挿規約を合わせるだけで済んだ。
   検査は `tests/unit/test_cond_air.cpp` (h): 200 K 未満で暗黙の $c_l$ が 4228 一定であること、200 K で $dL/dT$ が連続であること。
4. **液の $pv$ 項を落としている。** $e_l\approx h_l$ とする近似で、無視量は $p/(\rho_l L)$。
   N2 の onset 実測点 (800 Pa) で $3.5\times10^{-6}$、case/16 の凝縮域 (20 kPa) で $7.7\times10^{-6}$。
   $10^{-3}$ を超えるのは N2 で 1.4 bar、水で 26 bar から。臨界直下は $L\to0$ のため最悪で N2 125 K では 15 %。
   超音速ノズルの凝縮は必ず膨張後の低圧側で起きるので用途とは整合しているが、高圧凝縮 (蒸気タービン等) には使えない。

#### 373 K 超の $L$ — Watson 外挿 (2026-09-14)

液相フィット (CEA H2O(L)) は 373.15 K までしか無い。**2026-09-14 以前は $h_l$ をそこでクランプ**していたため、
$dL/dT=c_{p,v}-c_{p,l}$ が $c_{p,l}$ を失って $+1890$ J/(kg·K) になり、$L$ が温度とともに**増加**していた
(473 K で 2.460 対 真値 1.940 MJ/kg、573 K で 2.657 対 1.404、$T_c=647.096$ K で 0 であるべきところ 2.807)。

現行は $h_l$ を伸ばすのをやめ、$L$ 自体を Watson 型で外挿する:

$$L(T)=L(T_0)\Big(\frac{T_c-T}{T_c-T_0}\Big)^{0.38},\qquad T_0=373.15\ \text{K},\ T_c=647.096\ \text{K}$$

アンカー $L(T_0)=2{,}269{,}079$ J/kg は従来の CEA 差そのままなので 373.15 K で値は連続。指数 0.38 は Watson の
標準値で、IAPWS 飽和線の $h_g-h_f$ に対する誤差は 400 K で $-0.6\%$、473 K で $-1.6\%$、523 K で $-2.2\%$、
573 K で $-1.7\%$、623 K で $+0.6\%$ (rms 1.4 %)。0.28–0.40 を振ると 0.38 が最小。

**傾きは 373.15 K で不連続**: 直下 $-2327$、直上 $-3148$ J/(kg·K²) で、暗黙の液比熱が $4217\to5038$ J/(kg·K) と
19 % 跳ぶ。傾きまで繋ぐには指数 0.281 が要るが、それだと 573 K で $+12\%$、623 K で $+28\%$ と遠方が悪化するため、
遠方の精度を取った (373 K に液相が存在するケースが先に無い)。

**$T_{fr}=646.15$ K で凍結する**。$L\to0$ は物理的には正しいが、(a) 成長則の前因子 $kRT^2/(\rho_l L^2)$ が
ゼロ除算になり、(b) $dL/dT\to-\infty$ が 0.25 K 刻みの float 物性表の最終区間を壊す (647.0 K で $-24\%$)。
$T_{fr}$ は**物性表の格子点そのもの** ($(646.15-120.15)/0.25=2104$) なので、表の片側微分構成が折れを正確に表す。
凍結値は 263.3 kJ/kg で $1/L^2$ は $L(373)$ 比 74 倍に収まる。**$T\ge T_c$ に液相は存在し得ないので、この
0.95 K の帯はモデルの定義域外**であり、凍結値に物理的意味は無い (数値ガード)。

表の再現誤差は、host で区間を 16 分割して評価すると 374–600 K で 5.8e−8、600–640 K で 5.4e−8、
640–647.2 K で 7.1e−6、647.2 K 以上で 3.7e−8。**実際の単体試験 (`test_cond_float.cpp`) の値は 646.04 K で
2.37e−5** で、こちらは入力温度を float に丸めた上で別の正規化 (`|Δ|/(|L|)`) を使うので一致しない。
どちらも既存の許容 1e−4 の内側。$dL/dT$ の比較は凍結点をまたぐ 645.8–646.5 K を除いて 7.43e−5 (許容 1e−4)。

$T>T_c$ に $g>0$ が残る状態 (強制全蒸発にするか反転失敗として弾くか) は未決で、
`plans/active/condensation-followups.md` の F-cf10 に残してある。

#### TP のときの潜熱とエンタルピー

**TP でも潜熱は種 DB を使わない。** 気相エンタルピーは種 DB の NASA-9 混合 (`thermo_cph_mix`, datum は `thermoHrefTemp`)、
潜熱は凝縮種の独立相関 (`cond_latent`) から来る。したがって $L$ の外挿規約は CPG でも TP でも同じで、違うのは $e_v$ の側だけになる。

$$e = e_v^{\text{全蒸気}}(Y,T) + g\,(R_w T - L(T)),\qquad p = \rho T\,(R_{mix} - g R_w)$$

| | H2O (TP carrier) | N2 (TP carrier) |
| --- | --- | --- |
| $L$ の出どころ | $h_v-h_l$ の差 (自己完結の CEA 多項式) | $L(T)$ の直接フィット |
| 凝縮域 | 200–240 K | 40–80 K |
| 種 DB の $T_{lo}$ | 200 K | 200 K |
| 凝縮域と $T_{lo}$ の関係 | ちょうど境界。上は多項式、下は定 $c_p$。$c_p$ は 1851 → 1855 J/(kg·K) とほぼ平ら | **全域が定 $c_p$ 外挿**。$c_p(200\,\mathrm{K})=1039.0$ J/(kg·K) |
| $L$ との整合 | 二重ソース (既知の限界 3)。180 K で 0.04 kJ/kg | $c_p(200)=1039.0$ と $L$ フィットの前提 $c_{p,v}=1038.8$ が **0.02 % 一致** |

N2 を TP で凝縮させると、凝縮域 (40–80 K) は NASA-9 の $T_{lo}=200$ K より 120–160 K 下なので**全域が定 $c_p$ 外挿**になる。
その $c_p(200\,\mathrm{K})=1039.0$ J/(kg·K) は $L$ フィットが仮定する $c_{p,v}=1038.8$ と 0.02 % しか違わないので、
TP の N2 は低温で自動的に CPG の N2 に縮退し、暗黙の液相熱力学 ($h_l=h_v-L$) も整合する。
ただし TP × N2 凝縮の実ケースは無く (case/34 は全て CPG)、検証されていない。

**実際に使われている組合せ**: TP + H2O carrier (`condGasSpecies`) が case/16 生産 run と case/44、
CPG pure N2 と CPG carrier 空気 (`condVaporMassFraction`) が case/34。
なお CPG に `condGasSpecies` を与える組合せは検証で弾かれず、ソース項と EOS が食い違う
(followups F-cf7; 該当は使用停止済みの旧 run のみ)。

#### float 経路との関係

物性表 (実装 §9) は**この外挿規約を含めて** double 関数からサンプルするので、表を引く限り float 経路と double 経路の
外挿挙動は同一になる。表の範囲外 (N2 20–125.6 K、H2O 120.15–647 K の外) は表を使わず旧 double 関数を呼ぶので、
そこでも規約は変わらない。判定は数値ヤコビアンの摂動点 $T+0.1$ K を含めて行う。
...
#### 空気: CPG carrier 形 (N2 選択凝縮 + O2 キャリア; `condVaporMassFraction`, 計画 [condensation-air](../plans/accepted/condensation-air.md))
...
- 温度反転 `cond_T_from_e_cpg` は非有限の $e,g,T_{guess}$・非正の熱容量を反転不能 (ok=false) とし、成功条件にも $T,G$ の有限性を要求する ($e=\pm\infty$ は tol=∞ で「成功」に化けていた; codex 2026-09-13)。失敗セルでは $T,p,c,H_t$ を前ステップ値のまま残し、$\rho e$ を反転結果で上書きしない (密度床と速度の更新は通常どおり先に済んでいる) — `[condensation] WARNING: CPG two-phase temperature inversion failed in N cells` を出す。
...
- pure TP: $R_{eff}=(1-g)R_v$、$c_p^{全蒸気}=c_p(T)$。**CPG 分岐 (`thermalMethod 0`, pure N2 case/34) は旧式のまま**:
  ...
  (codex 指摘 2026-09-10)。潜熱の低温整合 (`condN2LatentLowT`, 上記) で $c_l>0$ は解消済みで、残る理由は CPG 分岐の一般 EOS 固有系・Jacobian への切替 (γ_2φ, 実 $H_t$) を二相音速と一括で行う必要があること (後続 [condensation-followups](../plans/active/condensation-followups.md) #6)。
