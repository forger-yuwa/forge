...
当てており、**そもそも凝縮種固有の $c_p$ を表現できない**。config に統一するのはその枠内で自己整合を取る選択で、
...
| $p_{sat}$ (38 K) | — | ×0.983 |
| onset ($\Delta p/p_{dry}>1\%$) node / cell | 2.20 / 2.21 in | **2.20 / 2.21 in (変化なし)** |
| onset ($g>10^{-4}$) node / cell | 2.10 / 2.13 in | **2.10 / 2.13 in (変化なし)** |
| $g_{exit}$ node / cell | 0.0583 / 0.0590 | 0.0581 / 0.0589 |
| 保存モーメント $\rho Q_0$ の最大差/基準場最大値 | — | node 2.43 %, cell 2.92 % |

**検証されている量 (onset) は動かない**。純窒素の run は `physProp.cp: 1038.8` で内蔵値と同じなのでノイズ床以内で不変。
