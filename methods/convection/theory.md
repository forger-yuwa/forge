...
$\widehat{M}$ は**速度ベクトルの大きさ**で組む (面法線成分ではない)。これは原典が多次元マッハ数を
数値実験にもとづいて選んだ結果である。

**既知の限界 (2026-09-23, case/46 で実測)**: node 方式で `nodeWallDirichlet: 1` の壁ノード ($\mathbf u = 0$) が、
**接線速度の大きい内点**と面を共有すると、$\widehat M$ が $|\mathbf u|$ で決まるために $\chi = 0$ となり、
**この面の圧力差項が恒等的に消える**。壁ノードは $V_n=0$ で移流の流入経路を持たないので、
壁 CV は補充を失って指数的に排出される。実測 (接続模型の側壁後端面、面法線速度 276 m/s・接線込み 1316 m/s・
