...
def van_driest_ii(M, T_inf, T_w, air, Re_theta):
    """圧縮性 Cf を返す。Fc / F_theta は Hopkins-Inouye の表記。"""
    gam = air.cp(T_inf) / (air.cp(T_inf) - air.R)
    m = 0.5 * (gam - 1.0) * M ** 2
    T_aw = T_inf * (1.0 + R_FACTOR * m)
    tw = T_w / T_inf
    A = np.sqrt(R_FACTOR * m / tw)
    B = (1.0 + R_FACTOR * m) / tw - 1.0
    den = np.sqrt(B ** 2 + 4.0 * A ** 2)
    Fc = R_FACTOR * m / (np.arcsin((2.0 * A ** 2 - B) / den) + np.arcsin(B / den)) ** 2
...
def h_vandriest(M, fs, T_w, air, theta):
    ...
    Cf, T_aw, Fc, F_th = van_driest_ii(M, fs["T"], T_w, air, Re_theta)
...
def main():
    ...
    for T_w in (300.0, 400.0, 480.0):
        ...
        for s in der["series"]:
            ...
            theta = theta_of[key]
            fs = dict(ro=s["ro_inf"], U=s["U_inf"], mu=s["mu_inf"], T=s["T_inf"])
