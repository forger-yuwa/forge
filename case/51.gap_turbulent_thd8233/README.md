...
> **$z/W$ 3.0 の値は float32 の量子化床である (2026-09-22 に測定で確定)**。最終 step の後壁 $q_w$ は
> $z/W$ 3〜18 で **0.7211 W/m² に完全に張り付き**、値は 1 ulp 刻みに量子化されている。階段幅の実測
> 1.717e−2 W/m² は $k\,\Delta T_{1\rm ulp}/y_1$ = 2.5726e−2 × 3.052e−5 / 4.580e−5 = **1.714e−2** と **0.2 % 一致**
> (床 = 42 ulp)。$h/h_{fp}$ に直すと **7.24e−4** で、抽出値 7.42e−4 と同一。
> 減衰率も 0.7→1.4 が 3.11/W と**スロット純伝導の固有値 $\pi/W$ = 3.14 に一致**したあと、1.4→2.2 で 2.68/W、
> 2.2→3.0 で 1.25/W、3.0 以深で **−0.01/W (= 完全に平坦)** と床へ倒れる。
> → **$z/W$ 3.0 は上界として「≤ 7.4e−4」と書く**。$z/W$ 2.2 (床の 2.4 倍) は床の上だが余裕は小さく、
> $z/W$ 1.4 (床の 22 倍) は影響を受けない。**分類 α には影響しない** (真値が床以下なら比はさらに小さくなる側)。
...
**原典の還元手順を確認した (2026-09-22)**: TN D-8233 p.7 "Data Reduction" は、投入直後の **0.5 s を捨て**、
**続く 3 秒間**に最小二乗で 2 次曲線を当て、**その初点 ($t$ = 0.5 s) で傾き $\partial T_w/\partial t$ を解析的に評価**した、
と書いている。さらに「長い時間窓による伝導の影響は代表データで **10 % 未満**」とも明記している。
→ **$t$ = 0.5 s で帯を取り直すと係数は 1.073 / 1.038 / 1.313** と、仮置き (0.25–5 s の 1.512 / 1.725 / 2.689) より
**1.4–2 倍狭い**。$z/W$ 1.4・2.2 の補正量 7 % / 4 % は原典の「< 10 %」とも整合する。
仮置きの広い帯 (保守側) で既に α だったので、狭い側でも同じ結論になる。

**事前登録ゲートの結果**:

> **T2p-Q の基準は原典の記述そのもの**: TN D-8233 p.8 は「downstream wall heat transfer is always higher than
> that to the upstream gap wall for depths into the gap of less than four gap widths ($z/w<4$)」「深部 ($z/w>5$) では
> 両壁がほぼ等しい」と書いている。したがって登録した基準は恣意的なものではない。
