...
| `run_0002_t2p_g51_viscnum` | **A/B で真因を確定させた run** (AWS)。`run_0001` から**変えたのは 1 箇所だけ**: `physProp.visc` を μ(T∞)=3.41e-6 → **2.4e-5** (= μ_Sutherland(425 K)、近壁の代表値)。メッシュ・初期場・CFL・`nStepInner`・`implicitRelax` は同一 | **暖機 6 段すべて完走** (lam/soft/mid 3000 step、ramp0/1/2 2500 step、NaN 無し)。本段 400k step を走行中 | active |
