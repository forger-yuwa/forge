...
## 前処理 A/B の決着 (2026-09-19) — `lowMachPrecond: 2` の場は非物理

下の「留保」で挙げた前処理依存 (平均比 0.580 → 0.258) は、**precond 2 側が壊れている**と判定した。

| 診断 | precond 0 | precond 2 |
| --- | --- | --- |
| 閉じたすきま断面の質量流束 $\int\rho U_y dx$ ($z/W$=5) [kg/(m·s)] | **−2.7e−7** (丸め) | **+1.85e−4** |
...
しかも温度差はゼロ — 駆動キャビティの構造になっていない。**→ precond 2 の低い値は採用しない**。
(memory [node-precond-boundary-freeze-and-tp-outlet-gamma] の境界凍結と同系統の疑い。閉領域内部での発現)

**したがって本 case の結果は `lowMachPrecond: 0` (既定) を採る**。残る未解決は**格子依存のみ** (run_0016)。

## ⚠ 留保 (2026-09-19) — 深部の数値感度

**`lowMachPrecond` を 0 → 2 に変えるだけで、同じ格子・同じ場から、すきま内部の熱流束が
平均で 2.2 倍・局所で 1–2 桁変わる** (run_0013)。両方とも `check_quasisteady` は STEADY。

| $x/d$ | 実験 | 理論 | precond 0 | precond 2 | 比 (2/0) |
| --- | --- | --- | --- | --- | --- |
| 0.010 | 0.55 | 0.469 | 0.829 | 0.629 | 0.76 |
| 0.030 | 0.42 | 0.187 | 0.221 | 0.023 | **0.11** |
| 0.050 | 0.34 | 0.113 | 0.066 | 0.0002 | ~0 |
| 平均比 | 1.07 | 0.576 | **0.580** | **0.258** | **0.44** |

- **外部流は無傷**: 上流平板の $q_w$ は $x$=20–150 mm で **−0.4〜−0.6 %** しか変わらない
  (T1 precond0 は T0 と 1.000–1.008 で一致)。→ **変化はすきま内部の近淀み領域だけ**。
- したがって「**forge は理論側に付き、実験だけが離れている**」という下の整理は
  **深く狭い側については保留**する。深部の予測は**現時点で数値的に定まっていない**。
- **決着済み**: precond 2 側が非物理 (上節)。以下は経緯の記録。
- **前処理を入れると内部が長い過渡に入る**: run_0015 (w/d 0.383 + precond2) は 30k step 時点で
  平均比 0.192 だが `DRIFTING 37 %/tail`。**この値は使えない** (run_0017 で 120k step 継続中)。
  細格子 run_0014 も同様に `DRIFTING 21 %/tail` (run_0016 で 150k step 継続中)。
- **現時点で「落ち着いている」数字は 2 つだけ**: 粗格子 precond0 = 0.580 (漸近 0.578)、
  粗格子 precond2 = 0.258 (STEADY 0.8 %/tail)。**この 2 点だけで 2.2 倍**。
- **case/49 への含意**: 深いすきまの熱負荷は、低マッハ扱いの選択だけで 2 倍動きうる。
  受け渡しは plan §4.11 の **(B) 未定量**のままとする。
...
| `run_0012_T1_wd0063_fine` (+`_long`) | すきま格子細分 (開口 60→120 セル, キャビティ深さ方向 200→300) | 実行中 | 投入中 |
