...
## T1 (深キャビティ w/d = 0.063) の暫定所見

**まだ収束していない** (10000 step 時点で `check_quasisteady` = **DRIFTING**, $Q_c$ が −12.8 %/tail で単調減)。
以下は暫定値で、run_0006 (80000 step) で更新する。

| $x/d$ | 実験 (Fig 6a) | forge (分母=文献) | 比 |
| --- | --- | --- | --- |
| 0.010 | 0.55 | 0.98 | 1.79 |
| 0.030 | 0.42 | 0.29 | 0.69 |
| 0.050 | 0.34 | 0.095 | 0.28 |
| 0.100 | 0.17 | 0.020 | 0.12 |
| 0.200 | 0.07 | 0.004 | 0.06 |
| ≥0.40 | 0.02 (計測限界帯) | 0.003 | — |

- **CFD の減衰が実験よりはるかに急**。開口面積平均は CFD 0.85 (文献分母) / 0.72 (forge 分母) vs 実験 1.07。
- **ただし [W70] Fig 10(a) を見ると、Burggraf の非粘性コア理論も同じく急減衰**で、実験だけが上に乗る
  (W70 は「粘性コアだと上端付近が高い」と説明)。**forge は粘性計算なのに理論側に乗っている**。
- **計測側の仮説 (定量)**: [`tools/skin_conduction_estimate.py`](tools/skin_conduction_estimate.py)。
  感温面は 0.305 mm の 304SS 薄板で、挿入 0.8 s の間にスキン内を熱が拡散する長さは
  $\sqrt{\alpha_s t}$ = **1.74 mm = 1.4 すきま幅 = $x/d$ 0.086** — **実験が理論を上回る深さ帯と一致**する。
  横方向伝導が作る見かけ熱流束は $\lambda_s \tau \Delta T/(L_{hot}L_{abs})$ で
  ΔT 5–22 K のとき $0.08–0.7\,q_{fp}$ と、実測の超過 (0.07–0.17) と**同じ桁**。
  W70 本文も「表面伝導・放射の補正はしていない」と明記している。
- **残る CFD 側の候補** (未検証): 低マッハ前処理の有無、すきま方向の格子細分、
  非定常 (せん断層振動によるポンピング) を定常解が捉えていない可能性。
...
| `run_0006_T1_wd0063_long` | run_0005 から 80000 step (res 5000 step 毎) — 内部の緩和を追い込む | 計算中 | 投入中 |
