#!/usr/bin/env python3
"""case/50 メッシュ生成 — 平面 2D 構造 (node 用)。T0 (すきま無し) と T1 (深キャビティ) を**同一ブロック構成**で作る。
...
"""
CONV_BC = """inlet:  {physID: 1, kind: inlet_uniformVelocity, outputHDFflg: 0, ints: , floats: {ro: 0.0354, Ux: 1877.0, Uy: 0.0, Uz: 0.0, Ps: 1883.0}}
outlet: {physID: 2, kind: outlet_statPress, outputHDFflg: 0, ints: , floats: {Ps: 1883.0, Pt: 1883.0, Tt: 180.0}}
top:    {physID: 3, kind: slip, outputHDFflg: 0, ints: , floats: }
plate:  {physID: 4, kind: wall_isothermal, outputHDFflg: 1, ints: , floats: {Ux: 0.0, Uy: 0.0, Uz: 0.0, Ts: 294.0}}
runup:  {physID: 5, kind: slip, outputHDFflg: 0, ints: , floats: }
cavity: {physID: 6, kind: wall_isothermal, outputHDFflg: 1, ints: , floats: {Ux: 0.0, Uy: 0.0, Uz: 0.0, Ts: 294.0}}
"""
...
def main():
    ...
    ap.add_argument("--wd", type=float, default=0.063, help="w/d (T1 用。T0 でもブロック分割に使う)")
    ap.add_argument("--y1", type=float, default=4.0, help="壁第一セル [µm]")
    ap.add_argument("--r-y", type=float, default=1.06)
    ap.add_argument("--H", type=float, default=0.040)
    ap.add_argument("--x-in", type=float, default=-0.020)
    ap.add_argument("--x-out", type=float, default=0.230)
    ap.add_argument("--x-plate-end", type=float, default=0.197, help="模型長 (ここから先は slip バッファ)")
    ap.add_argument("--nx-buf", type=int, default=61)
    ap.add_argument("--nx-up", type=int, default=41)
    ap.add_argument("--nx-plate", type=int, default=701)
    ap.add_argument("--nx-gap", type=int, default=61)
    ap.add_argument("--nx-down", type=int, default=201)
    ap.add_argument("--ny-cav", type=int, default=201)
