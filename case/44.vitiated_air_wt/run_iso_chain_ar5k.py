#!/usr/bin/env python3
"""case/44 等温壁チェーン (AR 緩和メッシュ版: ni 2401 / nj 121 / wfft 8e-7, AR ≤ 5000, スロート y1+_cold ≈ 1) (plan tooling-nozzle-isothermal-wall-chain §4.5) の実行ドライバ。
...
P_AD = CASE / "problem_va_R2_LU6_Lc8_ns_ar5k.yaml"
P_ISO = CASE / "problem_va_R2_LU6_Lc8_ns_ar5k_iso300.yaml"
RA = CASE / "run_0116_va_ns_ar5k_ad_samewall"; RB = CASE / "run_0117_va_ns_ar5k_iso300_samewall"; RC = CASE / "run_0118_va_ns_ar5k_iso300_ib_pass0"
IC_A = CASE / "run_0108_va_ns_fine_ad_samewall"   # 細分 AR846 メッシュの断熱場から cross-mesh
...
def main():
    ...
    if a.wait_for:
        ...
        log("GPU queue free")
    if a.stage in ("a", "all"):
        run_ns(P_AD, RA, ic_from=IC_A, delta_r_csv=str(REF / "delta_r_initial.csv"), offset="radial")
    if a.stage in ("b", "all"):
