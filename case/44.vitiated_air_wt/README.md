...
| `run_0429`–`run_0448_matrix_*` (+ `run_0449`–`0452` の細水準) | **§6-8 の検証行列 (正式)**: FCT 有効条件 (convMethod 1 + SFR 2 + passiveFct 1, IC `run_0394` res_40, 倍精度, 終了 0.64 ms) で **BDF2/BDF1 × `passiveImplicitCoupling` 0/1** の 4 組 (各 dt 4e-6/2e-6/1e-6 + nSub 倍増) + **FCT 無効の対照** (BDF2 × coupling 1)。実現可能性射影の半径 underflow 修復後の binary | **BDF2 × coupling 1 / coupling 0**: 次数ゲート **PASS** (g 1.740 / Q2 1.924 / Q1 1.875 / Q0 1.381 [FCT 緩和下限 1.3]、流れ・化学種 1.93–1.95 は厳格帯)、**収支ゲート 4 run とも PASS**、nSub 比 0.000–0.006。**coupling 0 と 1 の差は 4 桁目以降**。**BDF1 × coupling 1 / coupling 0**: モーメント **0.879 / 0.900 / 0.906 / 0.901** (1 次に落ちる = 正しい)、収支 4 run とも PASS。流れ 2 列 (roUy 0.627 / roe 0.692) が [0.7,1.3] をわずかに外れる (BDF1 は誤差が大きく dt 4e-6 は漸近域外) → 1 オクターブ細かい `run_0449`–`0452` (dt 2e-6/1e-6/0.5e-6) で再判定。**対照 (FCT 無効)**: モーメント **1.987–2.013** で次数 PASS、収支は設計どおり INDETERMINATE (FCT 記録が無いので保存は判定不能; 場ゲートは PASS) | active (**§6-8 の正本**) |
