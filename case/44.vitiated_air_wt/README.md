...
**訂正 (2026-09-15 同日, `run_0130`/`0131`)**: 上の run_0127 は**非平衡ソースの θ 律速 (潜熱 ΔT ≤ 1 K/step, `dt_local` 比例) が効いた状態**で、
内側で成長が 1/4 に絞られていた (`condLim_0` 0.25、壁 2 ノードは dt_local が半分で 0.55)。「壁第一層だけ g の増え方が激しい」のはこの偽の差で、
`cfl_pseudo` 0.5 (`run_0131`) では `condLim` が全域 1 になり壁と内側が一致する。定常解が擬似 CFL に依存する構造 (plan
[condensation-followups](../../plans/active/condensation-followups.md) F-cf7)。当時は `run_0131` (cfl 0.5) を正本としたが、**現在の正本は修正後バイナリの `run_0137` 系** (下の「θ 律速の更新クランプ化」節; run_0131 は比較参考):
onset は不変 (x 15.9 r_t)、g=30 % Y_H2O 到達が x 22.5 → 18.7 r_t (内側)、出口 g 平均 0.584 % (15.5 % of H2O、run_0127 比 +34 %)、出口 M 4.050、g max 1.63 %。
図 `figs/va3_inletTt_cfl_ab.png`。onset (核生成) 自体は律速されていないので Wilson 点の議論は不変。

### θ 律速の更新クランプ化 (`condLimiterMode 1`) の検証 — cfl 2/4 が旧 cfl 0.5 と一致 (2026-09-15, `run_0132`–`0136`)

plan [condensation-source-limiter-steady](../../plans/accepted/condensation-source-limiter-steady.md) の実装 (残差から θ を外し、蒸発を瞬間速度形に、
更新量の 4 本同率クランプ) を、入口 Tt 分布 noneq (run_0127 と同じ入力: IC = run_0092 一様収束場、node Euler TP、24000 step) で検証。

| run | cfl_pseudo / condFloat | 対 `run_0131` (旧 θ≡1, cfl 0.5) g L1 相対 (Ω_c) / max|ΔT| / max|ΔM| | 出口 g 平均 / 出口 M / g=30 % 到達 (壁・内側) | `condLim` (凝縮域) / `condClampCorr` | 状態 |
| --- | --- | --- | --- | --- | --- |
| **`run_0132_…_lim1_cfl2`** | 2 / 1 | **1.6e-5 / 0.007 K / 6e-5** | 0.584 % / 4.0499 / 18.79・18.69 r_t | 1.000 / 0 | NaN 0, warm 床 plateau |
| `run_0133_…_lim1_cfl05` → **`run_0136_…_cont`** (+48000) | 0.5 / 1 | 対 0132: 2.1e-3 / 0.23 K / 2.2e-3 (72000 step, なお 2.4→2.1e-3 と緩やかに接近) | 0.584 % / 4.0501 / 18.79・18.69 | 1.000 / 0 | NaN 0, plateau |
| `run_0134_…_lim1_cfl4` | 4 / 1 | 対 0132: 3.9e-5 / 0.010 K / 1e-4 | 同上 | 1.000 / 0 | 一様 IC からの起動 NaN 0 |
| `run_0135_…_lim1_cfl2_condFloat0` | 2 / 0 | 対 0132: 1.8e-5 / 0.004 K / 4e-5 | 同上 | 1.000 / 0 | NaN 0 |

- **定常解が擬似 CFL に依存しなくなった**: cfl 0.5 / 2 / 4 で凝縮量・onset・出口量が一致 (ゲート: g L1 ≤1 %, |ΔT| ≤0.5 K, |ΔM| ≤5e-3)。旧 mode 0 の cfl 2 (`run_0127`) とは g L1 で 50 % 違う (絞られていた分)。
- 収束時に θ_u<1 が残るのは 62 セル (g ≤ 2e-23 の数値塵, S 0.2–0.35, 総液相の 7e-25) のみ。硬クランプ補正 `condClampCorr` は全域 0。
- 収束判定は入口 Tt 分布 run 固有の warm 床 plateau (rms_roe 0.4, dry 一様 run と同じ床) で `NOT CONVERGED` 表示だが、未加工残差 `rms_rog` は 3.4 桁低下、報告量は 12000 step 以降不変。

**2 巡目 (2026-09-16, codex result レビュー反映後のバイナリ: 蒸発の一様 ṙ 形・`condEquilibrium 0` 限定・部分体積・補正診断)**: 凝縮固有量の時系列
(`cond_series_csv.py` → `cond_series.csv`: g_max / 出口 g 平均 / 壁 onset x / 内側 g=30 % 到達 x / 壁 M / 出口 M) を `check_quasisteady.py --series-csv --drift 0.2 --osc 0.2` で判定。

| run | 離散化 / cfl / condFloat / IC | 対 `run_0137` g L1 / max|ΔT| / max|ΔM| | 報告量 (出口 g 平均, 出口 M, onset x, x30) | series | 状態 |
| --- | --- | --- | --- | --- | --- |
| **`run_0137_…_lim1b_cfl2`** | node / 2 / 1 / 一様場 | — (対 1 巡目 `run_0132`: 2.1e-5 / 0.004 K / 3.7e-5) | 0.584 %, 4.0499, 15.92, 18.69 | ALL STEADY | **正本** |
...
| `run_0141_…_cell_lim1_cfl2` / `run_0142_…_cell_lim1_cfl05` + **`run_0146_…`** (0142 の流れだけを引き継いだ 48000 step; 2 巡目バイナリ) | **cell** / 2, 0.5 / 1 / node 場を最近傍 (**注**: `interp_field.py` が roY を新設しなかったため IC の H2O と液相は起動時に消え、入口から再発達した場 = 「流れだけ引き継いだ 48000 step」; codex result-2 M2, ツールは修正済) | cell cfl 0.5 vs cfl 2: **6.8e-4 / 0.12 K / 9.6e-4** (24000 step 時点 4.3e-3 / 2.8 K は未収束) | 0141: 0.578 %, 4.0509, 15.97, 18.64 / 0146: 0.578 %, 4.0509, 15.97, 18.74 | 両方 ALL STEADY | cell 離散化差 (対 node 出口 g −1 %); cell でも Δτ 非依存 |
...
**残差の床 (当時の基準; result-4 M3 で撤回、現行ゲートは plan §6)**: cell の残差 plateau は mode 1 (`run_0155`) と旧経路 mode 0 (`run_0159`) で流れ変数は同じ床 — rms_ro 2.59e-5 vs 2.61e-5、rms_roe 24.9 vs 25.4、rms_roQ0 2.33e10 vs 2.28e10 (rms_rog は 3.7e-8 vs 1.1e-8 で 3 倍)。
...
**4 巡目 = 最終確定バイナリ (2026-09-16, 小液滴の蒸発継続 [ṙ を r_min で評価] を追加; commit d618c0dc 以降, `run_0162`–`0167`)**: 3 巡目との差は node 1.2e-5 / cell 7.3e-5 (g L1)。
node cfl 0.5 (`0163`) vs cfl 2 (`0162`): 1.6e-3 / 0.18 K / 1.8e-3; node condFloat 0 (`0166`) vs float: 1.5e-5; cell cfl 0.5 (`0165`) vs cfl 2 (`0164`): 7.6e-4 / 0.11 K / 8.8e-4;
cell condFloat 0 (`0167`) vs float: 8.4e-5。全 run NaN 0・series ALL STEADY・`condLim` 凝縮域 1.000・補正 0。
**残差床 (末尾 20 % 平均)**: node は cfl 0.5/2 で全列 0.90–1.06、精度で 0.95–1.00 (Δτ 非依存の床)。cell は cfl 0.5/2 で流れ変数 0.31、モーメント 0.15–0.18
...
**cfl 2 の収束場は cfl 0.5 でも指定ノイズ床内に留まり、cfl 0.5 系の場は cfl 2 で cfl 2 の場の近傍 (床内) へ移る = 共通の最終場を指定精度内で維持・再現できる** (codex result-6 m1: 低 cfl 系列に履歴差が残る機構 [遅い過渡か、低 cfl 側の float 丸めによる停滞か] はこれだけでは確定しないので followups F-cf10)。
...
| **`run_0178`** cell **cfl 2 ← `0175` の場** | cfl 0.5 系の場が cfl 2 でどこへ行くか | 6.788 → 6.807 (4000 step) → 6.810 | 2.17e-3 → 1.41e-3 (4000) → **4.7e-4 (6000) → 6.0e-4 (24000) / 1.5e-4** | 1.68e-3 / 7.1e-4 (離れる) | **6000 step 以降 cell の許容範囲 (床 2 倍) 内、最終場も床内** |
...
| **`run_0180`** node **cfl 2 ← `0171` の場** | 同上 | 6.839 → 6.864 (2000) → 6.875 (4000) → 6.877 | 5.39e-3 → 1.29e-3 (6000) → **4.6e-4 (8000) → 1.3e-4 (24000) / 1.7e-5 = 反復床**; ro 5.3e-6, T 4.7e-6, |ΔT| 0.005 K | 5.3e-3 / 1.56e-3 | **8000 step までに差の大部分が減少 (途中 10000 step 6.9e-4, 18000 step 4.3e-4 と再増加あり)、24000 step の最終場は反復床内** |
...
結論: **§6 (b) は交差 restart 対 (`0177`−`0172`, `0178`−`0172`) で成立** (cell g_0 max ≤ 9.7e-4 < 床 2 倍 1.15e-3; ro/P/T・化学種・全モーメント保存量も床 2 倍内 [codex 再計算])。受け入れる成果は「ソース残差の明示的 Δτ 依存の除去」と「交差 restart で共通の最終場を指定ノイズ床内で維持・再現」。
cfl 0.5 系 (`0139`→…→`0171`, `0156`→…→`0175`) は 2 巡目の cfl 2 場から warm した後、onset 域の成長遅れが cfl 0.5 では 1 世代 (24000 step) あたり g L1 ~1e-4 しか減衰しない一方、cfl 2 では 8000 step までに大部分が減衰する。
この履歴差が残る機構 (低 cfl 側の遅い過渡か、`cond_moment_update_limited_d` の増分加算後の float 丸めで θ_u>0 でも保存値が変わらない停滞か) は未同定 → followups F-cf10 (更新前増分と丸め後変化量の比較を含む)。
...
### species 統一スキーマ (full | lumped, モル分率入力, CEA 直読み DB) の回帰 — node のみ (2026-09-16, `run_0190`–`0199`, `run_0202`/`0203`)

plan [thermophysics-cea-mole-fraction-species](../../plans/accepted/thermophysics-cea-mole-fraction-species.md) §6 (cell はユーザ指示で対象外)。
正本 `run_0170` (旧バイナリ, 質量分率 YAML `split_h2o` = `{lumped, MIXDRY: composition exclude [H2O], keep [H2O]}`) と同じ入口 Tt 分布・cfl 2・24000 step。
IC は `run_0170` の収束場 (`full` は `tools/convert_species_field.py` で MIXDRY を実種 4 種に展開; ΣρY=ρ 2e-16, 総水量保存, T 保持 5.8e-4 K)。
問題 YAML: `problem_va3_M4.19_Lc8_noneq_full.yaml` (`composition_basis: mole` の元モル分率 Σ 0.998825 → 正規化, `tp_species: {mode: full}`), `…_noneq_lumpedmole.yaml` (同モル分率入力で lumped), `…_dry_full_cea.yaml` (`gas.species_db: species_db_cea_va3.yaml`)。

| run | 内容 | 結果 (NaN / series 0.2 % / condLim / 補正) | 最終場 vs `run_0170` (ro / T / g_0 max / g L1 / \|ΔT\|) | ṁ (x=2 r_t, 出口) 相対差 / 軸 M 差 max | 報告量 (出口 g %, onset r_t, x30, M) |
| --- | --- | --- | --- | --- | --- |
| (参考) `run_0176` 旧バイナリ反復 | 同一入力反復 = 旧ノイズ床 | — | 5.0e-6 / 6.4e-6 / 2.0e-4 / 1.95e-5 / 0.008 K | 0 / 1.3e-5 | 0.58443, 15.8405, 18.6456, 4.0499 |
| `run_0195_…_newbin` | 新バイナリ (host DB 解決・rms_roY 列) × 旧入力 (A/B) | 0 / ALL STEADY / 1.0000 / 0 | 9.2e-6 / 1.6e-5 / 5.1e-4 / 2.7e-5 / 0.019 K | +1.7e-7 / 1.2e-5 | 0.58443, 15.8400, 18.6453, 4.0499 |
| `run_0199_…_newbin_rep1` | 新バイナリの反復 (ノイズ床) | 0 / ALL STEADY / 1.0000 / 0 | vs `0195`: **4.6e-6 / 5.1e-6 / 1.6e-4 / 1.6e-5 / 0.006 K** (= 新バイナリのノイズ床); vs `0170`: 1.0e-5 / 1.1e-5 / 3.5e-4 / 2.4e-5 / 0.013 K | +8.7e-8 / 1.2e-5 | 0.58443, 15.8401, 18.6454, 4.0499 |
| **`run_0196_…_full5`** | **`full` 5 種 [H2O, N2, O2, AR, CO2]** (`condGasSpecies 0` = `condensationSpecies H2O` から生成), IC 種変換 | 0 / ALL STEADY / 1.0000 / 0; `rms_roY0..4` 列あり | 3.0e-5 / 2.8e-5 / 3.5e-4 / 1.1e-4 / 0.015 K | +2.9e-6 / 1.5e-5 | 0.58448, 15.8404, 18.6451, 4.0499 |
| `run_0197_…_lumpedmole` | モル分率入力の lumped (旧 `split_h2o` と同値; `species_db.yaml` は解析後に MW 1e-8 相対で一致 [旧 YAML の質量分率 7 桁丸め分]) | 0 / ALL STEADY / 1.0000 / 0 | 3.0e-5 / 2.8e-5 / 3.1e-4 / 7.7e-5 / 0.011 K | +2.7e-6 / 1.4e-5 | 0.58447, 15.8406, 18.6453, 4.0499 |
| `run_0190`–`0192`, `0194` (500 step) | forge 側の入力検証: `X0/X1` (モル分率) 入力 / `condensationSpecies: H2O` / トレーサ `tracer: exhaust` + `Xi: 1`; `0191`/`0194` は Y 入力の反復 | 全 NaN 0; X 入力・トレーサとも同一設定反復ノイズ (rel ≤7e-6) 内; 起動ログの species 表と入口 Y/X; `rms_roY`/`rms_roXi` 列 | — | — | — |
...
| `run_0108_va_ns_fine_ad_samewall` | **等温壁チェーン S4a-A** ([plan](../../plans/active/tooling-nozzle-isothermal-wall-chain.md) §4.5): run_0107 と同じ設計・同じ物理壁 (`delta_r_initial.csv`) を冷却壁向け細分メッシュ (`problem_va_R2_LU6_Lc8_ns_fine.yaml`: ni 4001 / nj 113 / `wall_first_frac_throat` 2.5e-6, AR max 846 PASS, 452k 節点) で断熱のまま。IC = run_0107 cross-mesh、soft/mid + 本段 cfl 2 + relax 0.7、24000 step | 完走・NaN 0・残差床 (rms_roe 0.23) ・`wall_thermal_ledger.py --series` STEADY。ṁ_NS/ṁ_E 0.9988、出口コア M 4.1869 (run_0107 4.1900: メッシュ感度 −0.07 %)、出口 T0 1060.0 K / P0 1.139 MPa、y₁⁺ スロート 0.61 / 出口 0.39 | active (S4a 断熱基準) |
| `run_0109_va_ns_fine_iso300_samewall` | **S4a-B**: 同じ壁・同じメッシュで `wall_isothermal` 300 K (`spec.wall_thermal`)。IC = run_0108 (同一メッシュ + soft/mid), 24000 step | 完走・NaN 0・残差 still converging、series: M_core drift 0.07 % / Q_w 9.6 % (熱場未静定 → run_0114 で継続)。**暫定の壁温感度 (同壁)**: 出口コア M 4.1869 → 4.2071 (+0.48 %)、ṁ 比 0.9988 → 1.0011、出口質量平均 T0 1060 → 1048 K、Q_w 7.8 MW、q_w ピーク 2.83 MW/m² @ x −0.41、y₁⁺ スロート 3.0 / 出口 1.8 / 上流 max 10.8。抽出 δ_r は断熱の 0.6〜0.75 倍、スロートで ≈0 | active (固定形状の壁温感度) |
| `run_0110_va_ns_fine_iso300_ib_pass0` | **S4b**: 300 K での再設計 pass 0 (積分法初期壁 `thermal_bc` 300 K, 符号付き δ\*), IC = run_0109, 24000 step | 完走・NaN 0。ṁ 比 1.0017 (ゲート内)、出口コア M 4.1998 (+0.23 %, ゲート外 → pass 1)。積分法初期壁は冷却効果を過小評価 (出口 δ_r 0.115 vs 断熱 0.122; 抽出は 0.094) → pass 1 (`run_0115`) で補正 | active |
| `run_0114_va_ns_fine_iso300_samewall_cont` | run_0109 の継続 (+36000 step, 同壁・同メッシュ, `--stages none`) | 完走・NaN 0。出口コア M 4.2016 (drift 0.03 % STEADY)、ṁ 比 1.0024、Q_w 6.40 MW (drift 6 %, 熱場なお緩和中)、出口質量平均 T0 1043.8 K。rms_roOmega 末尾上昇 (要確認) | active (S4a 冷却壁, 台帳の正) |
| `run_0115_va_ns_fine_iso300_ib_pass1` | S4b pass 1: run_0110 の場から符号付き帯局所抽出 (ω 1.0), warm start 36000 step | 完走・NaN 0。**ṁ_NS/ṁ_E 1.0002、出口面コア M 4.1909 (+0.02 %) = 等温壁で生産ゲート達成**。series M_core drift 0.02 % STEADY、Q_w 6.8 MW (drift 8.6 %)。固定点比 0.94〜0.99 (x≥8)、スロート近傍はなお縮小 | active (**等温 δ\* 反復の正**) |
...
| `run_0190_…_xinput_check` / `run_0191_…_yinput_ref` / `run_0192_…_tracer_check` / `run_0194_…_yinput_ref_rep1` (500 step) / `run_0193_…_full_prep` / `run_0195_…_newbin` / **`run_0196_…_full5`** / `run_0197_…_lumpedmole` / `run_0198_…_dry_full_cea_prep` / `run_0199_…_newbin_rep1` / `run_0202_…_dry_runner` / `run_0203_…_dry_full_dry_runner` | **species 統一スキーマの回帰 (node のみ)**: forge 側 `X{s}`・`condensationSpecies`・トレーサの 500 step 検証、新バイナリ A/B + 反復、`full` 5 種 (IC 種変換)、モル分率入力 lumped、CEA 直読み DB (prepare)、dry lumped/full (上の「species 統一スキーマ」表; plan thermophysics-cea-mole-fraction-species §6) | 全 NaN 0・ALL STEADY; full と lumped は報告量 5 桁一致・ṁ 3e-6・軸 M 1.5e-5 | active |
...
| `run_0204_passiveA_bitid_head` / `run_0205_passiveA_bitid_new` / `run_0208_passiveA_bitid_head_rep1` (cfl 2) ・ `run_0206_passiveA_bitid_relax07_head` / `run_0207_passiveA_bitid_relax07_new` / `run_0209_passiveA_bitid_relax07_head_rep1` (`implicitRelax 0.7`) | **受動種経路 Phase A の無影響試験** (plan species-passive-scalar-unification §6-7, 2026-09-16): `run_0170` 入力 500 step (一様 IC から) を HEAD バイナリ (c14909e1) と新バイナリ (`passiveScalarScheme 0`, `speciesFaceReconstruction 0`) で。`_rep1` = 同一 HEAD バイナリの反復 (ノイズ床) | res_500 の場の差 (max\|Δ\|/max): cfl 2 HEAD vs 新 = リミッタ 5.1e-1 (`limiter_P`)・勾配 2e-4・ro 等 <1e-4 vs HEAD 反復 4.2e-1 / 2.3e-4; relax 0.7 HEAD vs 新 4.7e-1 vs 反復 4.0e-1。残差最終行 rms_ro 1.5e-6 / 3.4e-6 相対 → 全て node の run-to-run ノイズ床 (atomicAdd 順序) と同水準で旧経路の変更は検出されない。凝縮モーメントは 500 step ではまだ 0 (rms_roQ0 all-zero) | active (無影響回帰) |
| `run_0214_passiveA_scheme1_1st_cfl2` / `run_0215_passiveA_scheme0_1st_cfl2` | `run_0170` 入力 2000 step、1 次移流のまま `passiveScalarScheme` 1 / 0 (受動種経路 vs 旧経路; 差は入口ピン・更新後 floor・`transport_diag` 周期 gather 無し) | 場の差 ro 1.1e-5 / P 6.6e-7 / T 1.1e-5 / g 6.8e-5 / Q0 2.4e-4 (相対), 残差 rms 同値 (rms_ro 6.69e-4)、onset x 3.2335 同一、floor 補正 0 (roQ1 1e-17)。scheme 1 の 1 次経路は旧経路と同じ離散化 | active |
| `run_0210_passiveA_s3_cfl2_c0` / `run_0211_passiveA_s3_cfl2_c1` / `run_0212_passiveA_s3_cfl6_relax07_c0` / **`run_0213_passiveA_s3_cfl6_relax07_c1`** | **S3 smoke (受動種 S3 = ψ_P 無次元化 Venkat + `species_advection_faceY_d`)**: `run_0170` 入力 2000 step, `passiveScalarScheme 1`, `speciesFaceReconstruction 2`, **`speciesImplicitCoupling 0` (run_0170 既定のまま = plan §4.2 が S3 で不安定と特定した化学種 segregated 更新)**, `passiveImplicitCoupling` 0 (point-implicit × `passiveImplicitRelax`) / 1 (scalar-DPLUR sweep) × cfl 2 / cfl 6 + relax 0.7 | cfl 2: 両方完走 (NaN 0) だが化学種 S3 segregated の乱れで rms_ro 2.8e-2・rms_roY1 0.13 (1 次の 40×/5000×)、onset 1.85 (1 次 3.23)、`condLim` min 0、Q0 max 8.9e15 (c0) / 2.6e17 (c1)、floor 補正 (積算, 相対) Q1 3.6e3 / Q0 1e-2 (c0), Q1 4e5 / Q0 1.6 (c1)。cfl 6 + relax 0.7: **c0 は完走** (rms_ro 4.3e-2, Q0 max 3.8e19, floor Q0 0.12)、**c1 は step 1098 で NaN** (rms_rog が 1.5→2.6 と 100 step で成長 → 軸上 x≈1.01–1.03 で ρ<0, g<0 → 二相 T 反転失敗 → ρ NaN; `res_nan_1098.h5`)。→ 化学種側が segregated S3 のときは受動種の方式に依らず健全でない (下の `sc1` 系列が本番条件) | active (S3 smoke, 化学種 coupling 0) |
| **`run_0216_passiveA_s3_sc1_cfl2_c0` / `run_0217_passiveA_s3_sc1_cfl2_c1` / `run_0218_passiveA_s3_sc1_cfl6_relax07_c0` / `run_0219_passiveA_s3_sc1_cfl6_relax07_c1`** | 同上 S3 smoke を plan §4.2 の推奨 **`speciesImplicitCoupling 1`** (化学種 scalar-DPLUR) で: `passiveImplicitCoupling` 0 / 1 × cfl 2 / cfl 6 + relax 0.7 (2000 step) | **4 本とも完走 (NaN 0)**、流れ残差は 1 次と同水準 (cfl 2: rms_ro 6.8e-4 / 6.7e-4; cfl 6 r0.7: 2.5e-4 / 1.75e-4)、rms_roY1 2.5e-5 / 6.6e-6、onset x 3.274 (cfl 2) / 3.071 (cfl 6, 発達途中) ≈ 1 次 3.23、g max 1.39–1.61e-2。**受動種 DPLUR (c1) の方が良い**: floor 補正 (積算 |Δ|/∫ρφ) c1 = rog 3.8e-6 / Q2 4.0e-6 / Q1 4.5e-6 / Q0 2.7e-6 (cfl 2), 1.1e-5 / 8.5e-6 / 7.7e-6 / 7.5e-6 (cfl 6) に対し c0 = 1.7e-7 / 1.2e-2 / 1.5e-2 / 1.1e-2 (cfl 2), 4e-6 / 4.3e-3 / 1.8e-2 / 1.4e-2 (cfl 6); モーメント残差 rms_rog c1 3.9e-6 / 2.5e-6 vs c0 3.8e-5 / 4.6e-5, rms_roQ0 2e11 vs 3e13。`condLim` min 0 (発達過渡, 2000 step は準定常前)。step 時間 2.39 / 2.62 / 2.55 / 2.84 ms (1 次 scheme 1: 2.01, scheme 0: 2.54). ⇒ Phase A の S3 本番候補 = `speciesImplicitCoupling 1` + `passiveImplicitCoupling 1` (既定の自動値)。準定常・固定点判定は §6-4/6-5 の長い run で | active (S3 smoke, 本番候補) |
| `run_0224_passiveB_dt_cond_dt8e-6` / `run_0225_passiveB_dt_cond_dt4e-6` | **凝縮ソース作動下の dual-time (Phase B §6-6-iii)**: `run_0170` 入力 + IC = `run_0170/res_24000` (収束場), `passiveScalarScheme 1`, SFR 0 (1 次), `unsteady 1, dualTime 1, timeIntegration 11, blockDPLUR 1, control 0`, `nSubIterDualTime 10`, `0220` = dt 8e-6 (物理 CFL≈10) × 100 step, `0221` = dt 4e-6 × 200 step (同一物理時刻 8e-4 s); `condLimiterMode 1` は降格されず有効 | NaN 0、モーメントは定常場に留まる: 収束場からのドリフト (max\|Δ\|/max) `0220` @100: ro 2.5e-6 / T 2.9e-6 / g 8.5e-5 / Q0 3.2e-4 / Q1 1.9e-4 / Q2 1.2e-4、`0221` @200 (同時刻): ro 1.7e-6 / g 1.2e-4 / Q0 4.5e-4 (Δt 半減で同水準 = 時間離散誤差でなく反復/クランプ床); g max 1.6301e-2・onset x 3.0298 不変; `condLim` min 0.19–0.26 (少数セルで θ_u 作動); floor 補正 (積算, 相対) rog 1e-23 / Q2 1e-19 / Q1 3e-12 / Q0 2e-18 ≈ 0; BDF 込み sub-iter 残差の低下はモーメント 0.7–1.1 桁 (収束場の残差床から; 流れ・化学種は床 0.05 桁) | active (Phase B 検証) |
| `run_0226_passiveB_dt_cond_transient_dt8e-6` / `run_0227_passiveB_dt_cond_transient_dt4e-6` / `run_0228_passiveB_dt_cond_pin_diag_1step` | **凝縮発達中の過渡 IC からの dual-time (Phase B §6-6-ii ソース作動)**: `run_0170` 入力, IC = `run_0186/res_4000` (cfl 4 起動段の途中: rms_ro 5e-5, g max 1.63e-2, wet 2874 ノード), `passiveScalarScheme 1`, SFR 0, coupling 0, `nSubIterDualTime 10`, `0226` = dt 8e-6 × 100 step, `0227` = dt 4e-6 × 200 step (同一物理時刻 8e-4 s), `0228` = 1 step `FORGE_PIN_DIAG=1` | NaN 0 (両方)。BDF 込み sub-iter 残差低下 (初回→最終, min/median/max): `0226` rms_rog 2.24/2.43/2.46, roQ2 2.19/2.51/2.53, roQ1 2.21/2.32/2.38, **roQ0 1.44/1.58/1.68**, 流れ ro 0.95/1.31/1.33, **roY0 0.95/1.31/1.33, roY1 0.95/1.30/1.33**; `0227` (dt 半分) rog 2.30/2.48/2.52, roQ2 2.05/2.55/2.58, roQ1 2.21/2.32/2.39, roQ0 1.51/1.66/1.77, ro 1.60/1.77/1.94, roY0 1.60/1.76/1.93 → g/Q2/Q1 は ≥2 桁、**Q0 と流れ・化学種は nSub 10 では 1.3–1.8 桁 (目標 2 桁未達; sub-iter 数不足)**。condLim min 0.098–0.155 (発達域の θ_u 作動)、floor 補正 (積算相対) rog 2e-16 / Q2 8e-12 / Q1 8e-6 / Q0 1e-11。過渡の進行 (IC からの変化 @8e-4 s): ro 6.7 % / T 7.4 % / g 3.6 % / Q0 2.1 %, g max 1.6278e-2, onset x 3.0094 不変。**dt vs dt/2 (同一物理時刻) の差** max\|Δ\|/max: ro 3.6e-3 / P 5.9e-5 / T 4.7e-3 / Y1 1.6e-5 / g 9.5e-5 / Q0 4.6e-4 / Q1 2.2e-4 / Q2 1.5e-4 (過渡変化 [ro 6.7 %] の 5 % = 時間離散 + sub-iter 未収束分)。**ピン行 (`0228`)**: 入口ピンノード 65 個で BDF 後の \|res_roY0/1, res_rog/roQ2/roQ1/roQ0\| = 0.00 (全 sub-iter)、更新後の \|Y − Y_inlet\| = 0、モーメント入口値 = 0 | active (Phase B 検証) |
| `run_0229_passiveB_dt_cond_transient_nsub20` / `run_0230_passiveB_dt_cond_transient_nsub40` | `run_0226` 設定 (過渡 IC=`run_0186/res_4000`, dt 8e-6 × 100 step, scheme 1, SFR 0, coupling 0) で `nSubIterDualTime` 20 / 40 (sub-iter 収束の確認) | NaN 0 (両方)。BDF 込み sub-iter 残差低下 (初回→最終, min/median/max) は **nSub 20 と 40 で同値 = 頭打ち**: rog 2.26/2.43/2.47, roQ2 2.19/2.51/2.54, roQ1 2.21/2.32/2.38, roQ0 1.47/1.58/1.69, ro 1.77/1.93/2.03, roY0 1.77/1.91/2.02, roY1 1.62/1.77/1.90, roUx 1.49/1.76/1.94, roe 1.35/1.47/1.62, roUy 0.94/1.12/1.25 (nSub 40: ro 1.83/1.94/2.04, roQ0 1.46/1.59/1.67, 他同値) → **ro/roY0 は median ≈1.9 桁で 2 桁に僅かに届かず、roQ0 1.6・roe 1.5・roUy 1.1 は sub-iter を増やしても下がらない (擬似時間反復の残差床; cfl_pseudo 2, 発達域の θ_u 作動 condLim min 0.098)**。同一物理時刻 (8e-4 s) の場の差 max\|Δ\|/max: nSub 10 (`run_0226`) vs 20: ro 3.8e-3 / T 5.0e-3 / g 5.0e-5 / Q0 3.0e-4 / Q1 1.2e-4 / Q2 7.6e-5; 10 vs 40: ro 4.0e-3 / T 5.3e-3 / g 6.2e-5 / Q0 3.4e-4; **20 vs 40: ro 1.9e-4 / T 2.8e-4 / g 3.7e-5 / Q0 4.1e-4** → nSub 10 の sub-iter 誤差は Δt 半減差 (ro 3.6e-3) と同程度で不足、nSub 20 で ro/T は Δt 半減差の 5 % (Q0 は同程度 = 残差床)。step 時間 114 (20) / 219 (40) ms | active (Phase B 検証) |
| `run_0233_passiveC_m3_s1_ckpt100` / `run_0234_passiveC_m3_restart_s0` / `run_0235_passiveC_m3_restart_s1_from_s0` | **codex result M3: scheme 変更 restart (1→0→1)** — `run_0226` プロトコル (過渡 IC, dt 8e-6, nSub 20): scheme 1 で 100 step checkpoint → scheme 0 で restart → その res から scheme 1 で restart | `0234`: `history NOT restored (layout mismatch (file '...passiveScalarScheme=1...' vs run '...passiveScalarScheme=0...'))` → BDF1; `0235`: 同様に layout mismatch (file scheme 0) → BDF1。NaN 0 | active (codex M3) |
| `run_0236_passiveC_m4_cfl4_nsub20` / `run_0237_…_cfl8_relax07_nsub20` / `run_0238_…_pc1_nsub20` / `run_0239_…_cfl8_relax07_pc1_nsub20` / `run_0240_…_cfl2_nsub20_newbin` / `run_0241_…_conv0_nsub20` / `run_0242_…_limiter0_nsub20` | **codex result M4: 凝縮 dual-time の sub-iter 残差床の原因** — `run_0226` プロトコル (過渡 IC, dt 8e-6, 100 step, nSub 20) で cfl_pseudo 4 / 8+relax 0.7 / `passiveImplicitCoupling 1` / 両方 / 流れ 1 次 (`convMethod 0`) / リミッタ無し (`limiter 0`) | 桁低下 (median; roQ0 / roUy / roe / roY1): cfl2 1.57/1.12/1.47/1.77, cfl4 1.54/1.09/1.46/1.77, cfl8 r0.7 1.69/1.24/1.40/1.68, pc1 1.59/1.13/1.46/1.77, cfl8 r0.7 pc1 1.67/1.23/1.42/1.68, limiter 0 2.10/1.31/1.49/1.67, **convMethod 0 3.26/2.59/2.41/2.21 (全列 median ≥2.2; ro 2.47, rog 3.57, roQ2 3.68, roQ1 3.82; min は roQ0 1.69 以外 ≥1.99)** → 床は擬似 CFL・緩和・受動種の更新方式・リミッタではなく **流れの 2 次 MUSCL 再構成に起因** (擬似時間反復が 2 次流束で頭打ち)。時間次数試験は `convMethod 0` で実施 (`run_0243`–`0249`) | active (codex M4) |
| `run_0243`–`run_0249_passiveC_m4_order_*` / `run_0250`–`run_0253_passiveC_m4_order2_*` | **codex result M4: モーメント・ソース込みの 3 水準時間次数** — `run_0226` プロトコル (過渡 IC, ソース作動) を `convMethod 0` (sub-iter が全列 ≥2 桁に収束する設定) で同一物理時刻 8e-4 s: `0243`–`0245` BDF2 dt 8e-6/4e-6/2e-6 (nSub 20), `0246` dt 4e-6 nSub 40 (sub-iter 誤差), `0247`–`0249` BDF1 同 3 水準; `0250`–`0252` BDF2 dt 1.6e-5/8e-6/4e-6 (物理 CFL≈20, nSub 40), `0253` dt 8e-6 nSub 80 | NaN 0 (全 11 run)。sub-iter 低下 (nSub 20, dt 4e-6): ro 2.3 / roUy 2.7 / roY1 2.0 (min 1.9) / roQ0 median 3.3 (min 1.7)。**観測次数 (L2 差)**: BDF1 (dt 8e-6 系) g 0.89 / Q0 0.89 / T 2.1 / ro 2.1; **BDF2 (dt 8e-6 系) T 2.43 / ro 3.27 だが g −0.06 / Q0 0.19** — モーメントの水準差 (g 1.1e-7 = 場の 3e-5) は sub-iter 依存 (nSub 40 vs 20 の差 = 最小水準差の 0.44) と同じ大きさで **BDF2 のモーメント次数は分解できない**; 大刻み系 (dt 1.6e-5, nSub 40) でも g 1.13 / Q0 1.41 / T 1.27 / ro 2.30、nSub 80 vs 40 の差は最小水準差の 0.39–0.40 (g, Q0) → §6-6 の「倍増で 1/10 以下」未達。原因候補: 発達域で作動する更新クランプ θ_u (condLim min ≈0.1, 非線形スイッチ) が sub-iter 収束を roQ0 min 1.6–1.7 桁で止め、モーメントの時間離散誤差 (BDF2 では 1e-7 級) がその床に埋もれる。BDF1 (誤差 10 倍) は 0.89 で分解できている | active (codex M4, 未達) |
| `run_0254_passiveD_s0_noimpact_new` / `run_0255_passiveD_s0_noimpact_binA` | **scheme 0 無影響** (#19/#18 後): `run_0170` 入力 500 step, `passiveScalarScheme 0` を新バイナリ / Phase A snapshot (`forge-bin-passive-A`) で | 保存量の差 max\|Δ\|/max ro 2.5e-6 / roUx 1.8e-6 / roe 2.3e-6 / P 2.1e-6 / T 2.4e-6 / Y1 2.7e-6, モーメント 0 (500 step では未発生) = node の run-to-run ノイズ床 (limiter_P 等の感度配列は 1e0 級だが保存量は同水準) → 旧経路は不変 | active |
| `run_0256`–`run_0262_passiveD_order_*` | **plan §5.1 #18: dual-time の θ_u 閾値クランプ (dg_max/dT_max) を各物理 step の初回 sub-iter だけに** (実現可能性 [avail, 蒸発上限, 非負] は常時) + #19 の後、モーメント 3 水準時間次数を再試験: `run_0226` プロトコル (過渡 IC, ソース作動) を `convMethod 0`, nSub 40, dt 1.6e-5/8e-6/4e-6 (`0256`–`0258` BDF2, `0260`–`0262` BDF1), `0259` = dt 8e-6 nSub 80 | NaN 0。sub-iter 低下 (median; min): ro 2.46 (2.41), roUx 3.23, roUy 2.53, roe 2.39, roY0 2.28 (2.21), roY1 2.20 (2.14), rog 3.61 (2.48), roQ2 3.71, roQ1 3.83, **roQ0 3.29 (min 1.64)**; nSub 80 でも同値 (roQ0 min 1.66) = sub-iter を増やしても下がらない床 (condLim min 0.0: 実現可能性クランプ [avail/非負] が常時作動するセルで増分 0 に留まる)。**観測次数**: BDF2 g 1.30 / Q0 1.36 / T 1.27 / ro 2.29; BDF1 g 0.95 / Q0 0.70 / T 1.31 / ro 1.93。nSub 倍増 (80 vs 40, dt 8e-6) の差 / 最小水準差 = g 0.43, Q0 0.37, T 0.07 → **モーメント (g, Q0) の「倍増で 1/10」と BDF2≈2 は未達** (T/ro は BDF2 で 1.3/2.3, ρ は分解できる)。原因: 発達域の実現可能性クランプが sub-iter 収束を止め、モーメントの時間離散誤差がその床 (1e-7 級) に埋もれる | active (#18, 未達) |
| `run_0263`–`run_0269_passiveF_order1_*` | **plan §6-6 / §5.1 #12/#18/#20: §4.6 (モーメント非負化を成分ごと, codex result-2 M1) 後の `run_0256`–`0262` プロトコル再取得** (2026-09-16, forge-bin-passive 11:57 ビルド = 208be829 相当; 全 7 run 同一バイナリ): `run_0170` 入力 + `run_0256` の過渡 IC `ic.h5`, `convMethod 0`, SFR 0 (FCT 無効), nSub 40, dt 1.6e-5/8e-6/4e-6 → 100/200/400 step (`0263`–`0265` BDF2, `0267`–`0269` BDF1), `0266` = dt 8e-6 nSub 80。評価 `analyze_moment_order.py` (**`res_outlet_*`/`res_wall_*` で regex が落ちる**ので basename 限定に直した写しで実行) | NaN 0 (7 run)。**観測次数 BDF2**: g 1.08 / Q0 1.28 / Q1 1.35 / Q2 1.28 / T 1.28 / ρ 2.30 / P 1.85; nSub 倍増差/最小水準差 g 0.42, Q0 0.35, Q1 0.38, Q2 0.40, T 0.07, ρ 0.15 → **モーメントの 2.0±0.3 と ≤0.1 は未達 (`0256` 系の 1.30/1.36, 0.43/0.37 から変化なし)**; BDF1 g 0.98 / Q0 0.71 / Q1 0.83 / Q2 0.91 / T 1.31 / ρ 1.94。sub-iter 低下 (median; min, dt 8e-6): ro 2.46 (2.41), roUx 3.22, roUy 2.53, roe 2.39, roY0 2.28 (2.23), roY1 2.20, rog 3.61 (2.48), roQ2 3.86, roQ1 3.83 (2.42), **roQ0 3.30 (min 1.63)**; nSub 80 でも roQ0 min 1.68 (床)。`condLim_0` min 0.12–0.30 (22–62 cells <1; g>0 cells 内 min 0.20–0.99)、モーメント <0 は 0、Q1²>Q0Q2 違反 0、**実現可能性射影 nearest-point 2.7M–12.2M / degenerate→monodisperse 269k–880k 回** (sub-iter 累計)。`check_passive_budget.py`: roQ0 floor+lim rel 0–1.1e-6 (`0265` 1.13e-6 で SUM>tol) だが **clampBudget |Δ| rel Q0 1.5–4.9e-5 / Q1 3.1e-5–1.0e-4 / Q2 7e-6–2.3e-5 で全 run FAIL(SUM>tol)** → 「作動していれば次数の言い訳にせず原因を潰す」(§4.6) の前提未達: 発達域の実現可能性射影/クランプが常時作動し、モーメントの sub-iter 床 (roQ0 1.6 桁) と 1.1–1.35 次を作っている | active (#12/#18/#20, 未達) |
| `run_0270_passiveF_order3_bdf2_dt1.6e-5_nsub40` (完走) / `run_0271`–`run_0277_passiveF_order3_*` (**step 1 で NaN**) + `_probe_passiveF/newbin_*` | **§6-6 追加 (plan-3 M4): S3 + FCT 有効の時間次数** — `run_0256` プロトコル + `speciesFaceReconstruction 2`, `speciesImplicitCoupling 1`, `passiveImplicitCoupling` auto, `implicitRelax 0.7`, `convMethod 1`; BDF2 3 水準 + nSub 80 (`0270`–`0273`), BDF1 3 水準 (`0274`–`0276`), `0277` = dt 8e-6 BDF2 `passiveFct 0` | `0270` (11:57 ビルド, dt 1.6e-5): NaN 0、sub-iter 低下 ro 1.94 (min 1.44) / roUy 1.48 (0.96) / roe 1.79 / roY1 1.85 (1.43) / rog 3.61 / roQ0 3.61 (2.90) = **2 次流れでは流れ側が ≥2 桁を割る**; fctCorr roQ0: dropped 1.2e11 (rel 7.8e-3), baseViol rel 8.9e-8, bndFluxSigned 1.09e13, bndDropped 8.2e9, srcHist 1.65e13, remAbs rel 6.5e-5, qL 13 sweep, HO 残差 2.5e-6; 実現可能性 nearest-point 260k/10 step。**`0271`–`0277` は 12:07:31 に再ビルドされた working-tree バイナリ (未コミット: condensationRealizability_d.cuh / passiveFct_d.cuh / speciesTransport_d.cu / main.cpp) で回り全て step 1 で NaN** (`two-phase temperature inversion failed` 警告 → `roUx`/`ro` NaN)。切り分け `_probe_passiveF/newbin_order1` (= `run_0263` 設定, SFR 0, FCT 無効) も同じバイナリで **step 0 の初期残差から rms_rog/roQ1/roQ2 = NaN → step 1 で NaN** (`_nofct`/`_sfr0` 変種も同じ) = **12:07 ビルドの凝縮モーメント経路が壊れている** (case/09 トレーサ箱 `newbin_0136` は動く)。3 水準の次数・nSub 比・A/B は **未取得**: 修正バイナリで `0271`–`0277` を新番号で再取得すること | `0270` active (単発記録); `0271`–`0277` 破棄予定 (壊れたビルド); `_probe_passiveF/` 破棄予定 |
| `run_0278_passiveG_smoke_order3` / `run_0279_passiveG_smoke_order1` | 検証 1 巡目の不具合 5 件 (射影の $r=0$ NaN・restart layout・θ_b/floor・射影の時機・KEEP 警告) 修正後のスモーク (`run_0271` / `run_0264` 設定を 6 step) | NaN 0 (両方; 修正前は step 1 で NaN)。S3 側は `[passiveFct] active` (4 受動種), E_ρ rel 5–6e-8, 上限逸脱 0; 実現可能性補正は step 末尾のみで 数百/step (修正前は sub-iter 内で数百万) | 破棄予定 (スモーク) |
| `run_0280`–`run_0286_passiveG_order1_*`, `run_0287`/`0288` (診断), `run_0289`–`run_0296_passiveG_order3_*`, `run_0297`–`run_0308` (再取得 1 回目) | **検証 2 巡目の先行試行 (2026-09-16 20:34 / 20:46 ビルド)**: 下の `0311`–`0329` と同設定。バイナリ再ビルド (initial-total 記録 → double totalTime + double ρ_l 射影) とゲート更新のたびに新番号で取り直した | 場は下の系列と反復ノイズ内で同一 (double ρ_l 射影前の `0280` 系は BDF2 次数 g 0.94 / Q0 1.08 / Q1 1.18 / Q2 1.14 と僅かに低い)。切り分け結果は `0311` 系の行に統合 | **破棄予定** (ゲート要件 [initial-total / nominal time] 未充足の旧ビルド写し) |
| **`run_0311`–`run_0314_passiveG_order1_bdf2_*` / `run_0315`–`run_0318_passiveG_order1_bdf1_*`** (C: 1 次流れ) + 診断 **`run_0319_…_norealiz`** (`condRealizProject 0`) / **`run_0320_…_nothresh`** (`condDgMaxStep`/`condDTmaxStep` 1e9) / **`run_0309`/`run_0310_…_coupling1`** (`speciesImplicitCoupling 1` + `implicitRelax 0.7`, nSub 40/80) | **plan §6-6 検証 2 巡目 (バイナリ 20:58:22 = b437a45a, ゲート 22b7f0ba, 2026-09-16)**: `run_0263`–`0269` プロトコル (`run_0170` 入力 + `run_0256` の過渡 IC, `convMethod 0`, SFR 0 [FCT 非作動], nSub 40, dt 1.6e-5/8e-6/4e-6 → 100/200/400 step; BDF2 3 水準 + dt 8e-6 nSub 80, BDF1 3 水準 + dt 8e-6 nSub 80 [`0318`, 更新ツールが `--nsub` 必須]); 評価 `analyze_moment_order.py` (`--nsub`, BDF1 は `--bdf 1`) | NaN 0・WARNING 0 (全 12 run)。**観測次数 BDF2** (`0311`–`0313`): g 1.13 / Q0 1.43 / Q1 1.54 / Q2 1.47 / T 1.27 / ρ 2.29 / P 1.86 (1 巡目 1.08/1.28/1.35/1.28 から改善、**2.0±0.3 は未達**); **nSub 倍増差/最小水準差** g 0.41 / Q0 0.39 / Q1 0.43 / Q2 0.42 / T 0.08 / ρ 0.18 (**≤0.1 未達**, 1 巡目と同じ)。**BDF1** (`0315`–`0317`): g 0.95 / Q0 0.69 / Q1 0.81 / Q2 0.89 / T 1.31 / ρ 1.94 / P 0.60, nSub 比 0.018–0.019 (BDF1 は PASS 水準; 絶対差 Q0 9.6e11 は BDF2 の 1.3e12 と同桁 = **sub-iter 由来の差は BDF に依らず ~1e12 で、BDF2 では時間離散誤差 [3.4e12] が小さいので比が大きく出る**)。sub-iter 低下 (min/median, dt 8e-6): roQ0 2.06/2.88 (最終 rms 床 2.6e8), rog 2.04/3.22, roQ1 3.70, ro 1.87, **roY1 1.60 (min 1.47; dt 4e-6 で 0.99)** → 更新ツールの「全列 ≥2 桁」は流れ/化学種列で FAIL (1 巡目の「roY1 2.2」はツール定義違い; step 5/199 の生残差は 1 巡目 `run_0264` と同一)。`condLim_0` min 0.08–0.23 (26–95 cells<1)。実現可能性射影 (step 末尾のみ) nearest-point 6.7万–30万 / degenerate 1.3万–2.5万 (1 巡目の 270万–1220万 から 1/40)。**切り分け (dt 8e-6 BDF2, vs `0312`; e(dt)=\|0312−0313\|)**: nSub 倍増差 `0314` Q0 0.39, **射影 off `0319` 0.42, 閾値 off `0320` 0.42, coupling 1 `0309`→`0310` 0.69** (roQ0 の sub-iter 床 rms 2.6e8 は射影/閾値 off で不変, coupling 1 で 1.5e8 だが場の差は増える) → **射影・閾値クランプ・陰的結合のいずれも roQ0 の sub-iter 床を動かさない**; 倍増差はクランプ作動集合 (condLim<1) に 0 % しか乗らず発達湿り域全体に分布 (上位 20 ノードで 17 %)。`0319` (射影 off) は最終場で singular 458 / 違反 453 ノード = 射影が要ることの確認。`check_passive_budget.py`: **全 run FAIL** — clampBudget \|Δ\| rel Q0 1.8e-5–6.8e-5 / Q1 7e-6–4e-5 / Q2 1.0e-6–2.0e-6 (SUM>tol) + unsteady モードの conservation 判定; `check_passive_field.py` **FAIL (実現可能性違反 41–87 ノード)** だが違反ノードは全て **g ≤ 2.8e-38 (float32 非正規化域の塵; g>1e-30 の違反 0)**、最大違反 1.6e-2 は 1 ノード → 射影の限界ではなく塵ノードの判定閾値の問題 (ツール側に g 下限が要る)。壁時間 103–109 ms/step (nSub 80: 207–210) | active (§6-6 未達, 切り分け済) |
| `run_0330_passiveG_order1_diag_noevap` / `run_0331_…_diag_nsub160` / `run_0332_…_diag_cflp6` / `run_0333_…_diag_nsub41` | **sub-iter 床の診断** (plan §5.1 #12/#18; `run_0312` [1 次流れ, BDF2, dt 8e-6, nSub 40] の複製): `condEvaporation 0` / `nSubIterDualTime 160` / `cfl_pseudo 6 + implicitRelax 0.7` / `nSubIterDualTime 41` | 全 run NaN 0・exit 0。roQ0 の sub-iter 床 (最終反復の rms 中央値 2.5e8) は蒸発 off・nSub 160・cfl 6 のいずれでも不変 (1.8–2.5e8) = **ulp ノルム 2.3e8 (float32 の状態 ×2⁻²³ の L2)** に一致。終了場の差 (L2, `run_0312` 比): **nSub 41 と nSub 80 が同じ大きさ** (roQ0 1.29e11 vs 1.11e11 = e(dt) 2.92e11 の 0.44/0.38; rog 0.51/0.42; roY0 0.15/0.18; roe 0.08/0.10)、nSub 160 は nSub 80 から更に 1.45e11 (幾何収束しない)、cfl 6 は 2.7e11、蒸発 off 1.4e11 → sub-iter 反復の不足ではなく **float32 の丸め床が凝縮ソース (核生成率の T 感度) で増幅されたノイズ床** (モーメントは ulp ノルムの 500 倍、流れは 3 倍)。3 水準の e(dt) がノイズの 2 倍しかないので観測次数 1.35 は無意味 → 倍精度ビルドで同系列を再実行 (`run_0334`–`0337`) | active (診断; 結論は plan §9 2026-09-17) |
| `run_0334`–`run_0337_passiveG_order1_double_*` | **倍精度ビルド** (`flowFormat.hpp` の typedef を double にした detached worktree `forge-double` @ 8d2661b1, `_f` 熱力学の double 配列橋渡し 4 関数を局所パッチ, `FORGE_CUDA_BLOCKSIZE=64`) で `run_0311`–`0314` の入力を不変のまま再実行 (BDF2, dt 1.6e-5/8e-6/4e-6 nSub 40 + dt 8e-6 nSub 80) | `analyze_moment_order.py --double`: **nSub 比が全量 0.004–0.013** (float 0.07–0.44) → float の nSub 比はノイズ床で確定。次数: roUx 1.93 / roUy 1.95 / roe 1.89 (ok), ro・roY 2.39 (上限 2.3 超; 粗い水準が漸近域外 → `run_0347`/`0348` で下側 3 水準を検査), **モーメント g 1.32 / Q0 1.39 / Q1 1.59 / Q2 1.54 = float と同じ** → 次数不足はノイズでなく**核生成 onset 前線の非平滑性**: e(dt)² の 70–85 % が上位 1 % のセル (Linf 次数 0.1–1.0)、残り 99 % の L2 次数 **1.93–2.07**、体積重み総量の次数 2.2–3.2、T も同じ構造 (L2 1.30, 前線外 2.40)。sub-iter 低下 roQ0 min 2.05–2.30 桁 (float 1.61–1.69: 床 = ulp ノルム)。checkpoint の dt/totalTime は float32 丸めなし (`--double`) | active (§6-6 の判定材料; 結論 plan §9 2026-09-17) |
| `run_0338`–`run_0341_passiveG_smooth_double_*` / `run_0342`–`run_0345_passiveG_smooth2_double_*` | **滑らかなモーメント場の時間次数** (§6-6 (ii) の「滑らかな解」): `run_0312` の IC から液滴場を消し、ガウス状ブロブ (r_d 0.15 µm; `gen_smooth_moment_ic.py`) を置く。核生成は `condSigmaScale 3.0` で抑止。smooth = 乾き核 (y 0.36, 単分散 x=y=1 = 実現可能域の角) / smooth2 = 湿り域内 (y 0.69, 多分散 x=y=0.9 = 内部)。倍精度、BDF2 3 水準 + nSub 80 | いずれも収束 IC が S≈1 なのでブロブは**蒸発**して消える (rog 総量 2.75e-6 → 5.3e-10; 蒸発ソースの試験になった)。**モーメント次数 smooth: g 2.01 / Q2 1.97 / Q1 1.92 / Q0 1.82, smooth2: g 1.96 / Q2 1.91 / Q1 1.83 / Q0 1.74, nSub 比 0.000–0.006** (PASS 範囲)。smooth の sub-iter 低下は Q0 で負 (消滅寸前の場で射影 [28k 回] と非負化のリミットサイクル)、smooth2 は全列 ≥2.04 桁 (射影 18.6k 回)。流れ ro/roY 2.39 は同じ。成長のみの試験は乾き IC から `run_0353`–`0356` | active |
| `run_0346_passiveG_order3_bdf2_dt8e-6_nsub41_noise` | `run_0322` (S3 + FCT, float) の nSub 41 = 同一バイナリのノイズ床 (`analyze_moment_order.py --noise`) | nSub 41 差 ≈ nSub 80 差: g 1.6e-6 vs 1.7e-6, Q0 5.0e11 vs 5.5e11 (e(dt) 2.8e-5 / 5.7e12 → 比 0.06 / 0.09 = ノイズ床), **流れは nSub 差がノイズの 2–3 倍** (ro 6.5e-4 vs 2.4e-4, 比 0.47) で流れ側の sub-iter 未収束 (2 次 MUSCL + リミッタの床: 低下 1.3–1.9 桁) は実在 → 流れ 1 次 + FCT の系列 `run_0349`–`0352` (倍精度) | active |
| `run_0347_passiveG_order1_double_bdf2_dt2e-6_nsub40` / `run_0348_…_dt4e-6_nsub80` | 倍精度 1 次流れ系列の下側 3 水準 (dt 8e-6/4e-6/2e-6, nSub 80 は dt 4e-6) | **流れは漸近域で 2 次**: ro 1.82 / roUx 1.80 / roUy 2.02 / roe 1.92 / roY 1.82 (上側 3 水準の 2.39 は粗い水準 dt 1.6e-5 が漸近域外), nSub 比 0.010–0.017。モーメント L2 は前線で 1.22–1.58 のまま (上位 1 % に e(dt)² の 76–91 %, 残りの L2 1.77–1.81, 総量 2.1–5.9) | active |
| `run_0349`–`run_0352_passiveG_fctconv0_double_*` | **FCT 作動 (SFR 2 + passiveFct) × 1 次流れ (convMethod 0)** 倍精度, BDF2 3 水準 + nSub 80 (`run_0321`–`0324` の入力で convMethod だけ 0) | `[passiveFct] active`; sub-iter 低下は全列 ≥1.98 桁 (中央値 2.75–5.2) なのに **流れ・化学種の nSub 比 0.15–0.58** (ro 5.7e-4 / e(dt) 9.7e-4) と粗い水準の巨大な誤差 (e(2dt) が 1 次系列の 6–10 倍 → 見かけ次数 2.8–4.6): S3 化学種 (SFR 2) が dual-time で nSub 依存を持ち込む。FCT の有無を `run_0357`/`0358` (SFR 2, passiveFct 0) で切り分け。モーメントは前線律速 (L2 1.32–1.60, 前線外 1.94–2.07, 総量 2.3–3.2, nSub 比 0.006–0.011) | active |
| `run_0353`–`run_0356_passiveG_growth_double_*` | **成長のみのソース試験**: 乾き収束場 `run_0128` res_24000 (S 7–95, T 218–242 K) にブロブ (x 4.13, y 0.69; `gen_smooth_moment_ic.py --center`), `condSigmaScale 3`, 倍精度 | `analyze_moment_order.py --double` **PASS** (全保存量): ro 1.86 / roUx 1.80 / roUy 1.93 / roe 1.81 / roY 1.86 / **g 1.95 / Q2 1.90 / Q1 1.83 / Q0 1.73**, nSub 比 ≤0.007 (モーメント 0.000), sub-iter 低下 ≥2.41 桁。ただし液滴は成長 (rog 総量 3.0e-6 → 2.2e-5 @ step 81) の後 **出口から流出** (u≈1000 m/s, 1.6 ms で 1.6 m) して終了時刻には 5e-10 → 上流 (x 3.4) + 0.8 ms の `run_0359`–`0362` で再試験 | active (PASS; 終了場は流出後) |
| `run_0359`–`run_0362_passiveG_growth2_double_*` (前段 `run_0367_…_pre_dt5e-6_40steps`) / `run_0368`–`run_0371_passiveG_growth3_double_*` | **成長ソースの時間次数 (正式)**: growth2 = 乾き場 `run_0128` res_24000 に onset 直下流のブロブ (x 3.4, y 0.71, σ 0.20×0.045) を置き 0.8 ms; growth3 = growth2 の場を dt 5e-6 で 40 step 進めた `run_0367` res_40 を IC に 3 水準 (dt 1.6e-5/8e-6/4e-6, 0.64 ms) + nSub 80 (起動過渡を除いた発達場; どの水準とも刻みが違うので全 run 同じく BDF1 再開) | **モーメントの BDF2 が成立**: growth2 g 1.91 / Q2 1.93 / Q1 1.95 / Q0 1.97, growth3 g 1.92 / Q2 1.94 / Q1 1.95 / Q0 1.97、**nSub 倍増差は e(dt) の 0.000** (絶対値 6e-10〜1e6 = 場の 1e-16 相当)、sub-iter 低下 モーメント 6.6–7.1 桁・全列 ≥2.41 (growth2) / ≥3.19 (growth3) 桁。液滴は成長し続ける (rog 総量 1.86e-6 → 3.81e-5)。流れは growth2 で ro/roY/roe 2.00・roUy 1.55、growth3 で 1.62–1.91 (ノズル自身の非平滑性; `run_0347`/`0348` の漸近 3 水準では 1.80–2.02) | active (診断: 1 次風上でのモーメント次数) |
| `run_0357`/`0358_passiveG_s3nofct_double_*` / `run_0365`/`0366_…_s3coupling0_*` / `run_0363`/`0364_…_s1coupling1_*` / `run_0372`/`0373_…_s3venkat_norelax_*` | **S3 系列 (D) の nSub 依存の切り分け** (倍精度, dt 8e-6, nSub 40 vs 80 の終了場 L2 差): `passiveFct` 0/1, `speciesImplicitCoupling` 0/1, `speciesFaceReconstruction` 0/2, `limiter` 1/2, `implicitRelax` 有無を 1 つずつ入れ替え | nSub 差 (ρ): S1 Venkat 無緩和 **9.28e-6** / S1 coupling 1 9.20e-6 / S3+FCT Barth relax0.7 5.65e-4 / S3 FCT 無 Barth relax0.7 **5.65e-4** (FCT は無関係) / S3 coupling 0 Barth relax0.7 5.65e-4 (coupling も無関係) / **S3 Venkat 無緩和 9.04e-6 (S1 と同じ)** → **S3 そのものは無罪**で、`limiter 1` (Barth) か `implicitRelax 0.7` が sub-iter 不動点を壊している。切り分けは `run_0374`–`0377` | active |
| `run_0374`/`0375_…_s3barth_norelax_*` / `run_0376`/`0377_…_s3venkat_relax07_*` | 上の切り分けの最終 2 組 (S3 倍精度, dt 8e-6, nSub 40 vs 80): Barth リミッタだけ / `implicitRelax 0.7` だけを戻す | nSub 差 (ρ): Barth 無緩和 **8.98e-6** (= S1 並) / Venkat + relax 0.7 **5.65e-4** → **原因は `implicitRelax 0.7` 単独**。Barth リミッタは無罪。緩和は sub-iter の残差ノルムを見かけ上下げながら (低下 2.81–3.55 桁) 遅いモードを nSub 40 では収束させない (不動点は変えない: nSub を倍にすると差が縮む) | active (**結論: dual-time の時間次数・sub-iter 試験で `implicitRelax` を使わない**) |
| `run_0378`–`run_0381_passiveG_fctorder_double_*` (**棄却: `convMethod 0` で S3 が不活性、最粗水準が収支ゲート FAIL — 正式系列は `run_0429`–`0432`**) | (旧) §6-6 (ii) の FCT 作動版: `run_0368`–`0371` (growth3 の滑らかブロブ) と同じ IC・水準で `speciesFaceReconstruction 2` + `passiveFct 1` + `speciesImplicitCoupling 1` + Venkat + 緩和なし、倍精度 | `[passiveFct] active` (4 受動種)。**モーメント g 1.92 / Q2 1.92 / Q1 1.94 / Q0 1.97、nSub 倍増差 0.000** (6.6e-10〜1.1e6)、化学種 roY 1.70、sub-iter 低下 全列 ≥3.20 桁 (モーメント 6.7–7.0)。`analyze_moment_order.py` の VERDICT は roUx 1.62 / roe 1.65 で **FAIL** だが、この 2 列の誤差は**スロート軸近傍** (x≈0, y<0.05 に上位 1 % が集中、e(dt)² の 47–63 %) に局在し、その 1 % を除くと **1.89–1.92** → 軸対称スロート軸の非平滑性 (本 plan の対象外) で、受動種経路は 2 次 | 破棄予定 (codex result-3 M1/M2) |
| `run_0382_scheme0_realiz1_float_dt8e-6` / `run_0383_…_realiz0_…` / `run_0393_…_realiz1_…_rep` | **旧経路 (`passiveScalarScheme 0`) の無影響検査** (codex result-3 M3): `run_0312` の IC・設定を scheme 0 に落として 100 step、`condRealizProject` 1 (既定) と 0、および同一設定の反復 | **初回は差が出た** (Q1 max 1.2e3 等) → 真因は射影本体ではなく、物理 step 末尾の後処理ブロック (`main.cpp`) が `condRealizProject != 0` だけで有効になり **EOS 再更新 (`dependentVariables`) まで旧経路に入っていた**こと。射影と後処理ブロックを `passiveScalarScheme 1` 限定に修正した binary では、**1 vs 0 の差 (Q1 1.21e3, T 1.7e-3, P 0.375, ρ 6.2e-6) が同一設定の反復ノイズ (1.50e3, 1.6e-3, 0.5, 5.7e-6) と同等** = 旧経路は変わらない | active (§6-7 無影響) |
| `run_0384`/`0385_s1conv1_*` / `run_0386`/`0387_s3fctconv1_*` / `run_0388`/`0389_…_relax07_*` / `run_0390`–`0392_s3fctconv1_*` | **convMethod 1 (S3 が実際に効く条件) での再取得** (codex result-3 M1: `convMethod 0` では `interp_dispatch` が 1 次を返すので SFR 2 は不活性だった): growth3 の滑らかブロブ IC、倍精度、dt 8e-6/4e-6/2e-6 + nSub 80 | nSub 40→80 の ρ 差: S1 9.31e-6 / S3+FCT 8.85e-6 / +relax 0.7 1.76e-5。次数 (dt 8/4/2e-6): モーメント **g 1.89 / Q2 1.87 / Q1 1.91 / Q0 1.80 (ok), nSub 比 0.000** だが流れ・化学種が 1.18–1.29 → **IC が `convMethod 0` で作られていて空間スキーム切替の初期層が乗った**ため (`run_0394` で IC を作り直し)。収支は dt 4e-6/2e-6 PASS、dt 8e-6 は Q1/Q2 の履歴残り 2–4e-5 で FAIL (射影の補正が粗い刻みで大きい) | active |
| `run_0394_s3fctconv1_double_pre_dt5e-6_40steps` + `run_0395`–`run_0398_fctorder2_double_*` | **§6-6 (ii) FCT 版 (正式・再取得)**: `convMethod 1` + S3 + FCT + Venkat なし (Barth) + 緩和なしで IC を 0.2 ms 進めて空間スキームと整合させ (`run_0394`)、そこから dt 4e-6/2e-6/1e-6 + nSub 80 (終了 0.32 ms)、倍精度 | **流れ・化学種は全列 2 次** (ro/roY 1.85, roUx 1.77, roUy 1.85, roe 1.84), **nSub 比 ≤0.008**, sub-iter 低下 ≥2.52 桁, **収支ゲート 3 水準とも PASS**, 場ゲート PASS。モーメントは **g 1.60 / Q2 1.73 / Q1 1.79 / Q0 1.69** で g と Q0 が 1.7 をわずかに下回る → 実現可能性射影の補正が **dt^1.4 でしか縮まない** (Q0 の |Δ| 5.07e-5 → 1.96e-5 → 7.32e-6、作動セル数 8254 → 7108 → 6931) ことが律速候補 → `run_0405`–`0408` (`condRealizProject 0`) で切り分け | active |
| `run_0459`–`run_0480_sweep_cflp{2,6,12,20,40,80,160}_nsub{10,20,40,80}_float` | **dual-time の擬似時間 CFL と sub-iteration 数のスイープ** (生産 float, 湿り核生成場 `run_0312` IC, convMethod 1 + S3 + FCT + coupling 1, `implicitRelax` なし, dt 8e-6 200 step, 物理 cfl 2) | **cfl_pseudo 160 まで発散しない** (NaN 0, 全 run exit 0)。収束解は cfl_pseudo に依存しない (nSub 80 同士の ρ 差 3.1–3.3e-4 = 同一 cfl の nSub 40 vs 80 差 2.3e-4 と同程度 = float のノイズ床)。**必要な nSub は cfl_pseudo で決まる**: nSub 80 との ρ 差は cfl_pseudo 2 で nSub 10 → 1.2e-1 / 20 → 4.5e-3 / 40 → 2.3e-4 なのに対し、**cfl_pseudo 12 以上では nSub 10 で既に 2.2e-4** (ノイズ床)。壁時間は 0.045 s/step (nSub 10) vs 0.14 (nSub 40) vs 0.265 (nSub 80) → **cfl_pseudo 12–20 + nSub 10–20 が cfl_pseudo 2 + nSub 40 と同じ解を 1.8–3 倍速で出す**。収支ゲートは両設定とも同じ理由で FAIL (核生成が効く float の場では lim 1e-6–1e-5・|H_rem| 1e-4 級で、設定差では変わらない; 滑らかブロブの検証系列とは別物) | active (dual-time の設定指針) |
| `run_0457`/`run_0458_matrix_bdf2_cpl1_fct_double_dt2e-6_nsub40_rebuild{,2}` | **修正の無影響確認**: `run_0430` を、射影の書き戻しを指数分離に統一した binary で再実行 (`0457`) + 同一設定の反復 (`0458`) | `0430` (旧 binary) と `0457` の差は `0457` vs `0458` の反復ノイズと同等 (ρ 3.38e-7 vs 3.66e-7, roe 0.254 vs 0.296, roQ0 2.73e5 vs 2.85e5) → **検証行列の結果は今回の修正で変わらない** | active |
| `run_0429`–`run_0448_matrix_*` (+ 細水準 `run_0449`–`0452`, `run_0453`–`0456`) | **§6-8 の検証行列 (正式)**: FCT 有効条件 (convMethod 1 + SFR 2 + passiveFct 1, IC `run_0394` res_40, 倍精度, 終了 0.64 ms) で **BDF2/BDF1 × `passiveImplicitCoupling` 0/1** の 4 組 (各 dt 4e-6/2e-6/1e-6 + nSub 倍増) + **FCT 無効の対照** (BDF2 × coupling 1)。実現可能性射影の半径 underflow 修復後の binary | **BDF2 × coupling 1 / coupling 0**: 次数ゲート **PASS** (g 1.740 / Q2 1.924 / Q1 1.875 / Q0 1.381 [FCT 緩和下限 1.3]、流れ・化学種 1.93–1.95 は厳格帯)、**収支ゲート 4 run とも PASS**、nSub 比 0.000–0.006。**coupling 0 と 1 の差は 4 桁目以降**。**BDF1 × coupling 1 / coupling 0**: モーメントは 1 次に落ちる (正しい) が dt 4e-6 は漸近域外で流れ 2 列が [0.7,1.3] を僅かに外れる (roUy 0.627 / roe 0.692) → 水準を 2 度細かくした `run_0449`–`0452` (dt 2e-6/1e-6/0.5e-6) → `run_0453`–`0456` (dt 1e-6/0.5e-6/0.25e-6) で **両 coupling とも次数ゲート PASS** (モーメント **0.965 / 0.972 / 0.974 / 0.971**、流れ 0.771–0.875、nSub 比 ≤0.001、収支も PASS)。観測次数は水準を細かくするほど 1 に近づく (roUy 0.627 → 0.680 → 0.771)。**対照 (FCT 無効)**: モーメント **1.987–2.013** で次数 PASS、収支は設計どおり INDETERMINATE (FCT 記録が無いので保存は判定不能; 場ゲートは PASS) | active (**§6-8 の正本**) |
| `run_0425`–`run_0428_fctorder5nofct_double_*` | **対照系列**: `run_0417`–`0420` と同一 (IC・convMethod 1・S3・倍精度・0.64 ms) で **`passiveFct 0`** | **全保存量が 2 次で VERDICT PASS** (既定閾値): ro 1.95 / **g 1.99 / Q2 1.99 / Q1 2.01 / Q0 2.01**, nSub 比 ≤0.005 → **スキーム (S3 + BDF2 + dual-time) 自体は 2 次**。FCT を入れると g 1.74 / Q2 1.92 / Q1 1.88 / **Q0 1.38** に下がる = **FCT の Zalesak 制限 (非線形リミッタ) が極値近傍で L2 の観測次数を下げる**ことが確定 (射影・蒸発・丸め・緩和はいずれも無関係と個別に排除済み)。この分だけ `analyze_moment_order.py --expect-fct` の下限を 1.3 にし、スキームの 2 次は本対照系列で示す規約にした | active (**§6-6 (ii) の対照・正本**) |
| `run_0417`–`run_0420_fctorder5_double_*` (= `run_0413`–`0416` を射影修正後の binary で再実行) / `run_0421`–`0424_fctorder5noevap_*` (`condEvaporation 0`) | **§6-6 (ii) FCT 版の正式系列** (convMethod 1 + S3 + FCT, IC は `run_0394` res_40, 終了 0.64 ms, dt 4e-6/2e-6/1e-6 + nSub 80, 倍精度) と roQ0 の律速要因の切り分け | **収支ゲートは全水準 PASS**、場ゲートは当時のゲートでは PASS だったが、**改定後のゲート (半径 underflow を判定不能として拒否) では `run_0421` が FAIL** (6 セル; 旧ゲート判定として残す。修復後の binary で取り直した `run_0429`–`0456` は PASS)。次数: 流れ ro/roUx/roUy/roe **1.93–1.95**、化学種 roY **1.95**、**g 1.74 / Q2 1.92 / Q1 1.88 (すべて ok)**、**roQ0 のみ 1.381** (nSub 比は全量 ≤0.006)。roQ0 の誤差は上位 1 % のセルに 79 % が集中し、その 99 % を除くと **2.00**。`condEvaporation 0` でも 4 桁一致 (**蒸発・液滴消滅は無関係**)、`condRealizProject 0` でも一致 (**射影も無関係**)、倍精度なので丸めでもない → 残る候補は FCT の Zalesak 制限 (`run_0425`–`0428` で `passiveFct 0` と比較) | active |
| `run_0405`–`run_0408_fctorder2_norealiz_double_*` / `run_0409`–`0412_fctorder3_*` / `run_0413`–`0416_fctorder4_*` | **モーメント次数の律速要因の切り分け** (すべて `run_0395`–`0398` と同一の IC・設定からの 1 因子変更, 倍精度): norealiz = `condRealizProject 0`; fctorder3 = 終了時刻 0.16 ms; fctorder4 = 0.64 ms | **射影は無関係** (norealiz は 4 桁一致: g 1.602 / Q0 1.691)。**窓の長さが効く** (全水準が BDF1 の 1 step で始まるため): 0.16 ms で g 1.36 / Q2 1.26 / Q1 1.59 / Q0 1.62、0.32 ms で 1.60 / 1.73 / 1.79 / 1.69、**0.64 ms で g 1.74 / Q2 1.92 / Q1 1.88 (ok) / Q0 1.38**、流れ・化学種は 0.64 ms で 1.93–1.95 (ok)。`run_0415` (dt 1e-6) の確定場に**塵セル 2 個の実現可能性違反** (rog 1e-193/1e-116, x 1e155) → 射影の overflow 欠陥を発見・修正 (commit 25465361) | active |
| `run_0399`/`0400_wetconv1_s1_*` / `run_0401`/`0402_wetconv1_s3fct_*` / `run_0403`/`0404_…_relax07_*` | **sub-iter 依存の切り分け (正式)**: `run_0312` の湿り IC (実際に核生成する場) × `convMethod 1` × 倍精度、dt 8e-6 200 step、nSub 40 vs 80 | ρ の nSub 差: **S1 1.40e-5 / S3+FCT 1.32e-5 / S3+FCT+`implicitRelax 0.7` 6.32e-4 (45 倍)**; roY0 も同様 (1.35e-5 / 1.27e-5 / 6.08e-4)。**2 次再構成が実際に効く条件でも原因は `implicitRelax` 単独**で、S3 と FCT は 1 次種と同じ (codex result-3 M1 への回答) | active (**結論の正本**) |
| **`run_0321`–`run_0324_passiveG_order3_bdf2_*` / `run_0325`–`run_0328_passiveG_order3_bdf1_*`** (D: S3 + FCT 作動) + **`run_0329_…_nofct`** (`passiveFct 0`, dt 8e-6 BDF2) | **plan §6-6 追加 (plan-3 M4) 検証 2 巡目**: 同プロトコルを `speciesFaceReconstruction 2` + `speciesImplicitCoupling 1` + `implicitRelax 0.7` + `convMethod 1` + **`limiter 1` (Barth)**; BDF2 3 水準 + nSub 80 (`0324`), BDF1 3 水準 + nSub 80 (`0328`); 評価 `analyze_moment_order.py --expect-fct` | NaN 0・WARNING 0・`nonfinite 0` (全 9 run), `[passiveFct] active` (4 受動種)。**観測次数 BDF2** (`0321`–`0323`): **g 1.97 (ok)** / Q0 1.37 / Q1 1.63 / Q2 1.65 / T 2.89 / ρ 4.27 / P 0.12; **nSub 比 g 0.059 / Q0 0.097 / Q2 0.086 (ok), Q1 0.110 (僅か超過)**, T 0.19 / ρ 0.47 / P 0.19 (流れ側は sub-iter 未収束: min 1.0–1.3 桁)。**BDF1** (`0325`–`0327`): Q0 0.86 / Q1 0.81 / Q2 0.91 (ok), **g 1.97 (BDF1 でも 2 次 = g の水準差は BDF 打ち切り誤差でなく別要因 [S3 面値/FCT] が支配)**, T 3.26 / ρ 3.33, nSub 比 モーメント 0.011–0.025。1 次流れ (C) との比較: e(dt) が Q0 で 20 倍 (6.9e13 vs 3.4e12)、nSub 差は 5 倍 (6.7e12 vs 1.3e12) → **D の比が小さいのは時間離散誤差が大きいため**で sub-iter 床が下がったのではない。sub-iter 低下 roQ0 2.58/3.48 (床 3.1e8), rog 2.59/3.35。`condLim_0` min 1.0 (クランプ無作動; `0329` no-FCT は min 0.0, 118 cells)。実現可能性 nearest-point 5.3万–24万 / degenerate 3.5千–4.1万。**fct (roQ0, dt 8e-6 `0322`)**: dropped rel 2.0e-3 / base 2.6e-8 / upper 0 / bndFluxSigned 1.247e13 / bndDropped 2.6e5 / srcHist 1.864e13 / remSigned −1.5e6 / remAbs rel 4.0e-5 / qL run-max 1.0e-6 (8 sweep) / HO 2.1e-6 / E_ρ 6.3e-8 (dt 1.6e-5: dropped 1.1e-2, qL 13 sweep, HO 4.2e-6; dt 4e-6: dropped 8.1e-4, HO 1.5e-6)。`check_passive_budget.py`: **全 run FAIL** — lim (θ_b は FCT 下では tracer のみ off でモーメントは残る) rog 1.2e-4 / Q1 2.8e-5 / Q2 5.6e-5 + remainder Q2 5.5e-4 / Q1 2.5e-4 / Q0 3.9e-5 (SUM 5e-5–1.2e-3); `0329` no-FCT も lim で FAIL (rog 1.5e-4)。`check_passive_field.py` FAIL (違反 20–58 ノード, 全て g ≤ 2.5e-40 の塵, 最大 6.6e-4)。壁時間 FCT 129 ms/step (`0322`) vs no-FCT 125 (`0329`) → **+3 %** (nSub 80: 254) | active (§6-6: g の次数と nSub 比は達成, Q0–Q2 の 2 次は未達) |
| `run_0231_default_scheme_check` / `run_0232_scheme1_explicit_twin` | 既定キー確認 (2026-09-17): `run_0170` 入力 + res_24000 の場、`passiveScalarScheme` キー無し (既定 1) vs 明示 1、既定ビルド (HEAD 33dc6588)、500 step | ログで scheme 1 有効 (nPassive 4)、両者の差 ro 2.7e-6 / g 9.6e-5 / Q0 3.8e-4 = node 反復ノイズ内、NaN 0 | active (既定変更の根拠) |
| **`run_0220_passive_s0_sfr0_from0170`** / `run_0221_passive_s1_sfr0_from0170` / **`run_0222_passive_s1_sfr2_c1_cfl2_from0170`** / `run_0223_passive_s1_sfr2_c1_cfl6_relax07_from0170` (2026-09-17, Phase A snapshot バイナリ `forge-bin-passive-A`) | **受動種経路の凝縮回帰 (plan species-passive-scalar-unification §6-5) と無影響 (§6-7, 24000 step 全期間)**: `run_0170` 入力 (入口 Tt 分布) + IC = `run_0170` res_24000 を `nozzle.h5` に index コピー (交差 restart `run_0179` と同じ作法), 24000 step。0220 = scheme 0 / SFR 0 (旧経路対照); 0221 = scheme 1 / SFR 0 (1 次の化学種経路); 0222 = scheme 1 / SFR 2 / `speciesImplicitCoupling 1` / `passiveImplicitCoupling 1` / cfl 2; 0223 = 同 + cfl 6 + `implicitRelax 0.7` | 全 4 本 NaN 0, `cond_series.csv` **ALL STEADY** (0.2 %), `condLim` 凝縮域 min 1.0000, `condClampCorr` 凝縮域 0。**無影響 (0220 vs run_0170 最終場)**: g_0 max 7.2e-6 / g L1 3.2e-8 (反復床 2.0e-4 / 1.95e-5 内), ro 9.5e-6, T max 0.0165 K (p99.9 0.008 K; 壁 onset 域 x≈17.9 の 16 ノード, 反復 `run_0176` の 0.0074 K の 2.2 倍だが局所), onset 15.8401 / x30 18.6454 / 出口 g 0.5844 % / g_max 0.016302 = 正本と 5 桁一致。**0221 (1 次, scheme 1)** も同じ: g_0 6.6e-6 / gL1 3.4e-8 / T 0.0145 K, onset 15.8400, floor 補正 roQ1 累積 rel 1.5e-10 (他 0)。**S3 (0222/0223) は準定常量 (ALL STEADY) の比較で 1 次と異なる (ビット一致は要求しない)**: onset 16.024 (+0.18 r_t 下流), x30 18.497 (−0.15), 出口 g **0.5639 %** (−3.5 %), g_max 0.016451 (+0.9 %), exit M 4.0560; vs run_0170 g_0 max 6.2e-3 / g L1 4.2e-5 / T max 18.6 K — 差は出口付近 (x/r_t 24.24, r/r_t 2.93) の**半径方向の凝縮前線**に局在 (1 次は r/r_t 2.6〜2.9 に g を滲ませるが S3 は前線が鋭く前線ノードで過冷却が深い), core/壁の g は +0.9 %。**cfl 2 と cfl 6+relax 0.7 の S3 は準定常量 (ALL STEADY) の比較で一致** (残差は正本と同じ plateau なので固定点の同一性の証明ではない): 0222 vs 0223 g_0 max 3.5e-6 / T 4.5e-2 K / ro 相対 3.4e-6 (反復床内)。S3 の floor 補正: 累積 rel rog 9e-9〜1.5e-8 / Q2 1.3〜1.8e-6 / Q1 1.2〜1.6e-6 / Q0 0.7〜1.0e-6 (per-step は Q0/Q1 で微小に継続 = 乾き域の数値塵 floor)。残差床は 4 本とも run_0170 と同じ plateau (rms_ro 5.3〜5.6e-7, `check_convergence` NOT CONVERGED (stalled) = 正本と同じ)。step 時間 2.91 / 2.37 / 2.82 / 2.82 ms (正本 run_0170 系 2.4〜2.5) | active (§6-5 凝縮回帰・§6-7 無影響 24000 step) |
| `run_0487_reach_rhoylim` | `multispeciesRhoYCommonLimiter: 1` の分岐到達確認 (plan [config-key-pruning](../../plans/accepted/config-key-pruning.md) §6.4')。`run_0482` を 20 step に | 診断印字 `RHOYLIM call=` が 2 件出る = 再構成側の条件 (2 成分・`speciesFaceReconstruction 2`) を満たして分岐に入っている | ref (到達確認) |
| `run_0488_reach_fcttolabs_zero` / `run_0489_reach_fcttolabs_big` | `passiveFctTolAbs` の分岐到達確認。`passiveFctTol: 1e-30` を共通にし、`passiveFctTolAbs` を 0 と 1e30 に | FCT 低次解の相対残差が **4.32e-8 → 9.73e-22** (14 桁)。この値は受入判定の分母の加算項なので、読まれなければ動かない | ref (到達確認) |
| `run_0490_limA_va3` / `run_0491_limC_va3` / `run_0492_limD_va3` / `run_0493_limD_va3_long` | **リミッタ無次元化の回帰** (`run_0129` の `res_24000` から index コピー restart、A=`mr0/scaled0` 生産既定, C=`mr1/scaled1` 無次元化, D=`mr1/scaled2` 比の形。軸対称 TP + 多成分 + 非平衡凝縮) | **C は A の残差床に戻る** (`rms_ro` 5.45e-7 vs 5.26e-7, `roUx` 2.97e-4 vs 2.96e-4, `roUy` 4.08e-4 vs 4.10e-4, `roe` 4.15e-1 vs 4.04e-1)。**D は戻らない** — 12000 step 伸ばしても床が 2.1〜5.3 倍高い (`ro` 1.52e-6, `roUy` 2.15e-3) = **③ 比の形の棄却根拠** | ref ([../../plans/active/convection-node-wall-reconstruction.md](../../plans/active/convection-node-wall-reconstruction.md) §4.22) |
...
| `run_0505_fx_ctrl_a` / `run_0506_fx_ctrl_b` / `run_0507_fx_half_a` / `run_0508_fx_half_b` | **node 面補間重み `fx=0.5` の回帰** ([plan](../../plans/active/discretization-node-face-weight-midpoint.md) §5.1 #2)。`run_0494_v2_bitcheck` と同一入力 (廃止キー `limiterMatchRecon` のみ除去)、対照 2 本 + `FORGE_NODE_FX_HALF=1` 2 本、4000 step | `check_field_regress.py --boundary`: **`VERDICT: PASS`** (最大比 1.88 < 2)。Euler なので `fx` を読む粘性・拡散経路をほぼ通らない = 不変の確認 | active |
