...
| `run_0417`–`run_0420_fctorder5_double_*` (= `run_0413`–`0416` を射影修正後の binary で再実行) / `run_0421`–`0424_fctorder5noevap_*` (`condEvaporation 0`) | **§6-6 (ii) FCT 版の正式系列** (convMethod 1 + S3 + FCT, IC は `run_0394` res_40, 終了 0.64 ms, dt 4e-6/2e-6/1e-6 + nSub 80, 倍精度) と roQ0 の律速要因の切り分け | **収支ゲート・場ゲートとも全水準 PASS** (射影修正で `run_0415` の塵セル違反は消えた)。次数: 流れ ro/roUx/roUy/roe **1.93–1.95**、化学種 roY **1.95**、**g 1.74 / Q2 1.92 / Q1 1.88 (すべて ok)**、**roQ0 のみ 1.381** (nSub 比は全量 ≤0.006)。roQ0 の誤差は上位 1 % のセルに 79 % が集中し、その 99 % を除くと **2.00**。`condEvaporation 0` でも 4 桁一致 (**蒸発・液滴消滅は無関係**)、`condRealizProject 0` でも一致 (**射影も無関係**)、倍精度なので丸めでもない → 残る候補は FCT の Zalesak 制限 (`run_0425`–`0428` で `passiveFct 0` と比較) | active |
