...
| `run_0378`–`run_0381_passiveG_fctorder_double_*` | **§6-6 (ii) の FCT 作動版 (正式)**: `run_0368`–`0371` (growth3 の滑らかブロブ) と同じ IC・水準で `speciesFaceReconstruction 2` + `passiveFct 1` + `speciesImplicitCoupling 1` + Venkat + 緩和なし、倍精度 | `[passiveFct] active` (4 受動種)。**モーメント g 1.92 / Q2 1.92 / Q1 1.94 / Q0 1.97、nSub 倍増差 0.000** (6.6e-10〜1.1e6)、化学種 roY 1.70、sub-iter 低下 全列 ≥3.20 桁 (モーメント 6.7–7.0)。`analyze_moment_order.py` の VERDICT は roUx 1.62 / roe 1.65 で **FAIL** だが、この 2 列の誤差は**スロート軸近傍** (x≈0, y<0.05 に上位 1 % が集中、e(dt)² の 47–63 %) に局在し、その 1 % を除くと **1.89–1.92** → 軸対称スロート軸の非平滑性 (本 plan の対象外) で、受動種経路は 2 次 | active (**§6-6 (ii) FCT 版の正本**) |
