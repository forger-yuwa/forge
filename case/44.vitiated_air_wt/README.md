...
| `run_0487_reach_rhoylim` | `multispeciesRhoYCommonLimiter: 1` の分岐到達確認 (plan [config-key-pruning](../../plans/active/config-key-pruning.md) §6.4')。`run_0482` を 20 step に | 診断印字 `RHOYLIM call=` が 2 件出る = 再構成側の条件 (2 成分・`speciesFaceReconstruction 2`) を満たして分岐に入っている | ref (到達確認) |
