...
| `run_0175_r2_slau_visc_old` / `run_0176_r2_slau_visc_new` (同上 AWS) | **R2 (粘性 SLAU 2 次)**: Re=1600 (μ=2.5e-4)、Pr 0.71、定数粘性、`convMethod 1`+`limiter 2`、RK4 unsteady・dual-time なし、dt 0.007×3572 step。旧/新バイナリは上と同じ | 旧新とも **PASS** (新: 質量 max 2.3e-8、運動量 1.2e-8、全エネルギー 8.3e-9)。**旧新差 (記録のみ) max\|ΔK/K0\| 1.0e-1 (t=10.85)、final 4.5e-2**、max\|Δ(dS/S0)\| 3.4e-3。旧は t≲7 で K/K0 が 1.02 まで増え ΔS<0、新は単調 (図 `r2_ke_entropy.png`)。解釈は plan 側。先行の既定ブロック (512/256) 実行の結果は AWS の `_bs512/` に保存 (同値) | active (R2) |
