...
| `run_0165_limA_tgv` / `run_0166_limC_tgv` | **リミッタ無次元化の回帰** (`run_0159_passiveG_fct_step_seam_venkat` と同一入力、A=`mr0/scaled0` 生産既定, C=`mr1/scaled1` 無次元化), unsteady 100 step | **退行なし**: ρ/P/Ux は一様流のままビット同一、受動 ξ の最大相対差 2.0e-6、有界性 [0, 0.999995] は両者同一 | ref ([../../plans/active/convection-node-wall-reconstruction.md](../../plans/active/convection-node-wall-reconstruction.md) §4.22) |
| `run_0167_perA_tgv` / `run_0168_perC_tgv` | **周期経路に `limiterScaled` を配線した後の再実施** (codex plan-3 Critical 1。`run_0165/0166` は W1 のみだった) | **退行なし**: ρ/P/Ux は一様流のままビット同一、受動 ξ の A−C 最大相対差 2.0e-6、有界性 [0, 0.999995] は同一。配線前後の ξ 差は 1.5e-6 で配線は効いている | ref (plan §4.22 の正) |
| `run_0169_fx_ctrl_a` / `run_0170_fx_ctrl_b` / `run_0171_fx_half_a` / `run_0172_fx_half_b` | **node 面補間重み `fx=0.5` の回帰** ([plan](../../plans/accepted/discretization-node-face-weight-midpoint.md) §5.1 #2)。`run_0167_perA_tgv` と同一入力 (廃止キー `limiterMatchRecon` のみ除去)、対照 2 本 + `FORGE_NODE_FX_HALF=1` 2 本、unsteady 100 step | `check_field_regress.py`: **`VERDICT: PASS`** (候補/ノイズ床 最大 1.39 < 2)。一様直交の周期格子では幾何 `fx` も 0.5 なので不変 | active |
| `run_0173_r2_keep_eul_old` / `run_0174_r2_keep_eul_new` (**AWS** `~/forge-pgrad-new/case/09.Taylor-Green/` に同名。h5 は AWS のみ、ローカルは config・履歴・VERDICT) | **R2 (非粘性 KEEP 基準)** ([plan](../../plans/accepted/boundary-node-periodic-gradient-fix.md) §6 R2): 継ぎ目勾配の修正前 `1266aba1` (forge sha256 `f0505fe8…`) / 修正後 `565959c7` (`0bcff10d…`)、pure KEEP・RK4・dt 0.007×3572 step (t=10 t_c)、`FORGE_CUDA_BLOCKSIZE=128`/`_SMALL=128` (旧新共通)、同一 h5 (`mesh/_r2conv`, sha256 `59584fdf…`) | `r2_conservation.py` (体積重み `cv_weight` が ΣV=(2π)³ に一致): 旧新とも **PASS** (\|K/K0−1\| max 6.8e-3、\|ΔS/S0\| max 8.4e-6、運動量 max 1.6e-8/1.4e-8)。旧新差 max\|ΔK/K0\| 6.0e-8 (KEEP は勾配を読まない)。`r2_conservation.txt`・`r2_ke_entropy.png` | active (R2) |
| `run_0175_r2_slau_visc_old` / `run_0176_r2_slau_visc_new` (同上 AWS) | **R2 (粘性 SLAU 2 次)**: Re=1600 (μ=2.5e-4)、Pr 0.71、定数粘性、`convMethod 1`+`limiter 2`、RK4 unsteady・dual-time なし、dt 0.007×3572 step。旧/新バイナリは上と同じ | 旧新とも **PASS** (新: 質量 max 2.3e-8、運動量 1.2e-8、全エネルギー 8.3e-9)。**旧新差 (記録のみ) max\|ΔK/K0\| 1.0e-1 (t=10.85)、final 4.5e-2**、max\|Δ(dS/S0)\| 3.4e-3。旧は t≲7 で K/K0 が 1.02 まで増え ΔS<0、新は単調 (図 `r2_ke_entropy.png`)。KE 収支 `r2_ke_budget.txt`: 新は後処理の収支残差 r が全時刻で負、旧は K/K0>1 区間で r>0 が 15 snapshot 連続 (測定後に追加した診断、機構は言えない)。解釈は plan §6.2 側。先行の既定ブロック (512/256) 実行の結果は AWS の `_bs512/` に保存 (同値) | active (R2) |
