...
| `run_0169_fx_ctrl_a` / `run_0170_fx_ctrl_b` / `run_0171_fx_half_a` / `run_0172_fx_half_b` | **node 面補間重み `fx=0.5` の回帰** ([plan](../../plans/active/discretization-node-face-weight-midpoint.md) §5.1 #2)。`run_0167_perA_tgv` と同一入力 (廃止キー `limiterMatchRecon` のみ除去)、対照 2 本 + `FORGE_NODE_FX_HALF=1` 2 本、unsteady 100 step | `check_field_regress.py`: **`VERDICT: PASS`** (候補/ノイズ床 最大 1.39 < 2)。一様直交の周期格子では幾何 `fx` も 0.5 なので不変 | active |
