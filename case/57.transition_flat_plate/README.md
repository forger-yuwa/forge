...
- 残差は前縁の特異点で下げ止まる (SST 単体の `run_0003` でも同じ床)。報告量は `check_quasisteady.py --series-csv` の VERDICT で判定する。
