...
| `su2_t3a_lm_cont` | 上の解から restart で +10000 反復 (8 スレッド)。**SU2 側の量が落ち着いていることの確認** | 遷移開始・終了・$C_f$ 最小・$C_f$ 3 点とも 30000 反復の値から表示桁 (3〜4 桁) まで不変 | ref |
| `su2_t3b_lm` | 同 T3B 条件。7100 反復で一度止め (forge と CPU を取り合って 1 反復/s まで落ちたため)、5000 反復の restart から 4 スレッドで 25000 反復 (`plate_restart.cfg`、前半の履歴は `history_part1_0-5000plus.csv`) | rms 低下 (通算): ρ −2.1 桁 (−6.45 で停滞)、ρE −2.1 桁、k −5.9 桁、ω −3.7 桁、γ −2.2 桁、$\tilde{Re}_{\theta t}$ −1.8 桁。$x_{onset}$ 0.0900 m、$x_{end}$ 0.1952 m、$C_f$($x$ 0.05/0.1/0.2/0.6/1.3) = 0.00515 / 0.00478 / 0.00525 / 0.00445 / 0.00386 | ref |
