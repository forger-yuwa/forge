...
| `run_0354_kw3_sig097` / `run_0355_kw3_sig103` | mode 3 基準の σ 一定倍率局所感度 (`condSigmaScale` 0.97 / 1.03) | NS (SST) | on | onset **12.09 / 16.81 mm** (mode 3 の 14.28 から −2.2 / +2.5 mm; 符号は理論どおり)、壁偏差 @21 +21.2 / +6.2 %。一定倍率の局所感度であり σ の外挿精度や信頼区間の議論には使えない。σ 0.97 は 48000 step では未収束だったため **96000 step (cfl_pseudo 1.0) に延長**: 残差は plateau のまま (ro 2.1 / roUx 2.0 / roe 1.1 桁) だが報告量は 60000–96000 の窓で頭打ち (onset 振幅 4e-5 mm, 壁偏差 1e-3 %pt, g_exit 3e-8)、`--series` は h0err だけ振幅 0.03 kJ/kg (≈1e-4 h0) の OSCILLATING → リミットサイクル準定常として報告 | 0.97: NOT CONVERGED (plateau), series OSCILLATING (h0err のみ; 報告量は STEADY) / 1.03: PASS, STEADY | active (σ 感度) |
