...
| `run_0486_passiveB_dt_species_smoke` / `run_0487_passiveB_dt_ckpt100` / `run_0488_passiveB_dt_restart_from0487` / `run_0489_passiveB_dt_restart_oldformat` | node (同上) | Euler, TP 5 種, **dual-time** (`unsteady 1, dualTime 1, timeIntegration 11, blockDPLUR 1, control 0`, dt 5e-7 s [物理 CFL≈8.4], `nSubIterDualTime 15`, cfl_pseudo 2, `passiveScalarScheme 1`), IC = `run_0471/res_12000` (収束場) | off | **Phase B (plan species-passive-scalar-unification §4.4/§6-6) の多成分 dual-time smoke と restart 契約**: `0476` = 200 step 連続; `0477` = 100 step で checkpoint (`res_100.h5` の `/CHECKPOINT`: roN..roeN, roKN, roOmegaN, roY0P..roY4P + totalTime/dt/nHistoryValid/layout); `0478` = `0477/res_100` から 100 step (履歴復元, ログ `history restored ... nHistoryValid=2`); `0479` = 同じ入力から `/CHECKPOINT` を削除 (旧形式) → `history NOT restored ... BDF1`。結果: NaN 0、max\|ΣY−1\| 6.2e-8 (全出力)、化学種は動く (max\|ΔY\| 4.3e-7, Δρ 1.6e-6: 収束場なので微小)、sub-iter の残差低下は 0.03–0.3 桁 (**収束場では初回 sub-iter の残差が既に float 床 [rms_ro 6e-9, rms_roY 2e-10] で BDF 込み残差の桁低下は観測できない**; 過渡 IC の `run_0480` と case/09 `run_0071`–`0078` で判定)。restart: 連続 `0476/res_200` vs `0478/res_100` (同一物理時刻) ro 1.2e-6 / roUx 2.6e-6 / roe 3.5e-6 / roY 1.7e-6 = 同一設定の反復差 (`0476/res_100` vs `0477/res_100`: roUy 4.8e-5, ro 級 1e-6) 以内; 旧形式 BDF1 再開 `0479` も 1.5e-6 / 2.2e-6 / 3.6e-6 で完走 | 200/100 step (途中) | active (Phase B 検証) |
