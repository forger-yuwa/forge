...
| `run_0353_kw3_feder_surf` | mode 3 (Feder carrier + 表面仕事; 物理モデル) | NS (SST) | on | onset **14.28 mm**、壁偏差 @21 **+17.5 %** / @42 +3.5 / @52 +2.9。θ 0.03–3.0。**結合 run の onset 序列は局所 θ の序列どおり (0 < 3 < 2 < 1)**。carrier 形は等温寄りで実験 (onset 帯 −4.5 % の mode 1 より早い側) から離れる → 純蒸気 Kantrowitz の「良い一致」は CNT の J 過大を補償している可能性 | PASS; STEADY | active |
| `run_0354_kw3_sig097` / `run_0355_kw3_sig103` | mode 3 基準の σ 一定倍率局所感度 (`condSigmaScale` 0.97 / 1.03) | NS (SST) | on | onset **12.09 / 16.81 mm** (mode 3 の 14.28 から −2.2 / +2.5 mm; 符号は理論どおり)、壁偏差 @21 +21.4 / +6.2 %。σ ±3 % で onset が ±2.3 mm 動く = 実験一致の議論は σ の外挿精度 (Vinš 2020 の −20 °C 未満の偏差) の範囲内。**σ 0.97 は未収束** (rms_roe 1.2 桁で plateau、中心線 h0 偏差 0.8 kJ/kg で drift; onset が絞り近くに寄り過渡が残る) → 過渡比較 | 0.97: NOT CONVERGED (plateau), series DRIFTING (h0) / 1.03: PASS, STEADY | active (σ 感度) |
...
| `run_0357_cell_kw1` / `run_0358_cell_kw3` (cell, 48000, dry 場から; 非ゲート) | cell 対照 mode 1 / mode 3 (run_0341 プロトコル) | NS (SST) | on | mode 1: main の run_0341 と報告量同一 (onset 22.49, 壁偏差 +2.42/−4.5/+5.0/+5.3)、場の差は cell 反復ノイズ床の ≤1.2 倍 (ρ 5.3e-5 vs 床×2 4.5e-5)。**mode 3: onset 14.28 mm = node と同一**、壁偏差 +17.5/+3.4/+2.8 %。`verify_theta` PASS | plateau (cell の既知の床) = 未収束の準定常比較; series STEADY | active (cell 対照, 非ゲート) |
