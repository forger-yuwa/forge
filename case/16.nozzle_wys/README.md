...
| `run_0354_kw3_sig097` / `run_0355_kw3_sig103` | mode 3 基準の σ 一定倍率局所感度 (`condSigmaScale` 0.97 / 1.03) | NS (SST) | on | onset **12.09 (過渡参考値) / 16.81 mm** (mode 3 の 14.28 から −2.2 / +2.5 mm; 符号は理論どおり)、壁偏差 @21 +21.4 / +6.2 %。一定倍率の局所感度であり σ の外挿精度や信頼区間の議論には使えない。**σ 0.97 は未収束** (rms_roe 1.2 桁で plateau、中心線 h0 偏差 0.8 kJ/kg で drift) → 96000 step 延長 + cfl 低減を残作業に | 0.97: NOT CONVERGED (plateau), series DRIFTING (h0) / 1.03: PASS, STEADY | active (σ 感度; 0.97 は要再取得) |
