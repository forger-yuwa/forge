...
| **carrier 中の非等温核生成補正 (Feder 形) と σ 感度 (2026-09-12, branch feature/condensation-air, plan [condensation-kantrowitz-carrier](../../plans/active/condensation-kantrowitz-carrier.md))** — run_0335 のプロトコル (IC=run_0213 dry 場 index コピー, outflow, cfl 2, TP MIXDRY+H2O, HK 成長, `condSonicModel` 自動=1, 48000 step, 出力 level 2)。`condKantrowitz` 0=等温 / 1=Kantrowitz 純蒸気形 / 2=Feder carrier 形 / 3=Feder carrier + 表面仕事。診断 `condTheta_0`・`condLim_0` を `verify_theta.py` で host 式と照合 | | | | | |
