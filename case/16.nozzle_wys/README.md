...
| `run_0505_node_lam_flag0` (2026-09-25, ローカル) | **#10 flag 0** ([plan](../../plans/active/convection-slau-wall-normal-chi.md) §5.1 #10 / §6 V3)。既製レシピ `run_user_profile.py --disc node --phys sst --laminar --bc-sub "kind: outlet_statPress=kind: outflow" --out-interval 500`、IC = 1D 等エントロピー、`FORGE_CUDA_BLOCKSIZE=128` (REG 上限)。メッシュ `nozzle_user_2d_planar.geo` 41,640 節点 PASS (AR 724) | 層流 NS (node, 2D) | off | 本段 1.77 ms/step。NaN 0。壁 p/p0 @16.4/45.6/85 = **0.3583/0.2459/0.1733** (8000 step 以降 4 桁不変)。1D 非粘性 0.340/0.227/0.157 と参照 (2D SST/3D 層流/実験) 0.367/0.255/0.187 の間 | `check_convergence` 本段: NOT CONVERGED (plateau, 1.1〜1.8 桁)。**前提ゲート ±2 % FAIL** (−2.4/−3.6/−7.3 %; 参照値が 2D 層流でない) | ref (#10 分岐元。前提ゲートは plan §6 V3 で差し替え) |
