...
| `run_0459_perf_cond3d_local` / `run_0460_condf_phys_node2d_cond` (ローカル RTX 3060, 2026-09-13) | 凝縮 float 化 (plan condensation-float-speedup): 0459 = run_0455 (3D 2.37M dry) + 凝縮 H2O carrier (AWS run_0420 の対応物) の速度 A/B (`_bench/` に 100 step ×4)、0460 = run_0456 config の 12000 step 物理検証 (merged 3b77cc4b / condf 両バイナリ; check_convergence・check_quasisteady・`cond_axis_h0.py`) | 0459 (起動区間): 凝縮 ON 197 → 84 ms/step (dry 63 不変)。0460 (12000 step): 同 verdict 区分・ALL STEADY・h0 +0.07 J/kg・場ノイズ床内 (plan §5.1 #5)。関連: `run_0457_perf_regress_cell2d_cond` (cell 2D H2O, 基準 4 本, PASS 30/30), `run_0458_frozen1step_node2d_cond` (condFloat 0 のビット一致), `run_0461_frozen_roe_protect` (強制反転失敗 20 セルで roe 保護), `run_0462_cond3d_local_spinup` (3000 step, 湿潤 22.9 %), `run_0463_perf_cond3d_local_dev` (発達場: 221 → 113 ms/step) | active (回帰・速度) |
