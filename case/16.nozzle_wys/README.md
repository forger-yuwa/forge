...
| `run_0459_perf_cond3d_local` / `run_0460_condf_phys_node2d_cond` (ローカル RTX 3060, 2026-09-13) | 凝縮 float 化 (plan condensation-float-speedup): 0459 = run_0455 (3D 2.37M dry) + 凝縮 H2O carrier (AWS run_0420 の対応物) の速度 A/B (`_bench/` に 100 step ×4)、0460 = run_0456 config の 12000 step 物理検証 (merged 3b77cc4b / condf 両バイナリ; check_convergence・check_quasisteady・`cond_axis_h0.py`) | 0459: 凝縮 ON 197 → 84 ms/step (dry 63 不変)。0460: plan §5.1 #5 参照 | active (回帰・速度) |
