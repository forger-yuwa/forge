...
| `run_0414_s2_off000_tw20_bigdom_ext1` | 計算領域拡大の感度 (#11) | 結論量の変化 **≤1.2 %**。**準定常は再検証不能** | ref |
| `run_0415_s2_off000_tw20_d07` / `run_0416_..._d14_ext1` | 流入 BL 厚 δ ×0.7 / ×1.4 の感度 (#11) | **Q ∝ δ^−0.127**、効くのは換気量 (ṁ ∝ δ^−0.177)。**準定常は再検証不能** | ref |
...
壁温スイープ 6 点 (同心 / 偏心 5-1 mm × 20 / 500 / 1000 ℃) の状態と結果は、
上の **「計算 run 一覧」** を参照 (`run_0401`〜`run_0406` = CPG 入口の初版、
**`run_0419`〜`run_0424` = TP 入口の正本**)。
...
| `run_0046_g5_3d_{off,on}` | **代表的 3D 規模での速度測定** (plan `time_integration-fp64-accumulator` §6.1 G5)。hex 100 万節点・300 step・5 対を交互。各回は `G5_3D_TIMINGS.txt` | OFF **29.8180** / ON **29.8320 ms/step**、差 +0.0140 (標準誤差 0.2801)、**片側 95 % 上限 +2.05 %** (許容 +3 %)。採用カウンタ 30610–30616/step (等温壁 37268 の上界内) | `ref` |
