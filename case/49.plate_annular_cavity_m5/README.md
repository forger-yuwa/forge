...
| `run_0003_stageB_cpg` | **Stage B 生産メッシュ** (330k 節点, y₁=10 µm, VL 11 層, リップ 0.45 mm) | 計算中 | active |
...
**メッシュ生成の要点 (実測 2026-09-19)**: **VL 総厚が壁面の接線セルサイズを超えると、開口リップの
凸角で層が自己交差して NETGEN の tet 充填が無言で失敗する** (`Compute: True` なのに tet 0 で
体積の 2.2 % しか埋まらない)。`setup.py` が全壁面サイズに下限 = VL 総厚を課してこれを防ぐ
(引き上げた面は manifest の `size_bumped_by_vl` に残る)。
