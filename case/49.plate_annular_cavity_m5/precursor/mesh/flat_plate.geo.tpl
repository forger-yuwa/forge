// =====================================================================
// case/49 前駆 2D 平板 (node 用, 平面メッシュ・押し出しなし)
//   3D キャビティ領域の入口に与える乱流 BL (delta ~ 目標値) を作るための前駆計算。
//   領域: x in [X0, X2], y in [0, H]。x<X1 は slip 助走, x>=X1 が断熱 no-slip 平板
//   入口 x=X0 (超音速一様), 出口 x=X2, 上面 y=H slip
//   派生元: case/48 mesh/flat_plate_cooled.geo.tpl (x 範囲をテンプレート化)
// =====================================================================
Geometry.PointNumbers = 0;
Mesh.ScalingFactor = 1.0;
lc = 0.05;
x0 = __X0__;  x1 = __X1__;  x2 = __X2__;  H = __H__;
...
Line(2) = {2, 3};   // bottom wall (平板, 断熱 no-slip)
Line(3) = {3, 4};   // outlet
Line(4) = {4, 5};   // top (平板上)
Line(5) = {5, 2};   // 平板前縁の垂直線 (上->下)
Line(6) = {5, 6};   // top (助走上)
