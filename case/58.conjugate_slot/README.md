...
計画: [`plans/accepted/boundary-conjugate-heat-transfer.md`](../../plans/accepted/boundary-conjugate-heat-transfer.md) §6 **V6′**
...
**判定 (2026-09-26): V6′ PASS (範囲限定)** — V6′ は、全域 FP64・本ケースの固定深部帯 211 節点において、登録した伝導漸近、連成保存性・温度連続、局所準定常、帯内 G-if、および `Df_scale` 5/20 × `interval` 50/200 の感度条件を満たして PASS とするが、流体残差と全域 G-if は NOT CONVERGED であり、全域 CHT 解の収束や固定点の設定非依存を実証したものではない。 根拠: `run_0012_v6p_df5_i50_nlog` (合否・基準) / `run_0013_v6p_{df20_i50,df5_i200}_ext394k` / `run_0014_v6p_df20_i200_ext688k`。経緯は [plan §5.1 #100–#103](../../plans/accepted/boundary-conjugate-heat-transfer.md)。旧「次」(#101 ①〜④) はすべて完了した: ① (e) を $Q_{\rm sol}=(K_su-b_s)_{\rm iface}$ に訂正、② `conjugate.node_log` + `check_cht_interface.py --band-y`、③ `check_quasisteady.py --series-csv`、④ `sens_v6p.py`。
