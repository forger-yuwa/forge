...
| `run_0100_merge_regress_cell_air` / `run_0101_merge_regress_node_air` / `run_0102_merge_regress_cell_dry` (ローカル RTX 3060, 2026-09-13) | 高速化ブランチ `feature/perf-3d-speedup` に main (空気凝縮) をマージした後の回帰: `run_0035` (cell 空気 CPG carrier) / `run_0044` (node 空気) / `run_0038` (cell dry) と同 config・同 IC で 12000 step を `merged` ラベルで再計算し、`tools/perf_regress.py cmp --noise` (床 = 同一バイナリ反復 run_0027/0030/0033, run_0042/0043, run_0036/0037) で比較 | **全 PASS** (30/30, 30/30, 20/20)。参照 res は凝縮 worktree の run をハードリンク (`SOURCE.txt`)。 2026-09-13 凝縮 float 化 (`condFloat: 1`, b021bca1) の `condf` ラベルも全 PASS (28/28 [condTheta/condLim を除外], 28/28, 20/20; 12000 step の壁時計 cell 空気 60→31 s, node 空気 55→26 s [ローカル, 参考])。plan [condensation-float-speedup.md](../../plans/accepted/condensation-float-speedup.md) §5.1 #5。plan [performance-3d-node-sst-speedup.md](../../plans/accepted/performance-3d-node-sst-speedup.md) §9 2026-09-13 | active (回帰) |
| `run_0103_condf_regress_n2_cell` / `run_0104_condf_regress_n2_node` (ローカル RTX 3060, 2026-09-13) | 凝縮 float 化 (plan condensation-float-speedup, codex plan M7) の pure N2 回帰: run_0029 (cell) / run_0032 (node) と同 config・同 IC で 12000 step、ラベル `condf` (float) と `merged`/`merged_r1,r2` (変更前); 床は cell = run_0045/0046, node = merged 反復 | **全 PASS** (cell 28/28 ノイズ ×0.7, node 28/28)。onset (`onset_analysis.py --series`) 全 STEADY: cell 2.112/2.131 in の 1 セル交互は基準反復 run_0045 と同じ、node 2.102 in で同一。参照 res は凝縮 worktree からハードリンク (`SOURCE.txt`) | active (回帰) |
| `run_0105_limiter_regress_n2_cell` / `run_0106_limiter_regress_n2_node` / `run_0107_limiter_regress_air_cell` (ローカル RTX 3060, 2026-09-15) | θ 律速の更新クランプ化 (plan condensation-source-limiter-steady §6-2) の回帰: run_0103 / 0104 / 0100 の入力を複製 (参照・ノイズ label はハードリンク) し、新バイナリで `lim1` (`condLimiterMode 1`, 既定) と `lim0` (旧経路) を `run_case.sh` 経由で 12000 step、`perf_regress.py cmp --noise` で比較 | **N2 node: lim1/lim0 とも 28/28 PASS**。**N2 cell: lim1 は 27/28 (ro 6.2e-4 = 床 2.9e-4 の 2.1 倍, 乾き入口域セル 38) だが同一バイナリ反復 `lim1_r2` が同じセルで 5.6e-4 差 = cell atomicAdd の反復ノイズ (`lim1_r2` は 28/28 PASS)**; onset (Δp 1 %) lim1 2.13 in = 参照、lim0 2.11 in (既知の 1 セル交互)。**空気 cell (CPG carrier, 蒸発分岐あり): lim1/lim0 とも 28/28 PASS (床 = run_0027/0030/0033)、onset 2.21 in (Δp 1 %) / 2.13 in (g>1e-4) = 参照と同一**。**2 巡目バイナリ (`lim1b`; 蒸発の一様 ṙ 形・部分体積・補正診断) も N2 cell `lim1b`/`lim1b_r2`/`lim1b_r3` 28/28 PASS (新バイナリ反復間の ro 広がり 2.4〜4.6e-4 = 参照反復と同水準)、N2 node 28/28、空気 cell 28/28、onset 同一**。**最終バイナリ (`lim1c`, 3 巡目)**: N2 cell `lim1c`/`lim1c_r2` 28/28 PASS、`lim1c_r3` は ro のみ 6.1e-4 (同じセル 38 の反復ノイズ, 新バイナリ反復間 2.4〜4.6e-4)、N2 node 28/28、空気 cell 28/28、onset 2.13 in / 2.21 in = 参照。**4 巡目 (`lim1d`, 最終確定)**: N2 cell 3 反復とも 28/28 PASS、node 28/28、空気 28/28、onset 同一 | active (回帰) |
| `run_0106_limiter_regress_n2_node/passA_s1_sfr2` / `run_0106_limiter_regress_n2_node/passA_s0` (ローカル RTX 3060, 2026-09-17, Phase A snapshot `forge-bin-passive-A`) | 受動種経路 (plan species-passive-scalar-unification §6-5) の pure N2 node 回帰: run_0106 の `lim1e` プロトコル (`condLimiterMode 1`, cfl 1, nStepInner 20, 12000 step, IC = `arthur_nozzle_planar.h5` の dry 場) で `passA_s1_sfr2` = `passiveScalarScheme 1` + `speciesFaceReconstruction 2` + `speciesImplicitCoupling 1` + `passiveImplicitCoupling 1` (モーメント S3, scalar-DPLUR), `passA_s0` = scheme 0 / SFR 0 (旧経路)。`run_case.sh` 経由、`perf_regress.py cmp run_0106 run_0032_n2_new_node_v2 passA_s1_sfr2 passA_s0 lim1e --noise merged_r1 --noise merged_r2` | **`passA_s0`: 28/28 PASS** (Q0/roQ0/roQ1 はノイズ ×0.9, 他は絶対基準内) = 旧経路は新バイナリでも不変; `lim1e` も 28/28 (対照)。**`passA_s1_sfr2`: 15/28 (期待どおり固定点が変わる)**: g_0 4.3e-2 / rog 8.4e-2 / Q0 1.9e-1 / T 3.4e-3 / P 2.6e-5 / sonic 4.0e-3 / h0 1.0e-4 (相対 max)。onset (`onset_analysis.py --dry arthur_nozzle_planar.h5 --series`): Δp/p_dry 1 % 基準は **両方 2.10 in (53.40 mm) = 参照と同一**、g>1e-4 基準は S3 2.01 in (51.09 mm) vs 1 次 2.03 in (51.55 mm) = 0.46 mm 上流; 壁圧比 r3/r4/r5 S3 1.2383/1.3715/1.4738 vs 1 次 1.2475/1.3790/1.4808 (−0.5〜−0.7 %); g_exit 0.0630 vs 0.0642; series STEADY (両方), 出口超音速。S3 の floor 補正: 乾き IC からの核生成過渡 (≤2000 step) で Q0 累積 5.5e14 (最終総量比 5.6e-3, Q1 2.7e-3, Q2 1.2e-3, g 8e-33)、**step 2001 以降 per-step 0**。NaN 0、`check_convergence` は両方 2 次リミットサイクル plateau (既知) で NOT CONVERGED。step 時間 3.06 (S3) / 2.02 ms (lim1e 2.36) | active (回帰; S3 は固定点差の記録) |
| `run_0108_passiveC_m5_arthur_s1_sfr2` | **codex result M5 (増分スケーリング θ_b) 後の補正収支再取得**: `run_0106_limiter_regress_n2_node/passA_s1_sfr2` の入力 (node, scheme 1, S3, coupling 1, pc1, condLimiterMode 1, 12000 step) を新バイナリで | NaN 0。**floor 収支 (root のみ, 相対積算)**: rog 0 / roQ2 9.8e-21 / roQ1 3.6e-21 / roQ0 1.3e-22 (旧 Q2 1.24e-3 / Q1 2.69e-3 / Q0 5.61e-3 → ≤1e-4 達成)。その分は **増分スケーリング limCorr** (共通 θ による非負化) へ移った: rog 9.2e-6 / roQ2 1.25e-3 / roQ1 3.11e-3 / roQ0 7.41e-3 (符号付き +; 起動過渡でのみ作動, per-step 0 で終了) | active (codex M5) |
| `run_0109_passiveD_rhoterm_arthur_s1_sfr2` | plan §5.1 #19 (φ_N δρ 項) 後の再取得 (`run_0108` と同入力) | NaN 0。floor 収支 (root のみ, 相対積算): rog 0 / roQ2 3.9e-21 / roQ1 1.3e-21 / roQ0 0 (≤1e-4 達成); limCorr (共通 θ 非負化 + z 縮小) rog 8.8e-6 / roQ2 1.25e-3 / roQ1 3.05e-3 / roQ0 7.21e-3 (起動過渡のみ, per-step 0 で終了) | active (#19) |
