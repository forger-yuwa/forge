...
| (解析) `run_0415` の `tMin` 張り付き 2 節点 | **R5h: カウル後縁が板厚 0 で閉じている** (plan sern-3d §4.35) | 節点 517197/517198 は x = **`L_cowl` ちょうど**・z は側壁直内・**壁第 1 層**、境界に属さない内部節点で 双対 CV は退化していない (第 1 層中央値の 0.31–0.36 倍)。板厚は x=`L_cowl` の **25 z-station すべてで 0.000e+00** (板全体の中央値 5.000e-04) = **厚さ 0 スリットが後縁 1 station だけ残っている**。第 1 層 1.6e-05 では **2772 K と 1000 K が 3.2e-05 m で向き合い** T 37.6 K / ρ 0.671 に潰れる。1 station 手前 (x=0.11936) は 801.9 / 2864.8 K で正常。**粗メッシュには無い** (T 最小はプルーム過膨張の 157.9 K @ `wall_dist` 0.25、T<120 K は 0 節点) | ref |
