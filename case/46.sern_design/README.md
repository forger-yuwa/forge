...
| `cad/hex_junction_model.py` | **接続模型** (plan §4.10/§4.11): 有限厚の側壁・カウル板、側壁後端面・カウル後端面・カウル側端、後流ブロック、外部の H 型ブロック。19 ブロック/断面 × 3 区間。受入検査 (頂点 Jacobian float32・位相とタグ・壁タグ別の第一層の法線距離・AR/skew) を内蔵し `*_report.json` に VERDICT を書く | `--scale 0.5`: 1,932,448 節点・216 ブロック **PASS** / `--scale 0.71`: 4,966,050 節点 **PASS** / `--scale 1.0` は約 1250 万節点で手元のメモリに載らず未実行。node 変換後 (`demo_out/conv_s05/`): 双対体積は全 CV 正、CV ごとの閉性 max 3.9e-4 (`check_dual_closure.py`) | active |
