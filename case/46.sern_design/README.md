...
| `run_0413_3d_wallres_y` (AWS) | **R5d 段階 1: y 法線壁のみ壁解像** (`problem_3d_sst_cycle_m6on_wallres_y.yaml`: `first_wall_frac` 1.6e-04 / `first_top_frac` 6.0e-04 / `first_wake_frac` 1.7e-04、`nj_bot` 55 / `nj_top` 73 / `nj_ext_top` 57 / `ni_plume` 153。側壁は現行のまま) | **1,919,057 節点・AR max 4819 (>5000 が 0 セル、p99 995)**。予想 y₁⁺ `cowl_in` 2.3 / `ramp` 1.0 / `cowl_out` 0.9 / `vehicle_top` 2.5、側壁は 14–64 のまま。**本段プラトーが下がるかを測る**。実行中 | active |
