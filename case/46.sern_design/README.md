...
| `run_0439_3d_junction_outflow_steady` (AWS) | run_0438 を 36000 step 継続 (通算 66000)。V1-d の STEADY 確認 | 実行中 (2026-09-23) | active |
...
| **`run_0440_3d_junction_outflow_flag0`** (AWS) | **2×2 の 4 つ目**: run_0437 と設定完全一致で **flag だけ 0** (出口は `outflow`)。codex result M2 の指摘で追加 | **step 815 で NaN**。run_0432 (statPress+flag0) と**排出の軌跡が 5 桁一致し NaN の step も同じ** = **$\chi_n$ と出口 BC は独立** | active |
| `run_0441_3d_junction_cfl04` / `run_0442_3d_junction_cfl04_long` (AWS) | `cfl_pseudo` 0.2 → 0.4 の固定点確認 (M3)。0442 は通算 33000 step = run_0439 と**同じ累積 CFL** | **PASS**: 全域 $\rho$ の相対 L2 差 **0.011 %**、$P$ **0.0033 %**、監視 CV 0.017–0.049 %。**6000 step 同士の比較 (run_0441) は両者 `still converging` で不成立だった** (8–16 % の差は収束の進み具合) | active |
| **`run_0443_3d_gs071_flag0`** / **`run_0444_3d_gs071_flag1`** (AWS) | **格子感度** (M3)。`cad/hex_junction_model.py --scale 0.71` の **1,108,434 節点** (採用格子の 45 %、品質 PASS)、出口 `outflow`、差分は flag のみ | flag0 **step 3031 で NaN** / flag1 **6000 完走**。**定性的には格子によらず同じ** (対策なしは落ち、対策ありは通る) = `mSLAU` の格子感度報告は局所適用では再現せず。NaN step 比 3.72 (壁 CV 体積比 1.98 + 局所 dt) で**細かいほど早く顕在化** | active |
...
| **`run_0445_3d_gs050_flag0`** / **`run_0446_3d_gs050_flag1`** | **格子感度の 3 水準目 (粗)** ([plan](../../plans/active/convection-slau-wall-normal-chi.md) §5.1 #9)。`cad/hex_junction_model.py --set H1=3.2e-4 H1_END=5.0e-3 G=1.44 HX=0.05 HX_FAR=0.08 NZ=16` = **494,037 節点** (中格子 1,108,434 の 45 %)。メッシャ内蔵検査 **PASS** (AR max 430.9 / skew max 0.502 / x 隣接比 1.311 ≤ 許容 1.469)、変換後 `check_mesh_quality.py` **VERDICT: PASS** (AR≤1000, skew≤0.90)。`check_dual_closure` は FAIL (変換器の単精度、既知 B1b)。暖機 1 次・層流・cfl 0.2・6000 step、**差分は flag のみ** (config diff・`sern.h5` の md5 一致で確認)。**注意: `HX` は採用格子と同じで、粗くしたのは壁法線 (`H1`,`G`) と spanwise (`NZ`) だけ** (`HX` を 0.1 にすると長さ 0.0574 の区間が 1 セルに潰れ x 比 1.886 で受入検査に落ちる)。**中格子 `run_0443`/`0444` を作ったコマンドは記録に残っていないので、設定が完全一致する保証はない** | 投入待ち (AWS) | active |
