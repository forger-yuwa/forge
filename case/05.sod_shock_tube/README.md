...
| `run_0040`/`run_0041_grid*` (メッシュ) + `run_0042`〜`run_0047_gr{100,200,400}_{A,C}` | **リミッタの格子細分試験** (plan convection-node-wall-reconstruction §4.30)。`.geo` の `Transfinite Line` を 101/201/401 に変え gmsh→convert→`gen_sod3d_ic.py`。**CFL 固定のため dt と step を h 比例** (5.0e-7×100 / 2.5e-7×200 / 1.25e-7×400、t=5e-5 s 共通) | **② は 3 解像度とも `ro` 逸脱 0** (① は 27335→49712→88433、率では 0.60→0.28→0.12 %)。L1(ro) 収束次数 ① 0.76/1.00 vs **② 0.82/1.04**。衝撃後 ρ は ② が常に厳密 (2.2240) に近い | ref (**W1g 格子細分の根拠**) |
| `run_0048_flush` | **診断の終了時 flush の検証** (`limiterDiag_finalize()` 追加後、`case/05` 200 step) | 窓は `sides=18057600` だが **`LIMG1 FINAL sides=54172800`** = 全期間。**全期間で `ro` 逸脱 0**・`badscale=0`。→ §4.30 の G1 は窓の値で、全期間は `Ux` 60282 / `Uy` 54791 / `Uz` 54804 / `P` 13030 | ref (plan §4.35 (4)) |
