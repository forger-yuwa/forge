#!/usr/bin/env python3
"""case/56 平板評価 (TP-1187 較正パネル相当) — q_w(x) と T_w(x) を場から出す (断熱 run では T_w = T_aw)。
...
def evaluate(res, x_plate_end=1.90):
...
def main():
    ...
    ap.add_argument("--x-trip", type=float, default=0.13,
                    help="トリップ位置 [m]。評価点 = 実位置 - これ (乱流 run)")
    a = ap.parse_args()
    rd = CASE / a.run
    ...
    print(f"run {a.run}  res {res.name}  壁 {len(A)} 点 "
          f"(x {A[0,0]*1e3:.0f}–{A[-1,0]*1e3:.0f} mm)  TP-1187 run {setup['tp1187_run']}"
          f"  {'層流' if setup.get('laminar') else '乱流'}  トリップ補正 {trip*1e2:.0f} cm")
    print(f"\n{'位置':>4} {'x 実':>7} {'x CFD':>7} {'q forge':>9} {'q 実測':>9} {'forge/実測':>11} "
          f"{'T_w':>7} {'δ99':>8}")
    for loc, x_phys in setup["x_eval_m"].items():
        qm = ser.get(f"qfp_loc_{loc}_kW_m2")
        x_cfd = x_phys - trip
        if qm is None or x_cfd > A[-1, 0]:
            continue
        i = int(np.argmin(np.abs(A[:, 0] - x_cfd)))
        print(f"{loc:>4} {x_phys:7.2f} {x_cfd:7.2f} {A[i,1]*1e-3:9.2f} {qm:9.2f} "
              f"{A[i,1]*1e-3/qm:11.3f} {A[i,2]:7.1f} {A[i,5]*1e3:8.2f}")
    print(f"\n  → {rd/'wall_q.csv'}")
