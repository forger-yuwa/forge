...
| `run_0004_prec_f64` | **精度 A/B の B**。同じ初期場・同じ設定で**全域 FP64 ビルド** (`~/forge56-double`、`flowFormat.hpp` の typedef のみ変更、git 4687c3cd) | **`PASS (converged)`** (全列 5.0–6.2 dec)。壁 q_w の A との差 最大 4.8e-5 (相対)、R_A/R_B/D は小数 3 桁まで同じ → **プラトーは丸め床で報告量に効かない**。T4-0a-0 は判定不能のまま (壁解像 FAIL: 前縁 0.72 mm) | active (収束した基準場)。**T4-0a-E (Re_H 座標)**: 前縁入熱の外挿 A/B で R の差 最大 0.62 % → 方法 OK。R 平均 1.200 [1.040, 1.414] (予備、壁解像 FAIL の場) |
| `run_0005_tw02_re027_fine` | **壁解像を満たすメッシュ** `fp_y1_0.35um_nx2800` (y1 0.35 µm、平板 2800 セル、39.7 万セル、AR 最大 1017 `SOFT-PASS`)。run_0004 の res_20000 から `interp_field.py` (別メッシュ)、本段設定のみ、**全域 FP64** (`~/forge56-double`)、30,000 step、出力 5000 毎 (AWS 空き 2.7 GB) | **`PASS (converged)`** (全列 4.2–8.7 dec)、NaN 0。比較点 St `ALL STEADY`。壁解像: y1+>1 は前縁 x=0 の 1 点 (1.35) のみ・長さ重み 0.02 %、比較域 20–50 cm は 0.11–0.12 (ツール VERDICT は FAIL のまま)。**Re_H 座標の R (G14-E 点 4000/4400/4800/5200/5600) = 1.129/1.134/1.113/1.128/1.196**。T4-0a-E: 前縁入熱の外挿差 最大 0.7 %。**格子感度**: 17 点平均 R が 1.200 (run_0004、y1 1.5 µm) → 1.260 (+5 %) | active (G14 の Tw/Tt 0.2・起点場) |
| `run_0006_tw07_re031` 〜 `run_0013_tw02_re014` | **G14 本体の残り 8 系列** (`run_g14_batch.sh`、2 レーン並列)。各系列の条件、細分化メッシュ・全域 FP64・本段 40,000 step、run_0005 の res_30000 から `restart_field.py` (同一メッシュ)。完走後に中間の全場スナップショットを削除 (AWS ディスク逼迫) | 実行中 (2026-09-27) | active |
