#!/usr/bin/env python3
"""case/59 平板メッシュ生成 (case/48/gen_mesh.py から派生。平板長 --L と出力名だけ違う): 第一セル高さ y1 [µm] ごとに .geo → .msh (msh4.1, forge) + .su2 (SU2) → node 変換 h5 + 品質判定。
...
"""
CONV_BC = """inlet:  {physID: 1, kind: inlet_uniformVelocity, outputHDFflg: 0, ints: , floats: {ro: 0.119, Ux: 970.0, Uy: 0.0, Uz: 0.0, Ps: 2205.0, k: 35.0, omega: 30000.0}}
outlet: {physID: 2, kind: outlet_statPress, outputHDFflg: 0, ints: , floats: {Ps: 2205.0, Pt: 2205.0, Tt: 65.0}}
...
def main():
    ...
    ap.add_argument("--y1", type=float, nargs="+", default=[1.5], help="第一セル高さ [µm]")
