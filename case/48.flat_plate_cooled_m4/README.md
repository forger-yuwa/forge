...
| `run_0018_cht_insolver` | **ソルバ内 CHT** (`conjugate: {mode: local1d, thickness: 1e-3, k_solid: 0.217, back: isothermal, T_b: 300}`, `interval: 50`, `warmup: 500`)。**1 run 30000 step (3.6 分)** | **収束傾向**: $T_w$ = 548 K (x=1.0) … 970 K (前縁)、x=0.5 で 591 K。更新量 max|d$T_w$| 0.043 → 0.028 K/更新で単調減衰。**両側 $q$ の不一致 中央値 0.13 % / 最大 0.60 %** (平均 64.9 / 64.8 kW/m²)。`q_2nd` と `q_compact` の差 1.59 %。同じ問題を外部ループ (run_0017) は 12 反復・15 分で収束させられなかった | active (**Phase 2a の実ケース実証**) |
