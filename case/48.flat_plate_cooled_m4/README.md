...
| `run_0020_fx_ctrl` / `run_0021_fx_half` | **node 面補間重み `fx` の A/B** ([plan](../../plans/active/discretization-node-face-weight-midpoint.md) §6 (b))。`run_0014_iface_diag` の入力 (収束場からの warm start) で 4000 step。`0021` のみ `FORGE_NODE_FX_HALF=1` | 平板では**不変**: `q_eff` 平均 +0.0000 % / 最大 0.0031 %、$\tau_w$ 最大 0.0020 % (run-to-run ノイズ床 1.8e−6 と同程度)。軸に沿った平面壁では幾何 `fx` が 0.5 になるため。`check_convergence`: 両方 `NOT CONVERGED (stalled/plateau)` = 収束場からの継続で低下桁が出ないだけ (対照と同じ) | active |
| `run_0022_rot30_fx_ctrl` / `run_0023_rot30_fx_half` | **回転不変性の直接試験** ([plan](../../plans/active/discretization-node-face-weight-midpoint.md) §5.1 #2b)。`run_0020` の `mesh.h5` の座標・面ベクトル・運動量と入口速度を **z 軸まわりに 30° 回した**同一問題を 4000 step | 回す前の同スキームの解との差 ($x/L$ 0.3–0.95 の rms): 幾何 `fx` は $\tau_w$ 0.085 %・壁面勾配流束 0.254 %・`q_eff` 平均 +0.28 %、`fx=0.5` は **0.024 % / 0.062 % / +0.01 %**。回した格子では壁 CV の質量残差 (float32 の面ベクトル閉性) 由来の `q_eff` 節点間ノイズが両方に 0.9 % 出る (旧定義 `iface_q_eff_raw` なら 3.1 %) | active |
| `run_0024_A_ad_y3_fx05` / `run_0025_B_tw300_y3_fx05` | **新スキーム (`fx=0.5`) での標準検証の冷間起動** ([plan](../../plans/active/discretization-node-face-weight-midpoint.md) §5.1 #2c)。手順書どおり A = 断熱・全段 + 本段 48000、B = 300 K・soft + ランプ 0.5/1/2 + 本段 48000 (`--ic-from` A) | B: $C_f$/VD-II **0.969–0.994**、$2St/C_f$ 1.15–1.16、エネルギー閉合 **1.016**、`SERIES VERDICT: STEADY`。基準 `run_0005_B_tw300_y3` 比 $C_f$・$q_w$ +0.1〜0.2 %、δ\* ±0.9 %、積分 +0.18 % = **回帰基準 3 % 以内**。`run_0025_B_tw300_y3_fx05/eval.png`・`eval.json`。`check_convergence`: 両方 `NOT CONVERGED (stalled/plateau)` = 前縁特異点の既知の残差床 (基準 run と同じ) | active |
