...
- 凝縮 (2026-09-13, branch feature/condensation-air; plan [condensation-kantrowitz-carrier](../plans/active/condensation-kantrowitz-carrier.md) /
  [condensation-air](../plans/active/condensation-air.md)):
  - `condKantrowitz`: 1=Kantrowitz (純蒸気形, 既定) / **2=Feder carrier 形** (キャリアの冷却込み, $\hat q=b-\tfrac12$) / 3=同+表面項 ($\hat q=b-\tfrac12-\ln S$)。
    Wysłouzil H2O では 2/3 は θ 167→3–4 で onset が 8 mm 上流 (実験より早い) になるため**既定は 1 のまま**。診断出力 `condTheta`/`condLim` (`extraFields`)。
  - `condSigmaScale` (既定 1.0): 表面張力の定数倍感度 (±3 % で onset ∓2.3 mm)。Tolman 補正の代替ではない。
  ...
  - N2 低温物性: `condN2LatentLowT: 1` / `condN2PsatLowT: 1` (既定, 70 K 未満の潜熱線形外挿と整合する飽和圧), `condN2LiquidCp: 2000` (J/kg/K, ±500 で onset ∓0.7 K)。
