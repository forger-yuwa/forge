...
**出力と後処理の原則**: `res_*.h5` は `output.level` (既定 1 = 保存量 + 原始量 + `h0`) で絞る。勾配・リミッタ・診断量は必要な run だけ `level: 2` か `extraFields` で取る。**全温・全圧は `VALUE/h0` (全エンタルピー; `sstEnergyIncludesK` のとき k 込み, 属性 `h0_includes_k`) から `solver_density_cuda/tools/total_quantities.py` で作り、スクリプトで `T + u²/2c_p` を自前で組まない** ([`procedures/solver-settings.md`](procedures/solver-settings.md) の「output」)。
