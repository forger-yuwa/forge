...
担当列は [`AGENTS.md`](../AGENTS.md)「モデル分担とエスカレーション」の区分: `F` = 上位モデルの判断が要る
(設計・真因切り分け・採否・数値カーネル変更)、`O` = 中位モデルで自走可 (実装・config・run・文書)。
`O` の項目は、触るファイルと合格条件 (どの VERDICT / どの lint) まで書く。
`F` の項目を完了にするときは、内容欄に「判断: 日付・結論 1 行」を書く (諮った痕跡が無い `F` は完了にしない)。

| # | 項目 | 内容 | 担当 |
| --- | --- | --- | --- |
| 1 | <最優先の残作業> | <何をどこで。合格条件> | F / O |
| 2 | <次> | <> | |
