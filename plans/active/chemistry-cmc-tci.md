...
- `2026-09-13 (20)` — 修正後の couple 7 dual-time `run_0125` (run_0077 の場から 30 ms): ΣY=1.000、基部 x/d 7.1〜8.2、半径 T 差 +2/−13/+45/+36 K と良好だが、**coflow 柱の音響モードが激励** (断面質量流量が −39〜+94 g/s、周期 ~25 ms、coflow が z/d 10 で逆流): 付着火炎の組成を CMC が未燃に戻す初期過渡が ±2 kPa の圧力波を出し、速度入口 + 静圧出口の低マッハ柱 (Δu = Δp/ρc ≈ 10 m/s ≫ 3.5 m/s) で減衰しない。対策の候補: (a) 組成が Q と整合した定常 CMC 場から始める (`run_0126`)、(b) 継続して減衰を待つ (`run_0127`)、(c) 結合をランプ (τ_c を大→小)、(d) 出口/外周を非反射化。結果待ち。
- `2026-09-13 (21)` — **dual-time は本メッシュでは時間精度が出ない (擬似定常反復と同義)**: サブ反復 20 回で ρ/ρe 残差は 1 step 内に 20〜25 % しか
  落ちず (60 回でも 25 %; `run_0128`)、擬似 CFL 2 (`run_0129`) でも同程度で火炎基部が 7.2→4.4〜5.7 に動く (擬似反復パラメータ依存)、擬似 CFL 4 は
  step 2 で NaN (`run_0130`)。原因はリップ近傍の µm セルで物理 CFL ≈ 7000 (dt 1e-5) — [[dualtime-subiter-divergence-fingerprint]] の「物理 CFL ≲ 12」
  を大きく破っており、coflow 柱の「音響モード」も未収束サブ反復の擬似時間ダイナミクス (サブ反復 60 回では断面質量流量が 37〜39 g/s に揃う)。
  時間精度を得るにはリップ格子の粗大化 (10〜20 倍) か 10³ 回級のサブ反復が要り、いずれも現実的でない。
  `run_0126` (定常 CMC 場 run_0101 から couple 7 dual-time 30 ms): ΣY=1、基部 x/d 6.2〜7.8、半径 T +83/+90/+116/+32 K、質量流量 24〜49 g/s で振動。
  `run_0127` (run_0125 継続): −31〜+106 g/s で減衰せず。
  **方針 (提案・ユーザ確認待ち)**: 検証と T_c スイープは定常 (局所 dt) 反復 + couple 7 + ξ 整合修正で行い、柱モードは周期平均 ± 振幅で報告する
  (3 倍速い; 定常でも擬似 CFL 依存の確認 0.5 vs 1.0 を付ける)。dual-time は「時間精度の担保が無い」ことを明記して補助に留める。
- `2026-09-13 (22)` — ユーザ指示で計算を一旦停止 (`run_0131` を step 1562 で中断)。再開時の入口: §9 (21) の方針 (定常反復 + couple 7 で評価、擬似 CFL 0.5/1.0 の依存確認、周期平均で報告) をユーザが承認したら `run_0131` を再投入 (`setup_cabra_case.py ... --couple 7 --cfl 1.0 --nstep 20000 --restart ../run_0101_cmc_c6/res_8000.h5 --cmcq ../run_0101_cmc_c6/cmc_Q_8000.bin`)、AWS 復帰後は `run_tc_sweep_polite.sh` に `--dualtime` を外す変更を入れてスイープ。
