...
| R5d | **3D を壁解像 (y⁺≈1) メッシュに作り直す** (§4.26)。`wallTreatmentSST: 1` を使わない方針になったため必須。壁法線に +30 層程度 (伸長比 1.15)、側壁も壁なので z 方向にも同等。**概算 2.3M 節点・AR 中央値 200〜3100** (≤5000 の内側)。変換と計算は **AWS** で (ローカルは 2.7M で OOM)。`first_wall_frac` / `first_z_frac` を y⁺ 実測から逆算して決める |
