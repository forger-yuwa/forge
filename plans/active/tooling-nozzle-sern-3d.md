...
2D では**厚さ 0 スリットが発散原因**だった実績がある (`case/46` `run_0035`: `m6_on`/`m10_on` とも発散。
`cowl_thickness` はそれを避けるために入れた) ので、**3D SST が生産点で立たない件 (R5) の候補要因**になる。
§5.1 に R5e を起票し、**壁解像メッシュ (R5d) を作る前に、現行解像度のまま板厚だけ直した再試験**を先に置く
(高価なメッシュを誤った幾何の上で作らないため)。
...
平均 y₁⁺ を 0.7 に落とすのに要る第一層厚と、伸長比 1.15 で要る層数 (現行値 → 目標値):

| knob | 現行 (/H) | 目標 (/H) | 比 | 追加層数 | 効く格子数 |
| --- | --- | --- | --- | --- | --- |
| `first_wall_frac` | 4.0e-03 | **7.0e-05** | 57.1 | +29 | `nj_bot` 31→60, `nj_top` 49→78 |
| `first_z_frac` | 4.0e-03 | **7.0e-05** | 57.1 | +29 | `nz_in` 25→54, `nz_out` 15→44 |
| `first_top_frac` | 2.0e-02 | **1.6e-04** | 125 | +35 | `nj_ext_top` 41→76 |
| `first_wake_frac` | 4.0e-03 | **1.7e-04** | 23.5 | +23 | `ni_plume` 110→133 |

**規模**: 主ブロック 257×137×97 = 3.42M、`ext_top` ~1.90M、`vehicle_side` ~0.20M で **概算 5.5M 節点**
...
| **R5e** | **カウル板厚を直した現行解像度メッシュで 3D を再試験する (§4.27、R5d より先)**。`cowl_thickness` が 3D で効いておらず上下の壁ノードが同一座標 (**厚さ 0 スリット**) だった。2D では同じ形が発散原因だった実績があるので、**R5 の生産点が立たない件の候補要因**。現行解像度のまま板厚だけ直して `problem_3d_sst_cycle_m6on.yaml` を再投入し、`check_convergence.py` / `check_quasisteady.py` / `check_wall_resolution.py` を取り直す。**壁解像メッシュ (R5d) はこの結果を見てから組む** (高価なメッシュを誤った幾何の上で作らない) | `mesh_sern3d.py` 修正済・AWS `~/forge-sern` |
| R5d | **3D を壁解像 (y⁺≈1) メッシュに作り直す** (§4.26)。`wallTreatmentSST: 1` を使わない方針になったため必須。壁法線に +30 層程度 (伸長比 1.15)、側壁も壁なので z 方向にも同等。**概算 5.5M 節点・AR 30〜300** (§4.26.1 で見積もり直し。旧 2.3M は `first_top_frac`/`first_wake_frac` を落としていた)。変換と計算は **AWS** で (ローカルは 2.7M で OOM)。`first_wall_frac` / `first_z_frac` を y⁺ 実測から逆算して決める |
