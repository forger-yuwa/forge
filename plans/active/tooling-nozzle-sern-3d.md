...
| **R5r** | **旧メッシャ: カウル板の自由端の二重節点を共有にする** | `check_dual_closure.py` を `run_0402_3d_wf1/sern.h5` に掛けると閉性 0.30 の開いた CV が 68 個 (全部 $z=W/2$・$x\in[L_{sw},L_{cowl}]$、cowl_in 34 + cowl_out 34)。`mesh_sern3d.py` の `dup1` が $k=k_{sw}$ を含み、側壁の無い区間でも板の側端の節点を二重化している。$i \ge i_{sw}$ では $k_{sw}$ を共有にし、粗格子で閉性と ρ 床ノードを確認する。R5q の原因の候補 (詳細は [tooling-sern-mesh-blocking.md](tooling-sern-mesh-blocking.md) §4.11) |
