...
| **R5p** | **§8 の許容値の内部矛盾 (ユーザ判断)**: $x_{\rm ref}=-20H$ で腕が 20–29 H あるため $\Delta C_L = 0.002$ だけで $\Delta C_M$ は 0.04–0.06 になる。実測も $\Delta C_L$ 0.00145 × 29 = 0.042 ≈ $|\Delta C_M|$ 0.0417 で説明できる。**C_L を ~0.0007 に締めるか、C_M を ~0.05 に緩めるか**を決める | plan §8 |
| **R5q** | **側壁 (`sidewall_in` y⁺ 188) が最後の阻害要因**。ρ が床に張り付いた**壁ノード 8 個**が z = 0.10000 の側壁に出る (T は等温壁 1000 K 固定、P ≈ 34 Pa)。側壁の解像には **z クラスタの局所化** (テンソル積の解消) が要る (§4.32)。受理を「側壁未解像・床張り付き 8 節点」と明記して通すか、構造変更に踏み込むかを決める | `run_0422` の ρ 床ノード |
