...
**残る仮説: ベース近傍が未解像で離散的な定常解が無い**。ベースを横切るノードは 9 個、直後の Δx は**ベース厚の 4.1 倍**で、
...
**1 次が安定なのは解けているからではない** — 1 次では壁ノード側が風上のとき `rho u = 0` なので壁 CV は
対流で質量を**失えない** (一方弁)。これも人工物。
さらに §4.15.7 の格子試験 (`split_plume_at_te`) は **Δx を縮めたが壁 CV も 3.3 倍小さくした**ので、
**後流を解像する試験になっていない**。ベースを横切る点数と後流の解像を**分けて振り直す**こと。

ソルバ側に残る課題は **リミッタ評価点の不整合 (§4.15.5) の 1 件**で、
...
| # | 項目 | 内容 |
| --- | --- | --- |
| **R4e-次** | **ベース近傍の解像を詰める (2026-09-19 改訂)**。SU2 との突き合わせで「ソルバの欠陥」説は棄却されたので (§4.15.7 の訂正)、codex 指定の順序どおり **④ 固定形状の局所格子 → ⑤ 肩の丸めと `t_base` の形状感度**へ進む。④ はやり直し: 前回の `split_plume_at_te` は Δx と壁 CV を同時に変えてしまったので、**(i) ベースを横切る点数 (`nj_wake`) と (ii) 後流の流れ方向解像 (Δx) を分けて振る**。ベース圧が外気の 0.3〜0.5 倍に収まるかを判定量にする (初期値 0.41 倍は妥当な値だった)。ソルバ側に残るのはリミッタ評価点のみで [`convection-node-wall-reconstruction.md`](convection-node-wall-reconstruction.md) が引き取った | §4.15.4 / §4.15.7 |
