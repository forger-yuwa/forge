...
| R5c | **SST 壁関数の壁モデル渦粘性に上限を入れる** (§4.25)。`ν_t,wall = ν(1/g − 1)` が `g→0` で発散し、低密度域 (ν が大きい) で 4660 m²/s に達して `k=2.06e9` を作り 3D SST を殺す。混合長 `κ u_τ y` で上限を掛ける案。**乱流モデルの変更なので別 plan**。診断 (`k_wf` 異常節点の計数) も併せて |
