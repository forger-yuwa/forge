...
| **R5o** | **x ブレンドの実装判断 (ユーザ待ち, §4.40)**。`first_wall_frac` を x でブレンドし、**壁が無い下流 (x > `L_ramp`, 中間線は x > `L_cowl`) には細層を置かない**。R5m の実測で AR 29712 → **10648**、節点も減る。これで `first_wall_frac` 4e-05 が AR ~6000 で射程に入り、摩擦が収束して `C_T_with_shear`/`C_M` が許容内に入る見込み。**生成器変更なので着手前に判断を仰ぐ** | `mesh_sern3d.py` の `s_bot`/`s_top` の `first` を station 依存にする |
