...
| **R5h** | **カウル後縁 × 側壁の三重線で T が床に張り付く** (§4.34-5)。2 節点が T 37.6 / 49.8 K (`tMin` 50)。位置は x = `L_cowl` ちょうど・z は側壁直内・壁第 1 層。全体では T<80 K が 4 節点だけで**完全に局所**。粗メッシュでは第 1 層が 25 倍厚くならされて見えなかった。**受理を止めているのはこれ 1 つ**。まず (a) 三重線のノード定義と半 CV の閉性、(b) `L_sw` を `L_cowl` からずらしたのと同様にカウル後縁を側壁から分離できるか、を見る | `run_0415_wallres_y_cont40k` の `res_40000.h5` 節点 517197/517198 |
