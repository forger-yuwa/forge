...
**⑤ (肩の丸め) に進む前の判断**: 丸めても**壁が高速流に隣接する no-slip 節点であることは変わらない**ので、
上の機構はそのまま残ると予想される。丸めは形状感度としては意味があるが、**この発散の対策としては筋が悪い**。
本件は**ソルバ側の別 plan** にすべき: 速度ピン壁節点からの対流再構成の扱い
(例: 壁法線成分を落とす / 壁節点の速度再構成だけ制限する)。`mesh.bndFirstOrder` は使用禁止であり、
そもそも粘性勾配まで壊すので代替にならない。§4.15.5 のリミッタ評価点の不整合と合わせて **2 件のソルバ課題**。
