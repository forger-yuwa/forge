...
| **R5g** | **未完了に戻した (2026-09-20, codex plan-2 M3)**。確かなのは **(i) 場の変化は 5 反復の再現性幅の 3〜62 倍、(ii) 力は 7 桁で定常 (`C_T_with_shear` 0.8595940 ± 8.01e-07)** の 2 点だけ。**ドリフトと反復振動を分けるには**符号付き局所量・力の密な時系列・独立した複数の末尾窓・CFL を変えたときの平均と変動幅の感度が要る。物理的変動と主張するなら**物理時間積分**で裏づける。**受理条件は置き換えない** (`NOT CONVERGED` を保持) | `run_0417_noise_r1`–`r5` |
...
| **R5i** | **進行中 (2026-09-20, §4.37)**: 生産条件を 2D 生産 YAML から確定し `problem_3d_prod_m6on_wallres.yaml` を作成。生成 config を検証済 (`thermalMethod: 2` / `wall_isothermal Ts 1000` / メッシュ PASS AR 4212.9)。**2D では「断熱 → 回復温度 4600 K → 残差プラトー 5e-5」「等温 1000 K で 3.5 桁低下」が既知**で、§4.31–§4.34 のプラトー追跡は全期間その断熱条件だった。`run_0418_3d_prod_wallres` で 断熱 CPG (`run_0415`) との差を測る。**γ が変わるので形状が動く** (別形状として扱う) | `case/46.sern_design/run_0418_3d_prod_wallres` |
