...
| **R5f** | **本段の残差プラトーを詰める** (§4.28)。全列 0.1–0.3 dec 横ばい。本段は 2 次 + リミッタ有効なので**リミッタのチャタリング**が第一候補 (2D は `limiter: 0` で 5.11e-7 まで落ちた)。`limiter: 0` / `venkatK` 振り / `cfl_pseudo` 依存で切り分ける。**R5 の受入には `require_residual_pass: true` を要求する** | `problem_3d_sst_cycle_m6on.yaml` の `opt:` |
