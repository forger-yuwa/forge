...
| 18 | **`cyl_top` の壁解像** | ⏳ 最細格子でも**全面 y1+>1** (平均 1.22)。入熱 84 W = キャビティ 3 壁の 2 倍を受ける面なので影響が大きい。`hex.n_up` / `up_first_um` を上げる (上向き押し出しの第一層が 14.3 µm では足りない) か、`cyl_top` を断熱にする。§4.11.1 |
