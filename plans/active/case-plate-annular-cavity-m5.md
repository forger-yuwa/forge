...
| 18 | **`cyl_top` の壁解像** | 🔄 **形状2 で再測 (2026-09-20)**: 20 ℃ で y1⁺ 平均 **1.53** / 最大 1.63、1000 ℃ では 0.30 と問題なし (冷壁のみの問題)。入熱 300 W (半割) = キャビティ 3 壁の 2.8 倍。`up_first_um` 20→10 µm の A/B (`run_0412_s2_off000_tw20_up10`, 1.17M 節点) を実行中。旧記述: 最細格子でも全面 y1+>1 (平均 1.22)。入熱 84 W = キャビティ 3 壁の 2 倍を受ける面なので影響が大きい。`hex.n_up` / `up_first_um` を上げる (上向き押し出しの第一層が 14.3 µm では足りない) か、`cyl_top` を断熱にする。§4.11.1 |
