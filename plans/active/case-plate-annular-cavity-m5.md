...
- **VL 総厚 ≤ 壁面の接線セルサイズ (必須ガード、2026-09-19 実測)**: VL 総厚が壁面サイズを超えると
  **開口リップのような凸角で層が自己交差し、NETGEN の tet 充填が無言で失敗する**
  (`Compute: True` なのに tet 0 / 体積の 2.2 % しか埋まらない。警告は層厚未達のみ)。実測:
  VL 0.8695 mm × リップ 0.30 mm → **tet 0**、VL 0.426 mm × リップ 0.50 mm と
  VL 0.8695 mm × リップ 1.00 mm → 成功。よって `setup.py` が **全壁面サイズに下限 = VL 総厚**を課し、
  引き上げた面を manifest の `size_bumped_by_vl` に記録する。Stage B は
  `vl_total_frac_gap: 0.20` (11 層・総厚 0.4257 mm・最終層 0.0931 mm) を採り、リップ 0.45 mm を保つ。
