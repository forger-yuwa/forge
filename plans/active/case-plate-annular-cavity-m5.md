...
### 4.7.7 壁間熱回路の同定 — 定数 $G$ モデルの当たり外れ (2026-09-20)

§4.7.6 の結果を使い、`tools/fit_wall_network.py` で

$$Q_i = G_{i0}(T_{aw}-T_i) + \sum_{j\neq i} G_{ij}(T_j-T_i)$$

を最小二乗で同定した (3 壁 = 未知数 6。一様 3 本 + mixA)。単位は W/K、$Q$ は半割。

| | $G_{i0}$ (外部流から) | | $G_{ij}$ (壁どうし) |
| --- | --- | --- | --- |
| 外筒内壁 | 0.02194 | 外筒 − 円柱 | **−0.000004 (≈0, 未励起)** |
| 円柱側面 | 0.01583 | 外筒 − 底面 | 0.01518 |
| 底面 | 0.00073 | 円柱 − 底面 | 0.01034 |

- **壁間コンダクタンスは外部流からのものと同オーダー**。外筒−底面 0.0152 は外筒自身の
  $G_{i0}$ 0.0219 の 69 % もある。「深部は熱的に不活性」で済まない理由がこれ。
- **外筒−円柱がほぼ 0 なのはデータ不足**。一様 3 本でも mixA でもこの 2 壁は常に同温で、
  この係数が**励起されていない**。`run_0408` (外筒 1000 ℃ / 円柱側面 20 ℃) が要る。
- **再現性**: 側壁は −5.7〜+17.6 %。**底面は一様 run で −55 %〜+99 % と外す**。
  底面の熱は §4.7.5 の熱的平衡長で決まる非線形な量なので、定数 $G$ では表せない。
  一方 mixA の底面は +9.3 % で合う (壁間項が支配的な領域では定数 $G$ が効く)。

**包絡法との比較 (mixA、半割 $Q$ [W])**:

| 壁 | 実測 | 包絡予測 | 実測/包絡 |
| --- | --- | --- | --- |
| 外筒内壁 | 18.14 | 37.45 | **0.48 倍** (2 倍の過大) |
| 円柱側面 | 13.06 | 26.09 | **0.50 倍** (2 倍の過大) |
| 底面 | 24.61 | 4.224 | **5.83 倍** (6 倍の過小) |

**包絡は側壁で 2 倍過大、底面で 5.8 倍過小**という、方向の揃わない外し方をする。
「全部安全側に振れるから一次スクリーニングには使える」という擁護も成立しない。
