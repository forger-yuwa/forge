...
| 15 | **codex result-1 の残り (M7〜M11, m12)** | ⏳ M7 開口エネルギー流束に**伝導・粘性仕事**が入っていない (収支残差の原因を後処理だけに断定できない。離散流束と蓄積項での独立検算も未実装) / M8 旋回ゲートが**終点一致だけ**で合格を返す (成長・末尾振幅・他モードを見ていない。dt 半減・nSub 倍増・細分・窓倍増も未実施) / M9 CV 閉性閾値が plan `1e-6` と実装 `5e-5` で不一致、prism 検査が不十分、`y₁⁺` が**第一内部ノードでなく ghost 間距離**から出ている / M10 `cavity_eval` が run manifest を読む一方 `load_conditions()` が**共有 `case.json`** を読み直す (M・壁温を変えると過去 run の評価が変わる)。CPG run でも `Taw_tp` を優先していて `h_aw` が約 9.8 % 過大 / M11 塞ぎ 3D BL 比較・TP 本計算・領域/入口 BL/緩和 感度が未実施なのに §5.1 の完了表記が実態と合っていない / m12 §7 の「ソルバコード変更なし」が誤り (`viscousFlux_d.cu` に診断出力を追加した)。`methods/` への反映と回帰証拠も無い |
