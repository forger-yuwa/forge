...
- q''max の増加率は $h^{-1/2}$ に一致する ($\ln 1.22/\ln 1.43 = 0.56$, $\ln 1.18/\ln 1.40 = 0.49$)。
  鋭角凸エッジの逆平方根特異性そのもの。
- したがって**総入熱と局所 q''最大値は「格子を細かくすると増え続ける量」**であり、
  数値として報告してはいけない。**実物のエッジ丸み (製造の面取り半径) で決まる量**である。
...
その結果**底面には熱が届かなくなる**ので、総入熱は偏心 0.5 mm で頭打ちになる。

**$h_{ref}$ の上昇は分母の効果ではない** (自分で一度誤って説明したので明記する):
$h_{ref}=q''/(T_{0,ref}-T_w)$ の分母に使う**壁面平均**の基準温度は 524.1 → 513.6 K と **2 % しか
動かない**。すきま中央の 1 点プローブ (`dT_mid` 11.9 → 5.3 K) を分母と取り違えないこと。
...
向きではない。偏心側の格子収束は `run_0204_off150_s14` (2.74M 節点) で確認する。
...
| 11 | 感度 run 群 | ✅ **格子収束 決着** (全ヘキサ 3 点 340k/1.00M/2.74M)。ASME/Celik: **Q(リップ除) p 3.26 GCI 0.31 % / 中央 T・底 T GCI 0.00 % / ṁ p 1.99 GCI 1.45 % / 侵入 p 2.61 GCI 1.46 % / h_ref p 1.87 GCI 1.61 %**。⚠ **総 Q (GCI 32 %) と q''max (GCI 224 %) は原理的に収束しない** = §4.4.2 のリップ特異点。⏳ 領域・流入 BL δ・`implicitRelax` 無しは未着手、`cyl_top` 断熱は run_0104 で投入 |
