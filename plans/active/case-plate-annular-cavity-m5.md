...
**偏心側の格子収束はまだ確定していない**: `run_0204_off150_s14` (2.74M) は**判定区間の残差が
末尾で上昇**し、結論量も `DRIFTING` (開口 ΔT の絶対 drift 12.8 K > 許容 2 K)。一方
`run_0203_off150` (1.00M) は結論量が全て `STEADY` (底面の 2 量は値がゼロ近傍で相対 drift だけ大)。
よって両者の差 (総 Q −9.5 %、リップ除 Q −14.2 %) を**格子効果と読んではいけない**。
`run_0205_off150_s14_ext` で 48000 step 伸ばして決着させる。狭窄部が 300 m/s 級で駆動されるので
**非定常化の可能性**も切り分ける。
