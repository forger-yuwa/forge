...
| 14 | **偏心・すきま変更 run** | ⏳ `sweep_offset.py` を**ヘキサ経路に差し替え済み** (FreeCAD/SALOME 不要 = AWS で完結。偏心はレイと内円の交点半径だけなのでブロッキング不変 = 偏心間で同一格子族)。投入予定 `run_0200_off000` / `run_0201_off050` / `run_0202_off100` / `run_0203_off150`。格子収束の確認後に投入 |
