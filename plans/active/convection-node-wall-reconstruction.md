...
| W1b | **実装 + A/B 完了 (2026-09-19) → §4.13 設計 / §4.17 結果**。`space.limiterScaled` + `venkatK` + `limiterRefLength`。**生産既定の Venkatakrishnan で `ro` の厳密有界 (0) を達成**し、目的量の変化は **+0.025 %**。`h_i` を `sqrt(A_planar)` (2D/軸対称) にするかが判定を分けた。**残**: 単位変換・相似拡大・格子細分・軸対称の試験、標準ケース回帰、既定化の判断 |
