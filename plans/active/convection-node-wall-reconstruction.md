...
| **W1b** | **Venkatakrishnan で効くようにする (最優先・ユーザ指摘)**。`limiter: 2` が生産既定なので Barth だけで通っても意味がない。原因は `eps2 = K³·volume` が**変数と無関係な次元**で `K = 1` 固定なこと (§4.11)。案 S (無次元化: 受動スカラー用 `limiter_r1_scaled_d` と同じ扱いを流れ 5 変数へ) / 案 K (`K` を config キーに、SU2 既定 0.05) / 両方。**codex レビューで確定させる** |
