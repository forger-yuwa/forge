...
| **W1b** | **Venkatakrishnan で効くようにする (最優先・ユーザ指摘)**。原因は `eps2 = K³·volume` が変数と無関係な次元で `K=1` 固定 (§4.11)。**設計確定 → §4.13** (固定物理参照で無次元化・速度は共通 `a_ref`・ε̂²=(K h_i/L_ref)³・K を config キー・式の約分と 0/0 NaN 修正・別キー)。**W1 の共有増分関数と対象変数の確定 (§4.14) が前提**。試験は単位変換・相似拡大・格子細分・軸対称の半径非依存 |
