...
**未検証の懸念**: 比の形は尺度を持たないので「ほぼ一様な領域の数値ノイズまで制限して収束を鈍らせる」
可能性がある (Venkatakrishnan が eps を入れた本来の目的)。**今回の `rms_ro` 低下は 6 構成とも 0.1 dec で差が出ていないが、
このケース自体がプラトーするので収束速度の試験になっていない**。深く収束するケースで別途確認が要る。
