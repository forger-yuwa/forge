...
| 13 | **調査中 (codex result-3 M2 の node 軸拘束回帰で発覚)**: node 軸対称 SST TP 等温壁 (case/44 `run_0200_perf_regress_node_axisym_sst_tp`, run_0117/res_24000 から 300 step) で T の差 3.8e-5 (Tmax 正規化; 0.03 K @838 K, 基準同士 4.9e-6 の 8 倍) | 所在: 燃焼室側 (x<−0.3 m) の等温壁境界層 wall_dist 1e-5〜1e-3 m の帯のみ (壁ノード自体・軸・ノズル部は ≤7e-7 = ノイズ級)、143 節点、符号は系統的でない (平均 −1.4e-6)。`thermoFloat: 0` でも同じ → ハイブリッド反転ではない。AWS の中間バイナリで二分: limtpl / lit1 (limiter・viscous・setDT のリテラル) は PASS、**lit2 (SLAU/対流共通/境界流束/scalar/calcGradient/ransSource/ransTransport/gasProperties/depVar CPG のリテラル f 化, 5c1d7455) で発生**。対角キャッシュ on/off は同値。同 case の Euler+平衡凝縮 (`run_0201`, 30/30 PASS) と cell 軸対称 TP (case/23) では出ない。ファイル単位の二分を実施中 (worktree `forge-perf-bisect` @5c1d7455) |
