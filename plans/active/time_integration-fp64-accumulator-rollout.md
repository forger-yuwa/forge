...
| 6c | O | **過去の生産 run の棚卸し** | 「昔の結果も桁落ちしていないか」(ユーザ質問 2026-09-24)。**`residual_history.csv` の `rms_dq_*` は列だけで中身が常に 0** (`main.cpp:450` に明記) なので**遡っては測れない**。<br>**やり方**: `extraFields` に `dq_block_old_0..4` を指定できる (確保済み変数まで拡張済み) ので、**収束済みの場から 1 step 回して `dq` を出し ULP と比べる** (1 run 数秒)。<br>**当たりを付けている先**: 止まりかけた領域を持つ run (溝・袋小路・死水域)、長く回した run、「なぜか下げ止まる」と記録されているもの (`case/24` 系の純伝導、袋小路すきまの 10^5 step 級)。**当たりであって測定ではない** |
