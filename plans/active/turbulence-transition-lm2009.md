...
| 13 | Mark II の入口粘性比 1、および壁解像 PASS の遷移なし対照 | 現在は 10 / 100 だけ。対照 `run_0036` は $y_1^+>1$ が 11 % で FAIL (0.7 µm 級が要る) |
| 15 | **$Re_{\theta t,min}$ を case ごとに振るか** (文献調査 2026-09-23、担当 **F**) | [`notes/investigations/2026-09-23-vane-transition-literature.md`](../../notes/investigations/2026-09-23-vane-transition-literature.md): Lin ら (JGPP 2014) は Mark II で**同じ 2 症状** (負圧面は実験と同位置で遷移・**正圧面は後縁まで層流のまま**) を報告し、輸送変数の下限 $Re_{\theta t,min}$ を標準の 20 から **100 / 130 / 200** に振って対処している (130 で負圧面、小さめで正圧面が合う = **両立しない**)。入口粘性比 10 は文献と同値。**採否は方針の決定なので上位モデルに諮ってから**。「実験に合う値を事後に選ばない」(§6.3) と衝突しうる |
