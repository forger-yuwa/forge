...
- **area**: `turbulence`
- **status**: `in_progress` (**2026-09-23 ユーザ判断で凍結**: 「難しそうだから、もういいや」。実装・平板検証・翼への適用・報告は済み、**残っているのは合わせ込みと未完の検証** (§5.1 #10–#14, #16)。`done` にはしない — codex result レビューが 2 巡とも NO-GO で、**T3B の実験照合と CHT が未実施**のため。再開するなら §5.1 の未取り消し行から)
- **related_docs**:
...
### 4.2 ソース (`transition_d.cu` を新設。当初案のファイル名は `transitionSource_d.cu`)
...
**陰的対角**は `update_d.cu` の既存式に合わせて $D=V/\Delta\tau+V\max(-J,0)+\texttt{transport\_diag}$ ($J$ = 体積あたりのソース微分。**実装は項ごとの負の部分** = $1.5c_{e1}F_{length}c_{a1}S\sqrt{F_{onset}\gamma}+\max(c_{a2}\Omega F_{turb}(2c_{e2}\gamma-1),0)$ と $(c_{\theta t}/t)(1-F_{\theta t})$。当初案は「全体の $J$ の負の部分」。SU2 はソース微分そのものを足すので、$\gamma$ 側は SU2 の負部の 1.5 倍になる。定常解は同じ)。
...
2. `transition_d.cu` (ソース + $\gamma_{eff}$ + 陰的対角。当初案は `transitionSource_d.cu`)、`ransTransport_d` に descriptor 2 本、入口ピン、周期 gather。
...
> **2026-09-23 凍結**。使える状態にはなっている (既定 `none`、`lm2009` は node + 低 Re SST + 定常陰解法で受付、平板で SU2 と ±1 %)。
> **再開の判断材料**: 翼の全体一致は入口の乱れの減衰の速さに支配され (§6.3/§6.5)、報告がその値を与えないので、
> **モデルを直しても「合った」とは言えない構造**にある。次に効くのは合わせ込みでなく (a) 壁近傍の乱流量の実測がある試験を選び直すか、
> (b) 遷移を実測位置に固定した計算で機構だけを見るか、のどちらか。
...
| 4b | ~~SU2 T3A の量の時系列 (codex result M4)~~ | **済 (2026-09-22)**: `case/57…/su2_t3a_lm_cont` (30000 反復の解から +10000 反復)。遷移開始 0.3675 m・終了 0.8635 m・$C_f$ 最小 0.00220・$C_f$(0.3/0.9/1.3) = 0.00227 / 0.00441 / 0.00418 が**表示桁まで不変** |
| 5 | T3B 平板 — **SU2 との照合は済、実験との照合は未完** (2026-09-22): `case/57…/run_0013_t3b_lm` 対 `su2_t3b_lm` で遷移開始 0.0 % ($x$ 0.090 m、$Re_x$ 5.6e4)、終了 −3.0 %、$C_f$ 5 点 +1.0〜+1.7 % (基準 ±10 % / ±5 % 以内)。全 9 量 STEADY (遷移開始 0.0900 m・終了 0.1893 m を含む。1 巡目の時系列は旧い極値検出で遷移位置が 1.5 m の定数列になっており、列名も 0.05 と 0.1 が衝突していた → 作り直した: codex result-2 M2)、`NOT CONVERGED (stalled/plateau)` (丸め床 1.11 倍)。SU2 側の平均流残差は −2.1 桁で停滞。**実験値は未入手** (ERCOFTAC サーバに接続不可) なので実験との比較は無い |
...
| 8 | codex result レビュー | 2 巡とも NO-GO (§6.1)。下の未完行を閉じてから 3 巡目 |
| 9 | ~~倍精度の対照 (codex result-2 M4)~~ | **済 (2026-09-22)**: `flowFormat.hpp` を double にした同一コミット (`5e9b0533`) のビルドで `case/57…/run_0016_t3a_lm_double` (float32 の準定常場 `run_0011` から 40000 step)。残差は float32 の床から `rms_ro` 1.3e−7 → 2.3e−10 (−2.6 桁、低下中)、`rms_roe` 4.2e−2 → 3.9e−5 (−3.0 桁)、`rms_roOmega` −5.2 桁、`rms_roGamma` −3.0 桁。`rms_roUy` だけ 3.7e−7 (−1.9 桁) で横ばい、判定は `NOT CONVERGED`。**報告量は不変**: 遷移開始 0.3595 m・終了 0.8496 m が同一、$C_f$ の差は $x>0.02$ m で最大 2.4e−4 (相対)。→ float32 の床は丸めで、報告量には効いていない |
| 10 | T3B の実験照合 (plan §2 のスコープ) | ERCOFTAC の T3B データを入手して $C_f$ 分布と比べる。サーバに接続できず未入手 |
| 11 | 両コードの報告量の時系列 (codex result-2 M3) — **SU2 側は済 (2026-09-22)**: `case/53…/su2_smooth_lm_cont` (+10000 反復) で壁熱流束の節点ごとの相対変化が中央値 9.5e−7・最大 2.0e−5、forge との領域差・遷移位置 0.337 も不変。**forge 側は未完**: 遷移位置・幅を `h_series.py` の列に足して `check_quasisteady` に掛ける |
| 12 | 翼の 1 µm 格子・実測壁温での SU2 LM 照合 | §6.4 の照合は 2 µm・一様壁・粘性比 10 の 1 組だけ。壁解像 PASS の格子で、粘性比 100 も |
| 13 | Mark II の入口粘性比 1、および壁解像 PASS の遷移なし対照 | **粘性比は 10 / 20 / 40 / 100 まで実施 (§6.5)**。残りは粘性比 1 と、対照 `run_0036` は $y_1^+>1$ が 11 % で FAIL (0.7 µm 級が要る) |
| 15 | ~~$Re_{\theta t,min}$ を case ごとに振るか~~ | **済 (2026-09-23)**: キー `turbulence.transitionRethMin` (既定 20) を実装し Mark II で 20 / 130 / 200 を実施 → **空振り** (§6.5)。既定 20 を変える理由は無い。**代わりに入口粘性比が効く** (10 → 40 で正圧面 −27.7 → −9.2 %) | [`notes/investigations/2026-09-23-vane-transition-literature.md`](../../notes/investigations/2026-09-23-vane-transition-literature.md): Lin ら (JGPP 2014) は Mark II で**同じ 2 症状** (負圧面は実験と同位置で遷移・**正圧面は後縁まで層流のまま**) を報告し、輸送変数の下限 $Re_{\theta t,min}$ を標準の 20 から **100 / 130 / 200** に振って対処している (130 で負圧面、小さめで正圧面が合う = **両立しない**)。入口粘性比 10 は文献と同値。**採否は方針の決定なので上位モデルに諮ってから**。「実験に合う値を事後に選ばない」(§6.3) と衝突しうる |
| 16 | 比較量を壁温へ寄せる (同上) | 報告の $h$ は固体の熱方程式を実測壁温で解いた**導出量**で、壁温より不確かさが大きい (Lin ら)。文献の誤差 2〜2.5 % は温度の値。#14 (CHT) が通れば壁温で直接並べられる |
| 14 | CHT (遷移モデルつき連成壁温) | 未実施。正圧面が入口の乱れの減衰の速さに支配されるので、入口条件の扱いをユーザと決めてから |
...
- **単体**: 相関・ソース・陰的対角・`gammaEff` を SU2 の式を写した numpy 倍精度参照と**全節点**で比較。許容は |差| ≤ 1e−3·|参照| + 1e−5·最大値 (当初案の「1e−5 相対」は float32 の相殺誤差で満たせないので改めた)。NaN・必須診断場の欠落・評価マスクの異常は FAIL (`test_check_lm_kernel.py`)。
...
- **(1) 数値誤差ゲート: 報告量の格子差は基準内、全壁の壁解像条件は未達** (codex result-2 M5)。細 2 格子 (基準 → 細) で遷移開始 −0.4 % (< 5 %)、$C_f$ +0.0 / +0.4 / +0.2 % (< 3 %)。
...
- **収束: 全 run が `NOT CONVERGED (stalled/plateau)`。したがって §6 の規定どおり T3A の結果は「未収束・報告量は準定常・比較差は基準内」と扱う** (codex result M4)。
  床の所在と大きさ: level 2 の残差場 (`run_0015_t3a_lm_resmap`、`tools/residual_map.py`) では平均流の残差は前縁に集中しておらず (前縁 10 mm 以内の寄与 0.0〜0.1 %、上位 10 節点 5 %)、
  **「前縁の特異点で止まる」という当初の説明は撤回**。`res_roOmega` だけは壁第一層に局在する (上位 10 節点で 38 %)。
  **丸めかどうかは次元の合った目安で見る** (codex result-2 M4: 1 巡目後に書いた「`rms_ro`/⟨ρ⟩ が eps の 0.8〜1.6 倍」は、`rms_ro` が質量流束の和 [kg/s] であって密度でないので無意味。撤回)。
  節点ごとに $e_i=\epsilon_{32}\sum_f(|\rho u\cdot S|+p|S|/c)$ (面の質量流束と、SLAU の圧力差項 $\Delta p/\bar c$ が絶対圧の引き算で持つ丸め) を作ると (`tools/rounding_floor.py`、`ROUNDING_FLOOR.txt`)、
  $|res_\rho|/e_i$ は中央値 0.11・99 % 点 0.46・**最大 2.0**、エネルギーも 0.11 / 0.48 / 2.2。**全節点で残差は流束の丸めの目安と同程度以下**で、丸めの床と整合する。
  **倍精度の対照で確定** (§5.1 #9、`run_0016_t3a_lm_double`): 同じ場から double ビルドで回すと残差は float32 の床から 2.6〜5.2 桁下がり、遷移位置は同一・$C_f$ の差は最大 2.4e−4。`NOT CONVERGED` の扱いは変えない。
  ...
  SU2 側は残差のみ (ρ −4.0 桁、$k$ −6.2 桁、ω −3.1 桁、γ −3.5 桁、$\tilde{Re}_{\theta t}$ −2.3 桁) で、量は +10000 反復で表示桁まで不変 (§5.1 #4b、`su2_t3a_lm_cont`)。
...
- **C3X・一様壁 566 K・同一メッシュ (cs2000、第一層 2 µm)・入口粘性比 10 で forge と SU2 の遷移モデルどうし**: `case/53…/run_0153_lm_cf0_su2ctrl` (260000 step) 対 `su2_smooth_lm` (40000 反復。積分熱流束は後半 20000 反復で 37105.7 → 37072.5 W/m (幅 0.09 %)、最後の 10000 反復は 5 桁不変)。
  ...
  → **この条件 (2 µm 格子・一様壁温・入口粘性比 10) の最終場では、「遷移が遅く急」「正圧面が層流のまま」は SU2 の同じモデルにも同じだけ現れる** = forge の実装に固有ではない、まで。
  「モデル固有と確定」とは言わない (codex result-2 M3): 遷移位置 0.337 は 41 点移動平均後に中間値を横切る節点という抽出での一致、SU2 側の局所量の時系列判定は無い、格子・入口条件は 1 組だけ。forge 側の量は PS・SS 層流域・全体 STEADY、SS 遷移後 OSCILLATING (1.1 ± 0.1)。
...
- **Mark II run 42** (`case/54…`、第一層 1 µm メッシュを新設: 品質 PASS AR 913): 遷移なし `run_0036` +10.5 / +82.4 / +23.8 / +35.0 (rms 56.4、壁解像 **FAIL** 1 超 11 %)、
  ...
  Mark II は負圧面 $s/S$ 0.27 の衝撃波で実測自体が急に遷移するので、モデルの急な遷移が合う側に働く (遷移なしの対照 `run_0036` が壁解像 FAIL なので、ON/OFF の差の定量は機構診断の範囲) (負圧面層流域の rms 95.0 → 9.3 %)。衝撃波の後ろの過大 (+20〜24 %) は遷移の有無に依らず残る。

### 6.5 Mark II の入口乱れと $Re_{\theta t,min}$ の掃引 (2026-09-23)

run と数字の正本は [`case/54.markii_vane_cht/README.md`](../../case/54.markii_vane_cht/README.md)。第一層 1 µm・実測壁温・壁解像ゲート全 run PASS・全 run `NOT CONVERGED (stalled/plateau)`。
$h$ の平均偏差 [%] = 正圧面 / 負圧面層流域 / 負圧面遷移後 / 全体。

| 振った量 | 正圧面 | 背 $s/S<0.25$ | 背 遷移後 | 全体 (rms) | 準定常 |
| --- | --- | --- | --- | --- | --- |
| 入口粘性比 10 (`run_0037`) | −27.7 | −5.5 | +20.0 | −7.6 (32.0) | STEADY |
| 入口粘性比 20 (`run_0039`) | −21.4 | −3.5 | +20.7 | −4.1 (30.0) | STEADY |
| **入口粘性比 40 (`run_0040`)** | **−9.2** | **−0.5** | +21.8 | **+2.2 (27.3)** | STEADY |
| 入口粘性比 100 (`run_0038`) | +22.9 | +5.3 | +23.3 | +18.0 (27.5) | 正圧面 UNSETTLED |
| $Re_{\theta t,min}$ 130 (`run_0042`, 粘性比 10) | −27.8 | −5.5 | +19.4 | −7.8 | 背遷移後 DRIFTING |
| $Re_{\theta t,min}$ 200 (`run_0043`, 粘性比 10) | −27.8 | −5.5 | +19.0 | −7.9 | STEADY |

- **Mark II で正圧面を動かすのは入口の乱れの減衰 (粘性比) であって、$Re_{\theta t,min}$ ではない**。粘性比 10 → 40 で正圧面 −27.7 → −9.2 %、
  しかも**負圧面層流域も −5.5 → −0.5 % と同時に良くなる** (C3X のような二律背反が出ない)。ゼロ交差は粘性比 50〜60 付近。残る +21 % は衝撃直後の過大で、粘性比では動かない。
- **$Re_{\theta t,min}$ (§5.1 #15、文献の摘み) は我々の設定では空振り**。20 → 200 で動くのは背側遷移後だけ (20.0 → 19.0 %)、他は 0.1 pt 以内。
  理由は診断 `run_0041_lm_diag` が示す: 相関値が下限 20 に触れるのは**衝撃下流の $\gamma\to1$ の領域だけ** (496 節点)、**正圧面の輸送値は 133–368** で下限から遠い。
  Lin らが同じ摘みで正圧面を動かせたのは、彼らの場では輸送値が下限付近まで下がっていたためと考えられる (彼らのコードの下限の当て方が違う可能性も残る)。
- **粘性比 40 は「採用値」にしない** (§6.3 の立場)。報告は入口 $Tu$ 6.5 % だけを与え長さスケールを与えないので、感度として 10 / 20 / 40 / 100 を並べる。
  参考: 粘性比 40 のとき前縁手前の $Tu$ は 3.6 %・$\mu_t/\mu$ 37 (粘性比 10 では $Tu$ 1.9 %)。
...
| result (2 巡目) | `2026-09-22` | [`notes/reviews/2026-09-22-turbulence-transition-lm2009-result-2.md`](../../notes/reviews/2026-09-22-turbulence-transition-lm2009-result-2.md) | **NO-GO**, C0/M6/m2。1 巡目 M1・M2 の修正は確認済み、SST 結合・拡散係数・周期処理に新たな誤りなし | **全件採用、`in_progress` を維持**。M1 (単体検査が NaN・診断場の欠落を PASS にする、評価マスクが検査される側の出力に依存) → 必須場・形状・有限性を先に検査、マスクは壁距離と速度から、NaN を違反に数える。回帰 11 件 `test_check_lm_kernel.py` PASS、`run_0014`・`run_0154` を再判定して PASS。M2 (T3B の時系列が旧い極値検出で遷移位置 = 1.5 m の定数列、列名 `cf_x0.1` が 0.05 と 0.1 で衝突) → 列名 `%g`、極値検出に閾値 (丸めの凹凸を拾わない)、全 run の時系列と VERDICT を再生成。本文の +1.7 % ($x$ 0.1 m) は正しく、codex の +8.75 % は衝突した列を読んだ値。M3 (「モデル固有と確定」は証拠を超える) → §6.4・変更ログ・README・報告・メモを「この条件では forge に固有でない」に改めた、未完は §5.1 #11–#12。M4 (丸め床の根拠の次元不整合) → 撤回して `rounding_floor.py` で測り直し (§6.2)、決め手の倍精度対照は #9。M5 (数値誤差ゲート「基準内」と $y_1^+$ 未達の矛盾) → §6.2 (1) を分けて記載。M6 (未完項目が完了行に埋もれている) → #9–#14 を独立行に。m7 (SU2 の熱流束「後半 5 桁不変」は誤り: 幅 0.09 %) → §6.4・README・報告を訂正。m8 (旧記述) → §4.2・§5・§6・§7 を実装に同期 |

## 7. 影響範囲

- 新規: `cuda_forge/transition_d.cu/.cuh` (当初案 `transitionSource_d.cu`)。変更: `variables.*`, `input/solverConfig.*`, `ransTransport_d.*`, `ransSource_d.cu`, `ransBlendF1`, `update_d.cu`, `main.cpp`,
...
- `2026-09-23` (Mark II 掃引) `turbulence.transitionRethMin` を実装。Mark II で $Re_{\theta t,min}$ 20/130/200 は空振り、入口粘性比 10/20/40/100 が正圧面を支配すると分かった (§6.5)。粘性比 40 で全体 +2.2 %・rms 27.3 % だが**採用値にはしない**。

- `2026-09-23` (方針決定) 文献調査 ([notes/investigations/2026-09-23-vane-transition-literature.md](../../notes/investigations/2026-09-23-vane-transition-literature.md)) を受け、**$Re_{\theta t,min}$ を設定キー化して Mark II で振る**ことをユーザが決定 (§5.1 #15)。値の採用でなく感度の提示が目的。

- `2026-09-22` (codex result 2 巡目, NO-GO M6/m2 全件採用) 単体検査の偽合格を修正、平板の時系列を作り直し (T3B の遷移位置を含め STEADY)、丸め床の説明を次元の合った目安に差し替え、「モデル固有と確定」「5 桁不変」を撤回、未完項目 #9–#14 を独立行にした。**status は `in_progress` のまま**。

- `2026-09-22` (SU2 照合・Mark II・報告) C3X で forge と SU2 の遷移モデルが一致 (領域別 $h$ −0.4〜+1.4 %、遷移位置 0.337 で同一) → この条件では翼での限界が forge の実装に固有でないことを確認 (§6.4。「モデル固有と確定」は result-2 で撤回)。T3B も SU2 と一致 (§5.1 #5)。Mark II に 1 µm メッシュを新設 (§6.4)。報告 V39 に章を追加。
