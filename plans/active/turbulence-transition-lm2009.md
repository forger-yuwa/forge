...
| 9 | 倍精度の対照 (codex result-2 M4) | T3A を `flowFormat.hpp` double ビルドで回し、残差の床が下がること・報告量が動かないことを確かめる。丸めの床という説明の決め手 |
| 10 | T3B の実験照合 (plan §2 のスコープ) | ERCOFTAC の T3B データを入手して $C_f$ 分布と比べる。サーバに接続できず未入手 |
| 11 | 両コードの報告量の時系列 (codex result-2 M3) | C3X の SU2 LM を restart で延長し、領域別 $h$ と遷移位置が動かないことを示す (T3A は #4b で済)。forge 側は遷移位置・幅も `h_series.py` の列に足す |
...
- **収束: 全 run が `NOT CONVERGED (stalled/plateau)`。したがって §6 の規定どおり T3A の結果は「未収束・報告量は準定常・比較差は基準内」と扱う** (codex result M4)。
  ...
  **丸めかどうかは次元の合った目安で見る** (codex result-2 M4: 1 巡目後に書いた「`rms_ro`/⟨ρ⟩ が eps の 0.8〜1.6 倍」は、`rms_ro` が質量流束の和 [kg/s] であって密度でないので無意味。撤回)。
  ...
  ただし目安であって証明ではない (倍精度ビルドでの対照は未実施 → §5.1 #9)。`NOT CONVERGED` の扱いは変えない。
