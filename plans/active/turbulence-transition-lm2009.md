...
- 初期値: 全域 $\gamma=1$、$\tilde{Re}_{\theta t}$ = 入口値。SST の収束場から遷移モデルを入れて継続する運用を標準にする (段階起動の最後の段)。
...
| 3 | ~~単体検査 (§5 の 4)~~ | **済 (2026-09-22)**: `tools/check_lm_kernel.py` (level 2 の診断場 vs `lm2009_reference.py`)。`case/57…/run_0004_t3a_lm` で全 8 量 差 ≤ 6e−6、VERDICT PASS |
...
| 5 | T3B 平板 | $Tu$ 6.5 % (翼列に近い)。SU2 参照 `case/57…/su2_t3b_lm` を実行中。**実験値は未入手** (ERCOFTAC サーバに接続不可)。入手できなければ SU2 との照合だけで閉じる |
| 5b | 圧力勾配のある検証 (codex plan M7) | 翼の前に 1 つ。候補: T3C 系 (圧力勾配つき平板) または C3X の固定壁温での遷移 ON/OFF の機構比較。$\lambda_\theta$ と剥離補正の空間的な動作を見る |
| 6 | 翼への適用 | **まず同じ実測壁温・同じメッシュで遷移 ON/OFF を比較し、その後に CHT** (codex plan M8)。壁解像ゲート・派生量の VERDICT・CHT 界面残差を必須にし、内部冷却条件は再同定しない (`solid_published.json` のまま)。$h$ の領域別偏差・連成壁温、入口粘性比 1 / 10 / 100 の感度。未収束場は機構診断として扱う |
| 7 | 報告の更新 | Cooled Vane CHT Validation に遷移モデルの章 |
| 8 | codex result レビュー | `done` にする前 |

## 6. 検証

- **ビルド / 不変性**: `transition: "none"` で既存 run (case/48 平板の短い継続) がビット不変。
...
- **(1) 数値誤差ゲート: 合格**。細 2 格子 (基準 → 細) で遷移開始 −0.4 % (< 5 %)、$C_f$ +0.0 / +0.4 / +0.2 % (< 3 %)。全格子で局所 $y_1^+\le1$。
- **(2) SU2 との照合: 合格**。遷移開始 −2.2 % (±10 % 以内)、$C_f$ +0.8 / +0.1 / −0.2 % (±5 % 以内)。
- **(3) 実験: 遷移開始は合格** (−9 %、暫定基準 ±20 %)。**「層流域が Blasius ±5 %」は不合格で、基準の立て方が誤りだった**: 遷移モデルつきの解は $x$ = 0.05–0.3 m で
  ...
  **基準 (3) を「層流 run が Blasius ±5 %、遷移つき run の層流域は SU2 LM と ±5 %」に改める** (後者は $x$ = 0.3 m で +0.8 %)。
- **(4) 自由流 $Tu(x)$** (forge / 実験、%): 0.045 m 2.85 / 3.04、0.495 m 1.78 / 1.88、0.995 m 1.35 / 1.36、1.495 m 1.12 / 1.10。
- **収束**: 全 run が `NOT CONVERGED (stalled/plateau)`。床は遷移モデルと無関係で、**同じメッシュの SST 単体 (`run_0003`)・層流 (`run_0010`) も同じ値で止まる** (`rms_roe` ≈ 5e−2。前縁の特異点)。
  報告量は `check_quasisteady.py --series-csv` で 3 格子とも全 7 量 **STEADY** (基準格子は 50000 step 以降 6 桁不変)。
...
- 既定では無効。既存ケースへの影響なし (ビット不変を検証で確認)。
