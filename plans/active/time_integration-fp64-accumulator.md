...
| **G2s** | **定常値の比較** (**実際に `STEADY` な量だけ**) | `check_quasisteady` で**両 run とも `STEADY`** の量に限る。`DRIFTING` な量 (`zW844`, `zW1411`, $z/W\ge3$ の $U_y$) は**ここで扱わない**。<br>実測 (`gap_series.csv`, 400k、全域 FP64 vs アキュムレータ):<br>・`zW139` 2943 vs 2943 ・`zW283` 163.3 vs 163.3 ・`zW422` 2.198 vs 2.213 (**0.7 %**) — 3 つとも両者 `STEADY` で**一致**<br>・**`U_rms_deep` 3.811e-5 vs 1.532e-4 — 両者 `STEADY` なのに 4 倍差** | **一部 PASS / 一部は v1a の対象外**。<br>`zW139`/`zW283`/`zW422` は一致 (0.7 %)。<br>⚠ **`U_rms_deep` を「ノイズ床と解決した」と書いたのは撤回** (2026-09-24, codex result-3 M2)。深さ別に分解すると**全域 FP64 側でも速度二乗和の 99.78 % が $4<z/W\le5$ に集中**し (250k–400k の 7 点でこの帯の RMS 2.1633e-4、寄与率 99.73–99.81 %)、**両系列ともその帯で `STEADY`**。**正味質量流束がゼロでも往復流は存在できる**ので、精度向上で RMS が下がる事実は数値誤差の寄与を支持するが、**参照側も含めて指標全体がノイズである証明にはならない**。100k で 1.17 倍だったことも 400k の定常差を説明しない。<br>→ **`U_rms_deep` は §6.0a に従って v1a の合否から外す** (深部の量として)。**除外理由は「ノイズと確定した」ではなく「深さ別の定常流と数値誤差の分離が未了」**。後続計画の未解決項目 |
...
| **G5** | 速度 | 1 step のコスト増が **+3 % 以内**。**同一バイナリで ON/OFF を交互に 3 回ずつ・native・同一の出力条件と `monitorInterval`** で測る。既存ログは使えない (`run_0021` OFF 2.30 ms/step vs `run_0022` ON 1.70 ms/step は別バイナリ・別日)。<br>⚠ **採用カウンタの読み出しは同期 D2H + `cudaMemset`** (`main.cpp:1822`) なので、「新規カーネルを融合した」だけでは同期費用を除外できない。**小規模と代表的 3D 規模の両方**で測る | **PASS (2026-09-24、小規模と 3D の両方を実測)**。<br>⚠ 最初の判定は 3 対で標準誤差 2.66 %・上限 +7.8 % だったので撤回済。<br>**小規模** (`case/56`, 65k CV, 10k step, **8 対**、`G5_TIMINGS.txt`): OFF 1.6475 / ON 1.6600 ms/step、差 +0.0125 (標準誤差 0.0088)、**片側 95 % 上限 +1.77 %**。<br>**代表的 3D** (`case/49` hex, **100 万節点** = 15 倍, 300 step, **5 対**、`G5_3D_TIMINGS.txt`、`run_0046_g5_3d_{off,on}`): OFF **29.8180** / ON **29.8320 ms/step**、差 +0.0140 (標準誤差 0.2801)、**片側 95 % 上限 +2.05 %**。<br>→ **両規模とも +3 % 以内**。後続へ移していた 3D は**ここで実施した** (完了条件を緩めない) |
