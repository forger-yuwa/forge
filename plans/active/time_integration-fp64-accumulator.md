...
| **S0** | O | **G0 の単体テスト** ← 次 | `solver_density_cuda/tests/unit/test_qacc_commit.cu` (前例 `test_passive_fct.cu`, `test_cond_limiter_steady.cu`)。**製品コードにフックを入れない**。3 セル: (a) 通常 $\rho$=0.017755, $dq$=2.97e-10, N=100 → `Qacc−Q0` = $100\,dq$ (相対 1e-6)・ミラーが 16 ULP 動く / (b) 毎 step FP32 writer が `roe` を書く「壁ピン」セル → `Qacc == (double)Q` (採用) / (c) OFF 経路 → 0.0。**ここまで通ってから配線する** |
