...
| **G2a** | **主検証・軌道の一致** (`case/56`。**定常化を要求しない**) | **同一 seed から出発し、同じ step で比べる**。落ち着いていない量でも成立する主張で、知りたいのは「**アキュムレータが全域 FP64 の軌道を再現するか**」だからこれでよい。<br>$z/W\le5$ の帯平均、step 100000: $q_w$ **5 % 以内** / $T-T_w$ **10 % 以内** / $U_y$ **10 % 以内**。<br>**「定常値が一致した」とは書かない** (2026-09-24 に分離。codex result-2 M1: `Uy` の $z/W$ 3–4 は**全域 FP64 でも `DRIFTING` 181.7 %** なので、同 step の一致は過渡どうしの比較である) | **PASS** — 下表 (0.00–3.77 %)。素の float32 は $z/W\ge3$ で崩れる |
| **G2s** | **定常値の比較** (**実際に `STEADY` な量だけ**) | `check_quasisteady` で**両 run とも `STEADY`** の量に限る。`DRIFTING` な量 (`zW844`, `zW1411`, $z/W\ge3$ の $U_y$) は**ここで扱わない**。<br>実測 (`gap_series.csv`, 400k、全域 FP64 vs アキュムレータ):<br>・`zW139` 2943 vs 2943 ・`zW283` 163.3 vs 163.3 ・`zW422` 2.198 vs 2.213 (**0.7 %**) — 3 つとも両者 `STEADY` で**一致**<br>・**`U_rms_deep` 3.811e-5 vs 1.532e-4 — 両者 `STEADY` なのに 4 倍差** | **PASS (2026-09-24)**。<br>`zW139`/`zW283`/`zW422` は一致 (0.7 %)。<br>**`U_rms_deep` の 4 倍差は解決した**: これは**物理量でなく残留運動のノイズ床**で ($z/W>4$ の $\lvert U\rvert$ の RMS。静止した溝ならゼロ)、`STEADY` なのは**床が一定だから**。<br>同じ step 100k では **1.17 倍**しか違わず (2.2455e-04 vs 1.9139e-04)、400k で 4 倍になるのは**倍精度だけがさらに落ち続ける**ため。時系列は両方とも 4.1e-03 から出発し、アキュムレータは 1.5e-04、倍精度は 3.81e-05 で平らになる。<br>**3 ビルドの順序**: 素の float32 **1.1685e-02** ≫ アキュムレータ **1.55e-04** > 倍精度 **3.81e-05** (アキュムレータが 75 倍下げ、倍精度がさらに 4 倍下げる)。<br>→ **$\lvert\dot m\rvert$ の床 (35 倍) と同じ話**を速度 RMS で見たもの ($\lvert\dot m\rvert$ は相殺があるので感度が違う)。**新しい未解決ではなく既知の床**で、帰属は後続計画 |
...
| **G3** | 標準検証ケース (**v1a の範囲内のものだけ**) | **`case/48` を `blockDPLUR: 1` (block commit) と `blockDPLUR: 0` (scalar commit) の両方で**。<br> | **PASS (2026-09-24 に測り直し)**。⚠ **最初の判定は撤回**: 比 0.71–1.16 は**自分で登録した判定不能条件 (「床と同桁なら合否を付けない」) にそのまま当たっていた** (codex result-2 M2)。**閾値を緩めるのでなく分解能を取りに行った**: HEAD OFF を **6 本** (`run_0042_g3blk_{off,offb}` + `run_0044_noise_{1..4}`) 回し **15 対**から床の**分布**を取り、判定を「導入前 vs OFF $\le$ 床の (平均+3標準偏差)」にした。<br>床平均/標準偏差/導入前vsOFF/床平均比: `ro` 4.2466e-05/6.24e-06/4.1502e-05/**0.98**、`roUx` 4.7519e-05/7.28e-06/4.4336e-05/**0.93**、`roUy` 1.6474e-04/6.56e-05/2.2521e-04/**1.37**、`roe` 4.7853e-05/7.25e-06/4.3475e-05/**0.91**。<br>→ **導入前との差は同一バイナリ間のばらつきの中**。1 対では同桁かどうかしか言えないが、分布があれば判定できる |
...
| **G4** | restart の契約 (**厳密一致は求めない**) | **`Qacc` は checkpoint しない** = restart は残余 (≤½ ULP) を失う、と**明記**したうえで:<br>**指定した再開間隔での量別許容誤差**で判定する。case/56 で **100k step に 1 回の restart** なら $\dot m$ の差が run 間ノイズ床 (絶対 1.3e-9) の中 (実測)。<br>**頻回 restart は別** — $dq$ が小さいと効果が丸ごと消えるので (§5.1 S1b-⑤ の反例)、**単体試験で限界を残す** | **PASS (2026-09-24)**。100k step に 1 回の restart は実測でノイズ床の中 (`run_0030_restart_cost`)。<br>**頻回 restart の限界を単体試験に残した** (`tests/unit/test_qacc_commit.cu` (e))。$Q=1$, $dq=0.125$ ULP, $N=100$ step で残余を捨てる間隔を変えると**ミラーの移動 [ULP]**: 捨てない **12.0** / 100 step ごと **12.0** / 10 step ごと **10.0** / **毎 step 0.0**。<br>→ **残余は常に ½ ULP 以下でも、捨てる頻度が上がると効果が丸ごと消える**。「½ ULP しか失わないから実害なし」と一般化しない |
| **G5** | 速度 | 1 step のコスト増が **+3 % 以内**。**同一バイナリで ON/OFF を交互に 3 回ずつ・native・同一の出力条件と `monitorInterval`** で測る。既存ログは使えない (`run_0021` OFF 2.30 ms/step vs `run_0022` ON 1.70 ms/step は別バイナリ・別日)。<br>⚠ **採用カウンタの読み出しは同期 D2H + `cudaMemset`** (`main.cpp:1822`) なので、「新規カーネルを融合した」だけでは同期費用を除外できない。**小規模と代表的 3D 規模の両方**で測る | **PASS (2026-09-24 に測り直し)**。⚠ **最初の判定は撤回**: 3 対では差の標準誤差 2.66 %・片側 95 % 上限 **+7.8 %** で「+3 % 以内」と言えていなかった (codex result-2 M3)。<br>**8 対**を交互に測り各回を `case/56.gap_tp1187/G5_TIMINGS.txt` に保存。OFF **1.6475** / ON **1.6600 ms/step**、差 **+0.0125** (標準偏差 0.0249、標準誤差 0.0088)、**片側 95 % 上限 +1.77 %** $\le$ +3 %。<br>⚠ **代表的 3D 規模は未実施** (後続計画 §5.1 #9) |
