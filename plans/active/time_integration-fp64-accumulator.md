...
| **S6** | F | **床の内訳を分ける 1 step 分解 (事前登録)** | **追加の 400k は回さない** (run 間ばらつきしか測れず、リップ振動のカオス・gather 順序・量子化を分けられない)。<br>**準備** (数値カーネルには触らない): (1) `res_ro`…`res_roe` を `variables.cpp` の `output_cellValNames` に登録 (現状未登録で `output/output.cpp:61` が黙って無視する。#15 と同じ作業)、(2) FP64 ビルドを **HEAD で再ビルド** (`run_0020` は `3952632`+dirty の stale build)。<br>**run** (すべて `nStepOuter: 1`、`FORGE_CUDA_BLOCKSIZE=128`): (i)(i') FP32・Qacc ON ← `run_0026/res_400000.h5` の**同一設定 2 本** / (ii) FP64 ← 同じ場 / (iii) FP64 ← `run_0020/res_400000.h5`。<br>**見る量** (深部 z/W>10 の `res_ro`): `E_order = rms(res_i − res_i')` / `E_eval = rms(res_i − res_ii)` / `S26 = rms(res_ii)` / `S20 = rms(res_iii)`。<br>**判定 (事前登録)**: **A** `E_order ≥ 0.5·E_eval` かつ `E_eval ≥ 3·S20` → gather 順序ノイズが主。**FP64 和は順序不変なので残差 gather の FP64 化は床を下げうる**が、実用影響が無いので**別計画・低優先で起票し本計画は閉じる**。/ **B** `E_order < 0.2·E_eval` → 決定論的な面フラックス丸め → gather FP64 では下がらない → 上の「実用上不要」で閉じる。/ **C** `S26 ≥ 3·E_eval` → 評価誤差でなく状態側 (`S26 ≫ S20` なら ½ ULP 量子化、`S26 ≈ S20` なら力学的) → 同じく閉じ内訳を記録。<br>⚠ restart 直後は `vis_turb` が再現されない実績があるので ([[forge-sst-restart-nonfidelity]])、step 1 出力の深部 `vis_turb` が `res_400000.h5` と一致するか確認し、違えば step 2 を使う |
