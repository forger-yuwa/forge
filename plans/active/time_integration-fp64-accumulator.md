...
| **G2s** | **定常値の比較** (**実際に `STEADY` な量だけ**) | `check_quasisteady` で**両 run とも `STEADY`** の量に限る。`DRIFTING` な量 (`zW844`, `zW1411`, $z/W\ge3$ の $U_y$) は**ここで扱わない**。<br>実測 (`gap_series.csv`, 400k、全域 FP64 vs アキュムレータ):<br>・`zW139` 2943 vs 2943 ・`zW283` 163.3 vs 163.3 ・`zW422` 2.198 vs 2.213 (**0.7 %**) — 3 つとも両者 `STEADY` で**一致**<br>・**`U_rms_deep` 3.811e-5 vs 1.532e-4 — 両者 `STEADY` なのに 4 倍差** | **一部未解決**: 3 つは PASS。**`U_rms_deep` の 4 倍差は説明できていない** (「深部は FP64 でも `DRIFTING`」ではこの量を説明できない。この量は `STEADY`)。自由流 2040 m/s の 7.5e-8 なので実用影響は無いが**原因は不明**。運動量残差の gather ノイズ (§5.2) が候補 → 後続計画 |
