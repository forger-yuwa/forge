...
- **質量・エネルギー**: 3608 / 3791 CV で並べ替えが起きているのに max\|Δ\| が **2.1e-16 / 5.8e-11**。
  ...
  → **総和の丸めは (順序依存・決定論的とも) この桁で除外できる**。
  M2 の「`E_order` では決定論的な総和丸めを測れない」は原理として正しいが、
  **質量・エネルギーは比でなく「並べ替え上界」で除外できる**。
...
~~`case/36` (block commit)~~ **差し替え (2026-09-24)**: `case/36` は**全 run が `wallTreatmentSST: 1`** で、
...
| **G5** | 速度 | 1 step のコスト増が **+3 % 以内**。**同一バイナリで ON/OFF を交互に 3 回ずつ・native・同一の出力条件と `monitorInterval`** で測る。既存ログは使えない (`run_0021` OFF 2.30 ms/step vs `run_0022` ON 1.70 ms/step は別バイナリ・別日)。<br>⚠ **採用カウンタの読み出しは同期 D2H + `cudaMemset`** (`main.cpp:1822`) なので、「新規カーネルを融合した」だけでは同期費用を除外できない。**小規模と代表的 3D 規模の両方**で測る | **PASS (2026-09-24)**: `case/56` 10k step を同一バイナリで OFF/ON 交互 3 回ずつ。<br>OFF 1.69/1.76/1.71 → 平均 **1.7200 ms/step**、ON 1.72/1.67/1.77 → 平均 **1.7200 ms/step**。**差 +0.00 %**。<br>⚠ 1 回ごとのばらつきが 5.8 % (0.100 ms/step) あるので、言えるのは「**差はばらつきの中で区別できない**」まで。平均の標準誤差は約 1.4 % なので、実差があっても +3 % 以内。<br>**3D 規模は未測定** |
...
**採用カウンタの等号が別ケースでも成立** (G1 の計器の独立確認):
