...
| 11 | O | ~~**`mdot_decay.py` に区間分離を入れる**~~ **済 (2026-09-23)** ← M3 | 末尾から遡って単調減少が崩れる点を「膝」とし、**減衰区間と床を分けて**報告する。床は**平均±振幅**で出し「最小値を床と呼ばない」と明記。<br>これで `run_0020` が撤回なしに書ける: **減衰区間 (16 点) `\|mdot\| ∝ exp(-3.10e-5·step)`、e 折り 3.23e4 step、4.59e4 倍落ちた / 床 (末尾 2 点) 1.234e-11 ± 2.5e-12 (振れ幅 1.50 倍)**。<br>float32 (`run_0014`) は**膝が検出されない** (床に届いていない) = **べき乗則のまま、e 折り 4.98e6 step**。対比が明確になった |
...
| 13 | O | ~~**書き込み側の棚卸し**~~ **済 (2026-09-23)** ← M1 | 実測: `Q` 5 本への**代入は 3 ファイルに集中**している。<br>**`update_d.cu`** (15 箇所) — `ro[ic] = roN[ic] + dq` の形 (陰解法 commit、`:155` scalar / `:249` block)。<br>**`timeIntegration_d.cu`** (15 箇所) — 同形 (`:396` 陽解法 RK / `:408` dual-time)。<br>**`dependentVariables_d.cu`** (7 箇所) — **これが厄介**: `:76` の `ro_temp = max(ro[ic], roMin)` を `:207,271,282,296` で **毎 step 無条件に `ro[ic]` へ書き戻す** (床に当たっていなくても通る読み書き)。`roe` も `:209,283,297` で EOS から再構成される。<br>→ **`Q` を倍精度にしても、ここが float32 のままなら毎 step 下位ビットが落ちる**。`periodicNode_d.cu` は `Q` への代入 0 (別配列経由)。<br>**#4 への含意**: 正本を `Q` に置くだけでは足りず、**`Q_N` と `dependentVariables` の往復も FP64 にする**か、**床/EOS 再構成を「変化分だけ」の形に書き換える**かのどちらかが要る |
