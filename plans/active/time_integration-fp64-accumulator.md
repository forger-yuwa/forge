...
| **G5** | 速度 | 1 step のコスト増が **+3 % 以内**。**同一バイナリで ON/OFF を交互に 3 回ずつ・native・同一の出力条件と `monitorInterval`** で測る。既存ログは使えない (`run_0021` OFF 2.30 ms/step vs `run_0022` ON 1.70 ms/step は別バイナリ・別日)。<br>⚠ **採用カウンタの読み出しは同期 D2H + `cudaMemset`** (`main.cpp:1822`) なので、「新規カーネルを融合した」だけでは同期費用を除外できない。**小規模と代表的 3D 規模の両方**で測る | **未実施** |
| **G6** | メモリ | 増分が **40 B/CV** (5×double、内点のみ) であること。ログと `nvidia-smi` で確認 | **未実施** |
