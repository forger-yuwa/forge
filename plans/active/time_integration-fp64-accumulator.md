...
~~**判定 = B**: gather の順序ノイズは床と無関係。床は決定論的な FP32 面フラックス丸めなので、残差 gather を FP64 にしても下がらない。~~ **撤回 (M2)**。<br>**筋が通っている**: `E_eval` (2.91e-11) ≈ `S26` (4.34e-11) — **場は自分の評価誤差の水準まで収束して、そこから先へ行けない**。倍精度の場はその 22 分の 1 (`S20`)。<br>したがって §5.1a の第 2 仮説は「**残差 gather の FP64 化では解けない**」で閉じる。もし将来この床を下げるなら、必要なのは累積でも gather でもなく**面フラックス評価そのものの精度**である。<br>⚠ restart 忠実性の確認: step 1 の深部 `vis_turb` rms 9.1818e-36 は種と相対差 1.9e-08 で一致 (step 2 を使う必要は無かった)。<br>**副産物**: `output.outputExtraFields` が `output_cellValNames` に入っている変数しか受け付けず、`cellValNames` に在る診断量 (`res_ro` など) は**指定しても黙って無視**していた。確保済みなら出せるように直した (`output/output.cpp`)。#15 はこれで不要 |
