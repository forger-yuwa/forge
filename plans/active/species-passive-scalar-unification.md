...
| 28 | (result-4 M1–M3/m1) 場ゲートの倍精度・半径 underflow の見逃し / FCT 次数緩和の適用範囲 / §6-8 検証行列 / docs 整合 | **M1 済 (2026-09-17)**: 半径と (x, y) を**指数分離**で作る `cond_moment_xy` を solver (射影・クランプの double/float 3 か所) とゲートに入れ、Q3 が underflow する塵は Q1=Q2=0 に修復する; ゲートは半径・比が作れないセル (`wet && !okr`) と非有限を **FAIL** にし、許容は**ファイルの精度**の丸めから作る (単体試験 + codex の反例 `rog=1e-60, x=2, y=0.1` を追加)。**M2 済**: FCT の下限緩和 (`--fct-order-lo` 1.3) は **BDF2 の受動種だけ**に適用し、流れ・化学種は [1.7, 2.3]、BDF1 は [0.7, 1.3] のまま。**M3 取得中**: `run_0429`–`0448` (BDF2/BDF1 × coupling 0/1 の 4 組 + FCT 無効の対照、各 3 水準 + nSub 倍増、修復後の binary)。**m1 済**: `recommended-settings.md` の `implicitRelax` を定常/dual-time で分離、§5.1 #8 と case/44 README の旧系列 (`run_0378`–`0381`) の位置づけを訂正 |
