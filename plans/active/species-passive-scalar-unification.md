...
| 8 | 検証 (§6 1–8, node のみ) と codex result レビュー | §6-1/2/7 (短 run) Phase A; §6-3 拡散 (解析解 0.23 %, 2 次収束; 等拡散一致/混合平均非一致は実施中), §6-4 固定点, §6-5 凝縮回帰 (S3 は固定点を動かす), §6-7 24000 step は済 (§9); §6-6 dual-time は Phase B 済。**既定 `passiveScalarScheme` を 1 に変更 (2026-09-17)**。次: codex result レビュー |
