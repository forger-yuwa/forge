...
| 20 | (result-2 M1) モーメントの共通 θ 停止を撤回 (成分ごとの非負化, θ≥1e-12)、停止セルの残差寄与の記録、次数の再評価 (#12/#18 の実体) | forge (必須): 非負化のコードは済 (`cond_moment_update_limited_body`; 切った量 = その成分の未適用残差を limStats に記録)、次数の再評価は #21 の後に S3 でも (§6-6)  **済 (2026-09-17)**: 次数の再評価は S3 + FCT でも完了 (§6-6 (ii): モーメント 1.92–1.97, nSub 比 0.000)。共通 θ 停止の撤回は次数を直す要因ではなかった (真因は float ノイズ床と onset 前線; §5.1 #12/#18) が、成分ごとの非負化自体は正 (停止セルの残差寄与を記録)。 |
| 21 | (M2 → §4.7 v5) 物理 step 末尾の保存的 FCT 補正 (流束形 BDF2 履歴, 有効質量流束, 全面 Zalesak) | **実装済 (2026-09-17, commit 7544ca36)**: 単体 `test_passive_fct.cu` (i)–(vi) ALL PASS。CFD (§6-2 dual-time seam ステップ 1e-6, restart G/H, §6-6 の 1 次流れ + S3 次数) は検証中 (case/09 `run_0127`–, case/44 `run_0263`–)  **restart (§6-6 (iv)) 済 (2026-09-17)**: case/09 `run_0164` (修正 binary) は流束形履歴を復元し、連続 `run_0162` との差 max|ΔroXi| 3.2e-6 (修正前 8.35e-4)、流れはビット一致、収支ゲート PASS。  **§6-6 (ii) 達成 (2026-09-17, 倍精度 + 滑らか解)**: FCT 作動 (SFR 2 + passiveFct 1) で case/44 `run_0378`–`0381`: **モーメント g 1.92 / Q2 1.92 / Q1 1.94 / Q0 1.97, nSub 倍増差 0.000, sub-iter 低下 ≥3.20 桁**。流れ 2 列 (roUx 1.62 / roe 1.65) はスロート軸近傍の上位 1 % 除外で 1.89–1.92 = 軸対称スロートの非平滑性 (対象外)。 |
...
| 8 | 検証 (§6 1–8, node のみ) と codex result レビュー | §6-1/2/7 (短 run) Phase A; §6-3 拡散 (解析解 0.23 %, 2 次収束; 等拡散一致 [コア 2.7e-6] / 混合平均非一致 [3.7e-3 一定] 済), §6-4 固定点, §6-5 凝縮回帰 (S3 は固定点を動かす), §6-7 24000 step は済 (§9)。**§6-6 のモーメント時間次数と §6-2 の非定常保存は未達** (#20–#22 が必須残作業)。既定 `passiveScalarScheme 1` (2026-09-17)。codex result 2 回 NO-GO  **§6-6 のモーメント時間次数 (2026-09-17, case/44 `run_0378`–`0381`) と §6-2 の非定常保存・有界・restart (case/09 `run_0156`/`0157`/`0159`/`0164`) は達成**。残りは codex result レビュー 3 回目。 |
