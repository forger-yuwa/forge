...
- `2026-09-17` — codex result レビュー 4 回目 **NO-GO (M3/m1)** を全採用 (§6.1, §5.1 #28): (1) 半径 (Q3/Q0)^{1/3} が underflow するセルで solver もゲートも判定を諦めていた欠陥を、**指数分離 (`cond_moment_xy`)** と「半径が存在し得ない塵は Q1=Q2=0 に修復」で解消し、ゲートは判定不能セルを FAIL・許容をファイルの精度から作るように; (2) FCT の次数緩和を BDF2 の受動種だけに限定; (3) §6-8 の検証行列を FCT 有効条件で取得 (`run_0429`–`0452`): BDF2 × coupling 0/1 は次数・収支とも PASS、BDF1 × coupling 0/1 はモーメントが 0.88–0.91 で 1 次、FCT 無効の対照は 1.99–2.01; (4) docs (定常/dual-time の `implicitRelax`、旧系列の位置づけ) を整合。
