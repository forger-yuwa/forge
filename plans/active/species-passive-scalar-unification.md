...
| 27 | (result-3 M1–M5/m1) S3 を実際に効かせた再検証・収支が通る正式系列・旧経路の無影響・ゲートの偽合格 2 件・docs 同期 | **進行中 (2026-09-17)**: M3/M4/M5/m1 は修正済 (上記レビュー行)。M1/M2 は再取得中 — `convMethod 1` の IC を作り直した次数系列 (case/44 `run_0394`–`0398`, dt 4e-6/2e-6/1e-6 + nSub 80) と、実際に核生成する湿り場での sub-iter 切り分け (`run_0399`–`0404`: SFR 0 / SFR 2+FCT / +relax 0.7 を nSub 40/80) |
