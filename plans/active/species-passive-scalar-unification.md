...
| 20 | (result-2 M1) モーメントの共通 θ 停止を撤回 (成分ごとの非負化, θ≥1e-12)、停止セルの残差寄与の記録、次数の再評価 (#12/#18 の実体) | forge (必須): 非負化のコードは済 (`cond_moment_update_limited_body`; 切った量 = その成分の未適用残差を limStats に記録)、次数の再評価は #21 の後に S3 でも (§6-6)  次数の再評価は #27 で再取得中 (result-3 M1: それまでの S3 系列は convMethod 0 で S3 が不活性)。共通 θ 停止の撤回は次数を直す要因ではなかった (真因は float ノイズ床と onset 前線; §5.1 #12/#18) が、成分ごとの非負化自体は正 (停止セルの残差寄与を記録)。 |
| 21 | (M2 → §4.7 v5) 物理 step 末尾の保存的 FCT 補正 (流束形 BDF2 履歴, 有効質量流束, 全面 Zalesak) | **実装済 (2026-09-17, commit 7544ca36)**: 単体 `test_passive_fct.cu` (i)–(vi) ALL PASS。CFD (§6-2 dual-time seam ステップ 1e-6, restart G/H, §6-6 の 1 次流れ + S3 次数) は検証中 (case/09 `run_0127`–, case/44 `run_0263`–)  **restart (§6-6 (iv)) 済 (2026-09-17)**: case/09 `run_0164` (修正 binary) は流束形履歴を復元し、連続 `run_0162` との差 max|ΔroXi| 3.2e-6 (修正前 8.35e-4)、流れはビット一致、収支ゲート PASS。  **§6-6 (ii) 達成 (2026-09-17, 再取得後)**: `convMethod 1` + S3 + FCT + 倍精度 + 空間スキーム整合 IC の `run_0417`–`0420` で **収支・場ゲート全水準 PASS**、流れ・化学種 1.93–1.95、g 1.74 / Q2 1.92 / Q1 1.88 / Q0 1.38 (FCT の非線形リミッタによる低下; 対照 `run_0425`–`0428` は 1.99–2.01)。旧記録: `run_0378`–`0381` は `convMethod 0` で S3 が不活性だった (1 次風上 + FCT の結果として記録) ほか最粗水準が収支ゲート FAIL。`convMethod 1` で S3 が効く条件の `run_0386`/`0390`–`0392` ではモーメント **g 1.89 / Q2 1.87 / Q1 1.91 / Q0 1.80 (ok), nSub 比 0.000** だが流れ・化学種が 1.18–1.29 (IC が convMethod 0 で作られた初期層) → IC を作り直した `run_0394`–`0398` で再判定。 |
