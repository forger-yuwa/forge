...
| 27 | (result-3 M1–M5/m1) S3 を実際に効かせた再検証・収支が通る正式系列・旧経路の無影響・ゲートの偽合格 2 件・docs 同期 | **M3/M4/M5/m1 済 (2026-09-17)**: 射影と物理 step 末尾の後処理 (EOS 再更新を含む) を `passiveScalarScheme 1` 限定にし、旧経路で `condRealizProject` 1/0 の差が同一設定の反復ノイズと同等であることを確認 (case/44 `run_0382`/`0383`/`0393`); 独立照合は固定 tol・`--double` は step 比例の許容なし; 場ゲートは solver と同じ条件 (g>0 かつ Q0>0) の全セルを判定し許容はセルごとの float32 丸めから (反例 2 件を回帰試験に); docs の撤回済み説明と退化閾値を修正。**M1 済 (2026-09-17)**: `convMethod 1` (S3 が実際に効く条件) × 実際に核生成する湿り場で取り直し (case/44 `run_0399`–`0404`) — nSub 40→80 の ρ 差は S1 1.40e-5 / S3+FCT 1.32e-5 / +relax 0.7 6.32e-4 → **原因は `implicitRelax` 単独**という結論は 2 次でも成立 (S3・FCT は無関係)。**M2 進行中**: 空間スキームと整合した IC (`run_0394`) からの `convMethod 1` + S3 + FCT 系列で、終了時刻を延ばすほどモーメントの観測次数が上がる (0.16 ms 1.26–1.62 → 0.32 ms 1.60–1.79 → 0.64 ms g 1.74 / Q2 1.92 / Q1 1.88, 流れ・化学種 1.93–1.95) = 全水準共通の BDF1 起動 step の寄与。副産物の欠陥修正: 実現可能性射影が**極端な (x,y) で候補距離が overflow して違反状態を残す** (case/44 `run_0415` の塵セル 2 個) → 初期候補を必ず許容領域内に取る修正 + 単体試験 (25465361)。残り: `run_0417`–`0424` (修正 binary + `condEvaporation` 0 の切り分け) で roQ0 の次数を確定する |
