...
| 全ケース | **収束の悪化** (§3.1 の `mSLAU` 報告) | flag 0 と比べ $\Sigma$ CFL あたりの低下桁数が悪化しないこと (**済**: case/16 (c) 比 ≤ 2、case/48・SERN 2D は両側 VERDICT 併記) | — |
...
| R1-M3 | V3 の一部だけで受入全体を PASS にした | 受入ゲートを戻し、case/48・case/16・SERN 2D の 3 件をすべて判定 | case/48 V3 (§6.2)、case/16 (§6 V3 case/16 条件の結果、2026-09-25)、SERN 2D (#10b) | **限定** (case/48・case/16 は閉。SERN 2D は $C_T$ 閉、衝撃足の各 run 準定常は **未** → #14。格子感度は受入から外した → #9b) |
...
| R2-M1 | SERN の準定常ゲートが未完、受入条件が既定化条件へ移った | case/16 を受入ゲートに戻し (#10)、SERN は時系列を #10b の登録式で再判定 | §6.2「#10b の再判定」(PASS)、#10 (V2・V3 PASS) | **限定** ($C_T$ と case/16 は閉。衝撃足の各 run 準定常・$C_L$ は **未** → #14; codex result-3 M3) |
...
| R2-M4 | V0・V1-b/c の全面合格を支える記録が不足 | 単体試験を「Python による式の確認」と明記し、実カーネル照合を V5 で実施。3 CV × 3 dump を記録 | V5 (#10d)、§6.2「V1-b/c の完全な記録」 | **未** (V0・V1-c・V5 は閉。V1-b は起点直後の符号と $\Delta t_l$ 込み正規化収支が未測定 → #13; codex result-3 M1) |
...
- `space.slauWallNormalChi` (opt-in、既定 0) は **node・`nodeWallDirichlet: 1`・非周期/非軸対称**の構成で受入 PASS。
  ...
  - V1 (case/46 接続模型): 壁 CV の排出が止まり床到達 0、要求系列 ALL STEADY、**V1-c** (収支の観測/予測) 1.000。**V1-b は未測定** (#13)。
  - V3: case/48 は $C_f$/$q_w$ ±1 % 内。case/16 は壁 $p/p_0$ の差が V3 許容内で、**この 500 step 試験ではノイズ床比 0.80–1.47 (許容 2 倍以内)**。SERN 2D は $C_T$ 0.031 % を事前固定ノルムと #10b 式で PASS。**衝撃足 L2 0.842 % は差ノルムの判定のみで、各 run の準定常は未判定** (#14)。
