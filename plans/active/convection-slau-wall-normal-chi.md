...
| 全ケース | **収束の悪化** (§3.1 の `mSLAU` 報告) | flag 0 と比べ $\Sigma$ CFL あたりの低下桁数が悪化しないこと (**水準比較まで済**: case/16 (c) 比 ≤ 2、case/48・SERN 2D は両側 plateau の残差水準を併記。低下桁数そのものは未測定) | — |
...
- `space.slauWallNormalChi` (opt-in、既定 0) は **node・`nodeWallDirichlet: 1`・非周期/非軸対称**の構成で受入 PASS。
  ...
  - V1 (case/46 接続模型): 壁 CV の排出が止まり床到達 0、要求系列 ALL STEADY、**V1-c** (平衡壁圧の観測/予測、合格範囲 ×/÷1.5) 1.000。**V1-b** (#13): 起点で 3 CV とも正味流入、プラトー後の $\lvert\Sigma\dot m\rvert\Delta t_l/(\rho V)$ 最大 0.060 %/dump (< 1 %)。
...
- 収束の悪化 (§3.1 `mSLAU` の報告): 3 ケースとも flag 1 の残差水準は flag 0 と同等 (case/16 末尾平均比 1.0003–1.0024、case/48 `rms_ro` 1.83e-7 対 1.94e-7、SERN 2D `rms_ro` 8.09e-5 対 8.82e-5)。**いずれもプラトーでの水準比較で、ΣCFL あたりの低下桁数そのものは測っていない**。
