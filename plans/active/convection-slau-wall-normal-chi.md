...
**ツールが実装と同じ流束を計算するようにした途端、予測と観測が 5 桁一致した** = 平衡壁圧の式が機序を正しく捉えている。
...
**`cfl_pseudo` 0.4 の固定点確認 — 測り直し中**。最初の比較 (`run_0441`, 6000 step) は**成立していなかった**:
`run_0437` (cfl 0.2, 6000 step) と並べて 8〜16 % の差を得たが、**両者とも `still converging`** なので
差は「固定点が違う」ではなく**収束の進み具合の違い**。CFL が 2 倍なら同じ step 数での到達点が違うのは当然で、比較になっていない。
→ **累積 CFL を揃える** (`cfl 0.4` × 33000 step = `cfl 0.2` × 66000 step) `run_0442_3d_junction_cfl04_long` で測り直す。
比較相手は `run_0439` (`ALL STEADY`)。
