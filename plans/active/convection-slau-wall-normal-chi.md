...
- `space.slauWallNormalChi` (opt-in、既定 0) は **node・`nodeWallDirichlet: 1`・非周期/非軸対称**の構成で受入 PASS。
  ...
  - V1 (case/46 接続模型): 壁 CV の排出が止まり床到達 0、要求系列 ALL STEADY、**V1-c** (収支の観測/予測) 1.000。**V1-b** (#13): 起点で 3 CV とも正味流入、プラトー後の $\lvert\Sigma\dot m\rvert\Delta t_l/(\rho V)$ 最大 0.060 %/dump (< 1 %)。
  - V3: case/48 は $C_f$/$q_w$ ±1 % 内。case/16 は壁 $p/p_0$ の差が V3 許容内で、**この 500 step 試験ではノイズ床比 0.80–1.47 (許容 2 倍以内)**。SERN 2D は $C_T$ 0.031 % を事前固定ノルムと #10b 式で PASS。衝撃足は**各 run が ALL STEADY** (#14: 絶対 $x_{foot}$ 不変・窓内壁圧 4 量) を確かめたうえで差ノルム L2 0.842 % (許容 1 %)、$x_{foot}$ 差 0。
