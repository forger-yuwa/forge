...
**V0 マスクの実効範囲 (2026-09-23, run_0437 `res_6000` で実測)** — 設計の「効くのは剥離縁だけ」を面数で確認した:
...
**対象面の 3 分の 2 では $\chi$ が変わらない** (壁法線方向に速度段差が無い = 付着境界層の内部では
$\widehat M_n \approx \widehat M$)。壁ノードは 37,956 / 2,446,572 = 1.55 %。
→ **V3 で副作用が出なかったことの定量的な裏づけ**。冷却平板は全域が付着境界層なので触る面がほとんど無く、
SERN 2D も剥離縁が限られるので $C_T$ が 0.029 % しか動かない。
...
→ **$\chi_n$ と出口 BC は独立**。壁の排出は出口を直しても同じ軌跡で進み、同じ step で落ちる
(出口から 1.2 H 離れた局所現象なので整合する)。**出口だけ直しても成立せず、$\chi_n$ だけでも出口が壊れる。両方要る**。
...
**V1-c 合格** (run_0437 `res_6000`)。**床を割る節点が無いので dump の状態 = ソルバの状態**であり、
codex M2 が求めた「ソルバ前処理つきの再収支」は不要 ($\rho$ 最小 9.27e-4 > $\rho_{Min}$ 1e-4)。
`diag_wall_cv_budget.py --equilibrium` で**全接続面の $\Sigma\dot m = 0$ となる $P_w$ を隣接凍結で求根**した結果:

| CV | 予測 $P_w$ | 観測 $P_w$ | 観測/予測 | 判定 |
| --- | --- | --- | --- | --- |
| 153797 | 419.3 Pa | 565.9 Pa | **1.350** | PASS |
| 153880 | 459.1 Pa | 604.7 Pa | **1.317** | PASS |
| 189814 | 322.5 Pa | 449.3 Pa | **1.393** | PASS |

3 点とも事前に固定した ×/÷1.5 以内。系統的に予測より 1.32〜1.39 倍高いのは**隣接凍結近似のため**で、
...
**絶対水準が `rms_ro` 2.40e-10** = 通算 4.8 桁低下 (6.02e-06 → 2.40e-10) で、
「頭打ちで打つ手が無い」のではなく**収束しきってノイズ床に達した**姿。派生量が全て STEADY なのでこれで判定する。
...
**評価が割れる点 (既定化の判断に直結)**:
- 動かしている面は**全内部面の 0.53 %** (剥離縁のみ。V0 のマスク実測)。そこは元々 $\chi=0$ で
  **圧力–質量結合が欠落していた**箇所なので、**動いた方が物理的に正しい可能性がある**。
