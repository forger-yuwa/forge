...
| **10** | **case/16 の V2・V3** — M3 の残り | run データが AWS・手元とも無く**メッシュ生成から**要る。超音速内部流の壁 $p/p_0$ は `mSLAU` の格子感度懸念を試す 2 つ目のケース。基準 (flag 0) も同一バイナリで取り直す。**既定化の必須条件** | O |
| **11** | **既定化の判断** (**F**) | **前提**: #7・#9・#10 の完了 + $C_L$ 0.433 % / $C_M$ 0.316 % / 衝撃足の壁圧 0.842 % の**正否を独立に判定** (格子収束・別スキーム・実験/文献のいずれか)。これが無いうちは **opt-in のまま**。既定化は本 plan のスコープ外 (§2) | F |
...
**定性的には格子によらず同じ**: 対策なしは両方で落ち、対策ありは両方で完走する。
→ **`mSLAU` の「面法線のみは格子形状に敏感」(§3.1) という報告は、本件の局所適用では再現していない** (2 水準の範囲で)。
...
NaN までの step 比は **3.72** (3031 / 815) で、体積比 1.98 より大きい。差は**局所時間刻み**で説明できる
(粗い格子は $\Delta x$ が大きく局所 $\Delta t$ も大きいので、同じ step 数で進む物理時間が長い)。
$\rho$ の減少率は $\dot m/V$ なので**体積が小さいほど速い**。
