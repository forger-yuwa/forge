...
| **9** | **格子感度 (3 水準)** — M3 の残り | 接続模型のメッシュを `cad/hex_junction_model.py --scale` で 3 解像度生成 → 変換 → 各々 flag 0/1。`mSLAU` の**格子感度の報告** (§3.1) を局所適用で踏んでいないかを見る唯一の試験。**既定化の必須条件**。メッシュ生成 3 + 計算 6 本で数時間 | O |
