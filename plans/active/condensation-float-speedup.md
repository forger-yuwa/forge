...
| 13 | 発達場の速度・湿潤統計・A10G (codex result M6) | **A10G 実測済 (2026-09-13 夜, 34.201.22.141, 最終バイナリ `forge_condf3` = commit 1551099d, sha256 f3d03e2d…; 基準 `forge_merged` a2b931c4…; 100 step ×3 交互, 全反復 ±0.03 ms)**: dry `run_0400` 33.76–33.78 → 33.90–33.95 (+0.15 ms; 凝縮 OFF 経路は不変), **起動区間 `run_0420` 89.40–89.91 → 44.01–44.02 (目標 ≤45 達成)**, **発達場 `run_0421` (湿潤 22.5 %) 97.98–98.02 → 52.30–52.31 ms/step (目標 ≤50 に 2.3 ms = 4.6 % 届かず → followups F-cf3)**。発達場 nsys (30 step, 51.8 ms): block-DPLUR 10.0 / `condensation_source_f_d` 9.6 / `dependentVariables_d` 8.0 (dry 3.2) / SLAU 3.0 / `cond_primitive_multi_d` 1.3。ブロックサイズ 512/256/128 は 52.3/52.1/52.4 で効かない。反転失敗 0。3D 300 step (`run_0421`, 発達場 IC): `cmp --noise base_r2` **PASS 30/30** (P 4.9e-3 / T 1.6e-2 / ρg 9.8e-2 は基準同士 5.7e-3 / 1.8e-2 / 1.15e-1 の 0.8–0.9 倍)、分布統計 (湿潤 533,27x 節点, onset x 0.010649 m, g p50 2.777e-3 / p99 1.095e-2, T_wet p50 233.69 K / p01 205.48 K) は基準 2 本と一致。証拠 `case/16.nozzle_wys/_aws_perf_evidence/cond/`: 交互計測ログ 18 本 (`*__m3_*`/`*__f3_*`), `run_0421_CMP_VERDICT_condf3.txt`, `run_0421_wet_stats_condf3.txt`, `BINARY_SHA256.txt` (両バイナリ・メッシュ・IC・config の sha256), nsys CSV。ローカル RTX 3060 (旧 SHA): 起動 195→87, 発達場 218→111 |
