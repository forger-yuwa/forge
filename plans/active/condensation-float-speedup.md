...
| 14 | 文書同期と記録の修正 (codex result m7/m8, 2 回目 m5/m6) | **済**: performance.md / plans/README / solver-settings.md (`condFloat`) / plan §4.2-5 表記 / 平衡形の扱い (`condEquilibrium 1` の湿潤 TP セルはハイブリッド反転、2 は反転も double) / methods §9 の範囲外退避・蒸発端許容 / §4.3 の λ 5e-4 と h0 二重判定; run_0460 の ρg は絶対基準で PASS (ノイズ床内ではない) と訂正; 各ラベルに VERDICT ファイル保存 (§5.1 #15) |
