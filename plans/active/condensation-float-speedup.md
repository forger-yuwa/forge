...
| 8 | 表の範囲外は double 関数へ退避 (codex result M1) | **済** (ecd56f6a): 旧 kernel 本体を `condensation_source_cell_d` (`__device__`) にし、float kernel は湿潤セル (g>0 または Q0>0 または S>1) の T が表範囲 [Tmin, TwetMax] (N2 20–125.6 K, H2O 120.15–647 K) の外なら丸ごと委譲 (device 試験: 範囲外 32 状態が両実体でビット一致)。面潜熱 (`cond_latent_tab_or_d`) と clamp も範囲外は旧 double 関数。ln p_sat の dry 診断だけ表範囲 [Tmin, Tmax] (H2O 1200 K) で表 (外は端クランプ, 判定対象外)。双方の単体試験は実温度で比較 (ALL PASS)。double kernel の SASS は変更前バイナリと**同一** (40016 命令, `cuobjdump`) |
