...
| 13 | 発達場の速度・湿潤統計・A10G (codex result M6) | **A10G 実測済 (2026-09-13 夜, インスタンス 34.201.22.141, 最終バイナリ forge_condf2 = commit "table derivative …", 100 step ×3 交互, 全反復 ±0.02 ms)**: dry `run_0400` 33.77 → 33.92 (+0.15 ms, 0.4 %; 凝縮 OFF の経路は不変でビルド配置差), **起動区間 `run_0420` 89.40 → 44.0 ms/step (目標 ≤45 達成)**, **発達場 `run_0421` (湿潤 22.5 %) 98.02 → 52.3 ms/step (目標 ≤50 に 2.3 ms = 4.6 % 届かず)**。発達場の nsys (30 step, 51.8 ms): block-DPLUR 10.0 / `condensation_source_f_d` 9.6 / `dependentVariables_d` 8.0 (dry 3.2) / SLAU 3.0 / `cond_primitive_multi_d` 1.3。ブロックサイズ 512/256/128 は 52.3/52.1/52.4 で効かない (占有率律速でない; 湿潤セルの warp 分岐と二相反転の double 研磨が残り)。追加削減候補 (二相反転の湿潤セル分離起動、src_jac 摂動 2 回の削減、`cond_primitive` の 2 起動統合) は followups F-cf3 へ。反転失敗 0。3D 300 step の場: `cmp --noise base_r2` PASS 30/30 (P/T/ρg とも基準同士ノイズの 0.8–0.9 倍)、分布統計 (湿潤 533k, onset x 0.01065 m, g p50 2.777e-3 / p99 1.095e-2, T_wet p50 233.69 K) は基準 2 本と一致。証拠: `case/16.nozzle_wys/_aws_perf_evidence/cond/` (bench ログ 18 本, nsys CSV, `BINARY_SHA256.txt`)。ローカル RTX 3060: 起動 195→87, 発達場 218→111 |
...
- [x] §5.1 #1–#5 完了、§4.3 の判定 (場・物理量・単体・ビット不変) を満たす。**速度: A10G 起動区間 44.0 ms/step (≤45 達成)、発達場 52.3 ms/step (≤50 に 4.6 % 届かず; 98.0 からの −47 %)**。周期保存ゲート (≤1e-6) と cell の h0 絶対ゲートは既存挙動で未達 (followups F-cf1/F-cf2)、発達場の残り 2.3 ms は F-cf3
