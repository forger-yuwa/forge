...
- **status**: `in_progress`  <!-- codex plan GO-with-changes → 実装 → codex result 1〜5 回目 (NO-GO) 全件採用; 6 回目待ち -->
- **related_docs**:
...
| 15 | 証拠の統一 (codex result-2 M4/m6, result-4 M1/m3) | **済**: 最終バイナリは commit 1551099d (AWS `forge_condf3` sha256 f3d03e2d…, 二相音速の dL/dT を double 差分に戻した版)。**AWS で正式取得**: 速度 (dry/起動/発達場 ×3), 3D 300 step 回帰 + 分布統計 (#13)。**ローカル RTX 3060 で正式採用** (2D/小規模で GPU 種は判定に影響しない; 各ラベルに VERDICT ファイル): run_0456 (node 2D H2O) / run_0457 (cell 2D H2O) / run_0460 (12000 step 物理検証) / run_0201 (平衡) / case/34 run_0100–0104 / 凍結 run_0458 (ビット一致) / run_0461 (roe 保護) / run_0053 (周期)。ローカル結果はいずれも 1551099d 以前の中間 SHA で取得したものを含む (最終差分は二相音速 dL/dT の double 差分復帰のみ = 旧挙動への回帰; run_0456 は 1551099d で再 PASS) |
...
- [x] §5.1 #1–#5 完了、§4.3 の判定 (場・物理量・単体・ビット不変) を満たす。**速度 (最終バイナリ 1551099d, A10G): 起動区間 44.0 ms/step (≤45 達成)、発達場 52.3 ms/step (≤50 に 4.6 % 未達; 98.0 からの −47 %) — 未達分は followups F-cf3 に移管**。周期保存ゲート (≤1e-6) と cell の h0 絶対ゲートは既存挙動で未達 (followups F-cf1/F-cf2)、発達場の残り 2.3 ms は F-cf3
