...
| 2 (**$C_T$ 系 5 列は済 2026-09-25: 全列帯内**、§6.2。**膨張角窓の再計算は未** — codex result M4 で戻した) | 既存系列の再判定 (§4.3) + 前 plan #10b の差区間での再計算 | `_v3sern/ct_flag{0,1}.csv` (AWS から csv だけ取得) の **5 列 (`C_T`, `C_T_with_shear`, `C_L`, `C_L_with_shear`, `C_M`)** に §4.3 の閾値表で `check_quasisteady.py --series-csv`、`STEADY`/`OSCILLATING` の量だけ差区間 (§4.2)。前 plan #10b の $C_T$・膨張角窓も差区間で再計算し訂正行 | O |
...
**#2 既存系列の帯判定 (2026-09-25) — 全 5 列「帯内」= 生産許容内** (m6_on、生産格子、この窓)。`_v3sern/v3s_flag{0,1}_ext` の
