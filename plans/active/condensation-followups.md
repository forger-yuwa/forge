...
| F-cf8 | **凝縮モーメント (rog, roQ0..2) に dual-time の物理時間項が無い** (2026-09-15, codex plan レビュー M4 of condensation-source-limiter-steady): BDF 残差・対角・時間レベルシフトは平均流 5 変数と roK/roOmega のみで、モーメントはサブ反復ごとに N/M を現在値へコピーし定常と同じ point-implicit 更新。物理 Δt を小さくしてもモーメントがその物理時間で積分される保証がない。物理履歴・BDF 残差・対角を整備し、物理 Δt 半減とサブ反復数変更で検証する | **完了 (2026-09-17)**: plan [species-passive-scalar-unification](species-passive-scalar-unification.md) §4.3/§4.4 (モーメントの BDF 物理時間項・履歴・対角) で実装し、§6-8 の検証行列 (FCT 有効 × BDF1/BDF2 × coupling 0/1, case/44 `run_0429`–`0456`) で次数・収支とも PASS |
...
| F-cf12 | **SLAU + `convMethod 1/2` (または `limiter 0`) + node 周期 + dual-time BDF2 の一様流が丸め由来の不安定で t≈0.22 s に NaN** (2026-09-17, plan [species-passive-scalar-unification](species-passive-scalar-unification.md) §10; case/09 検証 1 巡目で流れだけで再現。`convMethod 0` と Barth `limiter 1` は安定)。受動スカラ経路とは独立 (流れ単独で出る) なので本 plan の対象外として引き継ぐ。切り分け: 同一メッシュ・同一 IC で convMethod/limiter/dualTime を 1 つずつ戻し、NaN の初発ノードが seam かどうかを見る | 未着手 |
...
- `2026-09-17` — F-cf8 は plan species-passive-scalar-unification (受動スカラの化学種経路化 + dual-time BDF) で実施し **完了** (モーメントに BDF 物理時間項; §6-8 の検証行列 BDF1/BDF2 × coupling 0/1 で次数・収支 PASS)。同 plan §10 の node 周期一様流の発散を F-cf12 として登録。
