...
| ~~F-cf5~~ | ~~H2O の潜熱が 373.15 K 超で増加する~~ **決着 (2026-09-14)**: `plans/active/condensation-h2o-latent-supercritical.md` で Watson 型外挿 (指数 0.38、646.15 K で凍結) に差し替え。IAPWS 飽和線と 400–623 K で rms 1.4 % | 完了 |
| F-cf10 | **$T>T_c$ に液相 ($g>0$) が残る状態の規約が無い** (2026-09-14, 上記 plan §5.1 #5): 物理的には臨界点以上に液相は存在し得ないので強制全蒸発が正しいが、現状は潜熱を 646.15 K の値で凍結して数値的に破綻しないようにしているだけ。強制全蒸発にするか二相反転の失敗として弾くかを決める。現行ケースは全て 373 K 未満なので未着手で可 | 未着手 |
| F-cf11 | **`cond_Tsat` の 50 K 床** (2026-09-14 確認、本変更以前からの仕様): 空気/N2 凝縮の onset は ~38–40 K なので `condTsat` 診断はそこで 50 K に張り付く。診断のみで場には効かないが、低温ケースで Tsat 診断を読むときは注意。床を下げるか診断を無効化するか決める | 未着手 |
...
| ~~F-cf9~~ | **蒸気 c_p,v の出どころ** — **決着 (2026-09-14)**: Kirchhoff の傾き L'=c_p,v−c_l に入る c_p,v は、**CPG (thermalMethod 0) では config の `physProp.cp` をそのまま使う** (`CondPropOpts::gasCp`)。CPG は「気相の c_p はこの 1 つの定数」というモデルなので、物性相関だけ別の定数を持たせない。TP には気相の c_p 定数が無いので内蔵の種固有値 (N2 1038.8 / H2O 1855)。**空気キャリアで実測**: L(38 K) +0.40 %、p_sat(38 K) ×0.983、onset は 2.20 in / 2.10 in で**変化なし**、g_exit 0.0583→0.0581 (−0.3 %)、液滴モーメントは最大 2.4 % 変化 (ノイズ床超)。純 N2 (`cp: 1038.8`) はノイズ床以内で不変。run: `case/34.arthur_n2_nozzle/run_0105_gascp_air_node_*`, `run_0106_gascp_n2_node_*`。残件: CPG carrier の二相 EOS が液相の顕熱にも混合気の熱容量 (c_v+R_w=1014.7) を当てている件は定式化そのものの近似で別課題 | 完了 |
