...
### 4.7 既定を `limiterScaled: 1` に切り替えた (2026-09-20, ユーザ決定)

**決定**: ソルバと設計チェーンの既定を **`limiterScaled: 0` → `1`** (`venkatK` 既定 0.05) にする。
「切り替えたら答えが変わるのは当たり前だが、変えるべきなら変えるしかない」(ユーザ)。

**根拠** (すべて実測済み):

| 観点 | 旧 (0) | 新 (1, K=0.05) |
| --- | --- | --- |
| 厳密 Riemann 解の L1 収束次数 (`case/05`) | 0.76 / 1.00 | **0.82 / 1.04** |
| 近傍逸脱 (`case/46`) | `ro` 8146 / `Uy` 919 / `P` 141 | **`ro` 0 / `Ux` 0 / `P` 0** (`Uy` 133) |
| 座標 ×1024 | ψ が 58761/62069 ノードで変化 (**実質 OFF**) | **ビット同一** |
| 格子細分 (100/200/400) | `ro` 逸脱 27335→49712→88433 | **全解像度で 0** |

**切り替えで実際にどれだけ動くか** (旧既定の結果 vs 新既定、同一入力):

| ケース | ρ | P | T | Ux | ρe | 備考 |
| --- | --- | --- | --- | --- | --- | --- |
| `case/05` Sod | 6.0e-2 | 3.5e-2 | 2.0e-2 | 8.2e-2 | 3.8e-2 | **衝撃で動く。厳密解には近づく** (下表) |
| `case/09` TGV | **0** | **0** | **0** | **0** | **0** | 一様流なので**完全に不変** |
| `case/44` 軸対称 TP 凝縮 | 1.6e-4 | 1.9e-4 | 3.4e-4 | 1.2e-4 | 1.2e-4 | 小さい |

`case/05` を厳密解との距離で見ると**改善している**:

| | L1(ro) | L1(P) | L1(u) | 衝撃後 ρ (厳密 2.2240) |
| --- | --- | --- | --- | --- |
| 旧既定 | 7.048e-3 | 3.212e-3 | 6.048e-3 | 1.9913 |
| **新既定** | **6.905e-3** | **3.062e-3** | **5.909e-3** | **2.0517** |

**適用範囲のガードは既存のものが効く**: node 以外 / `convMethod` 対象外では
**警告を出して自動で 0 に落ちる** (`solverConfig.cpp`)。メッシュ変換 (`convertGmshToForge`) が
同じ config を cell モードで読むので、例外ではなく警告で落とす設計は維持する。

**旧値を再現したい run は `limiterScaled: 0` を明記すること。**
`procedures/recommended-settings.md` に本節の要約と +0.54 % の注意書きを入れた。
...
| S5 | ~~ψ 凍結の要否判断~~ **決着: 入れない** (§4.3)。プラトー運用の受理条件は `methods/limiter.md` に記載済み |
| S6 | ~~既定の切り替え~~ **完了 (2026-09-20) → §4.7**。ソルバ・生成器とも既定 `limiterScaled: 1` / `venkatK: 0.05`。`procedures/recommended-settings.md` 同期済み |
