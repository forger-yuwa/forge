...
$\max(r_u, r_l) + \delta_r < z_w$、$t_c, t_{sw} > 0$、$x \ge L_{sw}$ で $r_u = r_l = 0$、各区間長 ≥ 下限。
...
- **3D 法線の補正 (M3)**: 断面は x = const の平面なので、傾いたランプでは法線距離が $\cos\theta_r$ 倍に縮む。
  ランプに付く辺 (e2, e4 と S の de 上端) は station ごとに $h_1^{eff} = h_1 / \cos\theta_r(x)$ を使う。
  上隅の対角線 e2 は TOP と SIDE が共有するので、側壁側の第一層は隅の近傍で最大 $1/\cos\theta_r$ 倍 (22° で +7.8 %) 厚くなる。
  **受入の許容は「隅から $2\delta_r$ 以上離れた壁で ±5 %、隅の近傍で ±10 %」**とする。
- **e5 (側壁内面に沿う y 分布) は両端細分** ($h_1$ から成長率 ≤ 1.2)。B・C では SW がこの分布を引き継ぎ、
  **側壁の跡に露出したランプ面・カウル上面の壁層**になる (一様だと露出面の第一層が mm 級になる)。$N_Y \approx 90$。
- **端面の直後の x 分布 (M3)**: `sidewall_end`・`cowl_base` の下流区間は、端面から第一間隔 $\le t/5$ ($t$ = 板厚) で始め成長率 ≤ 1.2
  ([[base-wake-resolution-rule]])。上流区間の末尾も同じ間隔に寄せて x 方向の段差を作らない。
