...
| 案 | 内容 | 見込み | 代償 |
| --- | --- | --- | --- |
| (a) 壁節点の Dirichlet 固定を外す (`nodeWallDirichlet: 0`) | 壁 CV の運動量が圧力勾配で応答でき、質量が戻る | A/B 中 (`run_0428_3d_junction_diag_nowd`, 同じ IC・同じ 1800 step) | 全壁に効く。壁速度が厳密 0 でなくなる ([[node-wall-velocity-needs-dirichlet-flag]])。生産の格子列は 1 で取っている |
...
| (d) 床を上げる (`roMin`/`pMin`) | 過渡を床で持たせる | 床は保存量の排出を止めない (ρ 3.6e-5 < roMin 1e-4 まで落ちた) | 負密度 → NaN の経路 ([[detectnan-reports-symptom-not-cause]]) |
...
| **B1d** | **接続模型の CFD 成立確認 (§4.13)** | AWS。発散しない・床ノード 0・壁解像を確認。run_0423 発散 → run_0424 (後流を静止で開始) |
