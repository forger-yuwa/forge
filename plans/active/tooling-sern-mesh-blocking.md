...
| **B1d** (**F**) | **接続模型の CFD 成立確認 (§4.13)** | 鈍頭のまま診断を先行 (codex 3 回目 NO-GO 採用): ① IC 修正 (済) → ② 修正 IC で 200 step 刻み出力の診断 run を取り直し、問題 CV (node ID 固定) の面別 SLAU 流束 (移流項・圧力項・χ・両側音速)・面積・体積・Δρ・床補正量を場から計算 → ③ 断熱壁 (運動量固定のみ) の A/B。合否は M5 の基準。テーパは候補として保留 |
