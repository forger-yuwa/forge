...
**判断 (2026-09-22, Fable)**: (a)(d)(e) は却下、(c) は (e) と同型なので見送り。**(b) 後端のテーパを推奨する** — 根拠は機体ベースの実績 (鉛直 base は node で発散、テーパ + 薄い残りベースで全段通過) と、
本模型の排出が「流れに直交する面 × 縁を回る高速流」の組合せで起きること。テーパなら面が流れに沿い、壁 CV は接線流を見る。
**ユーザ判断待ち** (形状の決定): 側壁後端・カウル後端の板厚を、後端の手前 $L_t$ で $t \to t_{base}$ に絞る。案: $L_t = 5t$、$t_{base} = t/5$ (側壁 5 → 1 mm、カウル 2 → 0.4 mm)、
...
| **B1d** (**F**: ユーザ判断待ち = 後端のテーパ) | **接続模型の CFD 成立確認 (§4.13)** | run_0423–0429 の 7 run で IC の問題 2 つを潰し、残る発散は**鈍頭後端面の壁 CV の排出** (node の Dirichlet 壁固有) と特定。推奨 (b) テーパ。承認後: メッシャに $z_o(x)$・$y_{cl}(x)$ のテーパを入れ、$t_{base}$ = t/5 と t/2 の 2 水準で A/B → 通ったら soft → mid → 本段 |
