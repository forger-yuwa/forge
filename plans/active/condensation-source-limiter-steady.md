...
| 6 | codex result レビュー (4 回目) → accepted | 3 回目 NO-GO (M2/m3) を反映中: 小液滴の蒸発継続 (r_min 評価) + 単体 (k)、残差床ゲートを同一モードの Δτ 比較に改定、Wys CSV を報告用抽出関数に統一、(h) の Δτ 非依存正規化、文書同期。根拠 run は最終セット `run_0151`–`0161` (`run_0147`–`0150` は準備のみで破棄)。 1 回目 NO-GO の M1–M7/m1 は反映済み (§6.1)。残: Wys の mode 1/0 の step 時間差 (同時実行時 7.4 vs 4.4 ms/step → 単独計測は §9 に記載)、F-cf9 (旧 mode 0 削除) は followups |
...
- **収束・定常の扱い (codex M5, result-2 M1 で明文化)**: 比較する run は `check_convergence.py` **PASS** を原則とする。PASS が取れない case では次を残差ゲートとする (result-3 M2 で改定: 旧経路 mode 0 はソースを θ 倍に絞るのでモーメント残差の大きさ自体が違い、床の直接比較は不適):
  ...
  (b) 同一モード (mode 1) で Δτ (cfl 0.5 と 2) と精度 (condFloat 0/1) を変えたとき、各モーメント残差の床が流れ変数の床と同じ比率で動く (Δτ 比の比が 0.6〜1.7、精度で ±10 %) =
  モーメント残差も同じ床機構で、ソースが未収束で残っているのではない; (c) 凝縮固有量 series STEADY (0.2 %) + 補正 0。報告は「定常解の一致」でなく **「プラトー上の最終場一致 + series STEADY + 補正 0」** と表現する。case/44 のような warm 床 plateau
