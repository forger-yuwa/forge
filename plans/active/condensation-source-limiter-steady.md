...
| 5 | 回帰 run (result-4 で再開) | **5 巡目バイナリで再取得済 (2026-09-16, `run_0170`–`0176`; §9)**: (a) node 床の定常性 (末尾/直前 0.97–1.01)・Δτ 非依存 (床比 0.86–1.05) 成立、(c) 全 run ALL STEADY 0.2 %・condLim 1.0000・補正 0 成立、**(b) cell は ro/P/T が反復床内だが g_0 の max 差 2.17e-3 が床 (4.4–5.8e-4) の 3.7–4.9 倍で不成立** (壁側液相帯で cfl 0.5 の g が系統的に −0.41 %、3 世代 warm 継続で頭打ち; node も同符号で 4 世代単調接近中 1.55e-3)。**→ result-5 M1 の交差 restart (`run_0177`–`0180`) で決着: cfl 2 の場は cfl 0.5 でも固定点 (cell 6.2e-4 = 床の 2 倍 1.15e-3 内, node 反復床)、cfl 0.5 系の場は cfl 2 で 6000–8000 step で cfl 2 の場へ収束 → 差は過渡 (履歴)。(b) 成立 (§9)。**1 巡目 (2026-09-15): case/44 cfl 0.5/2/4 × condFloat 0/1 一致 (`run_0132`–`0136`); Arthur N2 cell/node・空気 cell (`case/34 run_0105`–`0107`) 参照とノイズ床内 PASS・onset 同一; Wysłouzil (`case/16 run_0470`) 報告量同一・場の差 ≤1e-4 |
| 6 | codex result レビュー (5 回目) → accepted | 4 回目 NO-GO (M3/m2) を反映済 (5 巡目バイナリ 355f4e40/cdb4fda5)。5 回目の論点: (b) の g_0 max 差超過を「Δτ 依存の残り (ゲート改定 or 追加検証)」と「cell 限界サイクル平均の Δτ 依存」のどちらとみなすか。反映内容: Q0=0 減衰、0.2 % 閾値と補間、§6 ゲート改定 (node 定常性 / cell 反復ノイズ)。根拠 run は 5 巡目バイナリの `run_0170`–`0176` (反復 `0173`/`0174`/`0176` 込み)、Arthur `lim1e`、Wys `lim1e`/`lim0e`、seam `run_0064`: 小液滴の蒸発継続 (r_min 評価) + 単体 (k)、残差床ゲートを同一モードの Δτ 比較に改定、Wys CSV を報告用抽出関数に統一、(h) の Δτ 非依存正規化、文書同期。根拠 run は最終セット `run_0151`–`0161` (`run_0147`–`0150` は準備のみで破棄)。 1 回目 NO-GO の M1–M7/m1 は反映済み (§6.1)。残: Wys の mode 1/0 の step 時間差 (同時実行時 7.4 vs 4.4 ms/step → 単独計測は §9 に記載)、F-cf9 (旧 mode 0 削除) は followups |
...
| --- | --- | --- | --- | --- |
| result (5 回目) | `2026-09-16` | [2026-09-16-condensation-source-limiter-steady-result-4.md](../../notes/reviews/2026-09-16-condensation-source-limiter-steady-result-4.md) | **NO-GO**, C0/M1/m1 | **全採用 (2026-09-16 反映中)**: M1 (cell の g_0 max 差が反復床の 1.89 倍で §6 (b) 未達。積分量 0.2 % 一致・node 再現性は異なる cfl 間の固定点一致を証明しない → 同一保存場からの cfl 2/0.5 交差 restart で履歴依存と Δτ 依存を切り分けてから完了とする; ノルム条件だけへの切替は不可) → 交差 restart `run_0177`–`0180` (cell: cfl 0.5 from `0172` / cfl 2 from `0175`; node: cfl 0.5 from `0170` / cfl 2 from `0171`; 24000 step, 2000 step 毎に保存) で壁側液相帯の平均 g の時系列と両正本への距離を評価 (§9); `recommended-settings.md` の記述を「ソース残差の明示的 Δτ 依存を除去」に限定; m1 (Q0=0 復旧規則が methods 未記載) → `methods/condensation.md` §4c に適用条件・減衰式・r_min 依存・g≤g_rm で消滅処理へ渡すことを追記 |
| result (4 回目) | `2026-09-16` | [2026-09-16-condensation-source-limiter-steady-result-3.md](../../notes/reviews/2026-09-16-condensation-source-limiter-steady-result-3.md) | **NO-GO**, C0/M3/m2 | **全採用 (2026-09-16 反映)**: M1 (Q0=0 の液相が蒸発も消滅もしない) → `cond_evap_source_rate{,_f}` で Q0≤1e-30 は質量を r_min の自己相似縮小率 3ṙ(r_min)/r_min で減衰 (Δτ 非依存) させ g≤g_rm でクランプに渡す; 単体 (k) を double/float × {整合 r30=1.5e-9, Q0=Q1=Q2=0} の 4 組でソース→更新→消滅 (17/20 step) まで確認 (m4 も対応); M2 (`--drift 0.2` は 20 %) → 0.002 に修正し、交差位置 (onset, x30) を線形補間 (`cond_series_csv.py`)、全 run の VERDICT を再保存; M3 (残差床の比率は根拠にならない) → §6 を改定: node は床の時間定常性 + Δτ/精度非依存、cell は同一バイナリ反復ノイズの 2 倍以内 (perf_regress 規約) で判定、「3 桁低下 or 床比」は撤回; m5 (文書) → 旧セットの「正本」表記を履歴に、F-cf7 本文、eq=1 の数値 (ro 2.5e-6) を最終セットで更新 |
| result (3 回目) | `2026-09-16` | [2026-09-16-condensation-source-limiter-steady-result-2.md](../../notes/reviews/2026-09-16-condensation-source-limiter-steady-result-2.md) | **NO-GO**, C0/M2/m3 | **全採用 (2026-09-16 反映)**: M1 (r30<2 r_min で S=0 にすると g>g_rm の小液滴が蒸発も消滅もしない) → ṙ を max(r30, r_min) で評価して蒸発を続ける (消滅は従来どおりクランプ)、単体 (k) で反例状態 (g=1e-5, r30=1.5e-9) がソース→更新→消滅を通して消えることを確認; M2 (残差床が旧経路と 3.4 倍) → 旧経路はソースを θ=0.24 倍に絞るので rog/roQ2 の残差自体が小さい比較だった。ゲートを §6 (a)–(c) に改定し、mode 1 内の Δτ・精度比較で床が流れ変数と同じ機構であることを示す (§9 の表); m3 (Wys CSV の出口取り違え) → `compare_condfix.py --series-csv` (報告と同じ抽出) に統一し VERDICT 再保存 (`wys_cond_series.py` 廃止); m4 ((h) の判定が Δτ 依存) → |res|/(β V |ρφ|) に変更; m5 (文書) → methods/README/plan/plans README 同期、`run_0147`–`0150` は破棄と明記 |
...
- `2026-09-16` — **codex result-5 (NO-GO, M1/m1) を全採用**: M1 → 交差 restart `run_0177`–`0180` (同一 `res_24000` から cfl 入替, 24000 step, 2000 step 毎保存; case/44 README「交差 restart」表)。**cell**: cfl 0.5 ← cfl 2 場 (`0177`) は 24000 step 動かず (vs `0172`: g_0 max 6.2e-4 [最大 9.7e-4], g L1 1.4e-4 = 反復床 4.4–5.8e-4 / 1.1–1.3e-4 の 2 倍内; cfl 0.5 系 `0175` には近づかない 2.3–2.7e-3); cfl 2 ← cfl 0.5 系場 (`0178`) は 6000 step で cfl 2 場へ (2.17e-3 → 4.7e-4, 最終 6.0e-4 / 1.5e-4)。**node** (決定的): cfl 0.5 ← cfl 2 場 (`0179`) は 0.7–1.7e-4 / 1.4–2.0e-5 = 反復床 (2.0e-4 / 1.95e-5) を 24000 step 保持; cfl 2 ← cfl 0.5 系場 (`0180`) は 8000 step で 4.6e-4, 24000 step で 1.3e-4 / 1.7e-5 (ro 5.3e-6, |ΔT| 0.005 K) = 反復床。4 run NaN 0, series ALL STEADY, 残差床は正本と同じ (比 0.77–1.03), 報告量 5 桁一致。差の空間分布 (cfl 0.5 系 − cfl 2): onset 域 x 15–16 r_t で最大 (−1.4 % cell / −2.7 % node) → 下流で減衰、Q0 同じ・Q1/Q2/g 低・S 高 = 成長遅れの過渡。**結論: 固定点は Δτ 非依存、cfl 0.5 系の系統差は低 cfl 側で減衰の遅い過渡 (履歴)。§6 (b) を「同一場からの交差 restart」で判定する手順に確定し、成立**。減衰が擬似時間比 (4 倍) より大きく速い理由は未同定 (固定点の問題ではないので本 plan 外; followups へ)。`recommended-settings.md` の記述を「ソース残差の明示的 Δτ 依存を除去」に限定。m1 → `methods/condensation.md` §4c に Q0=0 復旧規則 (適用条件・減衰式・r_min 依存・g≤g_rm で消滅処理へ) を追記。
