...
| 5 | ~~回帰 run~~ | 最終バイナリで完了 (2026-09-16, §9 最上段)。1 巡目 (2026-09-15): case/44 cfl 0.5/2/4 × condFloat 0/1 一致 (`run_0132`–`0136`); Arthur N2 cell/node・空気 cell (`case/34 run_0105`–`0107`) 参照とノイズ床内 PASS・onset 同一; Wysłouzil (`case/16 run_0470`) 報告量同一・場の差 ≤1e-4 |
| 6 | codex result レビュー (4 回目) → accepted | 3 回目 NO-GO (M2/m3) を反映済み (§6.1, §9; 最終確定バイナリ d618c0dc の `run_0162`–`0167` 等): 小液滴の蒸発継続 (r_min 評価) + 単体 (k)、残差床ゲートを同一モードの Δτ 比較に改定、Wys CSV を報告用抽出関数に統一、(h) の Δτ 非依存正規化、文書同期。根拠 run は最終セット `run_0151`–`0161` (`run_0147`–`0150` は準備のみで破棄)。 1 回目 NO-GO の M1–M7/m1 は反映済み (§6.1)。残: Wys の mode 1/0 の step 時間差 (同時実行時 7.4 vs 4.4 ms/step → 単独計測は §9 に記載)、F-cf9 (旧 mode 0 削除) は followups |

## 6. 検証

- **単体 / ビルド**: `cmake --build build` (double + float 経路)、§5-6 のテスト (Δτ 不変性を含む)。
- **収束・定常の扱い (codex M5, result-2 M1 で明文化)**: 比較する run は `check_convergence.py` **PASS** を原則とする。PASS が取れない case では次を残差ゲートとする (result-3 M2 で改定: 旧経路 mode 0 はソースを θ 倍に絞るのでモーメント残差の大きさ自体が違い、床の直接比較は不適):
  (a) 流れ変数 (rms_ro, rms_roe) の床が mode 0 と同じ (±10 %) = 床は case 固有 (cell の atomicAdd 等) で修正由来ではない;
  (b) 同一モード (mode 1) で Δτ (cfl 0.5 と 2) と精度 (condFloat 0/1) を変えたとき、各モーメント残差の床が流れ変数の床と同じ向きに動く (精度で ±10 %; Δτ を下げると流れ変数と同じく単調に下がる。実測: node は全列 0.90–1.06、cell は流れ 0.31 / モーメント 0.15–0.18 で、閾値「Δτ 比の比 0.4〜2.5」は実測後に置いた) = モーメント残差も同じ床機構 (Δτ に比例する限界サイクル) で、ソースが未収束で残っているのではない (未収束なら Δτ に依らず残る); (c) 凝縮固有量 series STEADY (0.2 %) + 補正 0。報告は「定常解の一致」でなく **「プラトー上の最終場一致 + series STEADY + 補正 0」** と表現する。case/44 のような warm 床 plateau
