...
| 6 | codex result レビュー (2 回目) → accepted | 1 回目 NO-GO の M1–M7/m1 は反映済み (§6.1)。残: Wys の mode 1/0 の step 時間差 (同時実行時 7.4 vs 4.4 ms/step → 単独計測は §9 に記載)、F-cf9 (旧 mode 0 削除) は followups |
...
- `2026-09-16` — step 時間 (Wysłouzil 2D node SST 凝縮, 4000 step 単独計測, RTX 3060): mode 1 = 3.28 / 3.1x ms/step、mode 0 = 3.09 / 3.10 / 3.12 ms/step → 更新クランプの追加コストは +0〜6 % (48000 step 同時実行時の 7.4 vs 4.4 は GPU 共有の影響)。
