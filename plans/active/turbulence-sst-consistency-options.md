...
- ~~既定値: 3 は ON 確定。1/2/4/5 は A/B (§3.2, §3.3) と codex レビューを見て決める。4 は境界面未対応のまま既定 ON にしない。~~
  → 決着 (2026-09-08, §2.1): 1/2 は整理 (a)(b) の上で既定 ON。4/5 は R3 (`accepted/turbulence-sst-energy-includes-k.md`, `sstEnergyIncludesK`) として実装・検証したが、**ユーザ決定で既定 0 (roe に k を含めない) を維持、opt-in** (乱流の過渡が T に伝播する脆弱性を避ける)。4/5 の分離型キーも既定 0 のまま残す。
