...
- ~~既定値: 3 は ON 確定。1/2/4/5 は A/B (§3.2, §3.3) と codex レビューを見て決める。4 は境界面未対応のまま既定 ON にしない。~~
  → 決着 (2026-09-08, §2.1): 1/2 は整理 (a)(b) の上で既定 ON、4/5 は境界閉包 (§4.1) と時間離散収支 (残作業 5) の完備後に再判断。
...
| 3 | `sstIsotropicStress` の境界面 (入口/対称面/slip/壁関数壁) と軸対称 (τθθ, method 1) 対応 | 未 (方針 §4.1: 有効圧力 p*=p+2/3ρk で一括) |
