...
- **Aitken 加速は基礎反復 (受理判定つき) の安定性を確認してから**。使うときは (i) 係数を $[0.2, 1.5]$ に制限、
  (ii) 分母 $\|\Delta^2 T\|$ の退化時は無効化、(iii) 残差が増えたら前反復へ退避、を必須とする。
...
| 10 | 固体ソルバ | `local1d` / `shell2d` / `fem2d`。単体は解析解 (格子収束と離散残差の機械精度検査を**分ける**) |
