...
| 86 | **V6 を V6′ に差し替え / すきまへの適用可否は先方に渡す** (2026-09-25、`diagnostician` に諮った。担当 `F`) | **判断: 2026-09-25、結論 = 「V6 は外すのでも待つのでもなく、CHT 側で自己完結する V6′ に置き換える。すきまへの適用可否は case/49 側 + ユーザが決める」**。理由: §1 の目的が「すきま / 深いキャビティでは $T_w$ が解の一部」なので**すきま形状の試験ゼロで閉じると目的と食い違う**が、原 V6 は合否が他セッションの資産に依存していた。**V6′ は今すぐ回せる** (深部収束は `case/56 run_0020_double` と `case/50 run_0051_fp64` の FP64 対照で既に閉じており、待つ理由がない)。規模は 100k step ≈ 3 分。**受け渡しは §5.1 の本行 + [`notes/sessions/2026-09-25-cht-gap-application-handoff.md`](../../notes/sessions/2026-09-25-cht-gap-application-handoff.md) で行い、`case-plate-annular-cavity-m5.md` は編集しない** (AGENTS.md「相手の plan に書き込まない」)。**先方に立ててほしい行の本文は handoff ノートに置いた** |
...
| 89 | **`codex_review.py` が巨大 plan で無言終了する** (2026-09-25 実測) | 本 plan が 1800 行を超え、ツールが作るプロンプトが **237 kB** になった結果、**codex が何も返さずに exit 0 で終わった** (2 回再現)。出力 `.md` が生成されないだけで**エラーは出ない**ので、**気づかずに「レビュー済み」と扱う事故になりうる**。7 巡目は手書きプロンプト (6.5 kB、plan は `sed -n 'A,Bp'` の行単位読みのみ許可) で通した。**対処案**: (a) `codex_review.py` にプロンプト長の上限と警告を入れる、(b) 出力 `.md` が空/未生成なら非ゼロ終了させる、(c) plan が長い場合は節を選んで渡すモードを足す。**起動の罠も 1 つ**: `nohup ... &` で投げるとセッション終了時に刈られて途中で消える。`setsid` で切り離し、出力はスクラッチパッドでなく `notes/reviews/*.log` (git 追跡外) に置く |
| 90 | **V2 `case/48` 段の SU2 を収束させて判定する** (2026-09-25、codex result 7 巡目が #83 の「ここで止める」を非承認。担当 `O`) | **最有力仮説 (codex、config で裏取り済み)**: **外反復が足りない**。`case/48.flat_plate_cooled_m4/su2_cht/plate_cht.cfg`:9 が `OUTER_ITER= 200`、`fluid.cfg`:51 が `INNER_ITER= 200` なので、**「40000 反復」は 外 200 × 流体内 200 であって連成更新は 200 回しかない**。SU2 の CHT は**界面量の交換が外反復ごと**で、**1D スラブ段でさえ固体残差を下げるのに 3000 外反復**を要した (#81)。→ **同じ設定のまま `OUTER_ITER` を延ばし、両ゾーンの残差と界面熱収支の推移を見る**。**「熱容量が実質ゼロで振動」は現状の証拠では支持できない** ($\rho c_p$=1000 J/(m³K)・厚さ 1 mm で面積当たり 1 J/(m²K) はゼロでない。`solid.cfg`:1)。**比較の作り方の訂正**: 60 % の食い違いは固体の $T_w$ から $k_s(T_w-300)/t$ で逆算した $q$ と流体 `HF` を比べたものだが、**この逆算は面内伝導を落とす**。**同じ面積・単位・符号で積分した界面流束**で比べ直すこと。**収束後に §6 V2 の登録許容 (平均 2 % / 局所 2 % / $\int q\,ds$ 3 %) で判定する。FAIL なら原因を切り分ける** |
...
| **V6′** | **すきま形状での CHT (自己完結)** (2026-09-25 新設) | **他セッションの資産に依存しない 2D 深スロット** ($D/W\ge16$) を自作し、前壁・後壁に `conjugate` (`fem2d` 帯) を付けて**背面温度を壁ごとに変え、壁温差を作る**。深部は $Pe=\lvert u\rvert W/\alpha\ll1$ で伝導支配になるので (`case/56` の SU2 実測で $z/W$ 3–5 が $Pe$≈0.45、5–10 で 1.4e-3)、**幅方向 1D 伝導 + 固体の直列抵抗の解析解** (V1 と同型) に漸近しなければならない。**一様壁温では漸近解が $T\equiv T_w$・$q\equiv0$ の自明解 (null 検査) にしかならないので、壁温差をつけるのが要点** | **合格条件 (2026-09-25 に codex result 7 巡目の指摘で修正)**: 漸近解は $q_*=\dfrac{T_{b1}-T_{b2}}{t_1/k_{s1}+W/k_f+t_2/k_{s2}}$、$T_{w1,*}=T_{b1}-q_*t_1/k_{s1}$、$T_{w2,*}=T_{b2}+q_*t_2/k_{s2}$。**正規化は量ごとに分ける** (旧稿の「$q$ も温度差の 0.5 %」は**次元が合っていなかった**): **温度誤差は $\lvert T_{b1}-T_{b2}\rvert$ で、熱流束誤差は $\lvert q_*\rvert$ で**割って ≤0.5 %。$K$ (50/200)・$D_f$ 方式による $T_w$ 差も**同じ分母 $\lvert T_{b1}-T_{b2}\rvert$** で ≤0.5 %。G-cons ≤0.5 %。**$D/W\ge16$・$z/W\ge8$ だけでは 0.5 % を保証できない**ので、判定に使う深さ帯は**底面条件の影響・固体の深さ方向伝導・残存移流 ($Pe$ の実測) を確かめてから決める** (帯を先に決め打ちしない)。**全域 FP64 ビルドと float32+`qAccumulatorFP64` の両方で回し、深部 $\lvert U\rvert$ を併記** (`case/50` で 60 倍差が出たので、ここでも出るかを測る)。**事前確認 — 済 (2026-09-25)**: `main.cpp`:2150-2183 の `qAccumulatorFP64` 拒否リストに **`conjugate` は入っていない**ので併用は起動時に弾かれない。**ただし「拒否されない」は「検証済み」ではない** (固体側は host の `updateFem2dWall` で更新され流れの commit の外にあるため衝突しないはず、という推定)。**v1a の対応範囲が V6′ の設定を縛る**: `gpu=1`・`node`・`timeIntegration=11` && `unsteady=0`・**軸対称なし**・**周期なし**・`sstEnergyIncludesK=0`。平面 2D の深スロットならすべて満たす |
...
1. **V2 の `case/48` 段を、事前登録した許容 (平均 2 % / 局所 2 % / $\int q\,ds$ 3 %) で判定する**。
   **決着 (2026-09-25, codex result 7 巡目)**: 「判定不能のまま閉じる」は**承認されなかった**
   ...
   → **`active` に留め、未達条件として残す**。SU2 を収束させてから登録条件で判定する (#90)。
...
3. **放射**: §4.10 のとおり解かない。深いキャビティで桁が無視できないと分かったら別 plan にするか。
   ...
   **「まだやらない。将来的にもやらないとは宣言しない」**。すなわち**本計画の対象外として保留**であって、
   ...
   将来やるときは別 plan を起票する。
4. ~~**V2 の `case/48` 段を「判定不能」で閉じてよいか**~~ (2026-09-25 起票)
