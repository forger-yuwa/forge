...
1. **V2 の `case/48` 段の判定**。PASS か、または**「判定不能 — SU2 側の乱流 CHT が定常に達しない」として
   ...
   (界面温度は 0.13 K 一致だが熱収支 60 % 食い違い・`bgs[w]` +6.4) → **判定不能のまま**。**ユーザ確認待ち** (§8-4)。
2. **V6′** (#86)。すきま形状での CHT を 1 本だけ自己完結で通す。
3. **放射の決着** (§8-3)。別 plan を起票するか「やらない」と明記するか。
...
3. **放射**: §4.10 のとおり解かない。深いキャビティで桁が無視できないと分かったら別 plan にするか。
   **2026-09-25 に「無視できない」が数字で出た**: [`case-plate-annular-cavity-m5.md`](case-plate-annular-cavity-m5.md)
   ...
   → **別 plan を起票するか「やらない」と明記するかの決着が要る** (本計画を閉じる条件の 3 番目)。**ユーザ確認待ち**。
4. **V2 の `case/48` 段を「判定不能」で閉じてよいか** (2026-09-25 新規)。SU2 の乱流 CHT が 4 設定 40000 反復で
   定常に達しない (#83)。連成の定式化自体は **1D スラブ段で SU2 と +0.00001 K 一致**済み (#81) なので、
   そこで止める判断。**「判定不能」は合格ではない**ので、§6 の事前登録許容に対して**判定を出していない**ことを
   明記したうえで codex result 7 巡目に扱いごと諮る、という形を想定している。**ユーザ確認待ち**。
