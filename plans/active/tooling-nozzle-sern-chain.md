...
| R4e | **側面バンド末端の壁を除去する (codex C2)**: 上バンドの下端を z 依存にし、幅外では直線 y_veh のまま出口まで通す (末端の閉じ面が不要になる)。断面・タグ試験に「幅外に横断する固体面がない」「追加流体から下流への接続がある」を追加 | `mesh_sern3d.py` の `_add_vehicle_side3d` docstring に欠陥を明記済 |
