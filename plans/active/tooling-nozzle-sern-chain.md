...
| 4 | **側壁の 3D 効果をどう入れるか** (§6 の 2 % 基準が不成立) | **2 段構え**を推奨。**第 1 段**: 2D パレートの 3–5 点を 3D Euler (52.5 万セル / **3 分**、run_0027 で成立) で回し**順位が保たれるか**を見る (5 点 × 3 作動点 ≈ 45 分)。保たれれば 2D 設計 + 3D 検証で運用可。**第 2 段** (順位が崩れたら): $L_{\rm sw}$ を 6 番目の dv にし、(a) 3D Euler を in-loop (1 評価 ≈ 15 分 → 56 評価 14 時間) か (b) **補正サロゲート** ($L_{\rm sw}/H$, $W/H$, $L_{\rm ramp}$ の 3 変数で 3D Euler 20–30 点 ≈ 1 時間、$\Delta(C_T,C_L,C_M) = $ 3D − 2D を Kriging) のどちらか。**(b) が安い**。3D SST は後縁 3 重点の ω 発散 (§5.1-3) で当面使えないので、**3D は Euler で圧力側の 3 次元効果だけ取り、摩擦は 2D の値を使う**分離が実用的 |
