...
~~**確定した生産レシピ (2D、3 設計とも PASS)**~~ **撤回 (2026-09-19, codex plan レビュー M2)**:
下表の組み合わせ (梯子 1500×3 + 本段 500) は**一度も回していない**。ramp の成功例 `run_0178`–`0180` は
**4 段とも 2000 step** で、別実験の数字を組み合わせただけだった。さらに 500 step 時点で `vis_turb` が
局所 `DRIFTING` / ノード平均 `TRANSIENT-UNSETTLED` で、力係数は STEADY でも乱流粘性が未発達。
残作業と受理基準の改訂は [`tooling-nozzle-sern-startup.md`](tooling-nozzle-sern-startup.md) §5.1 / §6.2。
以下は撤回した記述:
