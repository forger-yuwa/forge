...
| R5b | **床に張り付いた解を受理しないゲート** (codex M2): `sern_gates.py:39` は有限・正値しか見ない。`pMin`/`DEPVAR_TMIN`/`roMin` 到達ノード数、床による保存量変更、床感度を受理条件に足す。m10 の解析値は 0.147 Pa / 18.0 K / 2.83e-5 で三つの床に抵触 | §4.14-5 |
