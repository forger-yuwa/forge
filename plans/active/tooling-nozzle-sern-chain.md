...
| R4e | **側面バンド末端の壁を除去する (codex C2)**。**設計確定 (2026-09-19, codex plan-3): 案 (d) = 全長機体 + 有限ベース + 出口まで維持する側面/後流バンド** → 詳細と実装前に固定する検証条件は **§4.15.3**。2026-09-19 に試した 3 案 (楔縮退 / 鈍頭化 / prism) はいずれも境界書き出しの連鎖で差し戻し済み。<br>**着手順 (codex 指定)**: ① 接続・物理寸法の確定 (格子依存の形状を断つ) → ② ベース込みの BC と帳簿の確定 (R4f を内包) → ③ `run_0034` の診断訂正 (済) と 2D 有限ベース起動試験の定義 → ④ R5b と定量検証条件の確定。**案 (d) が発散しない保証は無い** | `mesh_sern3d.py` の `_add_vehicle_side3d` docstring に現行欠陥を明記済 |
