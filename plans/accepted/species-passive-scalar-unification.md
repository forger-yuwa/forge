...
- **周期 node (codex M4)**: 名前ベースの `res_*` gather だけでは不足。処理順を固定する:
  1. 勾配積算は周期半割面を除外して行い (`species_gradient_d` に除外引数を追加)、勾配を gather (既存の周期勾配 gather に受動種・化学種の勾配を登録);
     **訂正 (2026-09-26)**: 除外条件 `ip >= nNormalPlanes && ic1 < nCells` は周期 bcond にもゴーストが付く (`mesh.cpp:443-456`) ため一度も成立せず、
     ...
     [boundary-node-periodic-gradient-fix](boundary-node-periodic-gradient-fix.md) §4.2a。
...
| 18 | dual-time の θ_u を物理 step あたりに; 凝縮の 3 水準次数 (g/Q0) | 実装済・**次数は未達** (2026-09-17, case/44 `run_0256`–`0262`; convMethod 0, nSub 40/80, dt 1.6e-5/8e-6/4e-6): sub-iter 低下は流れ・化学種 ≥2.1 桁、rog/Q2/Q1 ≥2.4 桁だが **roQ0 min 1.64** (nSub 80 でも同値; condLim min 0 = 実現可能性クランプ [avail/非負] が常時作動するセルで増分 0); 観測次数 BDF2 g 1.30 / Q0 1.36 / T 1.27 / ro 2.29、BDF1 g 0.95 / Q0 0.70 / T 1.31 / ro 1.93; nSub 倍増差/最小水準差 g 0.43 / Q0 0.37 / T 0.07。**結論**: 流れ・トレーサ・化学種の BDF2 (2.0–2.3) は成立、モーメントは硬い実現可能性制約が作動するセルで時間 1 次相当 (制約整合の時間離散が要る = 本 plan の範囲外; §10)  **決着 (2026-09-17)**: 倍精度 (`run_0334`–`0337`) でも L2 次数 g 1.32 / Q0 1.39 / Q1 1.59 / Q2 1.54 = float と同じ → **核生成 onset 前線の非平滑性** (e(dt)² の 70–85 % が上位 1 % のセル、Linf 次数 0.1–1.0; 残り 99 % は 1.93–2.07、体積重み総量 2.2–3.2、T も同構造)。滑らかなブロブ (核生成抑止, `run_0338`–`0345`) では **g 1.96–2.01 / Q2 1.91–1.97 / Q1 1.83–1.92 / Q0 1.74–1.82, nSub 比 ≤0.006** → モーメントの BDF2 は成立。制約由来説 (§10) は撤回。 |
...
| 20 | (result-2 M1) モーメントの共通 θ 停止を撤回 (成分ごとの非負化, θ≥1e-12)、停止セルの残差寄与の記録、次数の再評価 (#12/#18 の実体) | forge (必須): 非負化のコードは済 (`cond_moment_update_limited_body`; 切った量 = その成分の未適用残差を limStats に記録)、次数の再評価は #21 の後に S3 でも (§6-6)  次数の再評価は #27/#28 で完了 (S3 が実際に効く `convMethod 1` の検証行列 `run_0429`–`0456`: BDF2/BDF1 × coupling 0/1 すべて次数・収支 PASS)。共通 θ 停止の撤回は次数を直す要因ではなかった (真因は float ノイズ床と onset 前線; §5.1 #12/#18) が、成分ごとの非負化自体は正 (停止セルの残差寄与を記録)。 |
| 21 | (M2 → §4.7 v5) 物理 step 末尾の保存的 FCT 補正 (流束形 BDF2 履歴, 有効質量流束, 全面 Zalesak) | **実装済 (2026-09-17, commit 7544ca36)**: 単体 `test_passive_fct.cu` (i)–(vi) ALL PASS。CFD 検証済 (§6-2 seam・restart は case/09 `run_0156`–`0164`、§6-6/§6-8 は case/44 `run_0429`–`0456`)  **restart (§6-6 (iv)) 済 (2026-09-17)**: case/09 `run_0164` (修正 binary) は流束形履歴を復元し、連続 `run_0162` との差 max|ΔroXi| 3.2e-6 (修正前 8.35e-4)、流れは roUx/roY/roe/ρ がビット一致 (roUy/roUz のみ 7.3e-11/1.6e-10)、収支ゲート PASS。  **§6-6 (ii) 達成 (2026-09-17, 再取得後)**: `convMethod 1` + S3 + FCT + 倍精度 + 空間スキーム整合 IC の `run_0417`–`0420` で **収支・場ゲート全水準 PASS**、流れ・化学種 1.93–1.95、g 1.74 / Q2 1.92 / Q1 1.88 / Q0 1.38 (FCT の非線形リミッタによる低下; 対照 `run_0425`–`0428` は 1.99–2.01)。旧記録: `run_0378`–`0381` は `convMethod 0` で S3 が不活性だった (1 次風上 + FCT の結果として記録) ほか最粗水準が収支ゲート FAIL。`convMethod 1` で S3 が効く条件の `run_0386`/`0390`–`0392` ではモーメント **g 1.89 / Q2 1.87 / Q1 1.91 / Q0 1.80 (ok), nSub 比 0.000** だが流れ・化学種が 1.18–1.29 (IC が convMethod 0 で作られた初期層) → IC を作り直した `run_0394`–`0398` で再判定。 |
...
| 25 | ~~(plan-12 M1–M6/m1) ゲートの完備性: 反復 0 始点・全文解析・入力欠落・全保存量の次数・INDETERMINATE・restart 開始時刻・実効設定比較~~ | 済 (2026-09-17): `passive_gate_common.py` / `check_passive_budget.py` / `analyze_moment_order.py` (§6.1 plan-12 行)。float 丸め床の許容 (§6-2) を case/09 seam 3 run の実測から確定 (符号付き 0.9–1.9e-8/step, |H_rem| 4.5–4.8e-8/step)。実 run の再判定: case/09 `run_0156`/`0157`/`0159` **PASS**; case/44 `run_0321`–`0324` (S3+FCT 3 水準) は次数 (Q0–Q2 1.37–1.65, roY 未達) と sub-iter 桁 (1.3–1.7) で **FAIL** のまま (#21 の CFD 検証が残件) |
| 26 | ~~S3 系列 (D) の sub-iter 未収束の切り分け~~ | 済 (2026-09-17, case/44 `run_0357`–`0377`, 倍精度 nSub 40 vs 80 の終了場 L2 差): **原因は `implicitRelax 0.7` 単独**。`passiveFct` 0/1・`speciesImplicitCoupling` 0/1・`speciesFaceReconstruction` 0/2・`limiter` 1/2 はいずれも無関係 (ρ 差 9.0e-6 = S1 と同じ)、緩和を入れると 5.65e-4 (60 倍)。緩和は不動点を変えず遅いモードの収束を遅らせるだけ (nSub 80 で縮む) だが、残差ノルムには現れない → **dual-time の時間次数・sub-iter 試験では `implicitRelax` を使わない** (生産の定常計算での推奨は変更しない) |
| 27 | ~~(result-3 M1–M5/m1) S3 を実際に効かせた再検証・収支が通る正式系列・旧経路の無影響・ゲートの偽合格 2 件・docs 同期~~ | **済 (2026-09-17)**。M3: 射影と物理 step 末尾の後処理 (EOS 再更新を含む) を `passiveScalarScheme 1` 限定にし、旧経路で `condRealizProject` 1/0 の差が反復ノイズと同等であることを確認 (`run_0382`/`0383`/`0393`)。M4/M5: 独立照合は固定 tol・`--double` は step 比例の許容なし; 場ゲートは solver と同じ条件 (g>0 かつ Q0>0) の全セルを判定 (許容はセルごとの float32 丸め)。反例 2 件を回帰試験に。m1: docs 修正。M1: `convMethod 1` × 湿り場で取り直し (`run_0399`–`0404`) — nSub 差 S1 1.40e-5 / S3+FCT 1.32e-5 / +relax 6.32e-4 → **原因は `implicitRelax` 単独**が 2 次でも成立。M2: 空間スキームと整合した IC (`run_0394`) から 0.64 ms・dt 4e-6/2e-6/1e-6 + nSub 80 で **収支・場ゲート全水準 PASS**、流れ・化学種 1.93–1.95、g 1.74 / Q2 1.92 / Q1 1.88、**roQ0 1.38 の原因は FCT の非線形リミッタ**と確定 (対照 `run_0425`–`0428` [FCT 無効] は全モーメント 1.99–2.01 で PASS; 射影・蒸発・丸めは個別に排除)。副産物: 実現可能性射影が極端な (x,y) で候補距離 overflow により違反を残す欠陥を修正 + 単体試験 (25465361) |
| 28 | (result-4 M1–M3/m1) 場ゲートの倍精度・半径 underflow の見逃し / FCT 次数緩和の適用範囲 / §6-8 検証行列 / docs 整合 | **M1 済 (2026-09-17)**: 半径と (x, y) を**指数分離**で作る `cond_moment_xy` を solver (射影・クランプの double/float 3 か所) とゲートに入れ、Q3 が underflow する塵は Q1=Q2=0 に修復する; ゲートは半径・比が作れないセルと非有限を **FAIL** にし、許容は**ファイルの精度**の丸めから作る (codex の反例 `rog=1e-60, x=2, y=0.1`・半径 underflow・実 run `run_0421` の 6 セルを回帰試験と実測で確認)。**M2 済**: FCT の下限緩和 (`--fct-order-lo` 1.3) は **BDF2 の受動種だけ**に適用 (流れ・化学種は [1.7, 2.3]、BDF1 は [0.7, 1.3])。**M3 済 (`run_0429`–`0452`)**: FCT 有効条件で **BDF2 × coupling 1 / BDF2 × coupling 0** はいずれも次数ゲート **PASS** (モーメント 1.740/1.924/1.875/1.381、流れ・化学種 1.93–1.95) + **収支ゲート 4 run とも PASS**; **BDF1 × coupling 1 / coupling 0** は水準を詰めた `run_0453`–`0456` (dt 1e-6/0.5e-6/0.25e-6) で **次数ゲート PASS** (モーメント 0.965/0.972/0.974/0.971、流れ 0.771–0.875; 粗い水準は BDF1 の誤差が大きく漸近域外だった) + 収支 PASS; **FCT 無効の対照** (`run_0445`–`0448`) はモーメント 1.987–2.013 で PASS = スキーム自体は 2 次。coupling 0/1 の差は 4 桁目以降。**m1 済**: `recommended-settings.md` の `implicitRelax` を定常/dual-time で分離、§5.1 #8 と case/44 README の旧系列の位置づけを訂正 |
| 29 | ~~(result-5 M1/M2/m1) 場ゲートの丸め許容の不等式別化 / 書き戻しの指数分離 / 旧記述の整理~~ | 済 (2026-09-17): ゲートは保存量の 3 不等式それぞれに専用の許容 (関係する量の丸めだけ) を使い、厳密成立は許容不要・判定不能は FAIL; solver は `cond_moment_writeback` で書き戻しも指数分離に統一し、修復不能分岐を収支に記録。反例 (float32 非正規化 `rog` で Q1²/(Q0Q2)=250) と往復試験を回帰に追加。検証行列は再実行で不変 (`run_0457`/`0458` の差 = 反復ノイズ) |
| 30 | ~~(result-6 M1/m1) |H_rem| を補正収支の合計に入れる / 記録の同期~~ | 済 (2026-09-17): 合計に `rem_eff = max(0, |H_rem| − float 床)` を加え (float32 の step 比例床は |H_rem| だけに適用し他項は緩めない)、反例 (floor 6e-7 + |H_rem| 6e-7) を回帰試験に。検証行列 24 本を再判定して全 PASS。旧主張 (`run_0222`/`0223` で「S3 固定点は cfl/relax 非依存」) を準定常量の一致に訂正、`run_0457`/`0458` を run 索引に追加、§10 の周期一様流発散を [condensation-followups](condensation-followups.md) の残作業表に F-cf12 として登録 |
| 8 | 検証 (§6 1–8, node のみ) と codex result レビュー | §6-1/2/7 (短 run) Phase A; §6-3 拡散 (解析解 0.23 %, 2 次収束; 等拡散一致 / 混合平均非一致 済), §6-4 固定点, §6-5 凝縮回帰, §6-7 24000 step は済 (§9)。**§6-2 (非定常保存・有界・restart) 済** (case/09 `run_0156`/`0157`/`0159`/`0164`)。**§6-6 (ii) はスキームの 2 次 (FCT 無効の対照) と FCT 作動時の判定まで済**、**§6-8 の検証行列 (FCT 有効 × BDF1/BDF2 × `passiveImplicitCoupling` 0/1) も 4 組とも次数・収支 PASS** (#28)。既定 `passiveScalarScheme 1` (2026-09-17)。codex result 4 回 NO-GO |
...
2. **保存・有界**: 一様流中のステップ状 ξ の移流 (node 箱 + 周期, 1 次/S3): ∫ρξ dV の保存 1e-6、floor 補正量の記録、0 ≤ roXi/ρ ≤ 1 (floor 後)、
   ...
   **追加 (plan-3 M4/m1, 2026-09-17)**: 非定常の保存は float32 で 1e-6 (double 集計の 1e-12 は `test_passive_fct.cu` の double 参照実装のみ)。dual-time の seam ステップ (`run_0091` プロトコル + `run_0098` のステップ ξ, SLAU S3, FCT 有効) で root-only 総量保存 ≤1e-6 (**float32 の状態更新の丸め床は step 数比例で許容** [codex plan-12 後に実測で確定, 2026-09-17]: 保存 (final−initial) と符号付き残り H_rem は max(1e-6, 2e-8·nstep)、|H_rem| の絶対値積算は max(1e-6, 6e-8·nstep); 閉合 [増分 = −境界 + ソース + 残り] と独立照合は 1e-6 のまま。実測: `run_0156` 160 step +1.4e-6 / |H_rem| 7.2e-6, `run_0157` no-FCT −1.2e-6, `run_0159` 100 step +1.9e-6 / 4.75e-6)、`floorCorr+limCorr+fctPinCorr` の総量比 ≤1e-6、$0\le\xi\le1$; 非周期入口 (case/16 のトレーサ分布 run を dual-time で短時間) は境界流束・ソース込みの収支; 周期の辺・角 (33³ 箱の角を跨ぐガウス) でも同じ収支。定常の θ_b は「limCorr の停止」と「未制限残差 (縮めた候補増分の総量) の消滅」を別々に確認する。
...
6. **dual-time**: (i) 多成分 dual-time で ΣY=1 と化学種の時間発展 — 「`roY` が動く」だけでは不十分で、**非一様組成の化学種を含めて** 3 水準の時間次数で判定する;
   ...
   (物理 step の `outer_end` だけで判定しない); **判定の実体 (2026-09-17 改定, `case/44/analyze_moment_order.py`)**: (a) 正式な次数判定は**滑らかな解** (核生成を `condSigmaScale 3` で抑止したガウス状液滴ブロブ `gen_smooth_moment_ic.py`, 成長/蒸発ソース作動) で行い、
   核生成 onset を含む実 run は L2 次数が前線の非平滑性で 1.3–1.6 に落ちる (e(dt)² の 70–85 % が上位 1 % のセル) ので**診断** (L1 次数・体積重み総量の次数・上位 1 % を除く L2 次数を併記) に留める;
   (b) float32 ではモーメントが ulp の数百倍のノイズ床を持つ (核生成率の T 感度による増幅) ので、**同一バイナリの nSub+1 run (`--noise`) を必須**にし、nSub 倍増差は max(0.1 e(dt), 2×ノイズ) で判定、e(dt) < 5×ノイズ の量は NOISE-LIMITED (PARTIAL, exit 3) として次数を判定しない;
   (a') **FCT 作動試験の次数下限は 1.3** (`--fct-order-lo`; 非線形リミッタは極値近傍で L2 の観測次数を下げる)、**スキーム自体の 2 次は同一設定・`passiveFct 0` の対照系列**を既定閾値 [1.7, 2.3] で通して示す
   (2026-09-17 実測: case/44 `run_0417`–`0420` [FCT 有効] は g 1.74 / Q2 1.92 / Q1 1.88 / Q0 1.38、対照 `run_0425`–`0428` [FCT 無効] は g 1.99 / Q2 1.99 / Q1 2.01 / Q0 2.01;
   射影・蒸発・丸め・緩和は個別に排除済み);
   正式な 2 次の実証は倍精度ビルド (`--double`; float の生産バイナリと同一ソース、typedef のみ。`check_passive_budget.py` / `check_passive_field.py` にも `--double` があり、実効刻みを float32 に丸めない) で行う; (c) 評価量は config の全保存量 (流れ・化学種・受動種)、`--fields` の部分評価は PARTIAL; (iii) coupling 0/1/2 の各分岐でピン行の残差が 0、入口値が保たれる; (iv) **restart**: 連続実行 vs 途中 checkpoint からの
...
| result | `2026-09-17` (1 回目) | [2026-09-16-species-passive-scalar-unification-result.md](../../notes/reviews/2026-09-16-species-passive-scalar-unification-result.md) | **NO-GO**, C0/M6/m2 | **全採用 (2026-09-17 反映中, §5.1 #9–#16)**: M1 (周期 DPLUR が合併 CV の近傍補正を解かず root の dq をコピーするだけ) → sweep の近傍寄与を独立バッファに作り周期群で合算してから解く + 非一様組成/トレーサが seam・辺・角を横切る陰解法試験 (#9); M2 (周期で補正収支・総量が全メンバーで積分され重複; `run_0067` 総量 166.2 vs root のみ 148.8) → 積分は周期 root のみ合併体積で; seam 局在補正の収支試験 (#10); M3 (checkpoint の履歴有効判定不足: dt 変更で警告のみ、layout に scheme 無し) → dt・生成方式の整合を復元条件に、不一致は全系 BDF1; 非定常の組成/トレーサ/モーメントで連続・通常 restart・刻み変更・方式変更・履歴欠落を試験 (#11); M4 (凝縮 dual-time の sub-iter 桁低下 <2 [Q0 1.58, roY0 1.92, roY1 1.77]、sub-iter 依存 g 3.7e-5 が刻み半減差 9.5e-5 に対し小さくない、モーメントの 3 水準無し) → sub-iter の収束を改善 (擬似 CFL・モーメントの DPLUR 更新) しモーメント込み 3 水準の次数試験; §10 送りにせず §5.1 必須へ (#12); M5 (累積 floor 補正が上限 1e-4 超: `run_0476` ξ 2.2e-4、`run_0494` 5.0e-4、Arthur S3 Q0 5.6e-3) → 上下限を事後クリップでなく増分スケーリング (θ_u 型) で守り floor は最後の保険にする; 再検証 (#13); M6 (緩和比較が実効でない: coupling 1 は `implicitRelax` を使う; 収束場 restart の判定ツール無し; case/44 の「固定点は同一」表現) → `implicitRelax` 0.7/1.0 の交差 restart、収束場 restart の判定 (基準 PASS の床と量の変動幅) をツール化、case/44 は準定常量の比較に表現限定 (#14); m1 (solver-settings/recommended-settings/thermophysics の旧仕様) → 同期 (#15); m2 (§5.1 の済/未達併記、§10 ψ 切替未決、case/09/16 README の改番前番号) → 同期 (#16) |
| result (6 回目) | `2026-09-17` | [2026-09-17-species-passive-scalar-unification-result-4.md](../../notes/reviews/2026-09-17-species-passive-scalar-unification-result-4.md) | **GO-with-changes**, C0/M1/m1 (検証行列の PASS を再現) | **全採用 (2026-09-17, §5.1 #30)**: M1 (`|H_rem|` が補正収支の合計から漏れ、floor 6e-7 + |H_rem| 6e-7 = 1.2e-6 が合格していた) → 合計に `rem_eff = max(0, |H_rem| − float 床)` を加え、float32 の step 比例床は |H_rem| だけに適用 (他項は緩めない)、反例を回帰試験に。検証行列 24 本を再判定して**全 PASS** (codex の独立再集計でも最大 2.951e-7 < 1e-6 で、今回の行列がこの穴で誤合格した証拠はない)。m1 → `run_0222`/`0223` の「S3 固定点は cfl/relax 非依存」を準定常量の一致に訂正 (両 run は `NOT CONVERGED`)、`run_0457`/`0458` を run 索引に追加、§10 の周期一様流発散を condensation-followups に **F-cf12** として登録 |
| result (5 回目) | `2026-09-17` | [2026-09-17-species-passive-scalar-unification-result-3.md](../../notes/reviews/2026-09-17-species-passive-scalar-unification-result-3.md) | **NO-GO**, C0/M2/m1 (検証行列の PASS は再現) | **全採用 (2026-09-17, §5.1 #29)**: M1 (共通の丸め許容が `rog` の非正規化数で 1 を超え、`rog` を含まない不等式 Q1²≤Q0·Q2 [250 倍の破れ] まで無効化していた) → **判定を保存量の不等式 (Q1³≤Q0²Q3 / Q1²≤Q0Q2 / Q2²≤Q1Q3) に分け、許容はその不等式に入る量の丸めだけから作る**; 厳密に成り立つ不等式には許容を使わず、破れが許容を超え、かつその許容が 1e-2 超のときだけ「判定不能」(FAIL) にする; M2 (指数分離が書き戻しまで届かず、射影直後に x が 14 % も 1 を超える例) → 書き戻しを共通の `cond_moment_writeback` (q3/q0 が非正規化数なら対数経路) に統一し、修復不能分岐も収支記録に通す。往復 (射影→保存量→再判定) の単体試験を追加; m1 (旧系列の記述・残作業表・`solver-settings.md` の `implicitRelax`) → 修正。修正後の binary で検証行列を 1 本再実行し、旧結果との差が同一設定の反復ノイズと同等 (ρ 3.4e-7 vs 3.7e-7) であることを確認 (`run_0457`/`0458`) |
| result (4 回目) | `2026-09-17` | [2026-09-17-species-passive-scalar-unification-result-2.md](../../notes/reviews/2026-09-17-species-passive-scalar-unification-result-2.md) | **NO-GO**, C0/M3/m1 | **全採用 (2026-09-17, §5.1 #28)**: M1 (場ゲートが倍精度・半径 underflow の実現不能状態を合格させる; solver も修復を飛ばす) → 指数分離で x,y を作り、半径が存在し得ない塵は Q1=Q2=0 に修復、ゲートは判定不能を FAIL・許容はファイルの精度から (反例と実セルを試験に); M2 (FCT の次数緩和が流れ・化学種と BDF1 まで緩めていた) → BDF2 の受動種だけに限定; M3 (§6-8 の BDF1/BDF2 × coupling 0/1 が未取得) → `run_0429`–`0452` で取得 (BDF2 は次数・収支とも PASS、BDF1 はモーメント 1 次、対照は 2 次); m1 (推奨設定・残作業表・run 索引の不整合) → 修正 |
| result (3 回目) | `2026-09-17` | [2026-09-17-species-passive-scalar-unification-result.md](../../notes/reviews/2026-09-17-species-passive-scalar-unification-result.md) | **NO-GO**, C0/M5/m1 | **全採用 (2026-09-17; §5.1 #27)**: M1 (convMethod 0 では `interp_dispatch` が 1 次を返すので S3 が不活性 → 「S3 は無罪・implicitRelax 単独」の一般化が未成立) → `convMethod 1` で取り直し (case/44 `run_0384`–`0404`); M2 (正式系列が補正収支ゲートを満たしていない [`run_0378` の Q1/Q2 クランプ 6.3e-5/8.8e-5]、上位 1 % の事後除外は全保存量ゲートの代替にならない) → 空間スキームと整合した IC + 収支が通る刻み水準で取り直し、§5.1 の「達成」を撤回して再判定; M3 (`passiveScalarScheme 0` の無影響契約が破れている: 新射影が旧経路の Q1/Q2 を動かす) → **射影と物理 step 末尾の後処理ブロック (EOS 再更新を含む) を scheme 1 限定に修正** (`condensationTransport_d.cu`, `main.cpp`)、`condRealizProject` 1/0 の差が同一設定の反復ノイズと同じであることを確認 (`run_0382`/`0383`/`0393`); M4 (独立照合まで step 比例の float 許容で緩めていた) → 独立照合は固定 `tol`、step 比例の許容は float32 の状態更新に由来する項だけ、`--double` では許容なし、反例を回帰試験に追加; M5 (場ゲートが `rog≤1e-30` を検査対象外にして桁違いの不整合を見逃す) → **solver と同じ条件 (g>0 かつ Q0>0) の全セルを判定**し、許容はセルごとの float32 丸め (非正規化数は緩く) から作る、反例を回帰試験に追加; m1 (`methods/condensation.md` に撤回済みの説明と実装と違う退化閾値) → 修正。指摘外: 「流れはビット一致」は誤りで roUy/roUz に 7.3e-11/1.6e-10 の差 (case/09 README を訂正) |
| plan (12 回目) | `2026-09-17` | [2026-09-16-species-passive-scalar-unification-plan-12.md](../../notes/reviews/2026-09-16-species-passive-scalar-unification-plan-12.md) | **NO-GO**, C0/M6/m1 (ゲートの完備性) | **全採用 (2026-09-17 反映, 実装済; §5.1 #25)**: M1 (sub-iter の初回残差の取り違え) → 反復 0 (`inner_begin`) を始点に nSub−1 まで評価、outer_end = 最終 inner_iter の照合、重複・範囲外番号・未知 phase を拒否、ゼロ始点は反復 0 を含めて全反復ゼロのときだけ受理; M2 (解析不能・過去の異常の保持が未完成) → 各 [passive] 書式の数値欄を数値だけ受理する全文解析 (nan/inf/BROKEN は書式不一致 = 解析失敗)、過去の `nonfinite 1` と解析失敗は最新値の上書きと独立に保持、`initial total` 行の欠落と budget 行 initialTotal との不一致も FAIL; M3 (メッシュ・IC・BC 欠落が同一扱い、外部入力を見ない) → `required_inputs`: bcondConfig・メッシュ・IC・`inletProfile: 1` の入口が読む `inlet_profile_<physID>.csv`・`physProp.speciesDBFile` の存在を run ごとに要求し欠落を FAIL、内容 md5 を照合; M4 (化学種の時間 1 次誤差を既定ゲートが検出しない) → 評価量の既定 = config の全保存量 (ro,roUx,roUy,roUz,roe,roY*,受動種); `--fields` が必須集合を覆わなければ **PARTIAL (exit 3)**; dt 非依存の非ゼロ量は「次数未実証」で FAIL (全水準で恒等 0 の量だけ情報なしで通す); M5 (保存が判定不能でも PASS) → `unsteady` モードは **INDETERMINATE (exit 3)**、§6-2 の合格は fct / conservative モードの PASS だけ; M6 (restart の開始時刻を無視) → log の `[dual-time] history restored` / `passive FCT flux-form history missing` (時刻は引き継ぎ BDF1 再開) / `history NOT restored` を区別し、IC の /CHECKPOINT/totalTime を開始時刻として t0 + nStepOuter×dt_eff を照合 (restart の正常系試験を追加); m1 (比較に未正規化 YAML) → solver 既定 (bdfOrder 2, nSub 20, monitorInterval 1, scheme 1, passiveFct 1, SFR 0, tracer none, condensation 0) を埋めた実効設定で比較 (省略と明示既定は同一; 試験追加)。併せて float32 丸め床の許容を実測で確定 (§6-2): 保存・符号付き残り max(1e-6, 2e-8·nstep)、|H_rem| max(1e-6, 6e-8·nstep)。`test_check_passive_budget.py` / `test_passive_gates.py` ALL PASS; case/09 `run_0156`/`0157`/`0159` PASS |
...
- [x] 関連 methods を更新済み (`methods/condensation.md` §4c [成分別非負化・射影の scheme 1 限定・退化閾値・撤回した説明], `thermophysics.md`, `time_integration/theory.md`, `convection/theory.md`)
- [x] 実装・§6 の検証 **1–7** (原因切り分け / 保存・有界 / 拡散 / S3 安定性と固定点不変 / 凝縮回帰 / dual-time / 無影響) を満たす
- [x] codex レビュー (plan 12 回 + result 6 回) を §6.1 に記録し、Critical / Major を全採用して §5.1 に反映済み
- [x] `status: done`、§9 に変更ログ
- [x] `plans/active/` → `plans/accepted/`、`plans/README.md` 同期、followups F-cf8 を完了・F-sp1 (トレーサ拡散) を §6-3 で解消 (§10 の node 周期一様流発散は F-cf12 として引き継ぎ)

## 9. 変更ログ

- `2026-09-17` — codex result レビュー 6 回目 **GO-with-changes (M1/m1)** を全採用して**完了**: `|H_rem|` を補正収支の合計に入れ (float32 の step 比例床は |H_rem| だけに適用)、検証行列 24 本を再判定して全 PASS。旧主張の訂正・run 索引・F-cf12 の登録を済ませ、`status: done` で `plans/accepted/` へ移動。
- `2026-09-17` — codex result レビュー 5 回目 **NO-GO (M2/m1)** を全採用 (§6.1, §5.1 #29): 場ゲートの丸め許容を**不等式ごと**に分け (共通 eps が `rog` の非正規化数で 1 を超え、Q1²≤Q0·Q2 の 250 倍の破れを見逃していた)、射影の**書き戻しも指数分離**に統一 (直後に x が 14 % 1 を超える例を解消) して修復不能分岐も収支に記録。検証行列 (BDF2/BDF1 × coupling 0/1 + FCT 無効対照) はこの修正でも不変 (再実行の差 = 反復ノイズ)。
- `2026-09-17` — codex result レビュー 4 回目 **NO-GO (M3/m1)** を全採用 (§6.1, §5.1 #28): (1) 半径 (Q3/Q0)^{1/3} が underflow するセルで solver もゲートも判定を諦めていた欠陥を、**指数分離 (`cond_moment_xy`)** と「半径が存在し得ない塵は Q1=Q2=0 に修復」で解消し、ゲートは判定不能セルを FAIL・許容をファイルの精度から作るように; (2) FCT の次数緩和を BDF2 の受動種だけに限定; (3) §6-8 の検証行列を FCT 有効条件で取得 (`run_0429`–`0452`): BDF2 × coupling 0/1 は次数・収支とも PASS、BDF1 × coupling 0/1 は細水準 (`run_0453`–`0456`) でモーメント 0.96–0.97 の 1 次・全列 PASS、FCT 無効の対照は 1.99–2.01; (4) docs (定常/dual-time の `implicitRelax`、旧系列の位置づけ) を整合。
- `2026-09-17` — codex result レビュー 3 回目 **NO-GO (M5/m1)** を全採用 (§6.1, §5.1 #27) し再検証: (1) `convMethod 0` では S3 が不活性という指摘どおり全系列を `convMethod 1` で取り直し、**sub-iter 依存の原因は `implicitRelax` 単独**という結論は 2 次でも成立 (`run_0399`–`0404`); (2) 空間スキームと整合した IC + 0.64 ms + dt 4e-6/2e-6/1e-6 の正式系列で **収支・場ゲート全水準 PASS**、流れ・化学種 1.93–1.95、モーメント g 1.74 / Q2 1.92 / Q1 1.88 / Q0 1.38 — **Q0 の低下は FCT の非線形リミッタ**で、`passiveFct 0` の対照 (`run_0425`–`0428`) は全モーメント 1.99–2.01 (射影・蒸発・丸め・緩和は個別に排除) → §6-6 の判定規約を「FCT 作動試験は下限 1.3、スキームの 2 次は FCT 無効の対照で示す」に確定; (3) 旧経路 (`passiveScalarScheme 0`) の無影響契約を回復 (射影と物理 step 末尾の後処理を scheme 1 限定に); (4) ゲートの偽合格 2 件 (独立照合の許容、場ゲートの絶対閾値) を修正し反例を回帰試験に; (5) 実現可能性射影の overflow 欠陥 (極端な x,y で違反が残る) を発見・修正。
- `2026-09-17` — **§6-6 (ii) 達成**: 滑らかな液滴ブロブ (核生成抑止) + 倍精度で **モーメントの BDF2 を実証** — 成長ソース `run_0368`–`0371` (g 1.92 / Q2 1.94 / Q1 1.95 / Q0 1.97) と **FCT 作動版 `run_0378`–`0381`** (g 1.92 / Q2 1.92 / Q1 1.94 / Q0 1.97)、いずれも **nSub 倍増差 0.000**・sub-iter 低下 ≥3.20 桁。併せて S3 系列 (D) の nSub 依存の真因を **`implicitRelax 0.7` 単独**と特定 (`run_0357`–`0377`; FCT・coupling・SFR・limiter はいずれも無関係、緩和なしなら S1 と同じ 9e-6)。残る流れ 2 列 (roUx/roe 1.62–1.65) はスロート軸近傍の上位 1 % に誤差が集中し、除くと 1.89–1.92 (軸対称スロートの非平滑性, 本 plan 外)。
- `2026-09-17` — **§6-6 モーメント時間次数の真因を確定** (case/44 `run_0330`–`0346`; README): (1) 1 次流れの sub-iter 床 (roQ0 残差 2.5e8) は float32 の ulp ノルム (2.3e8) で、nSub 41 と nSub 80 の終了場の差が同じ大きさ (e(dt) の 0.44 / 0.38) = ノイズ床; 蒸発 off・nSub 160・cfl_pseudo 6 で不変; 倍精度ビルド (worktree `forge-double`, typedef のみ + `_f` 熱力学の double 橋渡し 4 関数) では nSub 比 0.004–0.013。(2) 倍精度でも実 run のモーメント L2 次数は 1.3–1.6 のまま = 核生成 onset 前線 (上位 1 % のセルに e(dt)² の 70–85 %); 前線外 1.93–2.07、総量 2.2–3.2。(3) 核生成を抑止した滑らかなブロブ (蒸発ソース作動) では g 1.96–2.01 / Q2 1.91–1.97 / Q1 1.83–1.92 / Q0 1.74–1.82, nSub 比 ≤0.006 → BDF2 成立。(4) S3+FCT 系列 D (float) は流れ側 (2 次 MUSCL + リミッタ) の sub-iter 未収束がノイズの 2–3 倍で実在、モーメントはノイズ床内。ゲート改定: `analyze_moment_order.py` に `--noise` (nSub+1 のノイズ床; NOISE-LIMITED は PARTIAL)、`--double`、診断 (L1・総量・上位 1 % 除外の次数) を追加 (§6-6)。
- `2026-09-17` — codex plan レビュー 12 回目 **NO-GO (M6/m1)** を全採用 (§6.1, §5.1 #25): sub-iter 低下は反復 0 始点、収支ログは書式ごとの全文解析 + 過去の異常の保持、入力欠落・入口プロファイル・物性 DB の照合、次数ゲートの既定評価量 = 全保存量 (PARTIAL/INDETERMINATE を exit 3 で分離)、restart 開始時刻、実効設定比較。float32 丸め床の許容を実測で確定 (§6-2)。restart 復元の予告 `passiveFctHistoryFromHost` を config 述語 (`passiveFctConfigured`) に修正 (検証 2 巡目: runtime 述語は Pface 割当前に偽で FCT 履歴を復元できなかった)。
...
- **SLAU + `convMethod 1/2` (または `limiter 0`) + node 周期 + dual-time BDF2 の一様流が丸め由来の不安定で t≈0.22 s に NaN** (case/09 検証 1 巡目, 流れだけで再現; `convMethod 0` と Barth `limiter 1` は安定; RK4 の `run_0098` は 0.085 s しか回していなかった)。流れ側の別件として followups へ (→ [condensation-followups](condensation-followups.md) F-cf12 に登録済み, 2026-09-17)。受動種の周期試験は Barth で行う。
...
- ~~発達中の凝縮 dual-time で sub-iter 残差が 2 桁落ちない列 (Q0, 流れ, 化学種; case/44 `run_0229`/`0230`) は擬似時間反復の床 (cfl_pseudo 2 + θ_u クランプ) で、nSub では解消しない。~~ 決着 (2026-09-17, §5.1 #12): 1 次流れでは float32 の丸め床 (モーメントは核生成率の T 感度で ulp の数百倍に増幅) であり倍精度で消える; 2 次 MUSCL + リミッタの流れ側の床 (S3+FCT 系列 D) は別件で実在。cfl_pseudo 上げ・`passiveImplicitCoupling 1`・更新クランプの dual-time 挙動の切り分けは後続 (時間精度の判定は nSub ≥20 で行う)。
