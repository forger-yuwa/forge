...
- `2026-09-13` — **main (64bb983d, 空気凝縮 + Feder carrier 形 Kantrowitz) を取り込み** (3b77cc4b)。衝突は `convectiveFlux_slau_d.inc.cuh` (perf の float 面エンタルピー + 凝縮側 `cnd.cprops`; CPG 二相枝 `cond_face_h_cpg` は double のまま) と `dependentVariables_d.cu` (CPG 凝縮枝は凝縮側の double 形) の 2 ファイル 4 箇所。検証: 凝縮単体試験 `test_cond_air` / `test_cond_kantrowitz_carrier` ALL PASS、perf 回帰 (case/44 run_0200 `--truth` / run_0201、case/16 run_0450 / run_0456) `merged` ラベル全 PASS (run_0456 はマージ前 final とノイズ無しでも PASS)、凝縮回帰 (case/34 `run_0100_merge_regress_cell_air` / `run_0101_merge_regress_node_air` / `run_0102_merge_regress_cell_dry`: 凝縮ブランチ最終 run と同 config 12000 step、床は同一バイナリ反復 2〜3 本) 全 PASS、A10G 速度 33.93/33.94 → 33.82/33.82 ms/step (run_0400 100 step ×2 交互, 不変; マージ後バイナリ sha256 a2b931c4…)。
