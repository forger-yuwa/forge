...
- **status**: `done`  <!-- codex plan GO-with-changes → 実装 → codex result 1〜5 回目 (NO-GO) 全件採用 → 6 回目 GO-with-changes (m2) 採用 → accepted (2026-09-14) -->
- **related_docs**:
...
- **related_plans**:
  - [`performance-3d-node-sst-speedup.md`](performance-3d-node-sst-speedup.md) (dry 経路の float 化。判定基準 §4.3・回帰ツール `perf_regress.py` を継承)
...
5. **温度反転** (`dependentVariables_d.cu`, `condensationEOS_d.cuh`; codex M3)
   ...
   - $g>0$ の一温度二相反転 (`cond_T_from_e_carrier` / `_onetemp`): float Newton (`SpeciesThermoF` の cp/h + 表の $L, dL/dT$、最大 12 回) →
     ...
     現行の TP carrier 反転にも同じ成功判定と保護を入れる (double 経路の挙動変更: 旧 Newton の後に同じ残差条件まで研磨するので T は ≤1e-3 K 変わり得る [v2 初稿の「成功時はビット不変」は誤り; ビット一致の保証はソース/clamp の凍結評価に限る]、失敗時は roe 保護)。
...
6. **`condFloat` の分岐表** (codex M5):
   ...
   | 温度反転 (g>0 セル) | float Newton + double 研磨 + 成功判定 | 現行 double Newton → 同じ残差条件まで研磨 + 成功判定 (T は ≤1e-3 K 変わり得る; 失敗時は roe 保護) |
   | 平衡形 (`condEquilibrium`≠0) / 二温度のソース kernel、`condEquilibrium: 2` の EOS 反転、CPG 二相反転 `cond_T_from_e_cpg`、二相音速 | double (共通; `condEquilibrium: 1` の湿潤 TP セルの反転はハイブリッド、面潜熱・clamp・g≈0 セルの組成/反転は float 経路) | double |
...
| 13 | 発達場の速度・湿潤統計・A10G (codex result M6) | **A10G 実測済 (2026-09-14 未明, 34.201.22.141, 最終バイナリ `forge_condf4` = commit「table-range test includes the T+0.1 K …」, sha256 は `BINARY_SHA256.txt`; 基準 `forge_merged` a2b931c4…; 100 step ×3 交互, 全反復 ±0.03 ms)**: dry `run_0400` 33.77–33.79 → 33.92–33.94 (+0.15 ms; 凝縮 OFF 経路は不変), **起動区間 `run_0420` 89.40–89.41 → 44.01–44.04 (目標 ≤45 達成)**, **発達場 `run_0421` (湿潤 22.5 %) 98.00–98.01 → 52.30–52.34 ms/step (目標 ≤50 に 2.3 ms = 4.6 % 届かず → followups F-cf3)**。発達場 nsys (condf2 版の取得, 30 step, 51.8 ms; 内訳確認用): block-DPLUR 10.0 / `condensation_source_f_d` 9.6 / `dependentVariables_d` 8.0 (dry 3.2) / SLAU 3.0 / `cond_primitive_multi_d` 1.3。ブロックサイズ 512/256/128 は 52.3/52.1/52.4 で効かない。反転失敗 0。3D 300 step (`run_0421`, 発達場 IC, condf4): `cmp --noise base_r2` **PASS 30/30** (P 5.84e-3 / T 1.91e-2 / ρg 1.14e-1 = 基準同士 5.65e-3 / 1.82e-2 / 1.15e-1 の 1.03 / 1.05 / 0.99 倍)、300 step 時点の湿潤統計 (湿潤 533,27x 節点, onset x 0.010649 m, g p50 2.777e-3 / p99 1.095e-2, T_wet p50 233.69 K / p01 205.48 K) は基準 2 本と同値 (定常性の主張ではない)。証拠 `case/16.nozzle_wys/_aws_perf_evidence/cond/`: 交互計測ログ 18 本 (`*__m4_*`/`*__f4_*`), `run_0421_CMP_VERDICT_condf4.txt`, `run_0421_wet_stats_condf4.txt`, `BINARY_SHA256.txt` (両バイナリ・メッシュ・IC・config), nsys CSV (condf2 版と明記)。ローカル RTX 3060 (旧 SHA): 起動 195→87, 発達場 218→111 |
...
| 15 | 証拠の統一 (codex result-2 M4/m6, result-4 M1/m3, result-6 m1) | **済**: 最終バイナリは commit ef3c6fae (`forge_condf4`, sha256 3604fd4e…; 範囲判定に摂動点 T+0.1 K を含む版)。**AWS で正式取得**: 速度 (dry/起動/発達場 ×3) と 3D 300 step 回帰 + 統計 (#13)。**ローカル RTX 3060 で正式採用** (2D/小規模で GPU 種は判定に影響しない; 各ラベルに VERDICT ファイル): run_0456 (node 2D H2O; ef3c6fae で再 PASS) / run_0457 (cell 2D H2O) / run_0460 (12000 step 物理検証) / run_0201 (平衡) / case/34 run_0100–0104 / 凍結 run_0458 (ef3c6fae でビット一致再確認) / run_0461 (roe 保護) / run_0053 (周期)。ローカル結果の多くは中間 SHA で取得したもの: 以後の差分は (a) 二相音速 dL/dT の double 差分復帰 (旧挙動へ) と (b) 表上端 ±0.1 K (N2 125.5–125.6 K, H2O 646.9–647 K) のセルの double 委譲で、(b) の温度帯は 2D/case34 の場に存在せず (N2 ≤ 110 K, H2O ≤ 400 K)、境界 device 試験で補完 |
...
- **短期回帰 (`perf_regress.py`, 基準 = 変更前バイナリ 3b77cc4b, ノイズ床は基準の反復; 2D/小規模はローカル RTX 3060 で正式取得、3D は AWS A10G)**:
...
| --- | --- | --- | --- | --- |
| result (6 回目) | `2026-09-13` | [`notes/reviews/2026-09-13-condensation-float-speedup-result-6.md`](../../notes/reviews/2026-09-13-condensation-float-speedup-result-6.md) | **GO-with-changes**, C0/M0/m2 | **全件採用**: m1 (#15・§8・#5 を最終バイナリ ef3c6fae/`forge_condf4` とローカル正式採用の範囲 [温度帯の補完] に同期) / m2 (methods §9 と分岐表の平衡形・`condFloat: 0` の研磨・退避条件「T+0.1 K を含む」を solver-settings と統一)。codex の推奨どおり訂正後に `accepted/` へ移動 |
| result (5 回目) | `2026-09-13` | [`notes/reviews/2026-09-13-condensation-float-speedup-result-5.md`](../../notes/reviews/2026-09-13-condensation-float-speedup-result-5.md) | **NO-GO**, C0/M1/m2 | **全件採用**: M1 (本体が表範囲内でも src_jac の摂動点 T+0.1 K が表外に出ると端クランプで Jacobian が 97 % 過小 [N2 125.6 K] → 範囲判定に摂動点を含め、跨ぐセルは double 委譲; 125.45–125.7 K の境界状態を device 試験に追加: #8) / m2 (#13 の 300 step 差を回収済み証拠の値に同期、README の最終版名、nsys CSV の取得版明記、分布比較は「当該時点の統計比較」と表現) / m3 (§4.2-5 の「成功時はビット不変」を訂正、methods §9 本文と分岐表の平衡形の扱いを solver-settings と統一) |
| result (4 回目) | `2026-09-13` | [`notes/reviews/2026-09-13-condensation-float-speedup-result-4.md`](../../notes/reviews/2026-09-13-condensation-float-speedup-result-4.md) | **NO-GO**, C0/M2/m1 | **全件採用**: M1 (証拠が旧バイナリのログだった → 最終バイナリ `forge_condf3` で交互計測をやり直し、300 step の CMP_VERDICT・分布統計・全 sha256 を `_aws_perf_evidence/cond/` に回収: #13/#15) / M2 (二相音速の dL/dT の表微分は 373.15 K の液相クランプ点で従来差分と 4 % 違い γ_2φ を変える → double 差分に戻す [1551099d]; 表微分は Newton の傾きのみ) / m3 (#13–#15・§6・§8 をコミット・SHA・実施環境付きで整理、ローカル正式採用とAWS取得を区別) |
...
- [x] `methods/condensation.md` 実装 §9 と `performance.md` を更新済み
- [x] §5.1 #1–#5 完了、§4.3 の判定 (場・物理量・単体・ビット不変) を満たす。**速度 (最終バイナリ ef3c6fae = forge_condf4, A10G): dry 33.9 (+0.15 ms)、起動区間 44.0 ms/step (≤45 達成)、発達場 52.3 ms/step (≤50 に 4.6 % 未達; 98.0 からの −47 %) — 未達分は followups F-cf3 に移管**。周期保存ゲート (≤1e-6) と cell の h0 絶対ゲートは既存挙動で未達 (followups F-cf1/F-cf2)、発達場の残り 2.3 ms は F-cf3
- [x] codex レビュー (`plan` 1 回 / `result` 6 回) を §6.1 に記録し Critical / Major の採否を §5.1 に反映
...
- `2026-09-13` — AWS 復旧 (34.201.22.141) で A10G 実測: dry 33.8 (不変), 起動 89.4 → 44.0 (達成), 発達場 98.0 → 52.3 ms/step (目標 50 に 4.6 % 届かず)。二相研磨の傾きを表微分に・Feder 項を mode≥2 に限定して 55.1 → 52.3。3D 300 step の場は基準ノイズ内、分布統計一致。証拠を `_aws_perf_evidence/cond/` に回収。codex result 4 回目へ。
- `2026-09-13` — codex result 4 回目 NO-GO (M2/m1) 全件採用: 二相音速の dL/dT を double 差分に戻し (1551099d)、最終バイナリで A10G を再計測 (dry 33.9 / 起動 44.0 / 発達場 52.3 ms/step)、証拠を最終 SHA で回収。5 回目へ。
- `2026-09-13` — codex result 5 回目 NO-GO (M1/m2) 全件採用: 範囲判定に摂動点を含める修正 (境界試験追加)、証拠・文書の同期。最終バイナリで A10G を再計測 (6 回目レビューへ)。
- `2026-09-14` — 最終バイナリ (摂動点を含む範囲判定, forge_condf4) で A10G 再計測: dry 33.9 / 起動 44.0 / 発達場 52.3 ms/step、300 step 回帰 PASS (ノイズ ×1.0)、証拠を最終 SHA で回収。AWS の disk full (bench の res_0 蓄積) を掃除。
- `2026-09-14` — codex result 6 回目 GO-with-changes (m2) 採用、記録を最終バイナリ ef3c6fae に同期。**status done、`plans/accepted/` へ移動**。最終: A10G 凝縮 ON 起動区間 89.4 → 44.0、発達場 98.0 → 52.3 ms/step (dry 33.8 → 33.9)、RTX 3060 起動 195 → 87、発達場 218 → 111。残件 (発達場 −2.3 ms, 周期モーメント保存, cell h0 絶対ゲート) は condensation-followups F-cf1〜F-cf3。
