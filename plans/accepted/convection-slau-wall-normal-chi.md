...
| **11** | **既定化の判断** (**F**。**判断: 2026-09-25 `diagnostician` — 既定化しない (既定 0 のまま、適用規則で使う)**。後継 [`convection-slau-wall-normal-chi-usage-rule.md`](../active/convection-slau-wall-normal-chi-usage-rule.md) で規則と 2D の正否判定を扱う。以下は当時の記述: **本 plan のスコープ外で未着手**。着手するときは後継 plan を起票し、#9b・V7 の固定点・$C_L$/$C_M$ の準定常と正否をそこで扱う) | **前提**: #7・#9a・#10・#10b–#10d の完了 + **#9b (格子収束)・V7 の固定点 (ドリフトの減衰・漸近先の確認)・$C_L$/$C_M$ の準定常確認 (両 run STEADY) → その後に独立精度評価** (2026-09-25 #14–#16 で受入から移した) + $C_L$ 0.433 % / $C_M$ 0.316 % / 衝撃足の壁圧 0.842 % の**正否を独立に判定** (格子収束・別スキーム・実験/文献のいずれか)。これが無いうちは **opt-in のまま**。既定化は本 plan のスコープ外 (§2) | F |
...
**訂正 (2026-09-25, 後継 plan [`convection-slau-wall-normal-chi-usage-rule.md`](../active/convection-slau-wall-normal-chi-usage-rule.md) の codex plan M7)**: 不確かさを「両 run の振幅の大きい方」としたのは過小 (位相が独立なら両側が寄与する)。**両 run の振幅の和** $U=\lvert\Delta\rvert+a_0+a_1$ で再計算すると $C_T$ 0.0309+0.0073+0.0076 = **0.046 %** (≤ 0.1 %)、衝撃足 0.8385+0.0091+0.0091 = **0.857 %** (≤ 1 %)。**判定は変わらない**。
