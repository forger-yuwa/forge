...
- **area**: `convection`
- **status**: `done` (2026-09-25。既定 0 の opt-in として受入。既定化は §5.1 #11 で未着手・スコープ外)
- **related_docs**:
...
- **related_plans**:
  - [`tooling-sern-mesh-blocking.md`](../active/tooling-sern-mesh-blocking.md) §4.13.1 (本件を発見した診断。run_0430–0434)
- **created**: `2026-09-23`
- **owner**: `CFD Dev`
...
([`tooling-sern-mesh-blocking.md`](../active/tooling-sern-mesh-blocking.md) §4.13.1)。
...
- **やらない**: **圧力束の $\chi$ の変更** (§4.2 で却下)。既定の変更 (§6 V3 が全部通ってから別途判断)。cell 方式の実装
  (明示的に無効化する)。EOS 床の意味の修正 ([`tooling-sern-mesh-blocking.md`](../active/tooling-sern-mesh-blocking.md) §5.1 B1e。
...
| **E: EOS 床の意味を直す** | 別 plan | [`tooling-sern-mesh-blocking.md`](../active/tooling-sern-mesh-blocking.md) §5.1 B1e。独立の欠陥なので**本計画と同じ commit にしない** |
...
| **9b** (**未**、#11 の前提へ移した、2026-09-25 #16。**本 plan の完了後も未解決として残す**: 既定化を検討する後継 plan で扱う) | **格子感度 — 定常解・壁圧・収束率の格子比較** | 受入条件から外した (codex result-3 M4)。flag 0 は細格子で解を持たないので、flag 1 の解の格子収束は生産の格子収束列 (R5n) と同じ作業になる (R5n `run_0419/0420` は flag 0 なので転用不可)。**測るときの合格 (事前に固定)**: 同一生成コマンドを記録・同一 blocksize で 3 水準、壁圧 L2 差と ΣCFL あたり低下桁数を表にする | F |
...
| **11** | **既定化の判断** (**F**。**判断: 2026-09-25 `diagnostician` — 既定化しない (既定 0 のまま、適用規則で使う)**。後継 [`convection-slau-wall-normal-chi-usage-rule.md`](../accepted/convection-slau-wall-normal-chi-usage-rule.md) で規則と 2D の正否判定を扱う。以下は当時の記述: **本 plan のスコープ外で未着手**。着手するときは後継 plan を起票し、#9b・V7 の固定点・$C_L$/$C_M$ の準定常と正否をそこで扱う) | **前提**: #7・#9a・#10・#10b–#10d の完了 + **#9b (格子収束)・V7 の固定点 (ドリフトの減衰・漸近先の確認)・$C_L$/$C_M$ の準定常確認 (両 run STEADY) → その後に独立精度評価** (2026-09-25 #14–#16 で受入から移した) + $C_L$ 0.433 % / $C_M$ 0.316 % / 衝撃足の壁圧 0.842 % の**正否を独立に判定** (格子収束・別スキーム・実験/文献のいずれか)。これが無いうちは **opt-in のまま**。既定化は本 plan のスコープ外 (§2) | F |
| ~~8~~ (**済 2026-09-25**: 3 回目 NO-GO → #12–#18) | codex result レビュー (**3 回目の発火条件**) | **受入セットが揃ったら**回す: case/16 の flag 0/1 完了 + #10b の時系列 VERDICT + #10c の固定点判定 (延長 run)。**揃う前に出すと同じ M1 で NO-GO になりレビューを 1 周捨てる** (2 回目の教訓) | F |
...
| ~~8b~~ (**済 2026-09-25**: 4 回目 GO-with-changes C0/M0/m3、全件採用) | **codex result 4 回目** | #12–#18 の後。対応表を「閉 / 限定 / 未」の 3 値に直し、**出す前に受入結論の各文が「閉」行だけを根拠にしているか 1 行ずつ照合する** (3 回とも「測れた範囲を超えて書いた」型で落ちた) | F |
...
| SERN 2D 生産 | ~~**カウル衝撃がランプに当たる衝撃足の壁圧分布**~~ → **x≈0 膨張角窓の壁圧** (**訂正 2026-09-25, 後継 plan codex plan-2 M7**: 定義の $\max|dp_w/dx|$ はランプ全域から圧縮/膨張を区別せずに選び、実際に選ばれた $x_{foot}=-0.000878$ m は **x≈0 のランプ膨張角** (窓内壁圧 99.6→56.6→53.6 kPa と低下)。**測ったのは「膨張角窓の壁圧」でカウル衝撃足ではない**。定義どおりの計算結果は有効だが、§4.2 の残リスク (衝撃衝突点) は**未測定** → 後継 plan で再同定) (§4.2 の残リスク) | **ノルムの定義 (2026-09-23 に固定。flag 0/1 の壁圧を並べる前に決めた)**: flag 0 の run で $\max\lvert \mathrm{d}p_w/\mathrm{d}x \rvert$ を与える位置を $x_{foot}$ とし、**ランプ上 $x \in [x_{foot}-2t,\ x_{foot}+5t]$** ($t$ = カウル板厚 2 mm) の $p_w/p_{ref}$ の**相対 L2 差 ≤ 1 %** かつ **$x_{foot}$ の差 ≤ 格子間隔 1 つ分** | 同上 |
| 全ケース | **収束の悪化** (§3.1 の `mSLAU` 報告) | flag 0 と比べ $\Sigma$ CFL あたりの低下桁数が悪化しないこと。**2026-09-25 (測る前に固定, `diagnostician`)**: SERN 2D は両 run が段階起動を各自回しているので登録量で判定する — `_v3sern/v3s_flag{0,1}` に `check_convergence.py --segment` (最終区間)、**合格 = flag 1 の各列の低下桁数 ≥ flag 0 − 0.2 桁**、ΣCFL を併記。case/16・case/48 は残差がプラトーの場からの分岐で低下桁数が定義できないので**代替量** (case/16 は事前固定 (c) の水準比 ≤ 2、case/48 は事後の水準比)。**結果 (2026-09-25)**: SERN 2D 本段 (12000 step × `cfl_pseudo` 0.5 = ΣCFL 6000、両 run 同一) で全 8 列 OK (flag0/flag1 の低下桁数 ro 0.4/0.4、roUx 0.4/0.4、roUy 0.3/0.4、roe 0.2/0.2、roK 0.4/0.4、roOmega 0.5/0.4、roY0 0.5/0.4、roY1 0.6/0.5) → **PASS**。証拠 `case/46.sern_design/_r3_m1m3/sern2d_conv/`。**判別力の限定**: 両 run ともプラトーで低下桁数自体が 0.2–0.6 桁と小さく、許容 0.2 桁と同程度 | — |
...
**壁節点の排出の軌跡が ~~5 桁目まで一致する~~ 0.0264 % 以内で一致する** (5 有効桁の一致ではない; codex result-3 m6 で訂正、下の「言えるのはここまで」も参照) (CV 153797 の $\rho = P/(RT_w)$):
...
([`tooling-sern-mesh-blocking.md`](../active/tooling-sern-mesh-blocking.md) §5.1 **B1f**)。**本計画の $\chi_n$ とは独立**で、
...
**訂正 (2026-09-25, 後継 plan [`convection-slau-wall-normal-chi-usage-rule.md`](../accepted/convection-slau-wall-normal-chi-usage-rule.md) の codex plan M7)**: 不確かさを「両 run の振幅の大きい方」としたのは過小 (位相が独立なら両側が寄与する)。**両 run の振幅の和** $U=\lvert\Delta\rvert+a_0+a_1$ で再計算すると $C_T$ 0.0309+0.0073+0.0076 = **0.046 %** (≤ 0.1 %)、~~衝撃足 0.8385+0.0091+0.0091 = **0.857 %** (≤ 1 %)~~ (振幅加算も上限にならないので撤回、後継 plan codex result M4)。**再判定 (2026-09-25、式だけ全 dump 対に差し替え、窓は旧定義のまま)**: $C_T$ は差区間で相対 [−0.043 %, −0.020 %]、膨張角窓 (旧称「衝撃足」) は末尾 29 dump の全 841 対で $\ell_{ij}$ = **0.714–0.736 %** (≤ 1 %) → **帯内**。**判定は変わらない**。$t$ = 2 mm は「カウル板厚」ではなく**実厚 0.5 mm と異なる登録済み評価長さ**。
...
**衝撃足の壁圧 — PASS** (**訂正 2026-09-25, 後継 plan codex plan-2 M7**: 定義の $\max|dp_w/dx|$ はランプ全域から圧縮/膨張を区別せずに選び、実際に選ばれた $x_{foot}=-0.000878$ m は **x≈0 のランプ膨張角** (窓内壁圧 99.6→56.6→53.6 kPa と低下)。**測ったのは「膨張角窓の壁圧」でカウル衝撃足ではない**。定義どおりの計算結果は有効だが、§4.2 の残リスク (衝撃衝突点) は**未測定** → 後継 plan で再同定)。ノルムは**比較する前に**固定した (§6 V3 の行。結果を見てから条件を作らない):
...
| 衝撃足の壁圧ノルム | **済・PASS** (事前固定、L2 差 0.842 %、位置ずれ 0)。**2026-09-25 訂正: 実体は x≈0 膨張角窓。カウル衝撃足は未測定 (後継 plan)** |
| `cfl_pseudo` 0.4 の固定点 | ~~**済・PASS** (累積 CFL を揃えて 0.01 % 一致)~~ **撤回・限定** (codex result-2 M2 / result-3 M2): 言えるのは「ΣCFL を揃えた CFL 間差 ≤ ε」まで。固定点は #11 の前提 (V7 結果、#15) |
...
| R3-M3 | SERN 衝撃足は差系列だけで、各 run の準定常が未判定。$C_L$ の「実効果」を DRIFTING のまま確定 | 各 run の量 (絶対 $x_{foot}$・窓内壁圧) を判定 (#14)、$C_L$ の確定を撤回 | §5.1 #14、`case/46.sern_design/_r3_m1m3/PERRUN_QS_flag{0,1}.txt` | **閉 (ただし対象は x≈0 膨張角窓。カウル衝撃足は未測定、後継 plan)** (両 run ALL STEADY、窓内分布の偏差上限 0.063 % / 0.035 % も追記 (result-4 m1)。$C_L$/$C_M$ は #11 の前提) |
...
**言ってよいこと**:
- `space.slauWallNormalChi` (opt-in、既定 0) は **node・`nodeWallDirichlet: 1`・非周期/非軸対称**の構成で受入 PASS。**意味は「試験したケース・設定・区間・判定量で、flag 1 使用時の差が回帰許容内」** (codex result-4: 既定 0 であることは flag 1 使用時の無害性の証明にならない。一般の無害性は主張しない)。
  ...
  - V3: case/48 は $C_f$/$q_w$ ±1 % 内。case/16 は壁 $p/p_0$ の差が V3 許容内で、**この 500 step 試験ではノイズ床比 0.80–1.47 (許容 2 倍以内)**。SERN 2D は $C_T$ が両 run STEADY で、#10b 式 (末尾平均差 0.031 % + 振幅 0.007 % =) **0.038 %** (許容 0.1 %)。~~衝撃足~~ **x≈0 膨張角窓の壁圧** (2026-09-25 訂正: 定義の $\max|dp/dx|$ が膨張角を選んでいた。**カウル衝撃足は未測定**で後継 plan で再同定) は**各 run が ALL STEADY** (#14: 絶対 $x_{foot}$ 不変・窓内壁圧 4 量) を確かめたうえで差ノルム L2 **全 dump 対 0.714–0.736 %** (≤ 1 %、2026-09-25 再判定。旧表記 0.8385 ± 0.0091 % は同 step 対の値)、$x_{foot}$ 差 0。
- 収束の悪化 (§3.1 `mSLAU` の報告): **SERN 2D は登録量で PASS** (本段 ΣCFL 6000 で全 8 列、flag 1 の低下桁数 ≥ flag 0 − 0.2 桁。ただし両 run ともプラトーで低下は 0.2–0.6 桁しかなく判別力は限られる)。**case/16・case/48 は残差がプラトーの場からの分岐で低下桁数が定義できないので代替量**: case/16 は事前固定 (c) の末尾 40 % 平均の残差比 1.0003–1.0024 (≤ 2)、case/48 は**事後評価**の末尾 40 % 平均の残差比 0.9760–1.0114 (非ゼロ全 6 列; `rms_ro` 1.0108。旧記載の 1.83e-7 対 1.94e-7 は終端の瞬時値だった — codex result-4 m2)。**有限区間の回帰根拠であって、漸近的な収束率の保証ではない**。
...
**未解決・適用限界 (受入根拠ではない。§5.1 #11 の前提)** (codex result-4 の提案で「受入根拠」から分離):
...
- 受入 = **試験した条件で回帰許容内であることの合格であって、効果の検証でも一般の無害性の証明でもない**。
...
| result | `2026-09-25` (4 回目) | [2026-09-25-convection-slau-wall-normal-chi-result-2.md](../../notes/reviews/2026-09-25-convection-slau-wall-normal-chi-result-2.md) | **GO-with-changes**, C0/M0/m3 (「既定 0 の opt-in として、検証したケース・設定・有限区間に限定した受入は支持」「固定点・格子収束を #11 へ移す判断も妥当」) | **全件採用** (Minor のみ)。m1 → #14 に窓内分布の偏差上限 (0.063 % / 0.035 %、既存 CSV から) を追記。m2 → case/48 の水準比を終端瞬時値から末尾 40 % 平均 (0.9760–1.0114、事後) に直し、「収束場」を「残差がプラトーの場」に。m3 → 撤回済み記述 5 箇所 (plan 3・case/16 README・case/46 README) を同期、受入文を「試験した条件・区間・判定量で回帰許容内」に限定し、格子・固定点を「未解決・適用限界」に分離。→ `done`・`accepted` へ |
...
- **制限事項**: flag 1 は**非周期・非軸対称の node 構成でのみ検証**する (§2)。~~周期・軸対称で使う前に §5.1 #7 が要る。~~ #7 (V6 小規模試験) は PASS、生産規模では未使用。
- block-DPLUR の Jacobian は近似 FVS のままでよい (`block_dplur_jacobian_d.cuh:22`)。
  ~~ただし CFL・内反復数による固定点と収束性は V3 で確認する。~~ (**2026-09-25 変更**: 固定点は受入条件から外し #11 の前提へ。V3 で見たのは ΣCFL を揃えた CFL 間差と、収束の悪化 (SERN 2D は登録量、分岐 2 件は代替量) まで。§6 V3、§5.1 #15)

## 8. 完了条件

- [x] 関連 `methods/convection/` の現在仕様を更新済み
- [x] 実装・検証完了 (§6 の V1–V3)
- [x] codex レビュー 2 回 (`plan` / `result`) を §6.1 に記録し、Critical / Major の採否を §5.1 に反映済み
- [x] `status` を `done` に変更し §9 に変更ログを記載
- [x] `plans/active/` → `plans/accepted/` へ移動
