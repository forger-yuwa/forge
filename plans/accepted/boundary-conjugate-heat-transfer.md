...
- **area**: `boundary`
- **status**: `done` (2026-09-26。V6′ PASS (範囲限定)・V2 は FAIL のまま 1 点を例外受け入れ・放射は保留。§6「本計画を閉じる条件」)
- **related_docs**:
...
- **related_plans**:
  - [`tooling-nozzle-isothermal-wall-chain.md`](../active/tooling-nozzle-isothermal-wall-chain.md) §4.6-4 / §5.1 #6・#7 — **本計画の親**。弱 CHT ループと `wallProfile` を移管済み
  - [`tooling-energy-balance-diagnostics.md`](../active/tooling-energy-balance-diagnostics.md) — **本計画の合格条件** (codex C1 採用で「audit」から格上げ)。拘束反力を含む実効界面熱量はこれが無いと取れない
  - [`case-hypersonic-gap-heating-validation.md`](../active/case-hypersonic-gap-heating-validation.md) — 発注元の 1 つ。壁温実測を `wallProfile` で与える**流体側検証**を担う
  - [`case-plate-annular-cavity-m5.md`](../active/case-plate-annular-cavity-m5.md) — case/49。**別セッション所有につき参照のみ**
- **related_notes**:
...
**符号規約を先に固定する** (依存先 plan [`tooling-energy-balance-diagnostics.md`](../active/tooling-energy-balance-diagnostics.md) §4.1 と同じ):
...
- **依存関係を格上げし、対応範囲のマイルストーンまで書く** (codex 2 巡目 #1):
  拘束反力の採取は [`tooling-energy-balance-diagnostics.md`](../active/tooling-energy-balance-diagnostics.md) が提供するが、
  同 plan は初版で **dual-time・周期・軸対称を明示的に拒否**している ([:32](../active/tooling-energy-balance-diagnostics.md))。
  ...
  **提供側 ([`tooling-energy-balance-diagnostics.md`](../active/tooling-energy-balance-diagnostics.md) §5.1) にも次を登録する**
  (登録しないと解除が誰の作業でもなくなる):
...
- 共有角の拘束反力は依存診断側で**接合部の別勘定**になる ([`tooling-energy-balance-diagnostics.md`](../active/tooling-energy-balance-diagnostics.md) §4.1)。
...
| SU2 (別ソルバ) との差 | **ソルバ内 −0.02471 K (−0.152 % of rise) / 外部 +0.00409 K (+0.025 %)** (1 次元スラブ `case/52`。SU2 自身は解析解と +0.00001 K) | §6 V2 1D スラブ, #81, [`case/52` README](../../case/52.conjugate_slab/README.md) |
| 深いスロット・片壁共役 (V6′、固定深部帯 211 節点) | 解析解との差 $T_{w1}$ 帯内 max 0.153 % of rise・連成保存 (e) ≤0.0025 %・感度 2×2 の差 ≤0.0004 % (温度) | §6 V6′, §5.1 #100–#103 |
| 外部ループ ↔ ソルバ内 (翼) | 平均 **+0.209 K** / 局所最大 **0.514 K** | §6 V4b, #79 |
| 連成が流体側に与える余計な作用 | $C_f$ **0.000 %** / $q_w$ **0.017 %** / $\delta^*$ **0.032 %** | §6 V3, #84 |
| 保存則 (G-cons) | **0.0016 %** | #79 |
| 再開の再現性 | **+0.0001 K** | #79 |

→ **上表は検証したケース・設定・領域での実測値**であり、他の case へ一律の上限として持ち込めるものではない (2026-09-26 訂正、codex result 8 巡目 M3)。旧記述「連成に由来する壁温の不確かさは 1 K 未満、case を変えても保持される」は**有限個の検証からは保証できないので撤回**する。V6′ も**深部帯限定**で全域の収束を保証していない。渡すときは「どの case・設定・領域で何 K / 何 % だったか」を表ごと渡し、受け取る case で同種の検査 (G-cons・G-if・感度) を回してもらう。旧行「SU2 との差 0.0001 %」は SU2–解析解の値を forge–SU2 と取り違えていた (同訂正)。
...
([`case-hypersonic-gap-heating-validation.md`](../active/case-hypersonic-gap-heating-validation.md) §2、
[`case-plate-annular-cavity-m5.md`](../active/case-plate-annular-cavity-m5.md))。本節はその見直しを求めるための材料であって、
**本計画が先方の方針を決めるものではない**。
...
1. ~~**S0 docs 先行 + 索引の同期**~~ — **完了 (2026-09-19)**。
   ...
   **撤回済みの旧方針が索引・親 plan に残っているので同時に直す** (codex 2 巡目 #7):
   ...
   [親 plan](../active/tooling-nozzle-isothermal-wall-chain.md) §5.1 の `wall_conjugate`。
2. ~~**S1 界面診断**~~ — **完了 (2026-09-19)**。`conjugateWall.{hpp,cpp}` 新設 (符号規約の正本)、
   ...
   [`tooling-energy-balance-diagnostics`](../active/tooling-energy-balance-diagnostics.md) の完成後。
...
#21 は提供側 [`tooling-energy-balance-diagnostics`](../active/tooling-energy-balance-diagnostics.md) §5.1 #7・#8 に登録済み。
...
| 3 | 依存: 拘束反力の採取 | [`tooling-energy-balance-diagnostics`](../active/tooling-energy-balance-diagnostics.md) の完了を **Phase 1 の合格条件**として登録 |
...
| 85 | **受け渡しの誤差幅を成分に分けて確定** (2026-09-24、§4.11 を新設) | 引き継ぎ (3) の「翼で得た誤差幅を付けて case/49・50/51/55/56 へ渡す」を、**まとめて ±N K で渡さず**4 成分に分けた。**(a) 連成そのもの = 検証した範囲の実測値として渡す**: 解析解 0.025 % of rise (外部) / 0.152 % (ソルバ内) / forge–SU2 −0.02471 K・+0.00409 K / 外部ループ↔ソルバ内 +0.209 K / V3 で流体側 0.03 % / G-cons 0.0016 % / 再開 +0.0001 K。~~連成由来の壁温の不確かさは 1 K 未満~~ (**2026-09-26 撤回**、codex result 8 巡目 M3: 有限個の検証から case 横断の上限は保証できない。SU2 0.0001 % は SU2–解析解の値との取り違え。§4.11(a) 参照)。**(b) 固体の離散化 = 形状で測り直す**: 厚さ方向は −0.0009 K だが**面内伝導の有無は局所で 16.2 K** → 勾配の急な所では `fem2d` 必須。**(c) 流体側のモデル誤差 = そのままは渡せない**: 翼の全体 +11.7 / +20.0 K は**層流域の遷移モデル欠如が主因** (SU2 も +45.8 %) で、前縁遷移の無いすきまに移せない。渡せるのは**正圧面の +7.5〜9.7 K だけ**、それも条件付き。**(d) 固体の条件同定 = 渡せない**: 翼は孔配置・$k_s$ の出典由来で 16〜17 K rms (bias −7〜−10 K) 残る翼固有項。**受け取る側の plan は現在 CHT を『やらない』と書いている**ので、本節は見直しを求める材料であって本計画が先方の方針を決めるものではない |
...
| 103 | **V6′ の判定 (3 回目) = 判定不能のまま。`20/200` だけ累積 688k まで延長して再判定** (2026-09-26、**codex (diagnose) に諮った**: [`notes/reviews/2026-09-26-v6p-ext394k-verdict-diagnose.md`](../../notes/reviews/2026-09-26-v6p-ext394k-verdict-diagnose.md)、ブリーフ [`notes/reviews/briefs/2026-09-26-v6p-ext394k-verdict.md`](../../notes/reviews/briefs/2026-09-26-v6p-ext394k-verdict.md)。担当 `F`→`O`) | **判断: 2026-09-26、全件採用・却下 0**。**記録のしかた**: 「基準と比較 2 条件 (`20/50`・`5/200`) は登録条件 A で合格、残る `20/200` は判別未完」。**採否**: ① 総合 PASS / 条件つきは却下 (`20/200` の `Tw_m300_208`…`212` が `TRANSIENT-UNSETTLED` で「局所 ALL STEADY」を満たさない。未静定を免除しない)。② 「進行尺度 = 更新数/`Df_scale`」は**確認済みと書かない** — 今回の結果と整合する経験的目安に留める (最終値の近接は過渡の相似性や 392 の必要十分性を証明しない)。③ 流体残差・全域 G-if の NOT CONVERGED は報告項目のまま、ただし**主張の範囲を限定**: 検証したのは登録した深部帯であって全域 CHT 解の収束ではない。「全域収束」「固定点の設定非依存を実証」とは書かない。④ 尺度 392 までの一括延長 (追加 1,176,000 step) は**修正採用**: 追加 294,000 step だけを事前登録する。⑤ `eval_v6p.py --fix-band` は固定帯の行を座標だけで選び適格条件を検査していなかった → **修正済み** (固定帯の全行に Pe・線形残差・勾配比を当て、1 行でも外れれば FAIL)。今回の 4 条件は全行適格 (Pe max 2.5e-4…5.7e-4 / 線形残差 ≤1.93e-4 / 勾配比 ≤2.40e-3) で判定は覆らない。**訂正 2 件**: (i) 非静定 5 節点の drift は 0.051–0.062 %/tail で、表示の「0.1 %」は丸め — 通常閾値の超過ではなく「末端極値かつ半閾値 0.05 % 超過」で未静定 (`check_quasisteady.py`:297)。(ii) ブリーフの代表点 i=213 ($y$ −4.119 mm) は**比較帯の外**。帯内の根拠は i=212: 延長後半の壁温 394.250030 → 394.290253 → 394.309886 K、物理残差 $r/A$ −5.142 → −2.803 → −1.181 W/m² (縮小中だが VERDICT は未静定)。「0.0000 %」は丸め表示で厳密一致ではない。**次の一手 (事前登録。結果を見る前に書いた)**: `case/58.conjugate_slot/run_0014_v6p_df20_i200_ext688k` — `run_0013_v6p_df20_i200_ext394k` の同時刻の流体・壁温・固体状態を引き継ぎ、**変えるのは計算長だけ** (+294,000 step、累積 688,000)。基準は `run_0012_v6p_df5_i50_nlog` に固定。帯は −19.779487…−4.166299 mm の 211 節点を維持し、**帯の適格条件と (a)〜(f) を再確認** (`eval_v6p.py --fix-band`)。**静定・釣合い**: 新しい延長区間の末尾半分 (累積 541,000–687,800 step) で帯平均・全局所 $T$/$q$ が `ALL STEADY` (`--tail 0.5 --drift 0.001 --osc 0.001`)、最後の 80 更新で帯内 G-if PASS、(e) ≤0.1 %。**A**: 上記を満たし基準との差が $T$・$q$ とも ≤0.5 % → この設定範囲・固定帯・登録許容内の感度検証を合格とする。**B**: 静定・釣合いを満たすが差 >0.5 % → 「過渡だけ」を棄却 (全域未収束なので、直ちに全域固定点の設定依存とは断定しない)。**いずれも満たさない**: 判別未完 (累積 688k は診断の打切り時点で、収束を保証する長さではない)。**やらない**: 帯を縮める / 準定常の閾値を緩める / `Df_scale` を変えた結果で元の `20/200` を合格扱いする <br>**結果 (2026-09-26、`case/58.conjugate_slot/run_0014_v6p_df20_i200_ext688k`、NaN なし) → 登録条件 A**: 固定帯 211 節点は全行適格 (Pe max 3.74e-4 / 線形残差 1.92e-4 / 勾配比 2.40e-3)、(a)〜(f) PASS ((e) **0.0018 %**)、延長区間の末尾半分 (累積 541,000–687,800) で**帯平均・局所 211 節点 $T$/$q$ とも ALL STEADY**、帯内 G-if PASS (① 0.196 W/m² / ② 2.44e-5 / ③ 5.5e-6 K / ④ 1.4e-9)、基準との差 温度 max **0.0004 %** / $q$ max **0.0017 %** (許容 0.5 %)。流体残差 `NOT CONVERGED (stalled/plateau)`・全域 G-if NOT CONVERGED (① 27.8、報告項目)。**これで感度 2×2 の全条件が登録条件を満たした** (基準 `run_0012_v6p_df5_i50_nlog`、`20/50`・`5/200` = `run_0013_*_ext394k`、`20/200` = 本 run)。**主張の範囲 (#103 ③)**: 検証したのは登録した深部帯での伝導漸近と感度であって、全域 CHT 解の収束や固定点の設定非依存の実証ではない。解釈の確定は codex (diagnose) と result 8 巡目に回す <br>**判断 (2026-09-26、codex (diagnose) に諮った: [`notes/reviews/2026-09-26-v6p-close-diagnose.md`](../../notes/reviews/2026-09-26-v6p-close-diagnose.md) — 登録条件どおり PASS、範囲限定)**: 採用。記録する範囲は §6「閉じる条件」2 の 1 文。追加計算は閉じる条件として要求しない。**`accepted/` への移動は result 8 巡目の承認後** |
| 104 | **codex result 8 巡目の反映 (GO-with-changes、Critical 0 / Major 3 / Minor 2)** (2026-09-26、全件採用・却下 0。担当 `O`) | **M1 採用・修正済み**: `eval_v6p.py` が両壁の `iface_q_eff` を絶対値にしていたので**後壁の符号反転を見逃した** (codex がメモリ上で再現: (c) 0.2072 % のまま PASS)。→ 符号を保ち前壁 $q_{\rm eff}$・後壁 $-q_{\rm eff}$ で比較、(d) は流体柱の符号つき収支 $|q_{\rm eff,1}+q_{\rm eff,2}|$。`sens_v6p.py` も符号つき。**M2 採用・修正済み**: `check_cht_interface.py --band-y` が節点表の有限性・ID・更新内の時刻一致を見ていなかった (座標 NaN の節点が帯から黙って落ちる・1 節点だけ step=0 でも PASS)。→ `load_node_log()` に集約 (節点表の有限性・$A_i>0$・ID 連番、ログの有限性・整数性・更新番号の単調性・更新ごとの ID の完全一致・時刻一致) し、`eval_v6p.py` の準定常系列も同じ関数を通す。帯の節点数を `v6p_band.json` の行数と照合。**負例試験** [`case/58.conjugate_slot/test_v6p_negative.py`](../../case/58.conjugate_slot/test_v6p_negative.py): 正例 2 件 PASS・後壁符号反転 → FAIL・座標 NaN / 1 節点 step=0 / ID 重複 → REFUSED で **6/6**。**罠 (本件で踏んだ)**: 試験の初版は一時ディレクトリに元 run の全ファイルを symlink したため、判定ツールが書く `CHT_INTERFACE_BAND_VERDICT.txt`・`v6p_band_series.csv` が**元 run の成果物を上書き**した → リンクは読むだけの h5 に限定し、元 run の成果物を作り直してテスト前後の md5 一致を確認。**再評価**: 修正後の評価器で 4 run を再判定し数値は不変 ((b)(c)(d) 同値、帯内 G-if PASS、準定常 ALL STEADY、感度 PASS)。`test_gate_bad_input.py` PASS。**M3 採用・修正済み**: §4.11(a) の「SU2 との差 0.0001 %」は SU2–解析解の値の取り違え → forge–SU2 は ソルバ内 −0.02471 K (−0.152 %) / 外部 +0.00409 K (+0.025 %) に訂正。「連成由来の不確かさは 1 K 未満・case を変えても保持」は**撤回** (有限個の検証から case 横断の上限は保証できない。V6′ も深部帯限定)。#85 も同期。**m4 採用・修正済み**: `procedures/su2-cross-check.md` の「`HF[0]`/`HF[1]` で収支を比べる」を訂正 (`HF[1]` = 適用荷重 ↔ forge Σ`iface_Qf_eff`、`HF[0]` は WLS 診断流束)、固体 `CFL_NUMBER= 1000` と界面温度を `restart_1.csv` から取る手順を追記。**m5 採用・修正済み**: `methods/boundary.md` に `node_log`・帯判定・$Q_{\rm sol}$ の説明、`case/48` README に SU2 対照の最終判定・例外受け入れ・成果物の保存先 (`/home/sano/work/forge/case/48.flat_plate_cooled_m4/su2_cht_cont/`)、`plans/README.md` は `accepted/` 移動時に同期 <br>**判断 (2026-09-26、codex (diagnose) に諮った: [`notes/reviews/2026-09-26-cht-result8-disposition-diagnose.md`](../../notes/reviews/2026-09-26-cht-result8-disposition-diagnose.md) — 全件採用は妥当、残る穴は負例試験の判定強度 1 件)**: `test_v6p_negative.py` を「判定語 + 終了コード」の照合に変更し、VERDICT の無い異常終了を合格と取り違えないことを自己検査 2 件で確認 → **8/8**。元 run の成果物は試験前後で md5 一致 |
...
3. **連成の保存性の検査を追加** (これが本来の「固体に依らない検査」): **(e) 帯内で $|q_{\rm iface}/L - q_{\rm eff}|\le0.1\,\%$ of $q_*$** (**2026-09-26 に定義を訂正**: `q_iface` は流体荷重 $Q_f$ のコピーで保存性を測っていなかった。**現行は $|Q_{\rm sol}-Q_f|/A_i$、$Q_{\rm sol}=(K_su-b_s)_{\rm iface}$** — §5.1 #100/#101)、**(f) 固体 $T(x{=}0)$ と流体 `Ts` の差 ≤1e-3 K**。**旧設計の (d) G-cons は「固体に依らない独立な検査」ではなかった** — 後壁は $q_{w2}$ を通じて $T_{w1}$ の誤差を隙間越しに継承する ($\delta q_{w2}/q_* = \delta T_{w1}/(T_{w2}-T_{w1,*})$ = 0.598/105.8 = **0.565 %** で観測 0.744 % の大半を説明)。**G-cons が測るのは流体柱の 1 次元性**であって連成の保存性ではない。
...
5. **G-if の判定範囲を帯に合わせる** (**閾値は据え置き**: `eps_abs_Wm2` 23.5 / `eps_rel` 1e-3 / `dT_K` 1e-2 / `n_consec` 80)。全域 G-if は**報告項目**にする — ①② は **lip 行の流束リップルに支配される** (伝達差は $y$=−0.777 mm で 51 %、−2.05 mm で 0.1 % 未満に落ちる)。**(2026-09-26 実装済み: `conjugate.node_log` + `check_cht_interface.py --band-y`、§5.1 #101 ②)** 当時の記述: **実装が要る**: 節点ごとの界面残差は `conjugateWall.cpp`:740-744 で作られるが履歴には全域 max しか出ず、`check_cht_interface.py` に帯オプションが無い → 固体ダンプに節点残差 `r_iface` を出すのが最小変更 (host 側・数値カーネルではない)。
...
2. **V6′** (#86)。すきま形状での CHT を 1 本だけ自己完結で通す。
   **判定済み (2026-09-26、§5.1 #100–#103、codex diagnose 4 回): PASS (範囲限定)**。V6′ は、全域 FP64・本ケースの固定深部帯 211 節点において、登録した伝導漸近、連成保存性・温度連続、局所準定常、帯内 G-if、および `Df_scale` 5/20 × `interval` 50/200 の感度条件を満たして PASS とするが、流体残差と全域 G-if は NOT CONVERGED であり、全域 CHT 解の収束や固定点の設定非依存を実証したものではない。 根拠 run: `case/58.conjugate_slot/run_0012_v6p_df5_i50_nlog` (合否・基準)、`run_0013_v6p_{df20_i50,df5_i200}_ext394k`、`run_0014_v6p_df20_i200_ext688k`。「0.0000 %」等は丸め表示で厳密一致ではない。
...
| **codex (diagnose) 4 回目** | `2026-09-26` | [`notes/reviews/2026-09-26-v6p-close-diagnose.md`](../../notes/reviews/2026-09-26-v6p-close-diagnose.md) (ブリーフ [`notes/reviews/briefs/2026-09-26-v6p-close.md`](../../notes/reviews/briefs/2026-09-26-v6p-close.md)) | **Minor 1** | **全件採用** → §5.1 #103・§6 閉じる条件 2。**結論: V6′ を範囲限定で PASS と記録し result 8 巡目へ**。旧 (e)・未実装記述・README の「次」を同期 |
| **result (8 巡目)** | `2026-09-26` | [`notes/reviews/2026-09-26-boundary-conjugate-heat-transfer-result.md`](../../notes/reviews/2026-09-26-boundary-conjugate-heat-transfer-result.md) | **GO-with-changes**、C0 / M3 / m2 | **全件採用・却下 0** → §5.1 #104 (すべて修正済み)。V2 の数値と V6′ の範囲限定 PASS を codex が独立に再現。「accepted への移動前に評価器 2 件と文書 3 件」を満たした |
...
3. **放射**: §4.10 のとおり解かない。深いキャビティで桁が無視できないと分かったら別 plan にするか。
   **2026-09-25 に「無視できない」が数字で出た**: [`case-plate-annular-cavity-m5.md`](../active/case-plate-annular-cavity-m5.md)
...
- `2026-09-26` **完了 (`status: done`、`plans/accepted/` へ移動)**。閉じる条件 3 つ: V2 (FAIL のまま 1 点を例外受け入れ) / **V6′ PASS (範囲限定)** / 放射保留。result 8 巡目 (GO-with-changes) を全件反映し、採否を codex (diagnose) に諮って承認 ([`notes/reviews/2026-09-26-cht-result8-disposition-diagnose.md`](../../notes/reviews/2026-09-26-cht-result8-disposition-diagnose.md) — 全件採用妥当、負例試験の判定を厳密化すれば 9 巡目不要)。負例試験は終了コードと判定語の組で照合し自己検査込み **8/8**。**後継**: 軸対称 `fem2d` (#95) → 3D (#96) を別 plan に切り出す (未起票)。

- `2026-09-26` codex result 8 巡目 (GO-with-changes) を全件反映 (§5.1 #104): 評価器の符号・節点ログ整合検査と負例試験、§4.11 の誤差保証の撤回と訂正、SU2 手順・methods・case/48 README の同期。

- `2026-09-26` `run_0014_v6p_df20_i200_ext688k` が登録条件 A → 感度 2×2 完了。**V6′ PASS (範囲限定、§6 閉じる条件 2)**。codex diagnose 4 回目で承認、result 8 巡目へ。
