...
- **related_plans**: [tooling-nozzle-tp-split-h2o-condensation.md](tooling-nozzle-tp-split-h2o-condensation.md) (MIXDRY の起源),
  [chemistry-finite-rate-h2.md](../active/chemistry-finite-rate-h2.md) Phase 0 (CEA `thermo.inp` → `species_db.yaml` 変換ツール), [condensation-kantrowitz-carrier.md](condensation-kantrowitz-carrier.md) (多成分 carrier の衝突項)
- **created**: `2026-09-15`
...
| 8 | F-sp1: トレーサ `roXi` の拡散 (SST の SERN で必要なら混合平均 Sc) | → plan [species-passive-scalar-unification](../accepted/species-passive-scalar-unification.md) (2026-09-17): トレーサを化学種カーネルの受動種にして 2 次 + Fick/乱流拡散 (粘性二流体試験もそこで) |
...
| 18 | (M10) 小型 5 種 node TP の `check_convergence` PASS ケース; 作動点変更 (m6_on→m10_on) と湿潤 restart の試験 | **小型 PASS ケース済 (2026-09-16)**: case/16 `run_0471_tp5_node_euler_va3` (5 種) / `run_0472_tp2_node_euler_va3_lumped` (lumped): node Euler TP, Tt 353 K, 12000 step で全列 3.3 (rms_roUy)〜4.8 桁低下 (rms_roY 4.2 桁) **PASS** [訂正: 当初「4.2–4.8 桁」と記載, result-2/3 で rms_roUy 3.3 桁と指摘], ΣY−1 ≤ 8.6e-8, 負値 0, 5 種 vs lumped の場差 ro 1.0e-6 / P 1.7e-6 / T 1.0e-6 / Y_H2O 3.9e-6 (float 精度), step 時間 +33 % (case/16 README run 表)。**作動点変更・湿潤 restart 済 (2026-09-16)**: case/46 `run_0105` (m6_on `run_0102` → m10_on を `warm_from_run`; ΣρY/ρ 1.2e-7, T 整合 1.8e-3 K, 適応段 2000 step で残差 4.5 桁低下, NaN 0); 湿潤 restart は `run_0196` の入力 (6365 湿潤セル) を二相 EOS 変換器で同一配置変換して T 保持 6.8e-13 K (#10); 同一条件の段階 restart (restart_by_index) は case/46 `run_0100`–`0103` の段階起動で全種を引き継ぎ最終 ΣY 1±2.4e-7。**トレーサ誤差は切り分け済** (2026-09-16, case/46 `run_0104`: lumped run に tracer を併用すると同一 run 内で `\|Xi−Y_EXH\|` 最大 1.8e-4 / 平均 4.6e-7 → 汎用スカラ輸送カーネルと化学種輸送経路の差であり配置差ではない [訂正 2026-09-17: 次数差ではなく (化学種の 2 次面移流は既定 OFF) 再正規化・クランプ等の差; plan species-passive-scalar-unification §4.0]; §6 の「ξ と Y_EXH 1e-6」は同一カーネルでないと成立しないので **「平均 1e-6 以内 + 最大はカーネル差 (同一 run 実測) 以内」に改定**)。README「同じ固定点」→「ゲート内で一致」は済 |
| 20 | (result-2 M1) 作動点変更の組成再初期化 | 設計側済 (2026-09-16): `composition.reinit_transport_vector` + `warm_from_run` は ξ (元 run の `exhaust_fraction`) と目標入口ベクトルから Y_t を再構成 (単体: m10_on full ξ=1 → H2O 0.24881767 / H2 0.01383296)。CFD: case/46 `run_0105` (lumped) / **`run_0106` (full 11 種, `tp_species.species` で順序固定)** を m6_on 24k 場から m10_on へ warm: 排気側 Y = 目標入口組成 (full 2.5e-4 = (1−ξ) 内), ΣρY/ρ ≤2.4e-7, roXi 引き継ぎ, 適応段 2000 step で残差 4.5 桁低下。変換器 `--mode reinit` も同式 (目標 meta の `Y_transport`; 元組成は ξ 以外捨てる = 情報を落とす操作と明記): `run_0102` res → m10_on full 配置で純排気セルの Y_H2O 0.24881767 / Y_H2 0.01383296 (目標一致), roXi 生成 |
...
| 30 | ~~(result-5 M1) `interp_field.py` の Xi → roXi 転送と転送配列の完備検査~~ | 済 (2026-09-16): ρ·Xi を転送、`required_conserved` ⊆ 転送配列を書込み前に検査; `tests/unit/test_interp_field_tracer.py` (8 ノード合成: 宛先 roXi/ρ 0.8 → 0.3) |
...
| --- | --- | --- | --- | --- |
| result | `2026-09-16` | 6 回目  [2026-09-16-thermophysics-cea-mole-fraction-species-result-6.md](../../notes/reviews/2026-09-16-thermophysics-cea-mole-fraction-species-result-6.md) | **GO**, C0/M0/m0 | result-5 M1/m1 の解消を実物で確認 (Xi→roXi 転送 0.8→0.3, 完備検査の拒否・宛先不変, 小型ケース PASS/ALL STEADY, case/44・46 は改定ゲート内, 166 スナップショットに NaN/Inf 無し)。**改定済みの node/Euler 検証範囲で accepted へ**。F-sp1 (トレーサ拡散 + 粘性二流体試験) は §5.1 #8 に残す |
| result | `2026-09-16` | 5 回目  [2026-09-16-thermophysics-cea-mole-fraction-species-result-5.md](../../notes/reviews/2026-09-16-thermophysics-cea-mole-fraction-species-result-5.md) | **NO-GO**, C0/M1/m1 | **全採用 (2026-09-16 反映, §5.1 #30)**: M1 (`interp_field.py` が必須検査では原始量 `Xi` を代替として受理するのに転送は `roXi` だけで、トレーサを黙って引き継がない) → res に `Xi` しか無ければ ρ·Xi を `roXi` として転送し、書込み前に「検査で要求した保存量が転送配列に揃っている」ことを確認; 回帰 `tests/unit/test_interp_field_tracer.py`; m1 (plans/README の「残」、case/44 README の「固定点の差ではない」断定) → 同期し「報告量は比較許容差内、差の原因・固定点の同一性は未確定」に修正。確認済: 前回 M1/M2、小型ケース PASS、準定常性 |
| result | `2026-09-16` | 4 回目  [2026-09-16-thermophysics-cea-mole-fraction-species-result-4.md](../../notes/reviews/2026-09-16-thermophysics-cea-mole-fraction-species-result-4.md) | **NO-GO**, C0/M2/m1 | **全採用 (2026-09-16 反映中, §5.1 #28–#29)**: M1 (変換器が float64 で有限性検査した後に float32 へ変換し、1e39 の roOmega が Inf で書かれる) → 宛先 dtype に変換してから全検査、dry-run も同じ検査、既存データセット削除は全検査後、overflow の失敗系追加; M2 (設計側 FrozenGas/ResolvedSpeciesDB が固定 T_FLOOR と Tmid だけで外部 DB の Tlo/Thi を無視: H2O Tlo=300 K の DB で e_sens の反転温度が 100→100.498 K) → 種ごとの Tlo/Thi で cp 固定・h 線形・s° 対数 (ソルバ `thermo_d.cuh` と同式) に統一し、`run_gas_tests` にソルバ式の独立移植との比較 (1e-14, 反転 T 差 3e-13 K) を追加; m1 (§5.1 #4b の「残」、plans/README の記述) → 同期。確認済: トレーサ保持、低温 EOS 反例、24 試験、小型ケース PASS |
| result | `2026-09-16` | 3 回目  [2026-09-16-thermophysics-cea-mole-fraction-species-result-3.md](../../notes/reviews/2026-09-16-thermophysics-cea-mole-fraction-species-result-3.md) | **NO-GO**, C0/M3/m1 | **全採用 (2026-09-16 反映中, §5.1 #24–#27)**: M1 (変換器がトレーサの有無を meta 優先で決め、config で有効な `roXi` を削除 [run_0104: config tracer exhaust / meta enabled false]) → 必須保存量は config (`species_signature`/`required_conserved`) から決め、meta とのトレーサ矛盾は書込み前に拒否 (#21 を未完了へ); M2 (温度保存検査の EOS が `_TPGas` で Tlo/Thi 外の cp 固定・h 線形外挿を持たず、低温 (100–150 K) で組成変更後の T が 0.1–1.4 K ずれるのに検査を通る) → `_TPGas` をソルバ (`thermo_d.cuh`) と同じ範囲外処理にし、独立参照で低温の期待エネルギーを作る試験を追加 (#22 を未完了へ); M3 (`roK`/`roOmega` が有限性検査から漏れ NaN を書く) → 書込配列を一括管理し全配列を検査、失敗系に追加; m1 (solver-settings の「lumped では ξ=Y_EXH」、§9 の「4.2–4.8 桁」、plans/README の段階) → 同期。確認済: 再初期化 (run_0105/0106 の式との差 1e-7, 目標組成一致)、署名 (係数 +1・tracer 差を検出)、小型ケース PASS、準定常性 |
| result | `2026-09-16` | 2 回目  [2026-09-16-thermophysics-cea-mole-fraction-species-result-2.md](../../notes/reviews/2026-09-16-thermophysics-cea-mole-fraction-species-result-2.md) | **NO-GO**, C0/M3/m1 | **全採用 (2026-09-16 反映中, §5.1 #20–#23)**: M1 (作動点変更の再初期化が目標組成にならない: `reinit` は元の実種質量を配分するだけ、`warm_from_run` は組成分率を持ち越すだけ) → 再初期化を「保存済み ξ と**目標**入口ベクトルから Y(r) = ξ Y_in^dst + (1−ξ) Y_ext^dst を再構成」に統一 (変換器 `--mode reinit` と `warm_from_run` の両方)、m6_on→m10_on で目標組成 (H2O 0.24881767, H2 0.01383296)・ΣY・T を検証; M2 (署名に NASA-9 係数・温度区切り・トレーサ設定が無く、メタだけで照合) → 実 config + 解決済み DB から署名 (種順序・MW・両区間係数・Tlo/Tmid/Thi・datum・tracer キー) を作り、メタとの矛盾・TP の照合不能・必要保存量の欠落を拒否; M3 (変換器の hard fail が NaN を通す、反転未収束を返す) → ρ>0・全保存量/組成/T の有限性・組成非負・EOS 反転残差の検査と失敗系試験; m1 (docs の ξ 説明・完了条件の二重記載・粘性試験の扱い) → methods 2 箇所を「純流入元ラベル種の有無」で統一、§6 の旧ゲートを履歴化、F-sp1 依存の粘性試験を §6/§8 で延期明記。M10 の「全列 4.2–4.8 桁」は `rms_roUy` 3.3 桁に訂正 |
| result | `2026-09-16` | 1 回目  [2026-09-16-thermophysics-cea-mole-fraction-species-result.md](../../notes/reviews/2026-09-16-thermophysics-cea-mole-fraction-species-result.md) | **NO-GO**, C0/M10/m1 | **全採用 (2026-09-16 反映中, §5.1 #9–#19)**: M1 (`NO` が YAML で False) → 種名を引用符付きで出力し、生成 config の Python 再読込試験; M2 (湿潤 input の T 反転が二相 EOS を無視・総水量二重計上) → 変換器に二相 EOS 反転と書込前の保存検査、総水量は ρY_H2O; M3 (restart の名前/DB/datum 照合が未実装) → `species_meta.yaml` + DB + datum の共通照合を interp_field / restart_by_index / warm_from_same_mesh / warm_from_run に適用 (照合不能はエラー); M4 (変換器が stream lump とトレーサ新設を扱えない) → 保存的展開と作動点変更の再初期化を分離、`lumped→full` の roXi 生成; M5 (roXi が周期 node の残差合算・同期から漏れ) → periodicNode に登録 + 周期移流試験; M6 (dual-time の roXi は物理時間項なし) → 入力時に tracer × dual-time を拒否 (対応は F-cf8 と同じ扱い); M7 (`condensationSpecies` と `condModel` の不一致を拒否しない) → host 入力段階で物質↔物性モデルの対応検査 + config 読込までの単体; M8 (`lumped+keep` で ξ=Y_EXH が成立しない, `exhaust_fraction(run)` 不在) → 単一種分率が流入元ラベルにならない配置でもトレーサ有効化 + 共通アクセサ実装; M9 (凝縮 OFF で vapor_array が None → 固定組成 24 % 誤り) → H2O を名前で解決し凝縮の有無と独立に分圧; M10 (未達ゲート: 小型 5 種 node PASS ケース無し、トレーサ場 1e-6 超 (最大 1.7e-4, 4.6k ノード)、CEA 物性差 rtol 1.11e-6、「同じ固定点」の断定) → §5.1 に戻し、小型 node PASS ケース・トレーサ誤差の切り分け・作動点変更/湿潤 restart 試験を実施、README の断定を「ゲート内で一致」に修正; m1 (docs 不整合) → methods/plans README/condensation §7b を同期 |
| plan | `2026-09-15` | 再 (§4.5 SERN)  [2026-09-15-thermophysics-cea-mole-fraction-species-plan-2.md](../../notes/reviews/2026-09-15-thermophysics-cea-mole-fraction-species-plan-2.md) | GO-with-changes, C0/M6/m1 | **全採用**: M1/M2 (元素混合分率では流入元を復元できない・power-off 退化) → 排気率は独立トレーサ `roXi`、元素は診断のみ (§4.5, §5-4c); M3 (stream lump と keep の質量配分) → 流れごとの配分正本 (§4.5); M4 (`restart_by_index`/`warm_from_same_mesh` が roY を落とす) → §4.5, §5-4b, §6; M5 (元素メタデータ) → `species_meta.yaml` (§4.5); M6 (SERN 検証ゲート) → §6; m1 (スキーマ一本化・完了条件) → §4.5, §8 |
...
- [x] 関連 methods を更新済み (thermophysics §5, design/overview, condensation §7b)
- [x] 実装・§6 の検証 (単体 / 小型収束ケース [node] case/16 run_0471/0472 PASS / case/44 1–5 / SERN 一式 [粘性二流体は F-sp1 へ延期]) を満たす
- [x] codex レビュー (plan 2 回 / result 6 回) を §6.1 に記録し、Critical / Major の採否を §5.1 に反映済み
- [x] `status: done`、§9 に変更ログ
- [x] `plans/active/` → `plans/accepted/`、`plans/README.md` 同期 (2026-09-16)

## 9. 変更ログ

- `2026-09-16` — **codex result 6 回目 GO (指摘 0)** → `status: done`、`plans/accepted/` へ移動。受け入れる成果: 統一 `tp_species` スキーマ (full | lumped, 別名), モル分率入力, 解決済み DB (CEA 直読み可) の全経路共有, 由来コメント付き `species_db.yaml` + `species_meta.yaml`, forge `X{s}`/`condensationSpecies`/`rms_roY`/`roXi`, 種変換・署名照合ツール; 検証は node/Euler 範囲 (小型ケース PASS, case/44・46 は改定ゲート内)。残件: F-sp1 (トレーサ拡散・粘性二流体試験) は §5.1 #8。
- `2026-09-16` — codex result 5 回目 **NO-GO (M1/m1)** を全採用: `interp_field.py` の Xi→roXi 転送 + 転送配列の完備検査 (回帰追加)、docs 同期 (§6.1, §5.1 #30)。**codex result 6 回目へ**。
...
- `2026-09-16` — result-1 M10 の小型収束ケース: case/16 node Euler TP `run_0471` (5 種) / `run_0472` (lumped [MIXDRY, H2O]) が `check_convergence` **PASS** (3.3 [rms_roUy]〜4.8 桁; 当初の「4.2–4.8 桁」は訂正, `rms_roY` 列込み), 5 種 vs lumped は float 精度 (1e-6) で一致 (非粘性 frozen の等価性, §3)。トレーサ誤差は同一 run 内のカーネル差 (case/46 `run_0104`) と切り分け。
