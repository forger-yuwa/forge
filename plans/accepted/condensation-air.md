...
- **area**: `condensation`
- **status**: `done`  <!-- codex plan 1 回目 NO-GO (C1/M8/m1) → v2 → 2 回目 GO-with-changes (M5/m2) 全件採用 → v3 で実装 -->
- **related_docs**:
...
| 6 | ~~codex result レビュー~~ | 決着 (2026-09-13, §6.1): 1 回目 NO-GO → 2 回目 NO-GO → **3 回目 GO-with-changes (m2) 全件採用** (§9 ③) → `status: done`, accepted へ |
...
後続 (#6b, #6c, #7–#10) の正本は [condensation-followups.md](../active/condensation-followups.md) §5.1 (codex result ② の要求)。
...
- **検証ケース**: `case/34.arthur_n2_nozzle/` (dry 収束場から restart, Euler, `convMethod 1`, cfl_pseudo 1, **12000 step, 1000 step 毎保存**)。全 run で NaN 0、
  ...
  | N3–N6 | `run_0036–0038_dry_slip_cell_rep`, `run_0039–0041_dry_slip_node_rep`, `run_0042–0044_air_cpgcarrier_node_rep`, `run_0045/0046_n2_new_v2_rep` | cell / node | 全系列 (dry cell / dry node / E1 node / R3 cell) の同一バイナリ 3 反復 → 変数別床 4 本 (codex result ③ m1) |

- **判定基準 (合否)**:
  1. 単体 PASS。
  0. **場差の判定方法** (codex result ② M2): 同一バイナリ・同一 config の反復を 3 run 以上取り、変数別に全ペアの max|Δ|/max|ref| の最大をノイズ床 JSON にする (`diff_res.py --dump` → 変数別 max)。判定は `diff_res.py REF NEW --tolfile 床.json --factor 2` の **exit code** (表示丸めで判断しない)。床に無い変数は判定しない。床は比較対象と同じメッシュ・離散化・プロトコル (dry 場 restart, 12000 step)・凝縮 ON/OFF の系列から取る: cell dry `noise_cell_dry_12000.json`, node dry `noise_node_dry_12000.json`, cell 空気 `noise_cell_air_12000.json`, node 空気 `noise_node_air_12000.json`, cell N2 `noise_cell_n2_12000.json`。R3 node (N2) は同メッシュ・同プロトコル・凝縮 ON の node 空気床を流用 (N2 node の反復は取っていない; 床の大きさは dry node の 1.3 倍程度で系列差は小さい)。
...
| --- | --- | --- | --- | --- |
| result | `2026-09-13` | 3 回目 [`notes/reviews/2026-09-13-condensation-air-result-3.md`](../../notes/reviews/2026-09-13-condensation-air-result-3.md) | **GO-with-changes**, C0/M0/m2 | **全件採用** (2026-09-13 ③, §9)。m1 (3 反復基準が dry cell/node・node に未適用、空気床の N2 流用根拠) → dry cell ×3 (run_0036–0038)、dry node ×3 (run_0039–0041)、E1 node ×3 (run_0042–0044)、R3 cell ×3 (run_0029/0045/0046) で床 4 本を追加し、slip 変更 (cell/node)・E1 node・R3 cell/node を全て exit 0 で再判定、流用根拠を §6 0 に明記。m2 (文書: methods の CPG 二相音速を見送る理由「潜熱整合が先」は古い、§9 の「N2 モデルの較正問題」断定、凍結の説明「原始量・ρe とも保持」が広すぎる) → 理由を一般 EOS 固有系側に、偏差は原因未切り分けに、凍結は「T/P/sonic/Ht を残し roe を上書きしない (密度床・速度は先に更新)」に訂正 (methods, plan, kernel コメント)。後続 (境界 kernel 試験・流束収支・check_quasisteady 統合・Arthur 偏差) は未実施の後続課題として [condensation-followups.md](../active/condensation-followups.md) に置く |
...
| result | `2026-09-13` | 1 回目 [`notes/reviews/2026-09-13-condensation-air-result.md`](../../notes/reviews/2026-09-13-condensation-air-result.md) | NO-GO, C0/M5/m2 | **全件採用** (2026-09-13, §9)。M1 (T 反転失敗でも T/P/sonic を更新) → 失敗セルは T/P/sonic/Ht を前ステップ値のまま残し roe を反転結果で上書きしない (密度床・速度の更新は先に済む) + `g_condTinvFail` を集計して警告 (再取得 run は全て 0 件)。M2 (消滅判定が全圧) → `cond_clamp_vapor_pressure` で source と同じ N2 分圧に統一、全圧/分圧で判定が分かれる状態を `test_cond_air` (h) に追加。M3 (境界種別の受付未実装・Yw NaN) → `main.cpp` で inlet_uniformVelocity/outlet_statPress/slip 以外を拒否、`isfinite` 検査。M4 (node 中心線・壁抽出) → 中心線 |y|<1e-9 ノード、壁は x ビン毎の y 最大ノード (全 slip の node メッシュは wall_dist が無い) で再計算 (onset 表は変わらず)。M5 (cell ノイズ根拠・float32 試験) → 同一バイナリ反復 run_0027/0030 (凝縮) と run_0015o/0031 (旧 dry) で床を実測し `--tolfile` 2 倍で判定、SLAU 面エンタルピーを `cond_face_h_cpg` に抽出して float 入力の試験 (g) を追加。m1 (「レート側」断定) → §5.1 #10 を原因切り分けに書き換え。m2 (文書の古い値) → methods「計画中」/0.59、README 旧注記、入口 325.12 m/s・6.137 kg/m³、c_l 符号の条件 (L'>c_pv)、procedures の新キーを同期 |
...
- `2026-09-13` — **実装・検証完了** (case/34 README「空気凝縮 (CPG carrier 形) と N2 低温物性の整合」節、図 `compare_air_n2_wall.png`):
  ...
  - **結果 (12000 step, 1000 毎保存, 全 run NaN 0・`--series` STEADY・出口 $u_n/c$>1; 残差は既知のリミットサイクル plateau)**:
    ...
    **潜熱整合の効果**: 潜熱だけ新では onset 不変で g_exit 0.081→0.059 (放出熱 +25 % で凝縮量減; 3 in の cond/dry 1.241→1.158 と実験 1.133 に近づく)、飽和圧も新にすると
    ...
    原因は未切り分け (核生成 J / 成長 dr/dt / 壁圧の抽出位置 / 記号の読み取り; 今回の物性変更では解消しなかった, §5.1 #10 → followups #5)。
- `2026-09-13` ③ — codex result 3 回目 **GO-with-changes** (m2, §6.1) を全件採用し `status: done`、`plans/accepted/` へ移動。同一バイナリ 3 反復の床を全系列に拡張 (case/34 run_0036–0046; 床 JSON 5 本): dry cell ρ 5.1e-4 / U_y 2.3e-3、dry node ρ 7e-7 / U_y 1.1e-5、node 空気 ρ 6.6e-7 / g 5.6e-5、cell N2 ρ 3.3e-4 / g 1.1e-3。再判定 (2 倍床, exit code): slip 変更 cell/node、E1 node (run_0026→0028)、R3 cell (run_0017→0029, N2 自身の床)、R3 node (run_0022→0032) すべて 0。 N2 cell の onset は 3 run で 2.112/2.131/2.131 in (1 セル揺れ = ±0.02 in / ±0.15 K)。文書の残り 3 点 (m2) を訂正。**採否の結論**: 空気凝縮 (CPG carrier 形) と N2 低温物性の整合は受理; 結果は残差 plateau の**未収束準定常比較**として支持 (§9 ①の表のまま)。
