...
| [turbulence-sst-energy-includes-k.md](active/turbulence-sst-energy-includes-k.md) | `turbulence / thermophysics / convection` | **R3: 全エネルギーに ρk を含める (SU2 形)**: E_t = ρ(e+u²/2+k)、p* = p+(2/3)ρk を圧力項に一括、エネルギー流束に k 拡散。分離型 `sstEnergyKSource`/`sstIsotropicStress` を置換。方針決定 2026-09-08、実装未着手 |
